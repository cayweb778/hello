...
interface AccountAccvoucherCdigestClassRepository : ReactiveCrudRepository<AccountAccvoucherCdigestClass?, String?> {
    fun findAllByOrderByClassCode(): Flux<AccountAccvoucherCdigestClass?>?

    /**
     * 获取最大的编码
     */
    @Query("select Max(class_code) as class_code from accvoucher_cdigest_class ")
    fun findMaxClassCode(): Mono<AccountAccvoucherCdigestClass?>?

    /**
     * 获取缺少的最小数字
     */
    @Query(
        "select MAX(cast(class_code as int))+1 as class_code from accvoucher_cdigest_class " +
    ...
    fun findBukongClassCode(): Mono<AccountAccvoucherCdigestClass?>?
}