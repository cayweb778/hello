...
import org.boozsoft.repo.CustomerClassRepository;
...
@Slf4j
@RestController
@RequestMapping("/sys/cusromerclass")
...
public class CustomerClassController {
    ...
    public Mono<R> verifyCusClassName(String parentId, String cusClassName) {
        return  StringUtils.isNotBlank(parentId)?
                customerClassRepository.countByParentIdAndCusCclassName(parentId, cusClassName).map(o -> R.ok().setResult(o)):
                customerClassRepository.countByCusCclassName(cusClassName).map(o -> R.ok().setResult(o));
    }
    ...
    public Mono<R> findAllCustomerClass(@RequestBody Map map, Pageable pageable) {
        ...
        .flatMap(item->{
            ...
            List<CustomerClass> list=item.stream().skip((page - 1) * size).limit(size).collect(Collectors.toList());
            ...
            list.stream().forEach(v->{
                List<CustomerClass> collect = item.stream().filter(f -> v.getParentId().equals(f.getUniqueCustclass())).collect(Collectors.toList());
                if(collect.size()>0){
    ...
    @PostMapping("/save")
    public Mono save(@RequestBody CustomerClass customerClassMono) {
        Mono add = Mono.just(customerClassMono).flatMap(item -> {
            ...
            return customerClassRepository.countByParentId(item.getParentId())
                ...
                .flatMap(customerClassRepository::save)
        ...
        Mono edit = Mono.just(customerClassMono).flatMap(item -> {
            ...
            return customerClassRepository.save(item);
        })
        .flatMap(customerClassRepository::save)
        .map(o -> R.ok().setResult(o));
    ...
    public Mono save(@RequestBody List<CustomerClass> customerClassMono){
        return customerClassRepository.saveAll(customerClassMono).collectList().map(a->R.ok().setResult(a));
    }

    @PostMapping("/del")
    public Mono<R> delete(@RequestBody List<CustomerClass> list) {
        list.sort(Comparator.comparing(CustomerClass::getCusClassGrade).reversed());
        // 不可删除
        List<String> noBend0Codelist=new ArrayList<>();
        // 可删除
        List<CustomerClass> Bend1Codelist=new ArrayList<>();
        ...
        .flatMap(listAll->{
            ...
            .flatMap(cusList->{
                ...
                .flatMap(listAll2->{
                    ...
                    List<String> collect = list.stream().filter(a -> !a.getParentId().equals("0")).map(CustomerClass::getParentId).distinct().collect(Collectors.toList());
                    collect.forEach(f->{
                    ...
                    // 删除
                    // 去除包含不能删除的编码
                    List<CustomerClass> str=Bend1Codelist.stream().filter(a->listStr.indexOf(a.getUniqueCustclass())==-1).collect(Collectors.toList());
                    ...
                    for (int i = 0; i < listStr.size(); i++) {
                        int finalI = i;
                        List<CustomerClass> collect1 = listAll2.stream().filter(v -> v.getUniqueCustclass().equals(listStr.get(finalI))).collect(Collectors.toList());
                        if(collect1.size()>0){
                            listStr.remove(i);
    ...
    public List<AAtemp> getChildDel(String pid, List<CustomerClass> allList) {
        List<AAtemp> childList = new ArrayList<>();//用于保存子节点的list;
        for (CustomerClass ms : allList) {
...
//                    } catch (IOException e) {}
                ...
                .flatMap(mapArr -> {
                    ...
                    List<CustomerClass> editBendlist = new ArrayList<>();
                    ...
                    for (int i = 1; i < excellist.size(); i++) {
                        ...
                        if(StringUtils.isNotBlank(excellist.get(finalI)[0].toString().trim())) {
                            List<CustomerClass> collect = cusclasslist.stream().filter(cusc -> cusc.getCusClass().equals(excellist.get(finalI)[0].toString().trim())).collect(Collectors.toList());
                            if (collect.size() > 0) {
                        ...
                        if(StringUtils.isNotBlank(excellist.get(finalI)[1].toString().trim())) {
                            List<CustomerClass> collect = cusclasslist.stream().filter(cusc -> cusc.getCusCclassName().equals(excellist.get(finalI)[1].toString().trim())).collect(Collectors.toList());
                            if (collect.size() > 0) {
                        ...
                        if(StringUtils.isNotBlank(excellist.get(finalI)[2].toString().trim())) {
                            // 从表中查询
                            List<CustomerClass> collect = cusclasslist.stream().filter(cusc -> cusc.getCusClass().equals(excellist.get(finalI)[2].toString().trim())).collect(Collectors.toList());
                            // 从文件中查询
                        ...
                        if(errorText.size()==0){
                            if(cusclasslist.size()>0&&sums==1){
                                cusclasslist.sort(Comparator.comparing(CustomerClass::getCusGradeCode).reversed());
                                int max=Integer.valueOf(cusclasslist.get(0).getCusGradeCode())+1;
                                sums = max;
                            }
                            CustomerClass cusc = new CustomerClass();
                            ...
                            if (StringUtils.isBlank(excellist.get(finalI)[2].toString().trim())) {
                                ...
                                // 查找上级分类编码 是否存在系统
                                List<CustomerClass> collect = cusclasslist.stream().filter(cl -> cl.getCusClass().equals(excellist.get(finalI)[2].toString().trim())).collect(Collectors.toList());
                                if (collect.size() > 0) {
                                    CustomerClass c1=collect.get(0);
                                    c1.setCusBend("0");
                                    editBendlist.add(c1);
                                    ...
                                    // 查询有几个下级
                                    List<CustomerClass> collect1 = cusclasslist.stream().filter(cl -> cl.getParentId().equals(collect.get(0).getUniqueCustclass())).collect(Collectors.toList());
                                ...
                                // 查询 文件中是否包含下级
                                    ...
                                    List<CustomerClass> collect12 = newlist.stream().filter(ex -> ex.getCusClass().equals(excellist.get(finalI)[2].toString().trim())).collect(Collectors.toList());
                ...
                .flatMapMany(mapArr -> {
                    List<CustomerClass> newlist = (List<CustomerClass>) mapArr.get("list");
                    List<CustomerClass> editBendlist = (List<CustomerClass>) mapArr.get("editBendlist");
                    Mono add=customerClassRepository.saveAll(newlist).collectList();
                    Mono editBend=customerClassRepository.saveAll(editBendlist).collectList();
    ...
    // 查看客户分类是否有数据权限控制
    @PostMapping("getCusTypeSeeSwitch")
    public Mono<R> getCusTypeSeeSwitch(@RequestBody Map map){
        String accId=map.get("accId").toString();
        String dataBaseName=map.get("dataBaseName").toString();
        return groupDataSeeSwitchRepository.findAllByUniqueCodeAndDatabaseNameAndIsCtrlAndCtrlRange(accId,dataBaseName,"1","ZT").map(a->R.ok().setResult(a)).defaultIfEmpty(R.ok().setResult(""));
    }
    // 查看授权的客户分类
    ...
    public Mono<R> findByCusClassAuthData(@RequestBody Map map){
        ...
        String accId=map.get("accId").toString();
        return customerClassRepository.findByCusClassAuthData(tableName,accId,userId).collectList().map(a->R.ok().setResult(a)).defaultIfEmpty(R.ok().setResult(""));
    }


    public List<Map<String, Object>> getChild(String pid, List<CustomerClass> allList) {
