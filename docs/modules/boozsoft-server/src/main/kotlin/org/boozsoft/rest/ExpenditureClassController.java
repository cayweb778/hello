...
import org.boozsoft.repo.ExpenditureClassRepository;
import org.boozsoft.repo.group.GroupExpenditureClassRepository;
...
@RestController
@RequestMapping("/sys/expenditureClass")
public class ExpenditureClassController {

    @Autowired
    private ExpenditureClassRepository groupExpenditureClassRepository;

    @Autowired
    private GroupExpenditureClassRepository groupRepository;
...
//                    .map(list->TreeUtils.castTreeList(list,GroupExpenditureClassVo.class))
    ...
    public Mono<Map> treeDeptByFlag() {
        return groupExpenditureClassRepository.findByFlagAndIsDelOrderByEcCode("1","0")
                .collectList()
                .map(list->TreeUtils.castTreeList(list, GroupExpenditureClassVo.class))
                .map(list -> CollectOfUtils.mapof(
                        "code", 0,
    ...
    public Mono save(@RequestBody ExpenditureClass object){
        ...
        //有上级是末级时
        Mono<ExpenditureClass> sysDeptClassMono = null;
    ...
    public Mono delete(@RequestBody ExpenditureClass object) {
    ...
    public Mono editFlag(@RequestBody ExpenditureClass object) {
    ...
    @Transactional
    public Mono excel(@RequestBody List<ExpenditureClass> object){
    ...
    public Mono introduceData(@RequestBody List<GroupExpenditureClass> ids){
        return Mono.just(ids)
                .flatMap(list->{
                    List<ExpenditureClass> l =  new ArrayList<>();
                    list.forEach(v->{
                        ExpenditureClass b =  new ExpenditureClass();
                        BeanUtils.copyProperties(v,b);
                        b.setId(null);
                        b.setIsDel("0");
                        l.add(b);
                    });
                    return  groupExpenditureClassRepository.saveAll(l).then(Mono.just("200"));
                }).map(o->R.ok().setResult(o));
    }
    ...
    public Mono<R> findSettModesInAll(Pageable pageable){
        return groupRepository.findAll().collectList()
                .flatMap(list->{
                    return groupExpenditureClassRepository.findAllByOrderByEcCode().collectList()
                            .map(v->v.stream().map(t-> t.getEcCode()).collect(Collectors.toList()))
                            .map(v->{
                                return list.stream().filter(o-> !v.contains(o.getEcCode()));
                            });
                })
                .map(o-> R.ok().setResult(o));
    }
