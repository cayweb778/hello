...
class FuZhuHeSuan2Controller {
    ...
    @Autowired
    lateinit var fzProjectItemR: ProjectItemRepository

    @Autowired
    lateinit var fuzhuHesuanRepository: FuzhuHesuanRepository


    fun <T> toMap(flux: Flux<T>): Mono<MutableList<Map<*, *>>> {
        return flux.map { JSON.parseObject(JSON.toJSONString(it), Map::class.java) }.collectList()
    }
    ...
    fun findKeyLabelAll(@RequestBody params: Map<Any, Any>): Mono<R<Any>> {
        ...
        return Mono.fromSupplier { require.split(",").filter { !it.contains("fz") } }
                ...
                .mergeWith(if (!require.contains("fzCustom")) Mono.empty()
                else toMap(fzCustomR.findAll())
                        .map { it1 ->
                            mapOf(
                                    "key" to "fzCustom",
                                    "label" to "客户",
                                    "list" to it1.map { row ->
                                        if (params["toTarget"] == "true")
                                            mapOf(
                                                    "key" to row["uniqueCode"],
                                                    "label" to row["custName"],
                                                    "code" to row["custCode"],
                                                    "target" to row
                                            )
                                        else
                                            mapOf(
                                        ...
                                        mapOf(
                                                ...
                                                "list" to it1.map { row ->
                                                    if (params["toTarget"] == "true")
                                                        mapOf(
                                                                "key" to row["itemCode"],
                                                                "label" to row["itemName"],
                                                                "code" to row["itemCode"],
                                                                "target" to row
                                                        )
                                                    else
                                                        mapOf(
...
//                                data["deptCode"] = data["key"] as String
        ...
        if (params["key"] == "fzProjectCategory") {
            data["itemCode"] = data["key"] as String
            data["itemName"] = data["label"] as String
            data["itemCode"] = data["key"] as String

            return fzProjectItemR.save(JSON.parseObject(JSON.toJSONString(data), ProjectItem::class.java))
                    .thenReturn(R.ok());
...
//                                data["deptCode"] = data["key"] as String
        ...
        if (tableId.contains("fzProjectCategory")) {
            val data = (JSON.parseObject(params["data"], Map::class.java) as Map<String, String>).toMutableMap()
            return fzProjectItemR.deleteByItemCode(data["key"] as String).thenReturn(R.ok());
