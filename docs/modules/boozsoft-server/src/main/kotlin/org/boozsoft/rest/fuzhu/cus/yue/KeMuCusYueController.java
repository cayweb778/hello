...
@Slf4j
@RestController
@RequestMapping("/KeMuCusYue")
@Api(value = "客户科目明细账", tags = "API系统：客户科目明细账")
public class KeMuCusYueController {
    @Autowired
...
//        String maxDept=map.get("maxDept").toString();       // 结束部门
        ...
        return  kmYueCusService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,dynamicTenantId);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map, Pageable pageable){
        ...
        return  kmYueCusService.exportAll(pageable,minDept,strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map, Pageable pageable){
        ...
        String styleValue=map.get("styleValue").toString();         // 科目类型
        List<String>ccodeList= Arrays.asList(map.get("ccodeList").toString().split(","));
        return  kmYueCusService.exportAll2(pageable,minDept,strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
}
