...
import org.boozsoft.service.impl.fuzhu.dept.mx.KmDeptMxServiceImpl;
...
@Slf4j
@RestController
@RequestMapping("/KeMuDeptMingXi")
@Api(value = "科目部门明细账", tags = "API系统：科目部门明细账")
public class KeMuDeptMingXiController {
    @Autowired
    KmDeptMxServiceImpl kmDeptMxService;

    @PostMapping("findAll")
    public Mono<R> findAll(@RequestBody Map map, Pageable pageable){
        String database=map.get("database").toString();
        String minDept=map.get("minDept").toString();       // 结束部门
        ...
        return  kmDeptMxService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,database);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map){
        ...
        return  kmDeptMxService.exportAll(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map){
        ...
        String styleValue=map.get("styleValue").toString();         // 科目类型
        List<String> ccodeList= Arrays.asList(map.get("ccodeList").toString().split(","));
        return  kmDeptMxService.exportAll2(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
}
