...
import org.boozsoft.service.impl.fuzhu.pro.mx.KmProMxServiceImpl;
...
@Slf4j
@RestController
@RequestMapping("/KeMuProMingXi")
@Api(value = "科目项目明细账", tags = "API系统：科目项目明细账")
public class KeMuProMingXiController {
    @Autowired
    KmProMxServiceImpl kmProMxService;
    ...
    public Mono<R> findAll(@RequestBody Map map, Pageable pageable){
        ...
        return  kmProMxService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,database);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map){
        ...
        return  kmProMxService.exportAll(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map){
        ...
        String styleValue=map.get("styleValue").toString();         // 科目类型
        List<String> ccodeList= Arrays.asList(map.get("ccodeList").toString().split(","));
        return  kmProMxService.exportAll2(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
}
