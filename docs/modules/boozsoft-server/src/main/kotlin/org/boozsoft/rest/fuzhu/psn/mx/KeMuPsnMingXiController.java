...
import org.boozsoft.service.impl.fuzhu.person.mx.KmPsnMxServiceImpl;
...
@Slf4j
@RestController
@RequestMapping("/KeMuPsnMingXi")
@Api(value = "科目个人明细账", tags = "API系统：科目个人明细账")
public class KeMuPsnMingXiController {
    @Autowired
    KmPsnMxServiceImpl kmPsnMxService;
    ...
    public Mono<R> findAll(@RequestBody Map map, Pageable pageable){
        ...
        return  kmPsnMxService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,database);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map){
        ...
        return  kmPsnMxService.exportAll(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map){
        ...
        String styleValue=map.get("styleValue").toString();         // 科目类型
        List<String> ccodeList= Arrays.asList(map.get("ccodeList").toString().split(","));
        return  kmPsnMxService.exportAll2(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
}
