...
import org.boozsoft.service.impl.fuzhu.person.yue.PsnKmYueServiceImpl;
...
@Slf4j
@RestController
@RequestMapping("/PsnKeMuYue")
@Api(value = "客户科目明细账", tags = "API系统：客户科目明细账")
public class PsnKeMuYueController {
    @Autowired
    PsnKmYueServiceImpl psnKmYueService;
    ...
    public Mono<R> findAll(@RequestBody Map map, Pageable pageable){
        ...
        return  psnKmYueService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,database);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map){
        ...
        String ishaveRjz=map.get("ishaveRjz").toString();         // 是否包含未记账  0否/1是
        String styleValue=map.get("styleValue").toString();         // 科目类型
        return  psnKmYueService.exportAll(strDate,endDate,bz,ishaveRjz,styleValue);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map){
        ...
        String ishaveRjz=map.get("ishaveRjz").toString();         // 是否包含未记账  0否/1是
        String styleValue=map.get("styleValue").toString();         // 科目类型
        return  psnKmYueService.exportAll2(strDate,endDate,bz,ishaveRjz,styleValue);
    }
}
