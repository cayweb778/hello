...
import org.boozsoft.service.impl.fuzhu.sup.mx.KmSupMxServiceImpl;
...
@Slf4j
@RestController
@RequestMapping("/KeMuSupMingXi")
@Api(value = "科目供应商明细账", tags = "API系统：科目供应商明细账")
public class KeMuSupMingXiController {
    @Autowired
    KmSupMxServiceImpl kmSupMxService;
    ...
    public Mono<R> findAll(@RequestBody Map map, Pageable pageable){
        ...
        return  kmSupMxService.findAll(pageable,minDept,km,strDate,endDate,bz,ishaveRjz,styleValue,searchMap,filterMap,database);
    }
    ...
    public Mono<R> exportAll(@RequestBody Map map){
        ...
        return  kmSupMxService.exportAll(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
    ...
    public Mono<R> exportAll2(@RequestBody Map map){
        ...
        String styleValue=map.get("styleValue").toString();         // 科目类型
        List<String> ccodeList= Arrays.asList(map.get("ccodeList").toString().split(","));
        return  kmSupMxService.exportAll2(strDate,endDate,bz,ishaveRjz,styleValue,ccodeList);
    }
}
