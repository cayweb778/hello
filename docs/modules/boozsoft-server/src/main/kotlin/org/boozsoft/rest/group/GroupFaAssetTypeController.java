...
import org.boozsoft.repo.FaAssetTypeRepository;
...
import org.boozsoft.repo.group.GroupFaAssetTypeRepository;
...
@RestController
@RequestMapping("/group/faAssetType")
public class GroupFaAssetTypeController {

    @Autowired
    private GroupFaAssetTypeRepository groupExpenditureClassRepository;

    @Autowired
    private GroupFaPropertyRepository faPropertyRepository;

    @Autowired
    private GroupFaZhejiuMethodRepository faZhejiuMethodRepository;

    @Autowired
    private GroupSysUnitOfMeaRepository sysUnitOfMeaTypeRepository;
...
//                    .map(list->TreeUtils.castTreeList(list,GroupExpenditureClassVo.class))
    ...
    public Mono save(@RequestBody  GroupFaAssetType object){
        ...
        //有上级是末级时
        Mono<GroupFaAssetType> sysDeptClassMono = null;
    ...
    public Mono delete(@RequestBody  GroupFaAssetType object) {
    ...
    public Mono editFlag(@RequestBody GroupFaAssetType object) {
    ...
    @Transactional
    public Mono excel(@RequestBody List<GroupFaAssetType> object){
    ...
    @GetMapping("/getFaZhejiuMethod")
    @ApiOperation(value = "获取折旧方法", notes = "获取折旧方法")
    public Mono getFaZhejiuMethod(){
        return faZhejiuMethodRepository.findAllDeptCodeOrDeptNameByFlag()
                .collectList()
                .map(v-> R.ok().setResult(v));
    }

    @GetMapping("/getFaProperty")
    @ApiOperation(value = "获取资产属性", notes = "获取资产属性")
    public Mono getFaProperty(){
        return faPropertyRepository.findAllDeptCodeOrDeptNameByFlag()
                .collectList()
                .map(v-> R.ok().setResult(v));
    }

    @GetMapping("/getSysUnitOfMeaType")
    @ApiOperation(value = "获取计量单位", notes = "获取计量单位")
    public Mono getSysUnitOfMeaType(){
        return sysUnitOfMeaTypeRepository.findAllByFlag()
                .collectList()
                .map(v-> R.ok().setResult(v));
    }
