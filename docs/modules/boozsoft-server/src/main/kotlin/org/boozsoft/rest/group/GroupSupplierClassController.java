...
import org.boozsoft.repo.GroupSupplierClassRepository;
import org.boozsoft.repo.GroupSupplierRepository;
...
@Slf4j
@RestController
@RequestMapping("/sys/supplierclass_group")
@Api(value = "供应商分类档案", tags = "API系统：供应商分类档案")
public class GroupSupplierClassController {
    @Autowired
    GroupSupplierClassRepository groupSupplierClassRepository;
    @Autowired
    GroupSupplierRepository groupCustomerRepository;
    @Autowired
    ...
    public Mono<R> verifyCusClass(String cusClass) {
        return groupSupplierClassRepository.countByCusClass(cusClass).map(o -> R.ok().setResult(o));
    }

    @PostMapping("verifySuperCusClassName")
    public Mono<R> verifySuperCusClassName(String parentId, String cusCclassName) {
        return groupSupplierClassRepository.countByParentIdAndCusCclassName(parentId, cusCclassName).map(o -> R.ok().setResult(o));
    }

    @PostMapping("verifyCusClassName")
    public Mono<R> verifyCusClassName(String cusCclassName) {
        return groupSupplierClassRepository.countByCusCclassName(cusCclassName).map(o -> R.ok().setResult(o));
    }

    @GetMapping("findAllByCusBendEq1")
    public Mono<R> findAllByCusBendEq1() {
        return groupSupplierClassRepository.findAllByCusBend("1").collectList().map(R::page);
    }

    @GetMapping("treeCustomerClass")
    public Mono<R> treeCustomerClass() {
        return groupSupplierClassRepository.findAll()
                .collectList()
    ...
    public Mono<R> findAllCustomerClass(@RequestBody Map map, Pageable pageable) {
        ...
        return groupSupplierClassRepository.findAll()
                ...
                .flatMap(item->{
                    ...
                    List<GroupSupplierClass> list=item.stream().skip((page - 1) * size).limit(size).collect(Collectors.toList());
                    ...
                    list.stream().forEach(v->{
                        List<GroupSupplierClass> collect = item.stream().filter(f -> v.getParentId().equals(f.getUniqueCustclass())).collect(Collectors.toList());
                        if(collect.size()>0){
    ...
    @Transactional
    @PostMapping("/save")
    public Mono save(@RequestBody GroupSupplierClass customerClassMono, String username) {
        ...
        Mono add = Mono.just(customerClassMono).flatMap(item -> {
            ...
            if (StringUtils.isBlank(item.getParentId())) {
                ...
                return groupSupplierClassRepository.findByMaxParentId("0")
                        .flatMap(sums -> {
                            item.setCusGradeCode(String.format("%03d", Integer.valueOf(sums) + 1));
                            return Mono.just(item);
                        });
            }
            return groupSupplierClassRepository.countByParentId(item.getParentId())
                    .flatMap(sums -> {
                        return groupSupplierClassRepository
                ...
                .flatMap(groupSupplierClassRepository::save).map(o -> R.ok().setResult(o));
        ...
        Mono edit = Mono.just(customerClassMono).flatMap(item->{
            ...
            return groupSupplierClassRepository.save(item).map(o -> R.ok().setResult(o));
        });
    ...
    @Transactional
    @PostMapping("/del")
    public Mono delete(@RequestBody List<GroupSupplierClass> list) {
        list.sort(Comparator.comparing(GroupSupplierClass::getCusClassGrade).reversed());
        // 不可删除
        List<String> noBend0Codelist=new ArrayList<>();
        // 可删除
        List<GroupSupplierClass> Bend1Codelist=new ArrayList<>();
        return groupSupplierClassRepository.findAll().collectList()
        .flatMap(listAll->{
            ...
            .flatMap(cusList->{
                ...
                .flatMap(listAll2->{
                    ...
                    .flatMap(assignCus->{
                        ...
                        List<String> collect = list.stream().filter(a -> !a.getParentId().equals("0")).map(GroupSupplierClass::getParentId).distinct().collect(Collectors.toList());
                        collect.forEach(f->{
                        ...
                        Mono editFlg=groupSupplierClassRepository.updateBendByUniqueCust("1",editflag).then();
                        // 删除
                        // 去除包含不能删除的编码
                        List<GroupSupplierClass> str=Bend1Codelist.stream().filter(a->listStr.indexOf(a.getUniqueCustclass())==-1).collect(Collectors.toList());
                        Mono delData=groupSupplierClassRepository.deleteAll(str).then();

                        // 把不能删除的唯一码替换成名称
                        for (int i = 0; i < listStr.size(); i++) {
                            int finalI = i;
                            List<GroupSupplierClass> collect1 = listAll2.stream().filter(v -> v.getUniqueCustclass().equals(listStr.get(finalI))).collect(Collectors.toList());
                            if(collect1.size()>0){
                                listStr.remove(i);
...
//                    } catch (IOException e) {}
        ...
        .flatMap(item -> {
            ...
            mapArr.put("error", "");
            mapArr.put("code", "200");
            return groupSupplierClassRepository.findAll().collectList().flatMap(ite -> {
...
//            }
        ...
        .flatMapMany(mapArr -> {
            List<GroupSupplierClass> newlist = (List<GroupSupplierClass>) mapArr.get("list");
            return mapArr.get("code").equals("401")?Mono.just(mapArr):newlist.size()==0?Mono.just(mapArr):groupSupplierClassRepository.saveAll(newlist).map(t->mapArr);
        })
        .collectList()
    ...
    public Mono<R> findByLikeCusGradeCode(String cusGradeCode,String cusClassGrade,String cusBend) {
        return groupSupplierClassRepository.findByLikeCusGradeCode(cusGradeCode+"%",cusClassGrade,cusBend)
                .collectList()
                .map(a -> R.ok().setResult(a));
    }

    public List<Map<String, Object>> getChild(String pid, List<GroupSupplierClass> allList) {
        List<Map<String, Object>> childList = new ArrayList<>();//用于保存子节点的list;
        for (GroupSupplierClass ms : allList) {
    ...
    public List<AAtemp> getChildDel(String pid, List<GroupSupplierClass> allList) {
