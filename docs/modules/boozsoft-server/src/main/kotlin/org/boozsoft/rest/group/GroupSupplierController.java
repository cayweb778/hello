...
@Slf4j
@RestController
@RequestMapping("/sys/supplier_group")
@Api(value = "集团供应商信息", tags = "API系统：集团供应商信息")
public class GroupSupplierController {
    ...
    @Autowired
    GroupSupplierClassRepository supplierClassRepository;
    @Autowired
    ...
    public Mono<R> verifyByCustAbbname(String custAbbname) {
        return supplierRepository.countByCustAbbname(custAbbname).map(o -> R.ok().setResult(o)).defaultIfEmpty(R.ok().setResult(""));
    }
    @PostMapping("findByCustAbbnameAndCustName")
    public Mono<R> findByCustAbbnameAndCustName(String custAbbname,String custName) {
        return supplierRepository.findByCustAbbnameAndCustName(custAbbname,custName).map(o -> R.ok().setResult(o)).defaultIfEmpty(R.ok().setResult(""));
    }
    ...
    public Mono<R> findByCustAbbname(String custAbbname) {
        return supplierRepository.findByCustAbbname(custAbbname).map(o -> R.ok().setResult(o));
    }
    @PostMapping("findByCustName")
    public Mono<R> findByCustName(String custName) {
        return supplierRepository.findByCustName(custName).map(o -> R.ok().setResult(o)).defaultIfEmpty(R.ok().setResult(""));
    }
    ...
    public Mono<R> findAllByFlag(String flag) {
        return supplierRepository.findAllBySuccessStateAndFlag(flag,flag).collectList().map(R::page);
    }
    ...
    public Mono<R> importCus(@RequestPart("file") FilePart filePartParm) throws Exception {
        ...
        return Mono.just(filePartParm)
            ...
            .flatMap(objArr -> {
                ...
                String[] str = new String[]{"供应商编码", "供应商全称", "供应商简称", "税号", "分类名称","母公司","对应客户","国家", "区划(省-市-区)", "详细地址", "电话", "手机", "网站", "EMail", "行业性质", "开户银行", "开户地", "银行账户", "银行行号",""};
            ...
            .flatMap(item -> {
                ...
                return supplierRepository.findAll()
                    .collectList()
                    .flatMap(list -> {
                        mapArr.put("list", list);   // 集团供应商list
                                ...
                                .flatMap(country->{
                                    ...
                                    return groupCustomerRepository.findAll().collectList()
                                        .flatMap(groupCusList->{
                                            mapArr.put("groupCusList", groupCusList);// 客户信息list
            ...
            .flatMap(mapArr -> {
                ...
                List<GroupSupplierClass> classlist = (List<GroupSupplierClass>) mapArr.get("classlist");
                // 国家list
                List<SysCountry> countrylist = (List<SysCountry>) mapArr.get("country");
                // 集团客户list
                List<GroupCustomer> groupCusList = (List<GroupCustomer>) mapArr.get("groupSupList");
                // 文件list
                ...
                // 如果小于5 说明表头不对
                if (!excellist.get(0)[0].equals("供应商编码") && !excellist.get(0)[0].equals("供应商全称")) {
                ...
                for (int i = 1; i < excellist.size(); i++) {
                    ...
                    if(StringUtils.isNotBlank(excellist.get(finalI)[0].toString())){
                        List<GroupSupplier> collect = list.stream().filter(customer -> customer.getCustCode().equals(excellist.get(finalI)[0].toString())).collect(Collectors.toList());
                        if (collect.size() > 0) {
                    ...
                    if(StringUtils.isNotBlank(excellist.get(finalI)[1].toString())){
                        List<GroupSupplier> collect2 = list.stream().filter(customer -> customer.getCustName().equals(excellist.get(finalI)[1].toString())).collect(Collectors.toList());
                        if (collect2.size() > 0) {
                    ...
                    if(StringUtils.isNotBlank(excellist.get(finalI)[2].toString())) {
                        List<GroupSupplier> collect3 = list.stream().filter(customer -> customer.getCustAbbname().equals(excellist.get(finalI)[2].toString())).collect(Collectors.toList());
                        if (collect3.size() > 0) {
                    ...
                    if(StringUtils.isNotBlank(excellist.get(finalI)[3].toString())) {
                        List<GroupSupplier> collect4 = list.stream().filter(customer -> customer.getCustSregcode().equals(excellist.get(finalI)[3].toString())).collect(Collectors.toList());
                        if (collect4.size() > 0) {
                    ...
                    if(column.get()>5){
                        ...
                        if(StringUtils.isNotBlank(excellist.get(finalI)[17].toString())) {
                            List<GroupSupplier> collect4 = list.stream().filter(customer -> customer.getCustAccount().equals(excellist.get(finalI)[17].toString())).collect(Collectors.toList());
                            if (collect4.size() > 0) {
                    ...
                    if(errorText.size()==0){
                        ...
                        if (StringUtils.isNotBlank(excellist.get(i)[4].toString())) {
                            int finalI2 = i;
                            List<GroupSupplierClass> collect1 = classlist.stream().filter(cla -> cla.getCusCclassName().equals(excellist.get(finalI2)[4].toString().trim())).collect(Collectors.toList());
                            if (collect1.size() > 0) {
                        ...
                        if(excellist.get(finalI)[5]!=null) {
                            List<GroupSupplier> collect = list.stream().filter(customer -> StringUtils.isNotBlank(customer.getUniqueCodeCcus()) && customer.getUniqueCodeCcus().equals(excellist.get(finalI)[5].toString().trim())).collect(Collectors.toList());
                            if(collect.size()>0){
                        ...
                        if(excellist.get(finalI)[6]!=null) {
                            if(groupCusList.size()>0){
                                List<GroupCustomer> collect = groupCusList.stream().filter(v -> StringUtils.isNotBlank(v.getCustName()) && v.getCustName().equals(excellist.get(finalI)[6].toString().trim()) || v.getCustAbbname().equals(excellist.get(finalI)[6].toString().trim())).collect(Collectors.toList());
                                if(collect.size()>0){
...
//                        listStr.add(excellist.get(i)[1].toString());    // 全称
            ...
            .flatMapMany(mapArr -> {
                List<GroupSupplier> list = (List<GroupSupplier>) mapArr.get("list");
                return mapArr.get("code").equals("401") ? Mono.just(mapArr) :list.size()==0?Mono.just(mapArr): supplierRepository.saveAll(list).map(a->mapArr);
            })
            .collectList()
    ...
    public Mono<R> OrgApplyImportCus(@RequestPart("file") FilePart filePartParm,@RequestPart("orgUnique") String orgUnique,
        ...
        .flatMap(item -> {
            ...
            // 判断组织申请客户是否在分配表中
            return groupSupplierAccountRepository.findAllByCusAssignLeftJoinGroupCusData(orgUnique,"1").collectList()
                ...
                .flatMap(groupSupList->{
                    ...
                    .flatMap(orgSupClassList->{
                        ...
                        .flatMap(countryList->{
                            ...
                            .flatMap(obj->{
                                // 获取集团客户
                                return supplierRepository.getCusCode().collectList()
                                .flatMap(supcode->{
                                    // 组织供应商
                                    ...
                                    .flatMap(orgSupAll->{
                                        ...
                                        .flatMap(orgCusAll->{
                                            ...
                                            mapArr.put("orgSupClassList", orgSupClassList);
        ...
        .flatMap(mapArr -> {
            ...
            // 组织客户分类
            List<OrgSupplierClass> orgSupClassList= (List<OrgSupplierClass>) mapArr.get("orgSupClassList");
            // 国家表
            ...
            for (int i = 1; i < excellist.size(); i++) {
                ...
                if (StringUtils.isNotBlank(excellist.get(i)[4].toString())) {
                    int finalI2 = i;
                    List<OrgSupplierClass> collect1 = orgSupClassList.stream().filter(cla -> cla.getCusBend().equals("1")&& cla.getCusCclassName().equals(excellist.get(finalI2)[4].toString().trim())).collect(Collectors.toList());
                    if (collect1.size() == 0) {
                ...
                if(errorText.size()==0){
                    ...
                    String uniqueCodeCcus="";
                    if(excellist.get(finalI)[5]!=null) {
                        List<OrgSupplier> collect = orgSupList.stream().filter(customer -> StringUtils.isNotBlank(customer.getUniqueCodeCcus()) && customer.getUniqueCodeCcus().equals(excellist.get(finalI)[5].toString().trim())).collect(Collectors.toList());
                        if(collect.size()>0){
                            uniqueCodeCcus=collect.get(0).getUniqueCode();
                        }
                    }
                    ...
                    String uniqueCodeSupplier="";
                    if(excellist.get(finalI)[6]!=null) {
                        List<OrgCustomer> collect = orgCusAll.stream().filter(customer -> StringUtils.isNotBlank(customer.getUniqueCodeCcus()) && customer.getUniqueCodeCcus().equals(excellist.get(finalI)[6].toString().trim())).collect(Collectors.toList());
                        if(collect.size()>0){
                            uniqueCodeCcus=collect.get(0).getUniqueCode();
                        }
                    }
    ...
    public Mono<R>delCus(String id,String delName){
        return supplierRepository.editByidIsDel(id,"1",delName,new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())).then(Mono.just(R.ok()));
    }
    ...
    public Mono<R>delCus(String id){
        return supplierRepository.editByidIsDel(id,"0","","").then(Mono.just(R.ok()));
    }
    ...
    public Mono<R>delCusArr(String id){
        ...
        for (int i = 0; i < id.split(",").length; i++) {
            list.add(new GroupSupplier().setId(id.split(",")[i]));
        }
    ...
    public Mono<R>findAllByUniqueCustclass(@RequestBody Map map){
        ...
        return supplierRepository.findAllByUniqueCustclass(list).collectList().map(a->R.ok().setResult(a));
    }

    @PostMapping("/findAllByGroupAndAllot")
    public Mono<R> findAllByGroupAndAllot(String tenantId){
        return supplierRepository.findAllByGroupAndAllot(tenantId).collectList().map(a->R.ok().setResult(a));
    }

    @PostMapping("/countAllotByCustUnique")
    public Mono<R> countAllotByCustUnique(String cusUnique){
        return supplierRepository.countAllotByCustUnique(cusUnique).map(a->R.ok().setResult(a));
    }

    @PostMapping("/getCodingRule")
    public Mono<R> getCodingRule(String ctype){
        return groupFileEncodingRulesRepository.findByFileType(ctype).map(a->R.ok().setResult(a));
    }
    ...
    public Mono<R> findGroupCusByOrg(@RequestBody Map map){
        ...
        Map<String, String> searchMap = ((HashMap<String,  String>) map.get("searchConditon"));
        return supplierRepository.findGroupCusByOrg(orgUnique).collectList()
            .flatMap(list->{
                return Mono.just(
                        list.stream().filter(v->{
                            if (StringUtils.isNotBlank(searchMap.get("requirement")) && StringUtils.isNotBlank(searchMap.get("value"))) {
    ...
    public Mono<R> findGroupCusByTenantId(@RequestBody Map map){
        ...
        Map<String, String> searchMap = ((HashMap<String,  String>) map.get("searchConditon"));
        return supplierRepository.findGroupCusByTenantId(tenantId).collectList()
                .flatMap(list->{
    ...
    public Mono<R> editGroupCusAll(@RequestBody List<GroupSupplier> list){
        return supplierRepository.saveAll(list).collectList().map(a->R.ok().setResult(a));
    }
    ...
    public Mono<R> findAllByGroupCusNameAndCustAbbnameLike(String name,String abbName){
        return supplierRepository.findAllByGroupCusNameAndCustAbbnameLike("%"+name+"%","%"+abbName+"%").collectList().map(a->R.ok().setResult(a));
    }


    @PostMapping("/editByIdSuccessState")
    public Mono<R> editByIdSuccessState(String id,String approveUser){
        return supplierRepository.editByIdSuccessState("1",id,approveUser,new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())).then(Mono.just(R.ok()));
    }
    ...
    public Mono<R> delByIdCustNameAndCustAbbnameAndSuccessState(String custName,String custAbbName,String successState){
        return supplierRepository.delByIdCustNameAndCustAbbnameAndSuccessState(custName,custAbbName,successState).then(Mono.just(R.ok()));
    }

    @GetMapping("findAllSupplierByFlag")
    public Mono<R> findAllSupplierByFlag(String flag){
        return supplierRepository.findAllBySuccessStateAndFlag(flag,flag).collectList().map(R::page);
    }
     ...
     */
    @PostMapping("/save")
    public Mono<R> save(ServerWebExchange exchange, @RequestBody GroupSupplier customerMono) {
