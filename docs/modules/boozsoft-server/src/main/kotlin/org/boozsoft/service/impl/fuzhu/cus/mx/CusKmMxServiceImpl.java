package org.boozsoft.service.impl.fuzhu.cus.mx;
...
@Slf4j
@Service
public class CusKmMxServiceImpl {
     ...
     * 查询客户科目明细账
     **/
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue,
        ...
        return codeKemuRepository.findByBcusAndIyearOrderByCcodeAsc("1",year).collectList()
                ...
                .flatMap(map->{
                    ...
                    codeList.stream().forEach(ck->{
                        ...
                        // ************* 获取科目凭证 *************
                        List<DeptCodeAccvoucherVo> findByCodePzList = pzlist.stream().filter(pz -> pz.getCcode().equals(ck.getCcode()) && Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate)).collect(Collectors.toList());
                    ...
                    BigDecimal yuenum=mapList.stream().filter(f->f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_num).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal yuenfrat=mapList.stream().filter(f->f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_nfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
    ...
    public Mono<R> exportAll(String strDate,String endDate,String bz,String ishaveRjz,String styleValue,List<String> cusList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        ...
        return codeKemuRepository.findByBcusAndIyearOrderByCcodeAsc("1",year).collectList()
                                            ...
                                            return customerRepository.findAllOrderByCustCode().collectList()
                                                    .flatMap(cuslist->{
                                                        return customerRepository.findAllOrderByCustCode2().collectList()
                                                            .flatMap(pzCusCodeList->{
                                                                return Mono.just(
                                                                        CollectOfUtils.mapof("qclist",qcmap.get("tablesData"),
                                                                                "deptAccVoucherList", finalNewaccvoucherlist,"codeList", finalNewDeptCodeList,"cuslist",cuslist,"pzCusCodeList",pzCusCodeList)
                                                                );
                                                            });
                                                    });
                                        });
                ...
                .flatMap(map->{
                    ...
                    List<SubjectInitialBalanceVo> qclist = (List<SubjectInitialBalanceVo>) map.get("qclist");
                    List<DeptCodeAccvoucherVo> pzlist= (List<DeptCodeAccvoucherVo>) map.get("deptAccVoucherList");
                    List<CustomerVo> pzCusCodeList= (List<CustomerVo>) map.get("pzCusCodeList");
                    List<Customer> cuslist= (List<Customer>) map.get("cuslist");

                    cusList.forEach(cus->{
                        Customer customer = cuslist.stream().filter(v -> v.getUniqueCode().equals(cus)).collect(Collectors.toList()).get(0);
                        List<String> ccodelist = pzCusCodeList.stream().filter(pzsup -> pzsup.getUniqueCode().equals(cus)).map(code -> code.getCcode()).collect(Collectors.toList());

                        List<KeMuMingXiBigDecimalVo> mapList = new ArrayList<>();
                        ccodelist.stream().forEach(code->{
                            AtomicReference<BigDecimal> lastYue= new AtomicReference<>(new BigDecimal(0));
                            AtomicReference<BigDecimal> lastYue_num= new AtomicReference<>(new BigDecimal(0));
                            AtomicReference<BigDecimal> lastYue_nfrat= new AtomicReference<>(new BigDecimal(0));
                            AtomicReference<BigDecimal> QcCode= new AtomicReference<>(new BigDecimal(0));
                            ...
                            // ************* 获取科目期初 *************
                            List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCcusId()) && qc.getCcode().equals(ck.getCcode()) && qc.getCcusId().equals(cus)).collect(Collectors.toList());
                            BigDecimal qcmd=findByCodeQcList.size()>0?findByCodeQcList.get(0).getMd():new BigDecimal(0);
                            BigDecimal qcmc=findByCodeQcList.size()>0?findByCodeQcList.get(0).getMc():new BigDecimal(0);
                            BigDecimal qcndS=findByCodeQcList.size()>0?findByCodeQcList.get(0).getNdS():new BigDecimal(0);
                            BigDecimal qcncS=findByCodeQcList.size()>0?findByCodeQcList.get(0).getNcS():new BigDecimal(0);
                            ...
                            if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                                String lastmonth = Integer.valueOf(month) - 1 < 10 ? "0" + (Integer.valueOf(month) - 1) : month;
                                // 获取上个月凭证发生数
                                List<DeptCodeAccvoucherVo> lastMonthAccVoucher = pzlist.stream().filter(acv ->StrUtil.isNotBlank(acv.getCcusId()) && acv.getCcusId().equals(cus)&& Integer.valueOf(acv.getImonth()) <= Integer.valueOf(lastmonth) && acv.getCcode().equals(ck.getCcode())).collect(Collectors.toList());
                                lastmd=lastMonthAccVoucher.size()>0?lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                                lastmc=lastMonthAccVoucher.size()>0?lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                                lastndS=lastMonthAccVoucher.size()>0?lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                                lastncS=lastMonthAccVoucher.size()>0?lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            ...
                            if(yue.compareTo(BigDecimal.ZERO)!=0){
                                mapList.add(
                                    new KeMuMingXiBigDecimalVo()
                                        ...
                                        .setYue_num(yue_num.compareTo(BigDecimal.ZERO) < 0?yue_num.multiply(new BigDecimal(-1)):yue_num)
                                        .setYue_nfrat(yue_nfrat.compareTo(BigDecimal.ZERO) < 0?yue_nfrat.multiply(new BigDecimal(-1)):yue_nfrat)
                                        .setTemp1(customer.getUniqueCode())
                                        .setTemp2(customer.getCustName())
                                );
                            }
                            // ************* 获取科目凭证 *************
                            List<DeptCodeAccvoucherVo> findByCodePzList = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCcusId()) && pz.getCcusId().equals(cus)&& pz.getCcode().equals(ck.getCcode()) && Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate)).collect(Collectors.toList());
                            for (int i = 0; i < findByCodePzList.size(); i++) {
                                BigDecimal pzmd=findByCodePzList.size()>0?findByCodePzList.get(i).getMd():new BigDecimal(0);
                                BigDecimal pzmc=findByCodePzList.size()>0?findByCodePzList.get(i).getMc():new BigDecimal(0);
                                BigDecimal pzndS=findByCodePzList.size()>0?findByCodePzList.get(i).getNdS():new BigDecimal(0);
                        ...
                        // 总计
                        BigDecimal totalMd=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalMc=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalNds=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalNcs=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalNfratMd=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNfrat_md).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalNfratMc=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNfrat_mc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalYue=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getTempyue).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal yuenum=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_num).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal yuenfrat=mapList.stream().filter(f->f.getTemp1().equals(cus)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_nfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        String bprogerty=totalYue.compareTo(BigDecimal.ZERO) == 0 ? "平" : totalYue.compareTo(BigDecimal.ZERO) > 0 ? "借" : "贷";
                        if(totalMd.compareTo(BigDecimal.ZERO)!=0 || totalMc.compareTo(BigDecimal.ZERO)!=0) {
                            mapList.add(new KeMuMingXiBigDecimalVo()
                                    .setInoIdASC(0)
                        ...
                        excelvo.put("ccode",customer.getCustCode());
                        excelvo.put("ccodeName",customer.getCustName());
    ...
    public Mono<R> exportAll2(String strDate,String endDate,String bz,String ishaveRjz,String styleValue,List<String> cusList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        List<KeMuMingXiBigDecimalVo> mapList = new ArrayList<>();
        ...
        return codeKemuRepository.findByBcusAndIyearOrderByCcodeAsc("1",year).collectList()
            .flatMap(deptCodeList->{
                ...
                .flatMap(map->{
                    ...
                    cusList.forEach(cus->{
                        List<CustomerVo> collect = pzCusCodeList.stream().filter(pzcus -> pzcus.getUniqueCode().equals(cus)).collect(Collectors.toList());
                        if(collect.size()>0){

                            CustomerVo customerVo = collect.get(0);
                            CodeKemu ck = codeList.stream().filter(f -> f.getCcode().equals(customerVo.getCcode())).collect(Collectors.toList()).get(0);
                            ...
                            if(yue.compareTo(BigDecimal.ZERO)!=0){
                                mapList.add(
                                        new KeMuMingXiBigDecimalVo()
                                            ...
                                            .setYue_num(yue_num.compareTo(BigDecimal.ZERO) < 0?yue_num.multiply(new BigDecimal(-1)):yue_num)
                                            .setYue_nfrat(yue_nfrat.compareTo(BigDecimal.ZERO) < 0?yue_nfrat.multiply(new BigDecimal(-1)):yue_nfrat)
                                            .setTemp1(customerVo.getUniqueCode())
                                            .setTemp2(customerVo.getCustName())
                                            .setTemp3(customerVo.getCustCode())
                                );
                            }
