...
@Slf4j
@Service
public class KmCusMxServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String dept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        return customerRepository.findAllOrderByCustCode().collectList()
            .flatMap(deptlist->{
                ...
                return accvoucherRepository.findAllByAccVoucherCus(year,km+"%",year+"01",endDate).collectList()
                    .flatMap(accdeptlist->{
                        ...
                        if (StringUtils.isNotBlank(dept)) {
                            newaccdeptlist = accdeptlist.stream().filter(vo -> vo.getCcusId().equals(dept)).collect(Collectors.toList());
                        }
                        // 获取期初
            ...
            .flatMap(map->{
                ...
                List<Customer> deptlist = (List<Customer>) map.get("deptlist");
                ...
                deptlist.stream().forEach(d->{
                    ...
                    // ************* 获取科目期初 *************
                    List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCcusId()) && qc.getCcode().equals(km) && qc.getCcusId().equals(d.getUniqueCode())).collect(Collectors.toList());
                    ...
                    if(yue.compareTo(BigDecimal.ZERO)!=0){
                        mapList.add(
                            new KeMuMingXiBigDecimalVo()
                                    .setNumber(number)
                                    .setInoIdASC(0)
                                    .setCcode(d.getCustCode())
                                    .setCcodeName(d.getCustName())
                                    .setCdigest("期初余额")
                    ...
                    // ************* 获取部门发生凭证 *************
                    List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate) && pz.getCcusId().equals(d.getUniqueCode())).collect(Collectors.toList());
    ...
    public Mono<R> exportAll(String strDate, String endDate, String bz, String ishaveRjz, String styleValue,List<String> ccodeList) {
        ...
        return customerRepository.findAllOrderByCustCode2().collectList()
                .flatMap(deptlist->{
                    List<CustomerVo> finalNewdeptlist = deptlist;
                ...
                .flatMap(map->{
                    ...
                    List<SubjectInitialBalanceVo> qclist = (List<SubjectInitialBalanceVo>) map.get("qclist");
                    List<DeptCodeAccvoucherVo> pzlist = (List<DeptCodeAccvoucherVo>) map.get("accdeptlist");
                    List<CustomerVo> deptlist = (List<CustomerVo>) map.get("deptlist");
                    ...
                    ccodeList.stream().forEach(km->{
                        ...
                        List<CustomerVo> cusvolist = deptlist.stream().filter(v -> v.getCcode().equals(km)).collect(Collectors.toList());
                        cusvolist.stream().forEach(d->{
                            BigDecimal codeTotalMd=new BigDecimal(0);
                            BigDecimal codeTotalMc=new BigDecimal(0);
                            BigDecimal codeTotalNds=new BigDecimal(0);
                            BigDecimal codeTotalNcs=new BigDecimal(0);
                            ...
                            // ************* 获取部门发生凭证 *************
                            List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCcusId())&&pz.getCcode().equals(codeinfo.getCcode())&& Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate) && pz.getCcusId().equals(d.getUniqueCode())).collect(Collectors.toList());
                            for (int i = 0; i < collect.size(); i++) {
                                BigDecimal pzmd=collect.size()>0?collect.get(i).getMd():new BigDecimal(0);
                                BigDecimal pzmc=collect.size()>0?collect.get(i).getMc():new BigDecimal(0);
                                BigDecimal pzndS=collect.size()>0?collect.get(i).getNdS():new BigDecimal(0);
                ...
                .flatMap(map->{
                    ...
                    ccodeList.stream().forEach(km->{
                        ...
                        cusvolist.stream().forEach(d->{
                            ...
                            // ************* 获取部门发生凭证 *************
                            List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCcusId())&& Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate) && pz.getCcusId().equals(d.getUniqueCode())).collect(Collectors.toList());
                            for (int i = 0; i < collect.size(); i++) {
                                BigDecimal pzmd=collect.size()>0?collect.get(i).getMd():new BigDecimal(0);
                                BigDecimal pzmc=collect.size()>0?collect.get(i).getMc():new BigDecimal(0);
                                BigDecimal pzndS=collect.size()>0?collect.get(i).getNdS():new BigDecimal(0);
