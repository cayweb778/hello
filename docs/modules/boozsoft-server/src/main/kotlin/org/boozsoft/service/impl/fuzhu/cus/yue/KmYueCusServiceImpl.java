...
@Slf4j
@Service
public class KmYueCusServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        return customerRepository.findAllOrderByCustCode().collectList()
                .flatMap(pzcuslist->{
                    List<Customer> newpzcuslist=pzcuslist;
                    if (StringUtils.isNotBlank(minDept)) {
                        List<String> strings = Arrays.asList(minDept.split(","));
                        newpzcuslist = pzcuslist.stream().filter(vo ->strings.indexOf(vo.getUniqueCode()) >-1).collect(Collectors.toList());
                    }
                    // 获取 已发生客户辅助核算的凭证
                    List<Customer> finalNewpzcuslist = newpzcuslist;
                    return accvoucherRepository.findAllByAccVoucherCus(year,km,year+"01",endDate).collectList()
                                    ...
                                    .flatMap(qclist -> {
                                        ...
                                        return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                            .flatMap(code->{
                                                ...
                                                return Mono.just(
                                                        CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzcuslist", finalNewpzcuslist,"code",newdeptCodeList.get(0))
                                                );
                                            });
                                });
                            });
                ...
                .flatMap(map->{
                    ...
                    List<Customer> pzcuslist = (List<Customer>) map.get("pzcuslist");
                    ...
                    pzcuslist.stream().forEach(cus->{
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        String bprogerty = code.getBprogerty();  // 方向
                        // ************* 获取此科目下的客户期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCcusId()) && qc.getCcode().equals(km)&&qc.getCcusId().equals(cus.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth) && last.getCcode().equals(code.getCcode())&&last.getCcusId().equals(cus.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 检索科目获取凭证
                        List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCcusId().equals(cus.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 获取累计发生 【月份：<= endDate】
                        List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCcusId().equals(cus.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                            ...
                            vo.setUniqueCode(cus.getUniqueCode()).setCcode(cus.getCustCode()).setCcodeName(cus.getCustName())
     ...
     * @return
     */
    public Mono<R> exportAll(Pageable pageable,String minDept,String strDate, String endDate, String bz, String ishaveRjz, String styleValue,List<String>ccodeList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        AtomicReference<Long> totalAR = new AtomicReference(0);
        // 封装list
        ...
        // 获取 已发生凭证的客户辅助核算
        return customerRepository.findAllOrderByCustCode2().collectList()
                .flatMap(pzcuslist->{
                    List<CustomerVo> newpzcuslist=pzcuslist;
                    if (StringUtils.isNotBlank(minDept)) {
                        List<String> strings = Arrays.asList(minDept.split(","));
                        newpzcuslist = pzcuslist.stream().filter(vo ->strings.indexOf(vo.getUniqueCode()) >-1).collect(Collectors.toList());
                    }
                    // 获取 已发生客户辅助核算的凭证
                    List<CustomerVo> finalNewpzcuslist = newpzcuslist;
                    return accvoucherRepository.findAllByAccVoucherCus2(year,year+"01",endDate).collectList()
                            .flatMap(accvoucherlist->{
                                List<DeptCodeAccvoucherVo> newaccvoucherlist = accvoucherlist;
                                // 根据币种查询凭证
                                if (!"全部".equals(bz)) {
                                ...
                                return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false", year + "21", "")
                                        .flatMap(qclist -> {
                                            ...
                                            return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                                    .flatMap(code->{
                                                        return Mono.just(
                                                                CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzcuslist", finalNewpzcuslist,"code",code)
                                                        );
                                                    });
                                        });
                            });
                ...
                .flatMap(map->{
                    List<CodeKemu> findBycodelist = (List<CodeKemu>) map.get("code");
                    List<CustomerVo> pzcuslist = (List<CustomerVo>) map.get("pzcuslist");
                    List<DeptCodeAccvoucherVo> pzlist = (List<DeptCodeAccvoucherVo>) map.get("pzlist");
                    List<SubjectInitialBalanceVo> qclist = (List<SubjectInitialBalanceVo>) map.get("qclist");

                    ccodeList.stream().forEach(ck->{
                        ArrayList<KeMuBalanceVo> mapList = new ArrayList<>();
                        List<CodeKemu> collect = findBycodelist.stream().filter(v -> v.getCcode().equals(ck)).collect(Collectors.toList());
                        CodeKemu code=collect.get(0);

                        List<CustomerVo> collect1 = pzcuslist.stream().filter(v -> v.getCcode().equals(ck)).collect(Collectors.toList());
                        collect1.stream().forEach(cus->{
                            KeMuBalanceVo vo = new KeMuBalanceVo();
                            String bprogerty = code.getBprogerty();  // 方向
                            // ************* 获取此科目下的客户期初 *************
                            List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCcusId()) && qc.getCcode().equals(code.getCcode())&&qc.getCcusId().equals(cus.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal qcMd = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMd() : new BigDecimal(0);
                            BigDecimal qcMc = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMc() : new BigDecimal(0);
                            // 数量
                            BigDecimal qcNdS = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getNdS() : new BigDecimal(0);
                            ...
                            if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                                ...
                                vo.setTemp1(code.getCcode()).setTemp2(code.getCcodeName())
                                        .setUniqueCode(cus.getUniqueCode()).setCcode(cus.getCustCode()).setCcodeName(cus.getCustName())
                                        .setCclass(code.getCclass()).setFindByBend(code.getBend()).setUnitMeasurement(code.getMenterage())
                                        .setForeignCurrency(code.getCurrencyType())
                                        .setQcMd(qcMd).setQcMc(qcMc).setQcNum(qcNum).setQcNfrat(qcNfrat)
                                        .setPzMd(pzMd).setPzMc(pzMc).setPzNum(pzNum).setPzNfrat(pzNfrat)
    ...
    public Mono<R> exportAll2(Pageable pageable,String minDept,String strDate, String endDate, String bz, String ishaveRjz, String styleValue,List<String>ccodeList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        ArrayList<KeMuBalanceVo> mapList = new ArrayList<>();
                ...
                .flatMap(map->{
                    ...
                    ccodeList.stream().forEach(ck->{
                        ...
                        collect1.stream().forEach(cus->{
                            ...
                            if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                                ...
                                vo.setUniqueCode(cus.getUniqueCode()).setCcode(cus.getCustCode()).setCcodeName(cus.getCustName())
                                        ...
                                        .setPzMd(pzMd).setPzMc(pzMc).setPzNum(pzNum).setPzNfrat(pzNfrat)
                                        .setLjMd(pzLjMd).setLjMc(pzLjMc).setLjNum(pzLjNum).setLjNfrat(pzLjNfrat)
                                        .setQmMd(qmMd).setQmMc(qmMc).setQmNum(qmNum).setQmNfrat(qmNfrat).setTemp1(code.getCcode()).setTemp2(code.getCcodeName());
                                mapList.add(vo);
                            }
                        });
                        BigDecimal totalQcMdTotal=mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQcMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcMcTotal=mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQcMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcNumTotal=mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQcNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcNfratTotal=mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQcNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalPzMdTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getPzMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzMcTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getPzMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNumTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getPzNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNfratTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getPzNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalLjMdTotal=mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getLjMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjMcTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getLjMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjNumTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getLjNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);;
                        BigDecimal totalLjNfratTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getLjNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);;

                        BigDecimal totalQmMdTotal=  mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQmMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmMcTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQmMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmNumTotal=  mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQmNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmNfratTotal= mapList.stream().filter(v->v.getTemp1().equals(code.getCcode())).map(KeMuBalanceVo::getQmNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        ...
                        vo.setCcode("合计").setFindByBend("0")
                                ...
                                .setPzMd(totalPzMdTotal).setPzMc(totalPzMcTotal).setPzNum(totalPzNumTotal).setPzNfrat(totalPzNfratTotal)
                                .setLjMd(totalLjMdTotal).setLjMc(totalLjMcTotal).setLjNum(totalLjNumTotal).setLjNfrat(totalLjNfratTotal)
                                .setQmMd(total_qmyuemd).setQmMc(total_qmyuemc).setQmNum(totalQmNumTotal).setQmNfrat(totalQmNfratTotal).setTemp1(code.getCcode()).setTemp2(code.getCcodeName());
                        mapList.add(vo);
                    });
