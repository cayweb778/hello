package org.boozsoft.service.impl.fuzhu.dept.yue;
...
@Slf4j
@Service
public class KmYueDeptServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        return departmentRepository.findAll().collectList()
                .flatMap(pzdeptlist->{
                    List<SysDepartment> newpzdeptlist=pzdeptlist;
                    if (StringUtils.isNotBlank(minDept)) {
                        List<String> strings = Arrays.asList(minDept.split(","));
                        newpzdeptlist = pzdeptlist.stream().filter(vo -> strings.indexOf(vo.getUniqueCode())>-1).collect(Collectors.toList());
                    }
                    List<SysDepartment> finalNewpzdeptlist = newpzdeptlist;
                    // 获取 已发生项目辅助核算的凭证
                    return accvoucherRepository.findAllByAccVoucherDept(year,km,year+"01",endDate).collectList()
                                    ...
                                    .flatMap(qclist -> {
                                        ...
                                        return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                            .flatMap(code->{
                                                ...
                                                return Mono.just(
                                                        CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzdeptlist", finalNewpzdeptlist,"code",newdeptCodeList.get(0))
                                                );
                                            });
                                });
                            });
                ...
                .flatMap(map->{
                    ...
                    List<SysDepartment> pzprolist = (List<SysDepartment>) map.get("pzdeptlist");
                    ...
                    pzprolist.stream().forEach(dept->{
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        String bprogerty = code.getBprogerty();  // 方向
                        // ************* 获取此科目下的客户期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCdeptId()) && qc.getCcode().equals(km)&&qc.getCdeptId().equals(dept.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth) && last.getCcode().equals(code.getCcode())&&last.getCdeptId().equals(dept.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 检索科目获取凭证
                        List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCdeptId().equals(dept.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 获取累计发生 【月份：<= endDate】
                        List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCdeptId().equals(dept.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                            ...
                            vo.setUniqueCode(dept.getUniqueCode()).setCcode(dept.getDeptCode()).setCcodeName(dept.getDeptName())
                    ...
                    return Mono.just(mapList.stream().filter(item->{
                        ...
                        // 过滤漏斗-----------------期初借方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinQcMd")) && StringUtils.isNotBlank(filterMap.get("amountMaxQcMd"))) {
                        ...
                        // 过滤漏斗-----------------期初贷方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinQcMc")) && StringUtils.isNotBlank(filterMap.get("amountMaxQcMc"))) {
                        ...
                        // 过滤漏斗-----------------凭证借方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinPzMd")) && StringUtils.isNotBlank(filterMap.get("amountMaxPzMd"))) {
                        ...
                        // 过滤漏斗-----------------凭证dai方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinPzMc")) && StringUtils.isNotBlank(filterMap.get("amountMaxPzMc"))) {
                        ...
                        // 过滤漏斗-----------------期末借方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinQmMd")) && StringUtils.isNotBlank(filterMap.get("amountMaxQmMd"))) {
                        ...
                        // 过滤漏斗-----------------期末dai方
                        if (StringUtils.isNotBlank(filterMap.get("amountMinQmMc")) && StringUtils.isNotBlank(filterMap.get("amountMaxQmMc"))) {
