...
@Slf4j
@Service
public class KmPsnMxServiceImpl {
     ...
     * 查询科目明细账
     **/
    public Mono<R> findAll(Pageable pageable,String dept, String km, String strDate, String endDate, String bz, String ishaveRjz,
        ...
        return sysPsnRepository.findAllOrderByPsnCode().collectList()
                .flatMap(deptlist->{
                    List<SysPsn> newdeptlist=deptlist;
                    if(StringUtils.isNotBlank(dept)){
                        newdeptlist=deptlist.stream().filter(p->p.getUniqueCode().equals(dept)).collect(Collectors.toList());
                    }
                    List<SysPsn> finalNewdeptlist = newdeptlist;
                ...
                .flatMap(map->{
                    ...
                    List<SysPsn> deptlist = (List<SysPsn>) map.get("deptlist");
                    ...
                    deptlist.stream().forEach(d->{
                        ...
                        // ************* 获取科目期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(km) && qc.getCpersonId().equals(d.getUniqueCode()) ).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            // 获取上个月凭证发生数
                            List<DeptCodeAccvoucherVo> lastMonthAccVoucher = pzlist.stream().filter(acv -> Integer.valueOf(acv.getImonth()) <= Integer.valueOf(lastmonth) && acv.getCcode().equals(codeinfo.getCcode()) && acv.getCpersonId().equals(d.getUniqueCode()) ).collect(Collectors.toList());
                        ...
                        if(yue.compareTo(BigDecimal.ZERO)!=0){
                            mapList.add(
                                    new KeMuMingXiBigDecimalVo()
                                            .setNumber(number)
                                            .setInoIdASC(0)
                                            .setCcode(d.getPsnCode())
                                            .setCcodeName(d.getPsnName())
                                            .setCdigest("期初余额")
                        ...
                        // ************* 获取部门发生凭证 *************
                        List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate) && pz.getCpersonId().equals(d.getUniqueCode())).collect(Collectors.toList());
    ...
    public Mono<R> exportAll(String strDate, String endDate, String bz, String ishaveRjz,String styleValue,List<String> ccodeList) {
        ...
        return sysPsnRepository.findAllOrderByPsnCode2().collectList()
                .flatMap(deptlist->{
                    List<SysPsnVo> finalNewdeptlist = deptlist;
                    // 2 获取科目发生的部门核算凭证
                                ...
                                return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false",year+"21","")
                                        .flatMap(qclist->{
                                            ...
                                            return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                                    .flatMap(ccodelist->{
                                                        return Mono.just(
                                                                CollectOfUtils.mapof("qclist",qcmap.get("tablesData"),"accdeptlist", finalNewaccdeptlist,"deptlist", finalNewdeptlist,"ccodelist",ccodelist)
                                                        );
                                                    });
                                        });
                            });
                ...
                .flatMap(map->{
                    List<SubjectInitialBalanceVo> qclist = (List<SubjectInitialBalanceVo>) map.get("qclist");
                    List<DeptCodeAccvoucherVo> pzlist = (List<DeptCodeAccvoucherVo>) map.get("accdeptlist");
                    List<SysPsnVo> deptlist = (List<SysPsnVo>) map.get("deptlist");
                    ...
                    ccodeList.stream().forEach(km->{
                        ...
                        List<SysPsnVo> psnvolist = deptlist.stream().filter(v -> v.getCcode().equals(km)).collect(Collectors.toList());
                        psnvolist.stream().forEach(d->{
                            BigDecimal codeTotalMd=new BigDecimal(0);
                            BigDecimal codeTotalMc=new BigDecimal(0);
                            BigDecimal codeTotalNds=new BigDecimal(0);
                            BigDecimal codeTotalNcs=new BigDecimal(0);
                            ...
                            // ************* 获取部门发生凭证 *************
                            List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCpersonId())&& pz.getCpersonId().equals(d.getUniqueCode())&&pz.getCcode().equals(km)&& Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate)).collect(Collectors.toList());
                            for (int i = 0; i < collect.size(); i++) {
                                BigDecimal pzmd=collect.size()>0?collect.get(i).getMd():new BigDecimal(0);
                                BigDecimal pzmc=collect.size()>0?collect.get(i).getMc():new BigDecimal(0);
                                BigDecimal pzndS=collect.size()>0?collect.get(i).getNdS():new BigDecimal(0);
