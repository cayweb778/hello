package org.boozsoft.service.impl.fuzhu.person.mx;
...
@Slf4j
@Service
public class PsnKmMxServiceImpl {
     ...
     * 查询科目明细账
     **/
    public Mono<R> findAll(Pageable pageable,String minPsn, String km, String strDate, String endDate, String bz,
                           String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        AtomicReference<Long> totalAR = new AtomicReference(0);
        // 1、获取个人辅助核算的科目
        return codeKemuRepository.findByBpersonAndIyearOrderByCcodeAsc("1",year).collectList()
                .flatMap(deptCodeList->{
                    ...
                    return accvoucherRepository.findAllByIyearAndPsnId(year,minPsn+"%",year+"01",endDate).collectList()
                ...
                .flatMap(map->{
                    ...
                    codeList.stream().forEach(ck->{
                        ...
                        // ************* 获取科目期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(ck.getCcode()) && qc.getCpersonId().equals(minPsn)).collect(Collectors.toList());
    ...
    public Mono<R> exportAll(String strDate, String endDate, String bz,String ishaveRjz, String styleValue,List<String> psnList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        ...
        return codeKemuRepository.findByBpersonAndIyearOrderByCcodeAsc("1",year).collectList()
                        ...
                        return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false",year+"21","")
                            .flatMap(qclist->{
                                ...
                                return sysPsnRepository.findAllByFlag("1").collectList()
                                    .flatMap(psnlist->{
                                        return sysPsnRepository.findAllOrderByPsnCode2().collectList()
                                            .flatMap(pzPsnCodeList->{
                                                return Mono.just(
                                                        CollectOfUtils.mapof("qclist",qcmap.get("tablesData"),
                                                                "deptAccVoucherList", finalNewaccvoucherlist,
                                                                "codeList", finalNewDeptCodeList,"psnlist",psnlist,"pzPsnCodeList",pzPsnCodeList)
                                                );
                                            });
                                    });
                            });
            ...
            .flatMap(map->{
                ...
                List<SysPsnVo> pzPsnCodeList= (List<SysPsnVo>) map.get("pzPsnCodeList");

                psnList.stream().forEach(psn->{
                    SysPsn sysPsn = psnlist.stream().filter(v -> v.getUniqueCode().equals(psn)).collect(Collectors.toList()).get(0);
                    List<String> collect = pzPsnCodeList.stream().filter(pzpsn -> pzpsn.getUniqueCode().equals(psn)).map(code -> code.getCcode()).collect(Collectors.toList());
                    List<KeMuMingXiBigDecimalVo> mapList = new ArrayList<>();
                    ...
                    collect.stream().forEach(code->{
                        ...
                        // ************* 获取科目期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(ck.getCcode()) && qc.getCpersonId().equals(psn)).collect(Collectors.toList());
                        BigDecimal qcmd=findByCodeQcList.size()>0?findByCodeQcList.get(0).getMd():new BigDecimal(0);
                        BigDecimal qcmc=findByCodeQcList.size()>0?findByCodeQcList.get(0).getMc():new BigDecimal(0);
                        BigDecimal qcndS=findByCodeQcList.size()>0?findByCodeQcList.get(0).getNdS():new BigDecimal(0);
                        BigDecimal qcncS=findByCodeQcList.size()>0?findByCodeQcList.get(0).getNcS():new BigDecimal(0);
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            String lastmonth = Integer.valueOf(month) - 1 < 10 ? "0" + (Integer.valueOf(month) - 1) : month;
                            // 获取上个月凭证发生数
                            List<DeptCodeAccvoucherVo> lastMonthAccVoucher = pzlist.stream().filter(acv ->StrUtil.isNotBlank(acv.getCpersonId()) && acv.getCpersonId().equals(psn)&& Integer.valueOf(acv.getImonth()) <= Integer.valueOf(lastmonth) && acv.getCcode().equals(ck.getCcode())).collect(Collectors.toList());
                            lastmd = lastMonthAccVoucher.size() > 0 ? lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            lastmc = lastMonthAccVoucher.size() > 0 ? lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            lastndS = lastMonthAccVoucher.size() > 0 ? lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            lastncS = lastMonthAccVoucher.size() > 0 ? lastMonthAccVoucher.stream().map(DeptCodeAccvoucherVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                        ...
                        if(yue.compareTo(BigDecimal.ZERO)!=0){
                            mapList.add(
                                new KeMuMingXiBigDecimalVo()
                                    ...
                                    .setYue_num(yue_num.compareTo(BigDecimal.ZERO) < 0?yue_num.multiply(new BigDecimal(-1)):yue_num)
                                    .setYue_nfrat(yue_nfrat.compareTo(BigDecimal.ZERO) < 0?yue_nfrat.multiply(new BigDecimal(-1)):yue_nfrat)
                                    .setTemp1(sysPsn.getUniqueCode())
                                    .setTemp2(sysPsn.getPsnName())
                            );
                        }

                        // ************* 获取科目凭证 *************
                        List<DeptCodeAccvoucherVo> findByCodePzList = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCpersonId()) && pz.getCpersonId().equals(psn)&& pz.getCcode().equals(ck.getCcode()) && Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate)).collect(Collectors.toList());
                    ...
                    // 总计
                    BigDecimal totalMd=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal totalMc=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal totalNds=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal totalNcs=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal totalNfratMd=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNfrat_md).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal totalNfratMc=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getNfrat_mc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                    BigDecimal totalYue=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getTempyue).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal yuenum=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_num).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    BigDecimal yuenfrat=mapList.stream().filter(f->f.getTemp1().equals(psn)&&f.getCdigest().equals("当前合计")).map(KeMuMingXiBigDecimalVo::getYue_nfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                    ...
                    excelvo.put("ccode",sysPsn.getPsnCode());
                    excelvo.put("ccodeName",sysPsn.getPsnName());
    ...
    public Mono<R> exportAll2(String strDate, String endDate, String bz,String ishaveRjz, String styleValue,List<String> psnList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        List<KeMuMingXiBigDecimalVo> mapList = new ArrayList<>();
