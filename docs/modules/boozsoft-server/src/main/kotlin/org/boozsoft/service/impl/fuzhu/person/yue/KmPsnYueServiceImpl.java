...
@Slf4j
@Service
public class KmPsnYueServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate,
                           String endDate, String bz, String ishaveRjz, String styleValue,
        ...
        // 获取 已发生凭证的个人辅助核算
        return sysPsnRepository.findAllOrderByPsnCode().collectList()
                .flatMap(pzcuslist->{
                    List<SysPsn> newpzcuslist=pzcuslist;
                    ...
                    List<SysPsn> finalNewpzcuslist = newpzcuslist;
                    // 获取 已发生个人辅助核算的凭证
                    return accvoucherRepository.findAllByAccVoucherPsn(year,km,year+"01",endDate).collectList()
                                    ...
                                    .flatMap(qclist -> {
                                        ...
                                        return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                            .flatMap(code->{
                                                ...
                                                return Mono.just(
                                                        CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzpsnlist", finalNewpzcuslist,"code",newdeptCodeList.get(0))
                                                );
                                            });
                                });
                            });
                ...
                .flatMap(map->{
                    ...
                    List<SysPsn> pzpsnlist = (List<SysPsn>) map.get("pzpsnlist");
                    ...
                    pzpsnlist.stream().forEach(psn->{
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        String bprogerty = code.getBprogerty();  // 方向
                        // ************* 获取此科目下的客户期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(km)&&qc.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth) && last.getCcode().equals(code.getCcode())&&last.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 检索科目获取凭证
                        List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 获取累计发生 【月份：<= endDate】
                        List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                            ...
                            vo.setUniqueCode(psn.getUniqueCode()).setCcode(psn.getPsnCode()).setCcodeName(psn.getPsnName())
    ...
    public Mono<R> exportAll(String strDate,String endDate, String bz, String ishaveRjz,
                             String styleValue,List<String>ccodeList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
        ...
        return sysPsnRepository.findAllOrderByPsnCode2().collectList()
                .flatMap(pzcuslist->{
                    List<SysPsnVo> finalNewpzcuslist = pzcuslist;
                    // 获取 已发生个人辅助核算的凭证
                                ...
                                return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false", year + "21", "")
                                        .flatMap(qclist -> {
                                            ...
                                            return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                                    .flatMap(codelist->{
                                                        return Mono.just(
                                                                CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzpsnlist", finalNewpzcuslist,"codelist",codelist)
                                                        );
                                                    });
                                        });
                            });
                ...
                .flatMap(map->{
                    ...
                    List<SysPsnVo> pzpsnlist = (List<SysPsnVo>) map.get("pzpsnlist");
                    List<DeptCodeAccvoucherVo> pzlist = (List<DeptCodeAccvoucherVo>) map.get("pzlist");
                    List<SubjectInitialBalanceVo> qclist = (List<SubjectInitialBalanceVo>) map.get("qclist");
                    ...
                    ccodeList.stream().forEach(ck->{
                        ...
                        pzpsnlist.stream().filter(v->v.getCcode().equals(ck)).forEach(psn->{
                            KeMuBalanceVo vo = new KeMuBalanceVo();
                            String bprogerty = code.getBprogerty();  // 方向
                            // ************* 获取此科目下的客户期初 *************
                            List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(psn.getUniqueCode())&&qc.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal qcMd = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMd() : new BigDecimal(0);
                            BigDecimal qcMc = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMc() : new BigDecimal(0);
                            // 数量
                            BigDecimal qcNdS = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getNdS() : new BigDecimal(0);
                            ...
                            // 检索科目获取凭证
                            List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCpersonId()) && Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal pzMd =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzMc =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzNfratMd =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzNfratMc =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            ...
                            // 获取累计发生 【月份：<= endDate】
                            List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCpersonId()) &&  Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal pzLjMd =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjMc =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjNfratMd =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjNfratMc =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            ...
                            if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                                ...
                                vo.setTemp1(code.getCcode()).setTemp2(code.getCcodeName())
                                        .setUniqueCode(psn.getUniqueCode()).setCcode(psn.getPsnCode()).setCcodeName(psn.getPsnName())
                                        .setCclass(code.getCclass()).setFindByBend(code.getBend()).setUnitMeasurement(code.getMenterage())
                                        .setForeignCurrency(code.getCurrencyType())
                                        .setQcMd(qcMd).setQcMc(qcMc).setQcNum(qcNum).setQcNfrat(qcNfrat)
                                        .setPzMd(pzMd).setPzMc(pzMc).setPzNum(pzNum).setPzNfrat(pzNfrat)
    ...
    public Mono<R> exportAll2(String strDate,String endDate, String bz, String ishaveRjz,
                             String styleValue,List<String>ccodeList) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 封装list
