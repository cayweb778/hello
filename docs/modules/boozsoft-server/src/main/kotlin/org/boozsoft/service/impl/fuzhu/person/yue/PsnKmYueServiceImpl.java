package org.boozsoft.service.impl.fuzhu.person.yue;
...
@Slf4j
@Service
public class PsnKmYueServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable, String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap, String database) {
        ...
        // 1、获取个人辅助核算的科目
        return codeKemuRepository.findByBpersonAndIyearOrderByCcodeAsc("1", year).collectList().flatMap(deptCodeList -> {
                    ...
                    // 查询个人辅助核算凭证
                    return accvoucherRepository.findAllByIyearAndPsnId(year, minDept, year + "01", endDate).collectList().flatMap(accvoucherlist -> {
                        ...
                        return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false", year + "21", database).flatMap(qclist -> {
                            ...
                            return Mono.just(CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "deptAccVoucherList", finalNewaccvoucherlist, "codeList", finalNewDeptCodeList));
                        });
                    });
                })
                // 获取部门科目凭证
                .flatMap(map -> {
                    ...
                    codeList.stream().forEach(ck -> {
                        ...
                        // ************* 获取科目期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(ck.getCcode()) && qc.getCpersonId().equals(minDept)).collect(Collectors.toList());
                        ...
                        if (qcMd.compareTo(BigDecimal.ZERO) == 0 && qcMc.compareTo(BigDecimal.ZERO) == 0 && pzMd.compareTo(BigDecimal.ZERO) == 0 && pzMc.compareTo(BigDecimal.ZERO) == 0 && pzLjMd.compareTo(BigDecimal.ZERO) == 0 && pzLjMc.compareTo(BigDecimal.ZERO) == 0) {
                            ...
                            vo.setCcode(ccode).setCcodeName(ccodeName).setCclass(cclass).setFindByBend(findByBend).setUnitMeasurement(unitMeasurement).setForeignCurrency(foreignCurrency).setQcMd(qcMd).setQcMc(qcMc).setQcNum(qcNum).setQcNfrat(qcNfrat).setPzMd(pzMd).setPzMc(pzMc).setPzNum(pzNum).setPzNfrat(pzNfrat).setLjMd(pzLjMd).setLjMc(pzLjMc).setLjNum(pzLjNum).setLjNfrat(pzLjNfrat).setQmMd(qmMd).setQmMc(qmMc).setQmNum(qmNum).setQmNfrat(qmNfrat);
                            mapList.add(vo);
                        }
                    });
    ...
    public Mono<R> exportAll(String strDate, String endDate, String bz, String ishaveRjz, String styleValue) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 1、获取个人辅助核算的科目
        return codeKemuRepository.findByBpersonAndIyearOrderByCcodeAsc("1", year).collectList().flatMap(deptCodeList -> {
                    ...
                    // 查询个人辅助核算凭证
                    return accvoucherRepository.findAllByIyearFuzhuAccvoucher(year, year + "01", endDate).collectList().flatMap(accvoucherlist -> {
                        List<DeptCodeAccvoucherVo> newaccvoucherlist = accvoucherlist;
                        // 根据币种查询凭证
                        if (!"全部".equals(bz)) {
                            newaccvoucherlist = accvoucherlist.stream().filter(a -> a.getForeignCurrency().equals(bz)).collect(Collectors.toList());
                        ...
                        return fuZhuBalanceService.findAllSubjectInitialFuZhuBalance(year, "false", year + "21", "").flatMap(qclist -> {
                            Map qcmap = (Map) qclist.getResult();
                            return sysPsnRepository.findAllByFlag("1").collectList().flatMap(psnlist -> {
                                return Mono.just(CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "deptAccVoucherList", finalNewaccvoucherlist, "codeList", finalNewDeptCodeList, "psnlist", psnlist));
                            });
                        });
                    });
                })
                ...
                .flatMap(map -> {
                    ...
                    psnlist.stream().forEach(psn -> {
                        ...
                        codeList.stream().forEach(ck -> {
                            ...
                            // ************* 获取科目期初 *************
                            List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCpersonId()) && qc.getCcode().equals(ck.getCcode()) && qc.getCpersonId().equals(psn.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal qcMd = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMd() : new BigDecimal(0);
                            BigDecimal qcMc = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMc() : new BigDecimal(0);
                            // 数量
                            BigDecimal qcNdS = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getNdS() : new BigDecimal(0);
                            ...
                            if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                                String lastmonth = Integer.valueOf(month) - 1 < 10 ? "0" + (Integer.valueOf(month) - 1) : month;
                                List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> last.getCpersonId().equals(psn.getUniqueCode()) && last.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth)).collect(Collectors.toList());
                                BigDecimal lastMd = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastMc = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastNdS = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastNcS = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            ...
                            // 检索科目获取凭证
                            List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz -> StrUtil.isNotBlank(pz.getCpersonId()) && pz.getCpersonId().equals(psn.getUniqueCode()) && pz.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)).collect(Collectors.toList());
                            BigDecimal pzMd = newpzlist.size() > 0 ? newpzlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzMc = newpzlist.size() > 0 ? newpzlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzNfratMd = newpzlist.size() > 0 ? newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzNfratMc = newpzlist.size() > 0 ? newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            ...
                            // 获取累计发生 【月份：<= endDate】
                            List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz -> StrUtil.isNotBlank(pz.getCpersonId()) && pz.getCpersonId().equals(psn.getUniqueCode()) && pz.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)).collect(Collectors.toList());
                            BigDecimal pzLjMd = pzljlist.size() > 0 ? pzljlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzLjMc = pzljlist.size() > 0 ? pzljlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzLjNfratMd = pzljlist.size() > 0 ? pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            BigDecimal pzLjNfratMc = pzljlist.size() > 0 ? pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                        ...
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        vo.setCcode("合计").setFindByBend("0").setQcMd(total_qcyuemd).setQcMc(total_qcyuemc).setQcNum(totalQcNumTotal).setQcNfrat(totalQcNfratTotal).setPzMd(totalPzMdTotal).setPzMc(totalPzMcTotal).setPzNum(totalPzNumTotal).setPzNfrat(totalPzNfratTotal).setLjMd(totalLjMdTotal).setLjMc(totalLjMcTotal).setLjNum(totalLjNumTotal).setLjNfrat(totalLjNfratTotal).setQmMd(total_qmyuemd).setQmMc(total_qmyuemc).setQmNum(totalQmNumTotal).setQmNfrat(totalQmNfratTotal);
                        mapList.add(vo);
                        Map<String, Object> excelvo = new HashMap<>();
                        excelvo.put("ccode", psn.getPsnCode());
                        excelvo.put("ccodeName", psn.getPsnName());
    ...
    public Mono<R> exportAll2(String strDate, String endDate, String bz, String ishaveRjz, String styleValue) {
        String year = strDate.substring(0, 4);
        String month = strDate.substring(4, 6);
        // 1、获取个人辅助核算的科目
        return codeKemuRepository.findByBpersonAndIyearOrderByCcodeAsc("1", year).collectList().flatMap(deptCodeList -> {
                    ...
                    psnlist.stream().forEach(psn -> {
                        codeList.stream().forEach(ck -> {
                            ...
                            if (qcMd.compareTo(BigDecimal.ZERO) == 0 && qcMc.compareTo(BigDecimal.ZERO) == 0 && pzMd.compareTo(BigDecimal.ZERO) == 0 && pzMc.compareTo(BigDecimal.ZERO) == 0 && pzLjMd.compareTo(BigDecimal.ZERO) == 0 && pzLjMc.compareTo(BigDecimal.ZERO) == 0) {
                                ...
                                vo.setCcode(ccode).setCcodeName(ccodeName).setTemp1(psn.getPsnCode()).setTemp2(psn.getPsnName()).setCclass(cclass).setFindByBend(findByBend).setUnitMeasurement(unitMeasurement).setForeignCurrency(foreignCurrency).setQcMd(qcMd).setQcMc(qcMc).setQcNum(qcNum).setQcNfrat(qcNfrat).setPzMd(pzMd).setPzMc(pzMc).setPzNum(pzNum).setPzNfrat(pzNfrat).setLjMd(pzLjMd).setLjMc(pzLjMc).setLjNum(pzLjNum).setLjNfrat(pzLjNfrat).setQmMd(qmMd).setQmMc(qmMc).setQmNum(qmNum).setQmNfrat(qmNfrat);
                                mapList.add(vo);
                            }
                        });
                        BigDecimal totalQcMdTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQcMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcMcTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQcMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalQcNumTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQcNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcNfratTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQcNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalPzMdTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getPzMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzMcTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getPzMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNumTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getPzNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNfratTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getPzNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalLjMdTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getLjMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjMcTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getLjMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjNumTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getLjNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        ;
                        BigDecimal totalLjNfratTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getLjNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        ;

                        BigDecimal totalQmNumTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQmNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmNfratTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQmNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalQmMdTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQmMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmMcTotal = mapList.stream().filter(f->f.getTemp1().equals(psn.getPsnCode())).map(KeMuBalanceVo::getQmMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        ...
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        if (total_qcyuemd.compareTo(BigDecimal.ZERO) == 0 && total_qcyuemc.compareTo(BigDecimal.ZERO) == 0 && totalPzMdTotal.compareTo(BigDecimal.ZERO) == 0 && totalPzMcTotal.compareTo(BigDecimal.ZERO) == 0 && total_qmyuemd.compareTo(BigDecimal.ZERO) == 0 && total_qmyuemc.compareTo(BigDecimal.ZERO) == 0) {
                        } else {
                            vo.setCcode("合计").setFindByBend("0").setTemp1("").setTemp2("").setQcMd(total_qcyuemd).setQcMc(total_qcyuemc).setQcNum(totalQcNumTotal).setQcNfrat(totalQcNfratTotal).setPzMd(totalPzMdTotal).setPzMc(totalPzMcTotal).setPzNum(totalPzNumTotal).setPzNfrat(totalPzNfratTotal).setLjMd(totalLjMdTotal).setLjMc(totalLjMcTotal).setLjNum(totalLjNumTotal).setLjNfrat(totalLjNfratTotal).setQmMd(total_qmyuemd).setQmMc(total_qmyuemc).setQmNum(totalQmNumTotal).setQmNfrat(totalQmNfratTotal);
                            mapList.add(vo);
                        }
                    });
