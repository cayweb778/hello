package org.boozsoft.service.impl.fuzhu.pro.yue1;
...
@Slf4j
@Service
public class KmYueProServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        return projectRepositoryBase.findAll().collectList()
                .flatMap(pzprolist->{
                    List<Project> newpzprolist=pzprolist;
                    if (StringUtils.isNotBlank(minDept)) {
                        List<String> strings = Arrays.asList(minDept.split(","));
                        newpzprolist = pzprolist.stream().filter(vo -> strings.indexOf(vo.getUniqueCode())>-1).collect(Collectors.toList());
                    }
                    List<Project> finalNewpzprolist = newpzprolist;
                    // 获取 已发生项目辅助核算的凭证
                                        ...
                                        .flatMap(qclist -> {
                                            ...
                                            return codeKemuRepository.findByIyearOrderByCcode(year).collectList()
                                                    .flatMap(code->{
                                                        ...
                                                        return Mono.just(
                                                                CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "pzlist", finalNewaccvoucherlist, "pzprolist", finalNewpzprolist,"code",newdeptCodeList.get(0))
                                                        );
                                                    });
                                        });
                            });
                ...
                .flatMap(map->{
                    ...
                    List<Project> pzprolist = (List<Project>) map.get("pzprolist");
                    ...
                    pzprolist.stream().forEach(pro->{
                        KeMuBalanceVo vo = new KeMuBalanceVo();
                        String bprogerty = code.getBprogerty();  // 方向
                        // ************* 获取此科目下的客户期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getProjectId()) && qc.getCcode().equals(km)&&qc.getProjectId().equals(pro.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth) && last.getCcode().equals(code.getCcode())&&last.getProjectId().equals(pro.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 检索科目获取凭证
                        List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getProjectId().equals(pro.getUniqueCode())).collect(Collectors.toList());
                        ...
                        // 获取累计发生 【月份：<= endDate】
                        List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)&& pz.getCcode().equals(code.getCcode())&&pz.getProjectId().equals(pro.getUniqueCode())).collect(Collectors.toList());
                        ...
                        if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                            ...
                            vo.setUniqueCode(pro.getUniqueCode()).setCcode(pro.getProjectCode()).setCcodeName(pro.getProjectName())
