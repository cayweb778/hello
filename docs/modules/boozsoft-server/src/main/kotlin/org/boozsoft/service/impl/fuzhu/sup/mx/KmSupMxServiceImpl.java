...
@Slf4j
@Service
public class KmSupMxServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String dept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        return supplierRepository.findAll().collectList()
            ...
            .flatMap(map->{
                ...
                List<Supplier> deptlist = (List<Supplier>) map.get("deptlist");
                ...
                deptlist.stream().forEach(d->{
                    ...
                    // ************* 获取科目期初 *************
                    List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc ->StrUtil.isNotBlank(qc.getCsupId()) && qc.getCcode().equals(km) && qc.getCsupId().equals(d.getUniqueCode())).collect(Collectors.toList());
                    ...
                    if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                        ...
                        // 获取上个月凭证发生数
                        List<DeptCodeAccvoucherVo> lastMonthAccVoucher = pzlist.stream().filter(acv -> Integer.valueOf(acv.getImonth()) <= Integer.valueOf(lastmonth) && acv.getCcode().equals(codeinfo.getCcode()) && acv.getCsupId().equals(d.getUniqueCode())).collect(Collectors.toList());
                    ...
                    // ************* 获取供应商发生凭证 *************
                    List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz -> Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate) && pz.getCsupId().equals(d.getUniqueCode())).collect(Collectors.toList());
        ...
        return supplierRepository.findAllOrderBySupCode2().collectList()
            ...
            .flatMap(map->{
                ...
                ccodeList.stream().forEach(km->{
                    ...
                    List<CustomerVo> supvolist = deptlist.stream().filter(v -> v.getCcode().equals(km)).collect(Collectors.toList());
                    supvolist.stream().forEach(d->{
                        BigDecimal codeTotalMd=new BigDecimal(0);
                        BigDecimal codeTotalMc=new BigDecimal(0);
                        BigDecimal codeTotalNds=new BigDecimal(0);
                        BigDecimal codeTotalNcs=new BigDecimal(0);
                        ...
                        // ************* 获取供应商发生凭证 *************
                        List<DeptCodeAccvoucherVo> collect = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCsupId())&& pz.getCsupId().equals(d.getUniqueCode())&&pz.getCcode().equals(km) && Integer.valueOf(pz.getIyperiod())>=Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod())<=Integer.valueOf(endDate)).collect(Collectors.toList());
                        for (int i = 0; i < collect.size(); i++) {
                            BigDecimal pzmd=collect.size()>0?collect.get(i).getMd():new BigDecimal(0);
                            BigDecimal pzmc=collect.size()>0?collect.get(i).getMc():new BigDecimal(0);
                            BigDecimal pzndS=collect.size()>0?collect.get(i).getNdS():new BigDecimal(0);
