...
@Slf4j
@Service
public class SupKmYueServiceImpl {
    ...
    public Mono<R> findAll(Pageable pageable,String minDept, String km, String strDate, String endDate, String bz, String ishaveRjz, String styleValue, Map<String, String> searchMap, Map<String, String> filterMap,String database) {
        ...
        // 1、获取供应商辅助核算的科目
        return codeKemuRepository.findByBsupAndIyearOrderByCcodeAsc("1", year).collectList()
                .flatMap(deptCodeList -> {
                    ...
                    // 查询供应商辅助核算凭证
                    return accvoucherRepository.findAllByIyearAndCsupId(year, minDept, year+"01", endDate).collectList()
                ...
                .flatMap(map -> {
                    ...
                    codeList.stream().forEach(ck -> {
                        ...
                        // ************* 获取科目期初 *************
                        List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCsupId()) && qc.getCcode().equals(ck.getCcode())&& qc.getCsupId().equals(minDept)).collect(Collectors.toList());
                        ...
                        if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                            ...
                            List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last -> last.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth)&& last.getCcode().equals(ck.getCcode())).collect(Collectors.toList());
                                        ...
                                        .flatMap(suplist->{
                                            return Mono.just(
                                                    CollectOfUtils.mapof("qclist", qcmap.get("tablesData"), "deptAccVoucherList",
                                                            finalNewaccvoucherlist, "codeList", finalNewDeptCodeList,"suplist",suplist)
                                            );
                                        });
                                });
                        });
                ...
                .flatMap(map -> {
                    ...
                    suplist.stream().forEach(sup->{
                        ...
                        codeList.stream().forEach(ck -> {
                            ...
                            // ************* 获取科目期初 *************
                            List<SubjectInitialBalanceVo> findByCodeQcList = qclist.stream().filter(qc -> StrUtil.isNotBlank(qc.getCsupId()) && qc.getCcode().equals(ck.getCcode())&& qc.getCsupId().equals(sup.getUniqueCode())).collect(Collectors.toList());
                            BigDecimal qcMd = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMd() : new BigDecimal(0);
                            BigDecimal qcMc = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getMc() : new BigDecimal(0);
                            // 数量
                            BigDecimal qcNdS = findByCodeQcList.size() > 0 ? findByCodeQcList.get(0).getNdS() : new BigDecimal(0);
                            ...
                            if (Integer.valueOf(strDate.substring(4, 6)) > 1) {
                                String lastmonth = Integer.valueOf(month) - 1 < 10 ? "0" + (Integer.valueOf(month) - 1) : month;
                                List<DeptCodeAccvoucherVo> lastpz = pzlist.stream().filter(last ->last.getCsupId().equals(sup.getUniqueCode()) && last.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(last.getImonth()) <= Integer.valueOf(lastmonth)).collect(Collectors.toList());
                                BigDecimal lastMd = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastMc = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastNdS = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getNdS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                                BigDecimal lastNcS = lastpz.size() > 0 ? lastpz.stream().map(DeptCodeAccvoucherVo::getNcS).reduce(BigDecimal.ZERO, BigDecimalUtils::sum) : new BigDecimal(0);
                            ...
                            // 检索科目获取凭证
                            List<DeptCodeAccvoucherVo> newpzlist = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCsupId()) && pz.getCsupId().equals(sup.getUniqueCode()) && pz.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(pz.getIyperiod()) >= Integer.valueOf(strDate) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)).collect(Collectors.toList());
                            BigDecimal pzMd =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzMc =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzNfratMd =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzNfratMc =newpzlist.size()>0?newpzlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            ...
                            // 获取累计发生 【月份：<= endDate】
                            List<DeptCodeAccvoucherVo> pzljlist = pzlist.stream().filter(pz ->StrUtil.isNotBlank(pz.getCsupId()) && pz.getCsupId().equals(sup.getUniqueCode()) && pz.getCcode().startsWith(ck.getCcode()) && Integer.valueOf(pz.getIyperiod()) <= Integer.valueOf(endDate)).collect(Collectors.toList());
                            BigDecimal pzLjMd =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjMc =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjNfratMd =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                            BigDecimal pzLjNfratMc =pzljlist.size()>0?pzljlist.stream().map(DeptCodeAccvoucherVo::getNfratMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum):new BigDecimal(0);
                        ...
                        excelvo.put("ccode",sup.getCustCode());
                        excelvo.put("ccodeName",sup.getCustName());
                    ...
                    suplist.stream().forEach(sup->{
                        codeList.stream().forEach(ck -> {
                            ...
                            if(qcMd.compareTo(BigDecimal.ZERO) == 0&&qcMc.compareTo(BigDecimal.ZERO) == 0 &&pzMd.compareTo(BigDecimal.ZERO) == 0&&pzMc.compareTo(BigDecimal.ZERO) == 0
                                ...
                                vo.setCcode(ccode).setCcodeName(ccodeName).setCclass(cclass).setTemp1(sup.getCustCode()).setTemp2(sup.getCustName())
                        ...
                        BigDecimal totalQcMdTotal=mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQcMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcMcTotal=mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQcMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalQcNumTotal=mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQcNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQcNfratTotal=mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQcNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalPzMdTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getPzMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzMcTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getPzMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNumTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getPzNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalPzNfratTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getPzNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalLjMdTotal=mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getLjMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjMcTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getLjMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalLjNumTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getLjNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);;
                        BigDecimal totalLjNfratTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getLjNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);;

                        BigDecimal totalQmNumTotal=  mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQmNum).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmNfratTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQmNfrat).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);

                        BigDecimal totalQmMdTotal=  mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQmMd).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
                        BigDecimal totalQmMcTotal= mapList.stream().filter(f->f.getTemp1().equals(sup.getCustCode())).map(KeMuBalanceVo::getQmMc).reduce(BigDecimal.ZERO, BigDecimalUtils::sum);
