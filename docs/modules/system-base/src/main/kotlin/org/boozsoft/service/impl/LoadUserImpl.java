...
//    }
    ...
    @Autowired
    RedisSystemUserRepository redisSystemUserRepository;
    ...
    public Mono<Map> saveSecurtyStateMap(Map map) {
        ...
        Mono<RedisSystemUser> userMono = Mono.just(new RedisSystemUser())
                .flatMap(redisSystemUser -> {
                    return tokenResource.getToken().map(token -> {
...
//                    redisSystemUser.setTargetId(systemUser.getId());
                    ...
                    redisSystemUser.setUser(JSON.toJSONString((SystemUser) map.get("user")));
                    redisSystemUser.setRoles(JSON.toJSONString((List<Role>) map.get("roles")));
                    redisSystemUser.setPermissions(JSON.toJSONString((List<Permission>) map.get("permissions")));
                    return redisSystemUser;
                }).map(redisSystemUserRepository::save);
    ...
    public Mono<Map> getSecurtyStateMap() {
        ...
        return tokenResource.getToken()
                .flatMap(token -> {
                    ...
                    RedisSystemUser byToken = redisSystemUserRepository.findByToken(token);
                    if (byToken == null) {
                        return Mono.empty();
                    }
...
//                );
    ...
    @Override
    public Flux<Role> loadRoles(String userId) {
...
//                });
  ...
  @Override
  @Deprecated
  public Flux<Permission> loadPermissions(List<Role> roleList) {
        return null;
