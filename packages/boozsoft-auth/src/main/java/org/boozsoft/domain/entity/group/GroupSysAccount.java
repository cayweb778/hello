...
@ApiModel(value = "数据表：集团账套表", description = "集团账套表")
@Table("_app_group_sys_account")
@Data
@EqualsAndHashCode(callSuper = false)
public class GroupSysAccount extends BaseModificationAwareEntity {
    ...
    @ApiModelProperty(value = "所属公司唯一编码", position = 1)
    ...
    @ApiModelProperty(value = "账套机构全称 ", position = 12)
    ...
    private String beiyong3; // 临时成存储主管id

    /*******************现金银行参数设置***********************/
    @ApiModelProperty(value = "对账单余额方向（1,借；0,贷）")
    private String statementFangxiang;
    @ApiModelProperty(value = "现金日记账排序方式（1,日期+凭证类别字+凭证号；2,日期+制单顺序号）")
    private String cashBankDayBookSort;
    @ApiModelProperty(value = "银行对账匹配优先级（1,对方单位+票号+日期+结算方式；2,对方单位+日期+票号+结算方式）")
    private String BankStatementSort;
    ...
    @ApiModelProperty(value = "科目对照辅助核算项是否必须一致（1是，0否，默认为是）")
    private String isame;

    @ApiModelProperty(value = "项目大类控制科目及项目（1是，0否，默认为否）")
    private String projectClassCtl;

    @ApiModelProperty(value = "打印时显示核算单位名称")
