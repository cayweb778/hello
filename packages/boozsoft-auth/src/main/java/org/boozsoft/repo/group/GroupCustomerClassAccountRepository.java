...
public interface GroupCustomerClassAccountRepository extends ReactiveCrudRepository<GroupCustomerClassAccount, String> {
    Flux<GroupCustomerClassAccount> findAllByOrgUniqueAndCtype(String orgUnique,String ctype);
    Flux<GroupCustomerClassAccount> findAllByOrgUniqueAndCtypeAndTenantId(String orgUnique,String ctype,String tenantId);

    @Query("delete from _app_group_customer_class_account where ctype=:ctype and org_unique=:orgUnique and data_unique in ( :orgDelList )")
    Mono<Void> delByCtypeAndOrgUnique(String ctype, String orgUnique, List<String> orgDelList);

    @Query("delete from _app_group_customer_class_account where ctype=:ctype and org_unique=:orgUnique and data_unique in ( :orgDelList ) and tenant_id=:tenantId ")
    Mono<Void> delByCtypeAndOrgUniqueAndTenantId(String ctype, String orgUnique, List<String> orgDelList,String tenantId);

    @Query("delete from _app_group_customer_class_account where ctype=:ctype and org_unique=:orgUnique and flag=:flag")
    Mono<Void> delByCtypeAndOrgUniqueAndFlag(String ctype, String orgUnique, String flag);
    @Query("delete from _app_group_customer_class_account where ctype=:ctype and org_unique=:orgUnique and flag=:flag and tenant_id=:tenantId")
    ...
    @Query("select distinct data_id\n" +
            "from _app_group_customer_class_account\n" +
    ...
    @Query("select g.*,(select cc.cus_cclass_name from _app_group_customer_class cc where cc.cus_bend='0' and cc.unique_custclass=g.parent_id ) as super_class_name\n" +
            "from _app_group_customer_class_account ga\n" +
            "         left join _app_group_customer_class g on g.unique_custclass = ga.data_unique\n" +
    ...
    @Query("update _app_group_customer_class_account set flag=:flag where ctype=:ctype and org_unique=:orgUnique and data_unique in ( :orgDelList )")
    Mono<Void> editFlagByCtypeAndOrgUnique(String flag,String ctype, String orgUnique, List<String> orgDelList);
    @Query("update _app_group_customer_class_account set flag=:flag where ctype=:ctype and tenant_id=:tenantId and data_unique in ( :orgDelList )")
    Mono<Void> editFlagByCtypeAndTenantId(String flag,String ctype, String tenantId, List<String> orgDelList);
    Flux<GroupCustomerClassAccount> findAllByCtype(String ctype);
}
