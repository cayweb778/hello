...
public interface OriginSupplierRepository extends ReactiveCrudRepository<OrgSupplier, String> {
    @Query("SELECT c.*,\n" +
            "       cc.cus_cclass_name                                                                           as unique_custclass_name,\n" +
            "       org.org_name,\n" +
            "       (select s.cust_name from _app_origin_customer s where s.unique_code = c.unique_code_supplier) as sup_name,\n" +
            "       (select c2.cust_name from _app_origin_supplier c2 where c2.unique_code = c.unique_code_ccus)  as parent_name\n" +
            "from _app_origin_supplier c\n" +
