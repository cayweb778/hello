...
在乾坤的`issue`中一个[议题](https://github.com/umijs/qiankun/issues/286)非常有意思，有个开发者提出能否利用`iframe`来实现`js`沙箱能力，这个`idea`启发了NC方案，下面详细介绍
...
NC采用[webcomponent](https://developer.mozilla.org/en-US/docs/Web/Web_Components)来实现页面的样式隔离，NC会创建一个`wujie`自定义元素，然后将子应用的完整结构渲染在内部
...
承载子应用的`iframe`和主应用是同域的，所以主、子应用天然就可以很好的进行通信，在NC我们提供三种通信方式
...
NC提供了`EventBus`实例，注入到主应用和子应用，所有的应用可以去中心化的进行通信
### 优势

通过上面原理的阐述，我们可以得出NC微前端框架的几点优势：
::: tip 优势
...
- **纯净无污染**
  ...
  - NC利用`iframe`和`webcomponent`来搭建天然的`js`隔离沙箱和`css`隔离沙箱
