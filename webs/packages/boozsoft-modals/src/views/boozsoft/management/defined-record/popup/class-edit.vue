...
const mul=null
import {saveDefinedRecord} from "/@/api/record/system/group-defined-record";
...
async function checkCode() {
  ...
  const res = await findByCode(formItems.value.definedCode)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
...
async function checkName() {
  ...
  const res = await findAllDefinedFileList()
