      ...
      <div class="app-container-neck">
        ...
        <div style="float: right; position: relative">
          <a-select v-model:value="formItems.selectType" style="width: 120px;font-size: 12px;" class="special_select">
            <a-select-option style="font-size: 12px;" value="ecCode">使用状况编码</a-select-option>
            <a-select-option style="font-size: 12px;" value="ecName">使用状况名称</a-select-option>
          </a-select>
          <a-input-search
            placeholder=""
        ...
        <div style="width: 60%; float: right;margin-left: 5px;">
          <BasicTable class="w-3/4 xl:w-4/5"
            ...
            <template #ecName="{ record }">
              <span style="float: left" v-if="record.ecName!='' && record.ecName!=null">{{record.ecName}}</span>
            </template>

            <template #isAccrual="{ record }">{{ formatisAccrual(record.isAccrual) }}</template>
...
import {deleteDept, saveDept, editFlag, getDeptListById, GetDeptTree} from '/@/api/record/group/fa-usage-status'
...
function formatisAccrual(flag:any) {
  let str = '计提折旧'
  switch (flag) {
    case '1':
      str = '计提折旧'
...
const exportExcelNow = async () => {
  ...
  // debugger
  const arrHeader = ['使用状况编码','使用状况名称','上级名称','状态'];
