  ...
  <div>
    <div class="app-container">
      <div class="app-container-head">
        ...
        <div class="ant-btn-group" style="float: right">
          ...
          <button type="button" class="ant-btn ant-btn-me">导入</button>
          <button type="button" class="ant-btn ant-btn-me" @click="startExport('table')">导出</button>
          ...
          <a-popover class="ant-btn-default" placement="bottom">
            ...
            <span class="group-btn-span-special2" @click="onChangeSwitch('all')"
              ...
              <span class="group-btn-span-special2" @click="onChangeSwitch('1')"
                ...
                <SafetyOutlined/>&nbsp;&emsp;可用&emsp;&ensp;<CheckOutlined
              ...
              <span class="group-btn-span-special2" @click="onChangeSwitch('0')"
                  ...
                  <RestOutlined/>&nbsp;&emsp;不可用&emsp;&ensp;<CheckOutlined
    ...
    <div class="app-container">
      ...
      >
        ...
        <template #plantNamej="{ record }"><u class="tableUStyle" @click="openEdit(record,true)">{{
            record.plantNamej
          }}</u></template>
        <template #plantNamefull="{ record }"><u class="tableUStyle" @click="openEdit(record,true)">{{
            record.plantNamefull
          }}</u></template>
        <template #corpUniqueCode="{ record }"> {{
            formatOrgnInCharge(record.corpUniqueCode)
          }}
        </template>
        <template #countryRegion="{ record }"> {{
            formatDqInCharge(record.countryRegion)
          }}
        </template>
        <template #plantClass="{ record }"> {{
            (record.plantClass == null || record.plantClass == '') ? '' : record.plantClass == '1' ? '自有工厂' : record.plantClass == '2' ? '联营工厂' : '代工工厂'
          }}
        </template>
...
import {
  changePlant, delPlant, savePlant, getPlantsList, getPlantDelAll, deletePlantTrue, reductionPlant
} from '/@/api/record/group/im-plant'
...
const CrudApi = {
  ...
  columns: [
    ...
    {
      title: '工厂简称',
      dataIndex: 'plantNamej',
      ellipsis: true,
      slots: {customRender: 'plantNamej'}
    },
    {
      title: '工厂全称',
      dataIndex: 'plantNamefull',
      ellipsis: true,
      slots: {customRender: 'plantNamefull'}
    },
    {
      title: '所属公司',
      dataIndex: 'corpUniqueCode',
      ellipsis: true,
      slots: {customRender: 'corpUniqueCode'}
    },
    {
      title: '国家(地区)',
      dataIndex: 'countryRegion',
      ellipsis: true,
      slots: {customRender: 'countryRegion'}
    }, {
      title: '城市',
      dataIndex: 'cityPostcode',
      ellipsis: true,
      slots: {customRender: 'cityPostcode'}
    }, {
      title: '收/送货地址',
      dataIndex: 'addr',
      ellipsis: true,
      slots: {customRender: 'addr'}
    }, {
      title: '工厂类型',
      dataIndex: 'plantClass',
      ellipsis: true,
      slots: {customRender: 'plantClass'}
    }
  ],
  editData: {
    id: null,
...
const [registerTable, {reload, setTableData, getDataSource, setColumns, getColumns}] = useTable({
  ...
  searchInfo: {pageMark: usePlatformsStore().getCurrentPlatformId == 1003 ? '1' : '0'}
})
...
const del = async (data: any) => {
  data.delName = lanMuData.username
  await delPlant(data)
...
const weedOut = (data) => {
  return ObjTool.dels(data, ['unitList', 'currencyList', 'countryList', 'accNameCn'])
}
...
const openRecycleDel = async () => {
  // 获取回收站数据
  recycleList.value = await getPlantDelAll()
...
const recycleModify = async (e) => {
  if (e.type == 'reduce') {
    ...
    const data = e.data
    let model = recycleList.value.filter(it=>it.id == e.data.id)[0]
    await deletePlantTrue(data).then(async (res) => {
      if (null != res && res.isOk == false) {
      } else {
        e.callback()
...
async function writeLog(action, a, content) {
  ...
  let old = getDataSource().filter(it => it.id == a.id)[0]
    ...
    let text = `操作内容【修改工厂信息】,`+'工厂代码【' + a.plantCode + '】,工厂名称【' + a.plantNamej + '】,'
  ...
  let map = {
    ...
    uniqueCode: '',
    optAction: action,
    optContent: content || `操作内容【${action}工厂信息】,` + '工厂代码【' + a.plantCode + '】,工厂名称【' + a.plantNamej + '】,工厂简称【' + a.plantNamefull + '】',
  }
  await saveLog(map)
}
...
function transformText(key, value) {
  ...
  switch (key) {
    ...
    case "countryRegion":
      ...
      break;
    case "plantClass":
      text = (value == null || value == '') ? '' : value == '1' ? '自有工厂' : value == '2' ? '联营工厂' : '代工工厂'
      break;
  }
  return text
}
