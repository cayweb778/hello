<template>
  <BasicModal width="940px" v-bind="$attrs"
    ...
    <div class="nc-open-content" :class="isLook?'nc-open-content-look':''" style="height: 100%">
      <div style="display: inline-flex;justify-content: space-between;width: 640px;margin:3%  14%">
        <div style="width: 600px;">
          <a-radio-group name="radioGroup" v-model:value="formItems.independent"
                         @change="independentChange"
                         :style="formItems.independent == '1'?{pointerEvents: 'none'}:{}"
                         style="margin: 0;">
      ...
      <!--      <label>所属集团组织：</label>
            <a-select
              ...
              :disabled="true"
            >
      &lt;!&ndash;        ref="focus3" @keydown.enter.native="$refs.focus3.focus()"&ndash;&gt;
      ...
      <a-input v-model:value="formItems.coCode" @keyup="inputLimit" @blur="checkCode()"
               ...
               style="width: 28%;text-align: center" ref="focus2"
               @keydown.enter.native="$refs.focus3.focus()"/>
      <span class="red_span">⋆</span>
      <br/>
      <template v-if="formItems.independent == '1'">
        <label>年度开始日期：</label>
        <MonthPicker  placeholder="" v-model:value="formItems.yearStartDate"
        ...
        <span class="red_span">⋆</span>
        <label>年度结束日期：</label>
        <DatePicker  placeholder="" v-model:value="formItems.yearEndDate"
                    value-format="YYYY-MM-DD"
                    format="YYYY-MM-DD"
                    style="width: 28%;pointer-events: none; cursor: default;text-align-last: center;"/>
        <br>
        <label>会计期间数量：</label>
        <a-select v-model:value="formItems.periodNum" @change="changePeriodNum"
                  style="text-align: center;width: 28%" :disabled="isEdit" ref="focus4"
      ...
      <label>会计准则：</label>
      <a-select v-model:value="formItems.accStandard" ref="focus5"
                @keydown.enter.native="$refs.focus6.focus()" style="width: 28%;text-align: center;"
      ...
      <span class="red_span"></span>
      <!--      <label>本位币：</label>
            <a-select v-model:value="formItems.currency" style="text-align: center;width: 28%"
                      ref="focus3" @keydown.enter.native="$refs.focus4.focus()">
      ...
      <div style="text-align: center">
        <label>预置会计科目：</label>
        <a-radio-group name="radioGroup" v-model:value="formItems.ibudgetAccStandard"
                       :disabled="isOrg" style="margin: 0%">
      ...
      <br/>
      <!--      <label>默认凭证类别：</label>
            <a-select v-model:value="formItems.voucherTypeNum" ref="focus5" @keydown.enter.native="$refs.focus6.focus()" style="text-align: center;width: 28%"
                      :disabled="isOrg">
...
import {
  ...
  findByName2,
  findByNameAssets,
  corpAvailable, findLastValByCoCode
...
async function handleOk() {
  ...
  let r = /^[a-zA-Z0-9]+$/g;
  if (hasBlank(formItems.value.accNameFull)) {
    createWarningModal({content: '财务账套名称为必填项！'})
  } else if (hasBlank(formItems.value.id)) {
    createWarningModal({content: '所属公司（单位）为必选项！'})
  } else if (formItems.value.coCode.length != 3 || !formItems.value.coCode.startsWith(unitCoCode.value) || (!r.test(formItems.value.coCode))) {
    ...
    if (hasBlank(formItems.value.currency)) {
      ...
      showNext.value = true
    } else if (formItems.value.independent == '0' && hasBlank(startYaerValue.value)) {
      createWarningModal({content: '财务账套开始年度为必选项！'})
      showNext.value = true
    } else if (hasBlank(formItems.value.startPeriod)) {
      createWarningModal({content: '财务账套启用期间为必选项！'})
      showNext.value = true
    } else if (formItems.value.independent == '1' && (hasBlank(formItems.value.accStandard) || hasBlank(levelPrefix.value))) {
      ...
      formItems.value.acountStandardList = []
      formItems.value.currencyList = []
      formItems.value.levelList = []
      ...
      if (!hasBlank(supervisors.value)) formItems.value.beiyong3 = JsonTool.json([supervisors.value])
      emit('save', {
        closeOpen() {
          showNext.value = true
...
const independentChange = (v) => {
  let text = formItems.value.accNameFull
  let code =   formItems.value.coCode
  let pid =   formItems.value.accGroup
  let id =   formItems.value.id
  resetForm()
  formItems.value.independent = '1'
  formItems.value.accNameFull = text || ''
  if (!hasBlank(pid)){
    formItems.value.id = id
    formItems.value.coCode = code
    unitCoCode.value = code
    isOrg.value = false
  }
}
...
const [register, {closeModal, setModalProps}] = useModalInner(async ({data}) => {
  ...
  supervisors.value = !hasBlank(formItems.value.beiyong3) ? (JSON.parse(formItems.value.beiyong3))[0] : []
  if (!isEdit.value) {
    ...
    formItems.value.id = data.uniqueCode
     startYaerValue.value = hasBlank(formItems.value.yearStartDate)?null:dayjs(formItems.value.yearStartDate.substring(0, 4))
...
const resetForm = () => {
  ...
  /*  formItems.value.currency = ''
    formItems.value.currencyName = ''
    formItems.value.currencyCh = ''*/
...
async function checkCode() {
  ...
  if (res != 0) {
    createWarningModal({content: '财务账套代码已存在，请重新输入！'})
...
async function checkName() {
  formItems.value.accNameCn = formItems.value.accNameCn.trim()
  ...
  const res = await findByName2(formItems.value)
  if (res != 0) {
    createWarningModal({content: '财务账套名称已存在，请重新输入！'})
    formItems.value.accNameCn = ''
    return false
  }
  return true
...
// 得到年份选择器的值
function panelChangeOne(m2) {
  let value = m2.$y + ''
  // 获取所属组织开始期间
  let arr = organizeList.value.filter(item => item.uniqueCode == formItems.value.accGroup)
  if (arr.length > 0) {
    let orgO = arr[0]
    formItems.value.yearStartDate = value + '-' + (isEdit.value ? formItems.value.yearStartDate.substring(5, 10) : orgO.yearStartDate)
    if (formItems.value.yearStartDate.substring(5, 10) != '01-01') value = (parseInt(value) + 1)
    formItems.value.yearEndDate = value + '-' + (isEdit.value ? formItems.value.yearEndDate.substring(5, 10) : orgO.yearEndDate)
    changePeriodNum()
  }
}
...
const orgChange = () => {
  ...
  if (null != obj && formItems.value.independent != '1') {
    ...
    orgStartYear.value = obj.createDate.split('-')[0]
...
const unitChange = (v) => {
  ...
  if (!hasBlank(thisUnit?.accGroup)) {
    ...
    independentChange('')
    formItems.value.id = v
    isOrg.value = false
  }
  formItems.value.coCode = thisUnit?.coCode
  unitCoCode.value = thisUnit?.coCode
  getVar(v, unitCoCode.value)
}
const disabledDate = (current) => {
  const aa = orgStartYear.value + '-01-01';
  return current && (current < dayjs(aa).startOf('year') || current > dayjs(aa).endOf('year'));
};
