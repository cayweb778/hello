    ...
    >
      <div class="open-content-up" style="text-align: center;margin-top: 5%;">
        <div style="text-align: left;padding-left: 8%">
          ...
          <a-input v-model:value="itemConfig.base.projectName" placeholder="现金流量项目名称" disabled style="width: 55%;"/>
          <span class="red_span">*</span>
        </div>
        <br/><br/>
        <div style="text-align: left;padding-left:  8%;">
          <label>编码：</label>
          <a-input v-model:value="itemConfig.base.projectCode" placeholder="现金流量项目编码" disabled  style="width: 30%;" />

          <label style="padding-left: 20px;">类别：</label>
          <a-select v-model:value="itemConfig.base.projectType" placeholder="所属类别" style="width: 30%;" disabled>
...
import {findCodeByIyear, findCodeByProjectCode, findCodeByYearAndBend} from "/@/api/record/system/group-project-cash";
...
async function reloadYear() {
  const res = await findYearByAccount(defaultAdName)
  yearList.value = res
...
async function reloadCode() {
  kemuList.value = []
  kemuList.value = await findCodeByYearAndBend({accStandard:itemConfig.value.base.accStandard})
}
...
const [register, { closeModal }] = useModalInner((data) => {
  ...
  findAllAcctandard().then(res => {
    ...
    accstandardList.value.forEach(
      function (item,index){
        if (data.accStandard==item.uniqueAccStandard){
          itemConfig.value.base.accStandard = item.uniqueAccStandard
        } else if (data.accStandard==''&& index==0) {
          itemConfig.value.base.accStandard = item.uniqueAccStandard
        }
      }
    )
    console.log(itemConfig.value.base.accStandard)
    //获取会计科目
    reloadCode()
  })
  // 方式2
  ...
  itemConfig.value.base.accStandard = data.accStandard
  itemConfig.value.base.flag = data.data.flag

  //给对应科目列表赋值 
  if (data.data.projectCode != null && data.data.projectCode != '') {
    findCodeByProjectCode({projectCode:itemConfig.value.base.projectCode, iyear:itemConfig.value.base.accStandard}).then(res => {
...
async function handleOk() {
  itemConfig.value.base.table.forEach((row: any) => {
    row.accStandard = itemConfig.value.base.accStandard
