  ...
  >
    ...
    <div style="height: 100%" :class="isEdit?'nc-open-content':'nc-open-show-content'">
      <div class="open-content-up">
        ...
        <a-input v-model:value="formItems.psnCode" :disabled="isDisabled" :placeholder="isEdit?'请输入编号（不允许重复）':''"/>
...
import {getDeptListById as getJobFileList} from '/@/api/record/group/job-file'
import {
  findByOriginIdAndUniqueCode,
  savePsn as originSavePsn
} from "/@/api/record/system/origin-psn";
...
const [register, {closeModal}] = useModalInner(async (data) => {
  ...
  if(isState.value=='1') {
    accountList.value = await findByUniqueCode(data.data.uniqueCode)
  ...
  //选择职务
  // const jobFileList1 = await getJobFileList()
...
async function handleOk() {
  ...
  if (isChanged) {
    if (formItems.value.cellPhoneNum != changeBeforeModel.cellPhoneNum) {
      // const accountList1 = await findByUniqueCode(formItems.value.uniqueCode)
      const accountList1 = accountList.value.filter(item => item.flag=='1')
      if (accountList1.length > 0) {
        createConfirm({
          iconType: 'error',
          title: '提示',
          content: '修改手机号会同步组织及账套档案,确定继续吗?',
          onOk: async () => {
            for (let i = 0; i < accountList1.length; i++) {
              const item = accountList1[i]
              if (item.ctype=='1'){
                const psn = await findByOriginIdAndUniqueCode(item.originId,item.uniqueCode)
                psn.cellPhoneNum = formItems.value.cellPhoneNum
