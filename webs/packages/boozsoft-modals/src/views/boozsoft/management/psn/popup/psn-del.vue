    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <DeleteOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;回收站 － 人员信息
        </span>
        </div>
      </template>
...
import {deletePsn, findPsnDelList, savePsn} from "/@/api/record/system/group-psn";
...
async function reloadPsn(){
  const res:any = await findPsnDelList()
  psnList.value = res
      ...
      onOk: async () => {
        for (let i = 0; i < checkRow.value.length; i++) {
          ...
          await saveLog({
            ...
            billDate: null,
            optAction: '还原',
            optContent: '操作内容【还原人员信息】,人员编码【' + item.psnCode + '】,人员姓名【' + item.psnName + '】,手机号码【' + item.cellPhoneNum + '】',
          })
        }
        checkRow.value = []
      ...
      onOk: async () => {
        for (let i = 0; i < checkRow.value.length; i++) {
          ...
          await saveLog({
            ...
            billDate: null,
            optAction: '彻底删除',
            optContent: '操作内容【彻底删除人员信息】,人员编码【' + item.psnCode + '】,人员姓名【' + item.psnName + '】,手机号码【' + item.cellPhoneNum + '】',
          })
        }
        checkRow.value = []
