...
const handleDownByData = async (val: any) => {
  ...
  const columns2 = ['客户编码','客户全称','客户简称','税号','分类名称','母公司','对应供应商','国家','区划(省-市-区)','详细地址','电话',
      ...
      if (info[0].code === '200' || info[0].error === 'error') {
        ...
        info[0].list.forEach(async (tx)=>{
          ...
          let map={
            ...
            optAction:'新增申请',
            optContent:'操作内容【组织导入客户信息】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,客户编码【'+tx.custCode+'】,客户全称【'+tx.custName+'】,客户简称【'+tx.custAbbname+'】',
          }
          await saveLog(map)
