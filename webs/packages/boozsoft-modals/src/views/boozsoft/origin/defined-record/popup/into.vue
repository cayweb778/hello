...
import {saveDefinedRecord} from "/@/api/record/system/origin-defined-record";
import {findByOriginIdAndUniqueCode,save as saveDefinedFileAccount} from "/@/api/record/system/group-defined-file-account";
import {saveDefinedFile} from "/@/api/record/system/origin-defined-file";
...
async function reloadDefinedRecord(){
  checkRow.value = []
  state.selectedRowKeys = []
  const definedRecordAccount = await findByOriginId(originId.value,definedCode.value)
  definedRecordAccountList.value = definedRecordAccount.items
  tableDataAll.value = definedRecordAccountList.value.filter(item=>{
    return item.flag != '1'
  })
