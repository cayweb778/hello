...
import {findAllFuzhuHesuanList, findByCankaoDuixiang, findByCode, findByName} from "/@/api/record/system/origin-fuzhu-hesuan";
import {getDefinedFileList} from "/@/api/record/system/origin-defined-file";
...
const [register, {closeModal}] = useModalInner((data: any) => {
  ...
  getDefinedFileList(data.data.originId).then(res => {
    res.forEach(item => {
      const item1: any = {}
      item1.key = '_app_origin_defined_code'
    ...
    findAllFuzhuHesuanList(data.data.originId).then(resList => {
...
async function checkCode() {
  ...
  const res = await findByCode(formItems.value.ccode,formItems.value.originId)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
...
async function checkName() {
  ...
  const res = await findByName(formItems.value.cname,formItems.value.originId)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
...
async function checkCankaoDuixiang() {
  ...
  const res = await findByCankaoDuixiang(formItems.value.cankaoDuixiang,formItems.value.originId)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
