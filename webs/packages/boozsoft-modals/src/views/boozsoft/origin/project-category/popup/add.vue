  ...
  <BasicModal width="900px" v-bind="$attrs" title="项目栏目样式" @ok="handleOk()" @register="register" :canFullscreen="false">
    ...
    <div class="nc-open-content">
      <div class="open-content-up" style="margin-top: 60px;">
        <label>编码：</label>
        <a-input v-model:value="itemConfig.base.projectCateCode" :disabled="true" @input.native="checkCateCode()" @blur="repeatCateCode()" placeholder style="width: 20%" />
        <label>名称：</label>
        <a-input v-model:value="itemConfig.base.projectCateName" @blur="repeatCateName()" placeholder style="width: 50%" />
        <br />
        <br />
        ...
        <div style="height: 350px;overflow: auto !important;">
          ...
          >
            ...
            <DeleteOutlined v-if="!(rowIndex<8)" @click="delRow(rowIndex)" style="color: rgb(1, 129, 226)" />
          </div>
        </div>
      </div>
    </div>
...
import {
  columnFindCate, findBukongProjectCateCode, findMaxProjectCateCode,
  findProCateByCode,
  findProCateByName,
  saveCate
} from '/@/api/record/system/origin-project-category'
...
async function handleOkAdd() {
  ...
  if (!(itemConfig.value.base.projectCateCode == changeBeforeModel.base.projectCateCode
    ...
    // emit('save', unref(itemConfig.value.base))
    // closeModal()
    await saveCate(itemConfig.value.base)
...
const [register, { closeModal }] = useModalInner((data) => {
  ...
  itemConfig.value.base.originId = data.data.originId
  ...
  if (data.data.projectCateCode != null && data.data.projectCateCode != '') {
    ...
    columnFindCate(data.data.projectCateCode,data.data.originId)
      .then(res => {
...
async function maxCode(){
  const buCate = await findBukongProjectCateCode(itemConfig.value.base.originId)
  if (buCate!=null && buCate.projectCateCode!=null && buCate.projectCateCode!=''){
    itemConfig.value.base.projectCateCode = pad(buCate.projectCateCode,3)
  } else {
    const cate = await findMaxProjectCateCode(itemConfig.value.base.originId)
    if (cate != null && cate.projectCateCode != null && cate.projectCateCode != '') {
      itemConfig.value.base.projectCateCode = pad(add(cate.projectCateCode, 1), 3)
    } else {
      itemConfig.value.base.projectCateCode = '001'
    }
  }
}
...
async function repeatCateCode() {
  if (itemConfig.value.base.projectCateCode != null && itemConfig.value.base.projectCateCode != '') {
    const res: any = await findProCateByCode(itemConfig.value.base.projectCateCode,itemConfig.value.base.originId)
    if (res.length != 0) {
      createErrorModal({
        iconType: 'warning',
        title: '提示',
...
async function repeatCateName() {
  if (itemConfig.value.base.projectCateName != null && itemConfig.value.base.projectCateName != '') {
    const res: any = await findProCateByName(itemConfig.value.base.projectCateName,itemConfig.value.base.originId)
    if (res.length != 0) {
      createErrorModal({
        iconType: 'warning',
        title: '提示',
