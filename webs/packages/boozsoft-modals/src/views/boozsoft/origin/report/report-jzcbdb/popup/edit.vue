    ...
    <div class="app-container" style="padding:10px;height:630px;overflow: auto !important;">
      <div class="nc-open-content">
        ...
        <div style="margin-top: 20px;width: 90%; float: left;margin-left: 5%;margin-right: 5%;">
          <p>1、净资产变动表只取科目余额或者行次加减值；若科目设置某科目5001+5002，则取两科目余额相加，若设置5001-5002，则取值两科目余额相减，设置1+2+3，则等于三个行次的值相加；</p>
...
async function handleOk() {
  itemConfig.value.base.reportName='jzcbdb'
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'jzcbdb',titleName:itemConfig.value.base.titleName,originId:itemConfig.value.base.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
