...
async function handleOk() {
  itemConfig.value.base.reportName='lrb'
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'lrb',titleName:itemConfig.value.base.titleName,originId:itemConfig.value.base.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
