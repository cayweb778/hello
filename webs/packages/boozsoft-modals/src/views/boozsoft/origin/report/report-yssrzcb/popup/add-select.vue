...
const [register, { closeModal }] = useModalInner((data) => {
        ...
        initBasisTabAccoutData().then(list => {
          ...
          levelList.forEach(item => {
            if (res.uniqueAccStandard == item.id) {
              ...
              findByAccStandardAndReportName(item.uniqueAccStandard, 'yssrzcb').then(template => {
  ...
  // 方式2
  /*findByReportNameAndFlag("yssrzcb").then(res => {
  ...
  formItems.value.templateName = '预算收入支出表'
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'yssrzcb',titleName:formItems.value.titleName,originId:formItems.value.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
