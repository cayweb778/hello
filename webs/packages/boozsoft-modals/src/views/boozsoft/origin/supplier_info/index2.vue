          ...
          <a-popover placement="bottom" v-model:visible="visible">
            <template #content>
              <DynamicColumn :defaultData="initDynamics()['DATA']" :dynamicData="dynamicColumnData"
                             :lanmuInfo="lanMuData" @reload="reloadColumns"/>              <span @click="pageParameter.showRulesSize = 'MAX'"
...
<script setup lang="ts">
  ...
  import {
    ...
    EditFilled,
    PieChartFilled,
    FilterFilled,EditOutlined,SortAscendingOutlined,CheckOutlined,SortDescendingOutlined,UnorderedListOutlined
  } from '@ant-design/icons-vue';
  import { editFlagByCtypeAndOrgUnique, } from '/@/api/record/system/supplier_group';
  import {
  ...
  import {saveAll, treeCustomerClass} from "/@/api/org/sup_class/org_sup_class";
...
const DynamicColumn=null
  ...
  async function bringGroupDataFunction(data) {
    ...
    await originCustomerSaveAll(data).then(async (t)=>{
      // 埋点-操作日志
      let log='操作内容【组织引入供应商信息】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,供应商编码【'+t.custCode+'】,供应商全称【'+t.custName+'】,供应商简称【'+t.custAbbname+'】'
      /************** 记录操作日志 ****************/
      let map={
        loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
        userId: useUserStoreWidthOut().getUserInfo.username,
    ...
    createConfirm({
      ...
      onOk: async() => {
        ...
        getSelectRows().forEach(async (a)=>{
          /************** 记录操作日志 ****************/
          // 埋点-操作日志
          let log='操作内容【组织删除供应商信息】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】'
          let logmap={
            loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
            userId: useUserStoreWidthOut().getUserInfo.username,
            userName: useUserStoreWidthOut().getUserInfo.realName,
          ...
          // 埋点-操作日志
          let log2='操作内容【修改组织供应商分配表状态】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】'
          let logmap2={
            loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
            userId: useUserStoreWidthOut().getUserInfo.username,
            userName: useUserStoreWidthOut().getUserInfo.realName,
