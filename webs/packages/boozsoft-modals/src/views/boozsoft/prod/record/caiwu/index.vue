...
import {getEmpAll} from "/@/api/admin/emp";
import {getCaiwuAll} from "/@/api/admin/caiwu";
...
export default defineComponent({
  ...
  setup() {
    // console.log('参照表头',getBasicColumns())
    // console.log('主要跟它一致', getTreeTableData())
    // 1
    ...
    // 2
    let dataFrom=async ()=>{
      state.data.push(...await (getCaiwuAll()));
    }
    // 3
    onMounted(()=>{
      dataFrom()
