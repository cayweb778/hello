...
const [register, { closeModal }] = useModalInner( async (data) => {
  ...
  getPriceData()
  // 是否已做过单子
  let stockIsData=await useRouteApi(verifyStockIsData,{schemaName: database})(data.data.stockNum)
...
async function handleOk() {
  ...
  if(oldStockName.value!==trim(formItems.value.stockName) || oldStockGgxh.value!==trim(formItems.value.stockGgxh)){
    ...
    if (!hasBlank(trim(formItems.value.stockName))) {
      item = await useRouteApi(findByStockNameAndGgxh, {schemaName: database})(map)
      if (item > 0) {
