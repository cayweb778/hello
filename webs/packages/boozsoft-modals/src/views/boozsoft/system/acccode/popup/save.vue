...
import {
  ...
  company_findByCodeNum, findByOrgLowerMaxCodeNum, findByOrgCcode,
...
async function getNewCcode(code,val) {
  if(code!=='不存在'){
    ...
    if(val!=undefined){
      ...
      let newccode:any =''
      if(pagetype.value!=='zz'){
        newccode=await findByLowerMaxCodeNum(templateID.value,val,code_length);
      }else{
        newccode=await findByOrgLowerMaxCodeNum(templateID.value,val,code_length,iyear.value);
      }
...
const ccodeblur = async () => {
  ...
  if(formItems.ccode.length>4){
    ...
    // 获取上级科目信息
    const superCodeInfo:any = await findByCcode(superiorCcode2,templateID.value);
  ...
  if(pagetype.value==='zz'){
    if(formItems.ccode!==''){
      let temp=0
      let orgDatabase= await findAllByAccGroup(orgUnique.value)
      orgDatabase.forEach(async (v)=>{
        let dataBaseCode= await useRouteApi(company_findByCodeNum,{schemaName:v.accId+'-'+iyear.value})({
      ...
      setTimeout(()=>{
        if(temp>0){
          createConfirmPop('本组织下已有账套使用此科目！不能添加');
          return false;
        }
      },150)
    }
...
async function handleOk(closeflag) {
  ...
  if(formItems.ccode.length>4){
    ...
    let superiorCcode2=formItems.ccode.substring(0,parseInt(codeLength))
    // 获取上级科目信息
    const superCodeInfo = await findByCcode(superiorCcode2,templateID.value);
    if(superCodeInfo==='不存在'){
      createConfirmPop('没有上级科目信息！')
      return false;
    }
