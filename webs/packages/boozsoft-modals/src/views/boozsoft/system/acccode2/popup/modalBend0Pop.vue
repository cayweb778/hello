...
import {
  company_findByLastCode, company_findByNoLastCode,
  company_findByNotLastCodeTree, company_saveCodekemu,
...
const [registerTable, { reload,getSelectRows,getDataSource,setTableData }] = useTable({
  api: useRouteApi(company_findByNoLastCode,{schemaName: database}),
  columns: columns,
...
async function fetch(map) {
  const deptTree = await useRouteApi(company_findByNotLastCodeTree,{schemaName: database})(map)
  function a(customerTree) {
    if(customerTree!==''){
      customerTree.forEach((item) => {
        ...
        item.title=setString(item.title,20)
