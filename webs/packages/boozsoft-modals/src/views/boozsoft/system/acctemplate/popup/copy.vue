    ...
    <div class="nc-open-content">
      <div class="open-content-up">
        <div style="margin-left: 20px;margin-top: 20px;">
          ...
          <br/>
          <label>科目级次：</label>
          <a-input disabled style="margin-left: 60px;width: 35%;" v-model:value="formState.tJici" @keydown.enter.native="$refs.focus16.focus()" autocomplete="off" />
        </div>
      </div>
    </div>
  </BasicModal>
...
const [register, { closeModal }] = useModalInner((data) => {
  findAllAcctandardList().then((res)=>{
      accStandardList.value=res
  })
  standardSelected.value=data.data.uniqueAccStandard+'_'+data.data.accStyleUnique+'BOOZ'+data.data.codeFirst
  formState.value.uniqueAccStandard = data.data.uniqueAccStandard;
  formState.value.tFlg=data.data.tflg
  formState.value.tName=data.data.tname
  formState.value.tJici=data.data.tjici
});
async function handleOk() {
  const name = await countByUniqueAccStandardAndTName(
    standardSelected.value.split('_')[0],
    formState.value.tName,'自定义模板'
  );
  if(formState.value.tName===''){
    message.error('请填写科目模板名称！');
    return;
  }else{
    if (name > 0) {
