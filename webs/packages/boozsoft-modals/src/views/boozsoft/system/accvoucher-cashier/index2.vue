    ...
    <div class="app-container-top">
      <div class="app-container-top-one">
        ...
        <div class="act-one-d-title">
          <span class="acto-d-title-span">出纳签字</span>
        </div>
        <div class="act-one-d-btn-group">
          <Button class="actod-btn" @click="openQueryPage()">查询</Button>

          <Button class="actod-btn" @click="openReviewPage(true)">签字</Button>
      ...
      <div style="position: relative;">
        ...
        >
          <template #ibook="{ record }">
            <a-tag v-if="record.inoId!=null" :color="record.ccashier != null ? 'green' : 'red'">
              {{ record.ccashier != null ? '已签字' : '未签字' }}
            </a-tag>
          </template>
          ...
          <span v-if="record.inoId!=null" style="color: blue;text-decoration: underline;cursor: pointer"
            ...
            <a-tag v-if="record.ifrag != 0 && record.inoId!=null"
                   :color="record.ifrag === '1' ? 'blue' : record.ifrag === '2' ? 'pink' : 'red'">
              {{
        ...
        <div class="pagination-text" :style="{top: (windowHeight+32)+'px'}"  v-show="showPaginationText">
...
import {
  findAllCashierAccvoucher as findAllAccvoucher,
  breakNumTidy,
  findDbLanMuList,
  saveLanMuList,
...
import Review from "/@/views/boozsoft/system/accvoucher-cashier/popup/cashier.vue";
import Query from "/@/views/boozsoft/system/accvoucher-cashier/popup/query.vue";
import {cloneDeep} from "lodash-es";
import {useMessage} from "/@/hooks/web/useMessage";
import {
  getCurrentAccountName, hasBlank, getThisIndexImg
...
const CrudApi = reactive({
  columns: JsonTool.parseProxy(accvoucherStore.getColumns(pageParameter.queryMark)).map(it=>{if (it.title=='记账状态'){it.title = '签字状态';} return it})
})
...
import {checkMeetSpecifyConditions} from "/@/views/boozsoft/system/accvoucher-cashier/bus";
import {
...
function resetDynamicColumnData() {
  ...
  findDbLanMuList(lanMuData).then(res => {
    ...
    let statiList = JsonTool.parseProxy(initDynamics()['DATA' + pageParameter.queryMark]).filter(item => item.name != '记账人' && item.name != '审核人' && item.name != '财务主管').map(it=>{if (it.name=='记账状态'){it.name = '签字状态';it.nameNew = '签字状态'} return it})
    if (dbList.length > 0) {
...
const reloadColumns = () => {
  ...
  let newA = (JSON.parse(JSON.stringify(accvoucherStore.getColumns(pageParameter.queryMark)))).filter(item => item.title != '记账人' && item.title != '审核人' && item.title != '财务主管')
