    ...
    <div class="app-container-top">
      <div class="app-container-top-one">
        ...
        <div class="act-one-d-title">
          <span class="acto-d-title-span">审核凭证</span>
        </div>
        <div class="act-one-d-btn-group">
          <Button class="actod-btn" @click="openQueryPage()">查询</Button>
          <Button class="actod-btn" @click="openReviewPage(true)">审核</Button>
    ...
    <div class="app-container-bottom"  :style="{height: (windowHeight+60)+'px',marginTop: '5px'}">
      ...
      >
        <template #ibook="{ record }">
          <a-tag v-if="record.inoId!=null" :color="record.ccheck != null ? 'green' : 'red'">
            {{ record.ccheck != null ? '已审核' : '未审核' }}
          </a-tag>
        </template>
...
import {
  ...
  CheckOutlined,
  PrinterOutlined,
  UsbOutlined, EditOutlined,FileSearchOutlined,
...
import Query from "/@/views/boozsoft/system/accvoucher-review/popup/query.vue";
...
const CrudApi = reactive({
  columns: JsonTool.parseProxy(accvoucherStore.getColumns(pageParameter.queryMark)).map(it=>{if (it.title=='记账状态'){it.title = '审核状态';} return it}).slice(0, 10)
})
...
function resetDynamicColumnData() {
  ...
  findDbLanMuList(lanMuData).then(res => {
    ...
    let statiList = JsonTool.parseProxy(initDynamics()['DATA' + pageParameter.queryMark]).filter(item => item.name != '记账人' && item.name != '出纳人' && item.name != '财务主管')
      .map(it=>{if (it.name=='记账状态'){it.name = '审核状态';it.nameNew = '审核状态'} return it})
    if (dbList.length > 0) {
...
const reloadColumns = () => {
  ...
  let newA = (JSON.parse(JSON.stringify(accvoucherStore.getColumns(pageParameter.queryMark).map(it=>{if (it.title=='记账状态'){it.title = '审核状态';} return it})))).filter(item => item.title != '记账人' && item.title != '出纳人' && item.title != '财务主管')
