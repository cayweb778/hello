    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span style="font-size: 16px;">导入内容：</span><span style="font-weight: bold;font-size: 16px;">账户档案导入</span><br/><br/>
...
import {getSysBankAccountList} from "/@/api/record/system/sys-bank-account";
const {
  createErrorModal
} = useMessage()
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length > 0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.cname = item['账户名称']
      item1.accStyle = item['账户类型']
      item1.accBank = item['开户银行']
      item1.accNumber = item['银行账号']
      item1.memo = item['备注']
      item1.status = '1'
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      if (cname == null || cname == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行账户名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i != j) {
          if (item1.cname == item2.cname) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行账户名称与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
const [register, {closeModal}] = useModalInner((data) => {
  ...
  useRouteApi(getSysBankAccountList, {schemaName: dynamicTenantId})({}).then(res => {
