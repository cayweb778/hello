    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">客户分类</span><br/><br/>
            <span>模板样式：</span>
            <Select
              disabled
    ...
    .then(async (info:any)=>{
      if(info[0].code==='200' || info[0].error==='error'){
        ...
        for (let i = 0; i < info[0].list.length; i++) {
          // 埋点-操作日志
          let log='操作内容【导入客户分类】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】' +
