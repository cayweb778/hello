  ...
  <div>
    <div class="app-container">
        ...
        <div class="ant-btn-group" style="float: right;">
          <span v-if="defaultPage">
            ...
            <button type="button" class="ant-btn ant-btn-me"
                    ant-click-animating-without-extra-node="false"
                    @click="rowOpenEdit"><span>修改</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    ant-click-animating-without-extra-node="false"
                    @click="openBatchEdit"><span>批改</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    ant-click-animating-without-extra-node="false"
                    @click="rowOpenDel"><span>删除</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    @click="openImportExcel"><span>导入</span></button>
          </span>
          <span v-else>
            <button type="button" class="ant-btn ant-btn-me"
                    @click="applyAdd"><span>新增申请</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    ant-click-animating-without-extra-node="false"
                    @click="rowOpenEdit"><span>修改</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    ant-click-animating-without-extra-node="false"
                    @click="rowOpenDel2"><span>删除</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    @click="introduce"><span>引入</span></button>
          </span>
          <button type="button" class="ant-btn ant-btn-me" @click="exportExcel123()"><span>导出</span>
          </button>
          <button type="button" class="ant-btn ant-btn-me"
                  ant-click-animating-without-extra-node="false" @click="jsonPrint"><span>打印</span>
          </button>
          <button type="button" class="ant-btn ant-btn-me"
                  @click="closeCurrent(),router.push('/one/home/welcome')"><span>退出</span></button>
        </div>
      </div>
      ...
      <div style="margin-top: -30px;margin-left: 80px;">
        ...
        <div style="display: inline-block;position:absolute;top:85px;left:110px;font-size: 10px;color:#999999;">
<!--          共 {{ custCountAllList }} 条记录-->
        </div>
        <div style="float: right; margin-left: 10px;">
          ...
          <a-popover placement="bottom">
            <template #content>
             <span class="group-btn-span-special2" @click="findByflagCheck('')"
                   :style="pageParameter.flag==''?{backgroundColor: '#0096c7',color: 'white'}:''">
              <ReadOutlined/>&nbsp;&nbsp;全部&emsp;&ensp;<CheckOutlined
               v-if="pageParameter.flag==''"/></span><br/>
              <span class="group-btn-span-special2" @click="findByflagCheck('1')"
                    :style="pageParameter.flag=='1'?{backgroundColor: '#0096c7',color: 'white'}:''">
              <SafetyOutlined/>&nbsp;&nbsp;启用&emsp;&ensp;<CheckOutlined
                v-if="pageParameter.flag=='1'"/></span><br/>
              <span class="group-btn-span-special2" @click="findByflagCheck('0')"
                    :style="pageParameter.flag=='0'?{backgroundColor: '#0096c7',color: 'white'}:''">
              <RestOutlined/>&nbsp;&nbsp;停用&emsp;&ensp;<CheckOutlined
                v-if="pageParameter.flag=='0'"/></span><br/>
            </template>
            <a-button>
    ...
    <div class="app-container">
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        <div class="bg-white m-4 mr-0 overflow-hidden" style="margin-top: -0.5px;margin-left: -5px;">
          <div style="width: 100%; height: 32px;background-color: #d9d9d9;padding-top:5px;font-weight: bold;text-align: center;">
            ...
            <div style="float: right;">
              <a style="color: black;font-size: 14px;" @click="openTreeAddPage()">
                <PlusCircleTwoTone/>
              </a>&emsp;
            </div>
          </div>
        ...
        <div class="temp" style="margin-left: 5px;">
            ...
            <template #action="{ record }">
              <div v-if="record.flag!=='11'">
                <a-popover placement="bottom">
                  ...
                  <template #content>
                    <a style="cursor: pointer" @click="editflag(record.id,'1')"
                       v-if="record.flag=='0'">
                      <CheckCircleOutlined/>
                      启用
                    </a>
                    <a style="cursor: pointer" @click="editflag(record.id,'0')"
                       v-if="record.flag=='1'">
                      <CloseCircleOutlined/>
                      停用
                    </a>
                    <!--已生效的删除-->
                    <a style="cursor: pointer" @click="del(record.id,record.uniqueCode,record)"
                       v-if="record.successState=='1'">
                      <DeleteOutlined/>
                      删除
                    </a>
                    <!--未生效的删除-->
                    <a style="cursor: pointer" @click="wsxdel(record.id,record)"
                       v-if="record.successState=='0'">
                      <DeleteOutlined/>
                      删除
                    </a>
                  </template>
          ...
          <div class="pagination-text" :style="{top: (windowHeight+37)+'px',left:(totalColumnWidth-330)+'px'}" v-show="showPaginationText">
            {{`共 ${totalColumnWidth}条记录&emsp;每页 200 条`}}
          </div>
        </div>
      </PageWrapper>
    </div>
    ...
    <ApplyList @closeOk="pageReload(),clearSelectedRowKeys(),fetch()"
               @register="registerApplyListPage"/>
...
import {
  ...
  initDynamics
} from "/@/views/boozsoft/system/customer_info/data";
...
import {
  customerSaveApi, findAllByCusBendEq1, getCusClassAuthorData,
  GetCustomerClassTree,
  getCusTypeSeeSwitch
} from "/@/api/record/system/customer_class";
...
const windowHeight = (window.innerHeight - (295))
...
const exportExcel123 = async () => {
  ...
  const columns = ['客户编码', '客户全称', '客户简称', '税号', '分类名称', '母公司', '对应供应商', '省', '市', '区', '详细地址', '电话', '手机', '网站', 'EMail', '行业性质', '开户银行', '开户地', '银行账户', '银行行号', '联系人', '价格级次', '税率']
  const dataArr: any = []
  data.forEach(v => {
  ...
  aoaToSheetXlsx({
    data: dataArr,
    header: columns,
    filename: '客户信息_' + databaseName.value + '.xlsx',
  });
}
...
const openAddPage = async () => {
  await useRouteApi(findAllByCusBendEq1, {schemaName: database})('')
    .then(async (data) => {
      if (data.total == 0) {
        await useRouteApi(customerSaveApi, {schemaName: database})({
          uniqueCustclass: '',
          cusCclassName: '默认分类',
          cusClass: '9999',
          cusClassGrade: '1',
...
const del = async (id, cusUnique, data) => {
  ...
  if (accvoucher > 0) {
    return createWarningModal({content: '此客户已生成凭证,不能删除'});
  }
  ...
  if (accvoucher21 > 0) {
    return createWarningModal({content: '此客户已设置辅助期初,不能删除'});
  }
...
// 查询 账套中 任务管理表 是否 有客户信息任务
const findByTask = async () => {
  let counttask = await useRouteApi(countByFunctionModule, {schemaName: database})({name: '客户信息'})
  if (counttask == 0) {
    const data = {
    ...
    let taskinfo: any = await useRouteApi(findByFunctionModule, {schemaName: database})({name: '客户信息'})
    if (parseInt(taskinfo.caozuoUnique) !== useUserStoreWidthOut().getUserInfo.id) {
      return createWarningModal({content: '操作员:' + taskinfo.username + ',正在编辑客户信息'});
      return false;
    } else {
      return true;
    }
...
async function bringOrgDataFunction(data) {
  data.forEach(async (a) => {
    ...
    await useRouteApi(duLiSaveApi, {schemaName: database})(a).then(async (t) => {
      // 埋点-操作日志
      let log = '操作内容【账套引入客户信息】,账套代码【' + accountInfo.value.coCode + '】,账套名称【' + accountInfo.value.companyName + '】,客户编码【' + t.custCode + '】,客户全称【' + t.custName + '】,客户简称【' + t.custAbbname + '】'
      /************** 记录操作日志 ****************/
      let map = {
        loginTime: new Date(+new Date() + 8 * 3600 * 1000).toJSON().substr(0, 19).replace("T", " "),
        userId: useUserStoreWidthOut().getUserInfo.username,
...
const dynamicAdReload = async (obj) => {
  ...
  // 独立账套
  if (obj.target.independent == '1' || obj.target.independent == undefined) {
    fetch()
  } else {
    findByCusClass()
  }
...
async function findByCusClass() {
  treeData.value = []
  // 是否设置权限控制
  let temp = await getCusTypeSeeSwitch({accId: database.value, dataBaseName: "customer_class"})
  if (temp !== '') {
    let cusClassAuthorData = await getCusClassAuthorData({
    ...
    if (cusClassAuthorData.length == 1) {
      treeData.value.push({
        title: '[' + cusClassAuthorData[0].cusClass + ']' + cusClassAuthorData[0].cusCclassName,
        key: cusClassAuthorData[0].uniqueCustclass,
        children: []
      })
      ...
      let deptTree = await getCusClassAuthorData({
        tableName: 'customer_class',
        userId: useUserStoreWidthOut().getUserInfo.id,
        accId: database.value
      })
    ...
    onOk: async () => {
      ...
      getSelectRows().forEach(async (v) => {
        ...
        let map = {
          ...
          optAction: '删除',
          optContent: '操作内容【删除客户信息】,账套代码【' + accountInfo.value.coCode + '】,账套名称【' + accountInfo.value.companyName + '】,客户编码【' + v.custCode + '】,客户简称【' + v.custName + '】,客户简称【' + v.custAbbname + '】',
        }
        await saveLog(map)
        /************** 记录操作日志 END ****************/
        ...
        v.optTime = logtime
        await useRouteApi(auditCustomerSave, {schemaName: database})(v)
      })
      pageReload()
    }
  })
...
async function findByCusAuthData() {
  // 是否设置权限控制
  let temp = await getCusTypeSeeSwitch({accId: database.value, dataBaseName: "customer"})
  if (temp !== '') {
    let data = await getCusDataAuthorData({
      tableName: 'customer',
      userId: useUserStoreWidthOut().getUserInfo.id,
      accId: database.value,
      classVal: pageParameter.uniqueCustclass,
