...
import {
  customerSaveCusClass,
  findAllByCusBendEq1, getCusClassAuthorData,
  getCusTypeSeeSwitch
} from "/@/api/record/system/customer_class";
import {findAllByFlag} from "/@/api/record/costomer_data/customer";
...
import {getCusDataAuthorData as getSupDataAuthorData} from "/@/api/record/supplier_data/supplier";
...
// 是否设置权限控制
  let temp=await getCusTypeSeeSwitch({accId:tenantId.value,dataBaseName:"supplier"})
  if(temp!==''){
    let temp=await getSupDataAuthorData({tableName:'supplier',userId:useUserStoreWidthOut().getUserInfo.id,accId:tenantId.value,classVal:'0',searchConditon: {requirement: 'custName',value: ''}})
  ...
  async function findByClass() {
    // 是否设置权限控制
    let temp=await getCusTypeSeeSwitch({accId:tenantId.value,dataBaseName:"customer_class"})
    if(temp!==''){
      let cusClassAuthorData=await getCusClassAuthorData({tableName:'customer_class',userId:useUserStoreWidthOut().getUserInfo.id,accId:tenantId.value})
      custmerClassList.value = cusClassAuthorData;
      formItems.value.uniqueCustclass=cusClassAuthorData[0].uniqueCustclass
    }else{
      // 全部客户分类
  ...
  async function findByCusAll() {
    // 是否设置权限控制
    let temp=await getCusTypeSeeSwitch({accId:tenantId.value,dataBaseName:"customer"})
    if(temp!==''){
      let temp=await getCusDataAuthorData({tableName:'customer',userId:useUserStoreWidthOut().getUserInfo.id,accId:tenantId.value,classVal:'0',searchConditon: {requirement: 'custName',value: ''}})
      custmerList.value =temp
    }else{
      // 全部客户信息
      await useRouteApi(findAllByFlag,{schemaName: tenantId})('1').then((res) => {
  ...
  const closeOk = async () => {
    ...
    let map={
      ...
      optAction:'新增申请',
      optContent:'操作内容【关闭新增申请-新增】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】',
    }
    await saveLog(map)
    /************** 记录操作日志 End ****************/
    emit('closeOk');
