...
import {saveAll as orgCusClassSave} from "/@/api/org/cus_class/org_cus_class";
    ...
    onOk: async () => {
      ...
      getSelectRows().forEach(async (a)=>{
        ...
        let map2={
          ...
          optRange:'2',
          optAction:'分配',
          optContent:'【集团客户分类分配】,分类编码【'+a.cusClass+'】,客户名称【'+a.cusCclassName+'】',
        }
        await saveLog(map2)
        let map3={
          loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
          ...
          optRange:'0',
          optAction:'分配',
          optContent:'【组织客户分类分配】,分类编码【'+a.cusClass+'】,客户名称【'+a.cusCclassName+'】',
        }
        await saveLog(map3)
        /************** 记录操作日志 END ****************/
      })
      ...
      if(orgCusClassAssignArr.length>0){
        orgCusClassAssignArr.forEach(async (a)=>{
          ...
          let map2={
            ...
            optRange:'2',
            optAction:'分配',
            optContent:'【集团客户信息分配】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
          }
          await saveLog(map2)
          let map3={
            loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
            ...
            optRange:'0',
            optAction:'分配',
            optContent:'【集团客户信息分配给组织】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
          }
          await saveLog(map3)
          /************** 记录操作日志 END ****************/
        })
      ...
      if(a==0){
        getSelectRows().forEach(async (a)=>{
          ...
          let map={
            ...
            optRange:'2',
            optAction:'分配',
            optContent:'操作内容【取消分配】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
          }
          await saveLog(map)
          /************** 记录操作日志 END ****************/
        })
