...
const CrudApi = {
  list: useRouteApi(findDefinedFileList, {schemaName: dynamicTenantId}),
  columns: [
    {
      title: '设置编码',
...
const del = async(data:any) => {
  await useRouteApi(deleteDefinedFile, {schemaName: dynamicTenantId})(data)
...
async function saveExcel(data:any) {
  await useRouteApi(excelDefinedFile, {schemaName: dynamicTenantId})(data)
...
async function reloadCurrentPage() {
  const res: any = await useRouteApi(getDefinedFileList, {schemaName: dynamicTenantId})({})
  cardList.value = res
...
async function saveData(data:any) {
  await useRouteApi(saveDefinedFile, {schemaName: dynamicTenantId})(data)
  await reloadCurrentPage()
  if (activeKey.value == true) {
    await reload()
  }
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let i = 0; i < checkRow.value.length; i++) {
          ...
          await useRouteApi(deleteDefinedFile, {schemaName: dynamicTenantId})(item)
        }
