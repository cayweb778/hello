        ...
        <div style="background:#0096c7;padding:10px;border-radius: 2px;margin-bottom:10px;display: flex;justify-content : space-between;">
          <div class="a1">
            <a-select v-model:value="formItems.selectType" style="width: 120px;font-size: 12px;" class="special_select">
              <a-select-option style="font-size: 12px;" value="recordCode">档案编码</a-select-option>
              <a-select-option style="font-size: 12px;" value="recordName">档案名称</a-select-option>
            </a-select>
            <a-input-search
              placeholder=""
...
import {findAllDefinedRecordList, saveDefinedRecord} from "/@/api/record/system/defined-record";
...
async function reloadRecord(){
  const res:any = await useRouteApi(findAllDefinedRecordList,{schemaName: dynamicTenantId})(definedCode.value)
...
async function reloadFile() {
  const res = await useRouteApi(findFileByCode, {schemaName: dynamicTenantId})(definedCode.value)
