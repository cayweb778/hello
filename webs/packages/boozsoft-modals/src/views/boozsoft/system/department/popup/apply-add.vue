  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;margin-top: 10px;margin-left: 10px;" v-if="isState=='0'">
    ...
    <div style="height: 100%" :class="isEdit?'nc-open-content':'nc-open-show-content'">
      <div class="open-content-up">
        ...
        <div>
          ...
          <a-input v-model:value="formItems.cellPhoneNum" :placeholder="isEdit?'请输入手机（不允许重复）':''"/>
          <span class="red_span"></span>
...
import {psnTypeFindAll} from "/@/api/record/system/psn-type";
...
import {findByTenantId, save} from "/@/api/record/system/group-psn-account";
...
const [register, {closeModal}] = useModalInner(async (data) => {
  ...
  const permissionList = await findByUserIdAndTenantId(useUserStoreWidthOut().getUserInfo.id,dynamicTenantId.value,'psn')
...
async function handleOk() {
  ...
  const psnList = await findByTenantId(formItems.value.originId)
  ...
  if (isAuto.value=='0'){
    ...
    //账套表插入数据
    await useRouteApi(saveSysPsn,{schemaName:dynamicTenantId})(psn)
    emit('save', unref(formItems))
    closeModal()
    saveClick.value=false
    return true
...
async function handleOkAdd() {
  ...
  if (isAuto.value=='0'){
    ...
    if (isChanged) {
      ...
      formItems.value.tenantId = changeBeforeModel.tenantId
      let dateTime = new_Date()
      formItems.value.successState = isAuto.value
      formItems.value.applyUser = useUserStoreWidthOut().getUserInfo.realName
      formItems.value.applyDate = dateTime
