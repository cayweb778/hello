  ...
  >
    ...
    <div class="bg-white" style="display: inline;width: 30%;float: left;margin-top: 5px;">
      <div style="width: 100%; height: 26px;text-align: center;background-color: rgb(216 216 216);">
        <label style="font-size: 14px;font-weight: bold;">支出功能分类</label>
      </div>
      <BasicTree
        title=""
...
import { introduceData, findSettModesInAll } from "/@/api/record/system/dept-class";

import {GetDeptTreeByFlag} from "/@/api/record/group/dept-class";
...
async function handleSelect(obj) {
  if(obj[0].toString()!==''){
    let d = await useRouteApi(findSettModesInAll,{schemaName: database})(pageParameter)
    setTableData(d.filter(item=> {
