    ...
    <div class="app-container-top">
      <div class="app-container-head">
        <div class="container-head-title" style="float: none;text-align: center;display: revert;">
          <b class="noneSpan">{{ titleName }}-部门总账</b>
        </div>
        ...
        <div style="margin-top: .5%;">
          <div style="display: inline-block;margin-left: 1%;font-size: 14px;width: 30%">
            <div style="display: inline-block;font-size: 14px;padding-top: 8px">
              <span style="font-size: 14px;color: #aaa;font-weight: bold">部门：</span>
              <a-select
                show-search
                placeholder="部门选择"
                ...
                v-model:value="pageParameter.dept"
              >
                <a-select-option style="width: 180px;font-size: 14px;" v-for="d in deptList" :value="d.uniqueCode">
                  {{ d.label }}
                </a-select-option>
              </a-select>
...
import { useDeptGlStore } from '/@/api/record/generalLedger/deptGeneralLedger'

import {onMounted, reactive, ref} from "vue";
import moment from "moment";
import {useCompanyOperateStoreWidthOut} from "/@/store/modules/operate-company";
import { initDynamics,assemblyDynamicColumn} from "./data";
import Query from "/@/views/boozsoft/system/dept-generalledger/popup/query.vue";
...
import {findAllDept, findDeptsheetDateOne, findDeptsheetDate} from "/@/api/record/generalLedger/data";
...
const lanMuData = {'accId':'','menuName':'部门总账','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
const CrudApi = {
  list: useRouteApi(findAllDept,{schemaName:databaseTrue}),
...
function resetDynamicColumnData() {
  // 先从数据查询是否已经设置
  lanMuData.accId = pageParameter.accId
...
const loadPage = (data)=>{
  ...
  titleName.value  = data.deptList[0].deptName
  ...
  pageParameter.minDept = data.minDeptCode
  pageParameter.maxDept = data.maxDeptCode

  pageParameter.jd = data.jd
  pageParameter.bz = data.bz

  deptList.value = data.deptList
  pageParameter.dept = data.deptList[0].uniqueCode
...
async function handleChangeMinDept(){
  titleName.value = deptList.value.filter(o=> o.uniqueCode === pageParameter.dept)[0].deptName
  closeFilterV()
  pageReload()
}
...
const exportExcelNow = async () => {
  ...
  //筛选条件
  const title = [
    "部门:"+ titleName.value +"   币种："+ bzName.value +"   样式："+ styleName.value +"   期间："+ pageParameter.strDate +'-'+ pageParameter.endDate +" ",
  ]
  ...
  exportExcel(sheet, 'xlsx',name+'-部门总账-'+pageParameter.strDate+'-'+pageParameter.endDate)
}
