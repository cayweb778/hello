...
<script setup="props, {content}" lang="ts">
  ...
  async function handleOk() {
    ...
    formItems.value.accId = pageParameter.accId
    formItems.value.databaseTrue = databaseTrue.value
    ...
    if(maxDept.value){
      formItems.value.minDeptCode = deptList.value.filter(o =>  o.uniqueCode === minDept.value)[0].deptCode
    }
    if(minDept.value){
      formItems.value.maxDeptCode =  deptList.value.filter(o =>  o.uniqueCode === maxDept.value)[0].deptCode
    }
    //科目 类别权限
    ...
    const queryPlanEntity={
      ...
      queryConditions:{
        ...
        maxDept: maxDept.value,
        minDept: minDept.value,
  ...
  async function AllCondition() {
    ...
    userId.value = userStore.getUserInfo.id
    // 会计科目
    kmList.value = await useRouteApi(findCodeKmDept,{schemaName:databaseTrue.value})({
      strDate: strDate.value,
      endDate: endDate.value,
    ...
    deptList.value = await useRouteApi(findDeptAll,{schemaName:databaseTrue.value})({
  ...
  async function getQueryPlan() {
    let queryPlanIngo= await findByQueryPlan(databaseTrue.value,'部门总账')
