...
<!--        <div style="float: right; position: relative">
          <a-select v-model:value="formItems.selectType" style="width: 120px;font-size: 12px;" class="special_select">
            <a-select-option style="font-size: 12px;" value="ecCode">存放位置编码</a-select-option>
            <a-select-option style="font-size: 12px;" value="ecName">存放位置名称</a-select-option>
          </a-select>
          <a-input-search
            placeholder=""
    ...
    <div class="app-container">
      ...
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        <div style="width: 60%; float: right;margin-left: 5px;">
          <BasicTable
                      ref="tableRef"
                      :loading="loadMark"
            ...
            <template #ecCode="{ record }">
              <span style="float: left" v-if="record.ecCode!='' && record.ecCode!=null">{{ record.ecCode}}</span>
            </template>
            <template #ecName="{ record }">
              <span style="float: left" v-if="record.ecCode.length === 5"> &#8195;&#8195;{{ record.ecName }}</span>
              <span style="float: left" v-else-if="record.ecCode.length === 7"> &#8195;&#8195;&#8195;&#8195;{{ record.ecName  }}</span>
              <span style="float: left" v-else>{{ record.ecName  }}</span>
            </template>
...
import {deleteDept, saveDept, editFlag, getDeptListById, GetDeptTree,excelDept,getTotalData} from '/@/api/record/system/fa-location'
...
const exportExcelNow = async () => {
  ...
  // debugger
  const arrHeader = ['存放位置编码','存放位置名称','上级名称','状态'];
