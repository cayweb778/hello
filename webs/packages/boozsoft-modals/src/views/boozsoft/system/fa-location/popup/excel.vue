    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">存放位置</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.ecCode = item['存放位置编码']
      item1.ecName = item['存放位置名称']
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行存放位置编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行存放位置名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.ecCode==item2.ecCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行存放位置编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.ecName==item2.ecName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行存放位置名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      for (let j = 0; j < deptList.value.length; j++) {
        ...
        if (dept.ecCode == item.ecCode) {
          msg = '第' + (i + 1) + '行存放位置编码重复,不能进行部门信息导入'
          return false
        }
        if (dept.ecName == item.ecName) {
          msg = '第' + (i + 1) + '行存放位置名称重复,不能进行部门信息导入'
          return false
        }
      }
    }
...
function exportExcel() {
  const arrHeader = ['存放位置编码','存放位置名称','上级名称'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '存放位置导入模板.xlsx',
  });
}
