      ...
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        <div class="bg-white" style="width: calc(100% - 650px); float: right;margin-left: 5px;margin-top: -0.5px;">
          <BasicTable
            ...
            <template #peopertyName="{ record }">
              <span style="float: left" v-if="record.peopertyName!='' && record.peopertyName!=null">{{record.peopertyName}}</span>
            </template>

            <template #faDepMethod="{ record }">
              <span style="float: left" v-if="record.faDepMethod!='' && record.faDepMethod!=null">{{jiti(record.faDepMethod)}}</span>
            </template>
...
const columns: any = [
  ...
  {
    title: '属性名称',
    dataIndex: 'peopertyName',
    ellipsis: true,
    width: '15%',
    customHeaderCell: () => {		// 表头加粗
      return {style: {'font-weight':'bold'}} // return 想要设置的样式
    },
    slots: {customRender: 'peopertyName'},
  ...
  },
  {
    ...
    slots: {customRender: 'faDepMethod'},
    customCell: () => {		// 在此处可以修改单元格中的样式
      return {style: {'text-align': 'center', '-webkit-line-clamp': 2}} // return 想要设置的样式
    },
  },
...
const [registerTable, {
  ...
  api: async (params) => {
    ...
    //const aa = await GetFaPropertyAll()
    const aa = await useRouteApi(GetFaPropertyAll, {schemaName: dynamicTenantId})({})

    if (params.data == null) {
      return aa
    }
    return aa.filter(it => it.peopertyId == params.data.keys[0])
  },
  columns: columns,
  bordered: true,
...
async function saveData(data: any) {
  isShowTree.value = false
  data.uniqueCustclass = buildShortUUID()
   await useRouteApi(SaveFaProperty, {schemaName: dynamicTenantId})(data)
  isShowTree.value = true
  reload2()
  reload()
  const aa = await useRouteApi(GetFaPropertyAll, {schemaName: dynamicTenantId})({})
...
async function delList() {
  createConfirm({
    ...
    onOk: async() => {
      ...
      await useRouteApi(DeleteFaPropertyBy, {schemaName: dynamicTenantId})({
        ids
      })
      reload2()
      //刷新tree
...
const exportExcelNow = async () => {
  ...
  // debugger
  const arrHeader = ['属性编码','属性名称','计提方式','是否计提','状态'];
  const columnList = ['peopertyId','peopertyName','faDepMethod','flag','status']
  data.forEach(v=>{
    v.status = v.status === '1'?'是':'否'
    v.faDepMethod =  jiti(v.faDepMethod)
...
const loadPrint = (obj) => {
  ...
  data.forEach((item,index) => {
    let item1 = {}
    item1[0] = item.peopertyId
    item1[1] = item.peopertyName
    item1[2] = jiti(item.faDepMethod)
    item1[3] = formatFlag(item.flag)
    item1[4] = item.status === '1'?'是':'否'
