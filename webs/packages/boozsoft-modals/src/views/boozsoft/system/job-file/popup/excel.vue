    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span style="font-size: 16px;">导入内容：</span><span style="font-weight: bold;font-size: 16px;">职务档案导入</span><br/><br/>
...
import {getTypeList,getDeptList} from "/@/api/record/system/job-file";
import {useRouteApi} from "/@/utils/boozsoft/datasource/datasourceUtil";
const {
  createErrorModal
} = useMessage()
const arrHeader = ['职务编码','职务名称','职务类别','','','','',''];
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.ecCode = item['职务编码']
      item1.ecName = item['职务名称']
      item1.pname = item['职务类别']
      item1.workInfo = item['工作概要']
      item1.psnLevel = item['职务级别']
      item1.mangeLevel = item['管理级别']
      item1.estDate = item['设立日期']
      item1.flag = item['状态']
      //匹配对应参数
      item1.bend = '1'
      //上级名称
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行职位编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行职位名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.ecCode==item2.ecCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行职位编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.ecName==item2.ecName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行职位名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function checkExcel(){
  msg=''
  if (delist.value.length>0) {
    for (let i = 0; i < delist.value.length; i++) {
      const item = delist.value[i];
      //判断导入编码是否为空
      ...
      for (let j = 0; j < list.value.length; j++) {
        const dept = list.value[j];
        if (dept.ecCode == item.ecCode) {
          msg = '第' + (i + 1) + '行职位编码重复,不能进行导入'
          return false
        }
        if (dept.ecName == item.ecName) {
          msg = '第' + (i + 1) + '行职位名称重复,不能进行导入'
          return false
        }
      }
    }
...
function exportExcel() {
  const arrHeader = ['职务编码','职务名称','职务类别','工作概要','职务级别','管理级别','设立日期','状态'];  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '职务编码导入.xlsx',
  });
}
