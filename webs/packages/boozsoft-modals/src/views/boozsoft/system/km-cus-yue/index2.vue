      ...
      <div class="app-container-top">
        <div style="width: 100%;height: 100px;">
          ...
          <div style="width: 60%;float: right;margin-top: -77px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目客户余额表</span>
            ...
            <span style="display: block;color: black;font-size: 14px;margin-left: 10px;margin-top: 8px;">期间：{{ time.strDate }} - {{ time.endDate }}
    ...
    <div class="app-container" style="margin-top: -35px;">
      <div class="app-container-bottom">
        <PageWrapper dense content-full-height fixed-height content-class="flex">
          ...
          <Query @save="loadPage" queryType="科目客户余额表" @register="registerQueryPage" />
...
import {exportAll, exportAll2, findAll } from '/@/api/record/km-cus-yue/data';
import Query from "/@/views/boozsoft/system/km-cus-yue/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-cus-yue/popup/print.vue";
import {kmCusYueGlStore} from "/@/api/record/km-cus-yue/kmCusYueColumn";
//////////////////////////////// END /////////////////////////////////////////
...
const lanMuData = {'accId':'','menuName':'科目客户余额表','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
async function goRouter(uniqueCode) {
  ...
  router.push({
    path: '/zhongZhang/account-book/abf-customer-contacts/abfcc-kemu-mxable',
    query: pageParameter,
  });
}
onMounted( async ()=>{
...
const loadPage = async (data)=>{
  ...
  pageParameter.dynamicTenantId=data.pageParameter.database
...
function thisPageImport(){
  ...
  exportExcel(sheet, 'xlsx',accNameAll.value+'-科目客户余额表-'+time.strDate+'-'+time.endDate)
...
async function onlySheetExport() {
  ...
  await useRouteApi(exportAll2,{schemaName:dynamicTenantId})(pageParameter)
    .then(list=>{
      ...
      exportExcel2(sheet, 'xlsx',accNameAll.value+'-科目客户余额表-'+time.strDate+'-'+time.endDate)
      compState.loading = false;
    })
}
...
const loadPrint = async (obj) => {
  ...
  if(pageParameter.queryMark==='J'){
    let qcMdMoney,qcMcMoney,pzMdMoney,pzMcMoney,qmMdMoney,qmMcMoney = 0
    let printList: any = []
