...
import {
  ...
  findBzAll,
  findCodeKmByPeriod, findDeptAll,
...
async function newfindAllCode(s,start) {
  ...
  return list.filter(ck=>ck.bdept==='1' && ck.bend==='1');
}
...
async function AllCondition() {
  ...
  deptList.value = await useRouteApi(findDeptAll,{schemaName:database.value})('');
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'科目部门明细账')
