...
import {
  ...
  findBzAll,
  findCodeKmByPeriod, findCusAll, findAllAuthPeriodListByUserAndCode, findProject
...
async function handleOk() {
  if(kmList.value.length===0){
    return createConfirmPop('请先设置项目辅助科目')
  }
  if(deptList.value.length===0){
    return createConfirmPop('请先设置项目信息')
  }
...
async function AllCondition() {
  ...
  // 客户
  deptList.value = await useRouteApi(findProject,{schemaName:database.value})('');
