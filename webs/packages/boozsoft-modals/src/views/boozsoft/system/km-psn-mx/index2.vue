<template>
  <div>
      ...
      <div class="app-container-top">
        <div style="width: 100%;margin-top: 5px;height: 100px;">
          ...
          <div style="width: 60%;float: right;margin-top: -40px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目个人明细账</span>
    ...
    <Query @save="loadPage" queryType="科目个人明细表" @register="registerQueryPage" />
...
<script setup lang="ts">
  ...
  import { finByParameterAccuracy } from '/@/api/record/km_mingxi/data'; // 参数规则
  import { findAll, exportAll, exportAll2 } from '/@/api/record/km-psn-mx/data';
  import Query from '/@/views/boozsoft/system/km-psn-yue/popup/query.vue';
  import Print from '/@/views/boozsoft/system/km-psn-mx/popup/print.vue';
  import { KmPsnMxGlStore } from '/@/api/record/km-psn-mx/psnKmMxColumn';
  //////////////////////////////// END /////////////////////////////////////////
  ...
  import { findPsnAll } from '/@/api/record/generalLedger/data';
  ...
  onMounted(async () => {
    if (JSON.stringify(routemsg.value).length === 2) {
      ...
      // 个人
      deptList.value = await useRouteApi(findPsnAll, {
  ...
  function tableRow(row: any, num: any) {
    return (
      (row.cdigest == '当前合计' ? 'table-month-striped' : null) ||
  ...
  const confirm = (e: MouseEvent) => {
    ...
    createConfirm({
      ...
      onOk: async () => {
        // 调整数据库 列参数
        lanMuData.accId = dynamicTenantId.value.substring(0, dynamicTenantId.value.length - 5);
  ...
  function thisPageImport() {
    ...
    exportExcel(
      sheet,
      'xlsx',
      accNameAll.value + '-科目个人明细账-' + time.strDate + '-' + time.endDate,
    );
      ...
      useNewPrint({ data: ['l', 'px', 'a4', true] }, (doc) => {
        ...
        doc.autoTable({
          head: [
            ...
            ['期间', '凭证号', '个人编码', '个人名称', '摘要', '借方', '贷方', '方向', '余额'],
          ],
          body: printList,
          // startY: 60,
          styles: tableStyle(),
