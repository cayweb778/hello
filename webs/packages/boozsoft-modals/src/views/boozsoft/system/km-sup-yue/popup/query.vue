...
import {
  ...
  findBzAll,
  findCodeKmByPeriod, findCusAll, findAllAuthPeriodListByUserAndCode, findSupAll
...
async function handleOk() {
  if(kmList.value.length===0){
    return createConfirmPop('请先设置供应商辅助科目')
  }
  if(deptList.value.length===0){
    return createConfirmPop('请先设置供应商信息')
  }
  ...
  let map={
    ...
    optContent:'操作内容【'+prop.queryType+'查询】,账套代码【'+pageParameter.companyCode+'】,账套名称【'+pageParameter.companyName+'】,查询条件【查询日期：'+strDate.value+','+endDate.value+'】,' +
      '科目类型【'+styleValue.value+'】,凭证类别【'+pzType.value+'】,会计科目【'+minKm.value+'】,供应商【'+minDept.value+','+maxDept.value+'】,币种【'+bz.value+'】,包含未记账【'+ishaveRjz.value+'】'
  }
  await saveLog(map)
  /************** 记录操作日志 END ****************/
