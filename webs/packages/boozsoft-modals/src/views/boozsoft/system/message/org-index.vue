    ...
    <div class="app-container-top">
      <div class="app-container-head"><img :src="getThisIndexImg()" class="container-head-img" draggable="false" >
        ...
        <div class="ant-btn-group">
          ...
          <button type="button" class="ant-btn ant-btn-me" ant-click-animating-without-extra-node="false" @click="del"><span>删除</span></button>
...
<script setup lang="ts">
  ...
  import { findAll,delMessage } from "/@/api/record/message/data";
  import {columnProps} from "ant-design-vue/es/table/interface";
  import {djStatus} from "/@/api/record/eletronicInvoice_data/eletronic_invoice";
  ...
  // 科目名称标题
  const titleName = ref<String>("消息公告")
      ...
      createConfirm({
        ...
        onOk: async () => {
          ...
          for (let i = 0; i < checkRow.value.length; i++) {
            item.push(checkRow.value[i].messageId)
          }
          if(item.length > 0){
            await delMessage({ids:item})
