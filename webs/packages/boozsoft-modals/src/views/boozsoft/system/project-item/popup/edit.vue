  ...
  >
    ...
    <div class="nc-open-content" :class="isState=='2'?'is_show':''">
      <div class="open-content-up" style="text-align: center;margin-top: 50px;">
        ...
        <label>项目栏目样式：</label>
<!--        <a-input v-model:value="formItems.cateCode" placeholder="项目栏目样式"/>-->
        <a-select v-model:value="formItems.cateCode" :disabled="isCateCode" placeholder="请选择项目样式" style="width: 50%;">
        ...
        <span class="red_span" v-if="projectClassCtl=='1'">*</span>
        <span class="red_span" v-if="projectClassCtl!='1'"></span>
        <br/>
...
import {findByCode, findByName, saveProjectItem} from "/@/api/record/system/project-item";
import {useRouteApi} from "/@/utils/boozsoft/datasource/datasourceUtil";
import {cateFindStateFlag} from "/@/api/group_project_category/project_category";
import {findAllList} from "/@/api/record/system/project";
...
const [register, {closeModal}] = useModalInner(async(data: any) => {
  ...
  isCateCode.value = false
  if (data.data.id!=null && data.data.id!='' && data.data.itemCode!=null && data.data.itemCode!=''){
    const res: any = await useRouteApi(findAllList, {schemaName: dynamicTenantId})({itemCode:data.data.itemCode,projectClassCtl:data.projectClassCtl})
    if (res.length > 0) {
      isCateCode.value = true
    }
  }
...
async function handleOk() {
  ...
  if (projectClassCtl.value=='1' && (formItems.value.cateCode == null ||formItems.value.cateCode == '')) {
    createErrorModal({
      iconType: 'warning',
      title: '保存失败',
...
async function handleOkAdd() {
  ...
  if (isChanged) {
    // emit('save', unref(formItems))
    // closeModal()
    await useRouteApi(saveProjectItem, {schemaName: dynamicTenantId})(formItems.value)
...
async function checkCode() {
  ...
  const res = await useRouteApi(findByCode, {schemaName: dynamicTenantId})(formItems.value.itemCode)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
...
async function checkName() {
  ...
  const res = await useRouteApi(findByName, {schemaName: dynamicTenantId})(formItems.value.itemName)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
