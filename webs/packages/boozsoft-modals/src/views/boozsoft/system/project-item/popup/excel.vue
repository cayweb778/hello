    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span style="font-size: 16px;">导入内容：</span><span style="font-weight: bold;font-size: 16px;">项目大类导入</span><br/><br/>
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      if (itemCode == null || itemCode == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行项目大类编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (itemName == null || itemName == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行项目大类名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i != j) {
          if (item1.itemCode == item2.itemCode) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行项目大类编码信息与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.itemName == item2.itemName) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行项目大类名称信息与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function checkExcel(){
  ...
  for (let i=0; i<list.value.length; i++){
    ...
    if (itemCode==null || itemCode==''){
      msg="第"+(i+2)+'行项目大类编码为空,不能进行信息导入'
      return false
    }
    //判断导入名称是否为空
    const itemName = item['itemName']
    if (itemName==null || itemName==''){
      msg="第"+(i+2)+'行项目大类名称为空,不能进行信息导入'
      return false
    }
    //根据导入类型导入数据是否重复
    for (let j = 0; j < projectItemList.value.length; j++) {
      ...
      if (object['itemCode'] == itemCode) {
        msg = "第" + (i + 2) + '行项目大类编码重复,不能进行信息导入'
        return false
      }
      if (object['itemName'] == itemName) {
        msg = "第" + (i + 2) + '行项目大类名称重复,不能进行信息导入'
        return false
      }
    }
  }
...
const [register, {closeModal}] = useModalInner(async(data) => {
  ...
  projectItemList.value = useRouteApi(findAllProjectItemList, {schemaName: dynamicTenantId})({})
  cateList.value = await useRouteApi(cateFindStateFlag, {schemaName: useProjectStoreWidthOut().getDynamicTenantId})({})
