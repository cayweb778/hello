    ...
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;账套授权引入 － 项目大类
        </span>
      </div>
    </template>
...
import {saveProjectItem} from "/@/api/record/system/project-item";
...
async function reloadProjectItem(){
  checkRow.value = []
  state.selectedRowKeys = []
  const projectItemAccount = await findByTenantId(dynamicTenantId.value)
  projectItemAccountList.value = projectItemAccount.items
  tableDataAll.value = projectItemAccountList.value.filter(item=>{
    return item.flag != '1'
  })
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await useRouteApi(saveProjectItem,{schemaName: dynamicTenantId})(item1);
      item.flag = '1'
