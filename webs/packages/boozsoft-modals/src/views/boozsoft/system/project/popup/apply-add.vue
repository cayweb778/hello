...
import {findByTenantId,save} from "/@/api/record/system/group-project-account";
import {saveProject as originSaveProject} from "/@/api/record/system/origin-project";
import {
  saveProject as sysSaveProject,
  findByCode as sysFindByCode,
  findByName as sysFindByName,
  findAllList,
  findAllProject
...
async function handleOk() {
  ...
  const projectList = await findByTenantId(formItems.value.originId)
  ...
  if (isAuto.value=='0') {
    ...
    const item2 = {
      ...
      ctype: '2',
      originId: formItems.value.originId,
      tenantId: formItems.value.tenantId,
      assignUser: useUserStoreWidthOut().getUserInfo.id,
      assignDate: dateTime,
      flag: '1',
      acceptUser: useUserStoreWidthOut().getUserInfo.id,
    ...
    //账套表插入数据
    await useRouteApi(sysSaveProject,{schemaName:dynamicTenantId})(project)
    emit('save', unref(formItems))
    closeModal()
    saveClick.value=false
    return true
...
const [register, { closeModal }] = useModalInner(async ({data}) => {
  ...
  dynamicTenantId.value = data.tenantId
  if (null != data.isEdit) isEdit.value = data.isEdit
  const permissionList = await findByUserIdAndTenantId(useUserStoreWidthOut().getUserInfo.id,data.tenantId,'project')
  ...
  const res:any = await useRouteApi(findAllProject,{schemaName: dynamicTenantId})({})
...
async function checkCode() {
  ...
  if (changeBeforeModel._value.projectCode != formItems.value.projectCode && formItems.value.projectCode != null && formItems.value.projectCode != '') {
    const res = await useRouteApi(sysFindByCode,{schemaName: dynamicTenantId})(formItems.value)
    if (res.length != 0) {
      createErrorModal({
        iconType: 'warning',
        title: '提示',
...
async function checkName() {
  ...
  if (changeBeforeModel._value.projectName != formItems.value.projectName && formItems.value.projectName != null && formItems.value.projectName != '') {
    const res = await useRouteApi(sysFindByName,{schemaName: dynamicTenantId})(formItems.value)
    if (res.length != 0) {
      createErrorModal({
        iconType: 'warning',
        title: '提示',
