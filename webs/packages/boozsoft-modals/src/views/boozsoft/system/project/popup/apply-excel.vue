...
import {saveList} from "/@/api/record/system/group-project-account";
import {excelProject as excelOriginProject} from "/@/api/record/system/origin-project"
import {excelProject as excelSysProject} from "/@/api/record/system/origin-project"
...
async function cateCodeChange(){
  projectList.value = await useRouteApi(findProjectByCateCode,{schemaName: useProjectStoreWidthOut().getDynamicTenantId})({projectCateCode:cateCode.value,itemCode:itemCode.value,projectClassCtl:projectClassCtl.value})
...
const [register, { closeModal }] = useModalInner(async (data) => {
  ...
  tenantId.value = data.dynamicTenantId
  projectClassCtl.value = data.projectClassCtl
  const permissionList = await findByUserIdAndTenantId(useUserStoreWidthOut().getUserInfo.id,data.dynamicTenantId,'project')
  ...
  cateList.value.forEach(
    function (item) {
      if (item.projectCateCode == data.projectCateCode) {
  ...
  const items: any = await useRouteApi(getProjectItemList,{schemaName: data.dynamicTenantId})({})
  ...
  if (itemCode.value == '' || itemCode.value == null) {
    itemCode.value = itemList.value[0].itemCode
    await itemCodeChange()
  }
...
async function handleOk() {
  ...
  if (msg=='') {
    ...
    if (isAuto.value=='0'){
      ...
      //组织表插入数据
      await excelOriginProject(list.value)
      //账套表插入数据
      await useRouteApi(excelSysProject,{schemaName:tenantId})(list.value)
    }
...
async function getMaxCode() {
  ...
  if(temp.serialNumStr==null){
    temp.serialNumStr = 1
  }
  if(temp.serialNumLength==null){
    temp.serialNumLength = 4
  }
