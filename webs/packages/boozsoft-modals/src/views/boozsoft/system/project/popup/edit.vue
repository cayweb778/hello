...
<template>
  <BasicModal width="900px" v-bind="$attrs" title="项目信息" @ok="handleOk()" @register="register" :loading="loadMark" :canFullscreen="false">
    ...
    <div :class="isEdit?'nc-open-content':'nc-open-show-content'" style="height: 100%;margin-top: 30px;">
      <div class="open-content-up">
        ...
        <label>项目编码：</label>
        <a-input v-model:value="formItems.projectCode" :disabled="isManual" @blur="checkCode()" :placeholder="isEdit?'请输入项目编码':''"/>
        <span class="red_span">*</span>
        <label>项目分类：</label>
        <a-select
...
const [register, { closeModal }] = useModalInner(async ({data}) => {
  ...
  const res:any = await useRouteApi(findAllProject,{schemaName: useProjectStoreWidthOut().getDynamicTenantId})({})
