...
async function AllCondition() {
  ...
  deptList.value = await useRouteApi(findPsnAll,{schemaName:database.value})({accId: database.value});
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'个人科目余额表')
