...
async function reloadTable() {
  ...
  let len = 0
  const res = await useRouteApi(findByReportName, {schemaName: dynamicTenantId})({reportName: 'xjllb',iyear:year.value})
...
async function exportExcel() {
  if (checkRow.value.length > 0) {
    ...
    const bookType = 'xlsx'
    // 类型默认为xlsx
    writeExcel(wb, bookType, '现金流量表_'+ year.value + '年_'+pageParameter.companyName)
  } else {
    createErrorModal({
      iconType: 'warning',
      title: '导出',
...
/**********************栏目设置**********************/
import {initDynamics,assemblyDynamicColumn} from "../report-zcfzb/data";
// import {useMessage} from "/@/hooks/web/useMessage";
import {
  findDbLanMuList,
  saveLanMuList,
...
const lanMuData = {'accId':'','menuName':'财务报表','type':'账套',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
