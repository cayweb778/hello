...
<!--          <Button class="actod-btn" @click="carryDown">结转上年</Button>-->
      ...
      <div class="app-container-top-two">
        <div class="act-two-d-left">
          <span style="font-size: 16px;display:none;"><b>年度：</b></span>
          <a-select v-model:value="iyearselected" style="width: 120px;display:none;"
                    @change="yearChange">
        ...
        <div class="act-two-d-right">
          ...
          <div class="acttd-right-d-btns">
            ...
            <Button class="acttdrd-btn" @click="()=>{if (!visible){ visible = true;}return false}">
    ...
    <div class="bg-white"
         :style="{height: (windowHeight+53)+'px',display: 'inline',width: showCatalog?'90px':'20px',float: 'left',marginTop: '5px',overflow:showCatalog?'auto':'hidden',overflowY:'hidden'}">
      ...
      <!--      <div v-show="showCatalog"
           style="width: 100%; height: 26px;text-align: center;background-color: rgb(216 216 216);">
        <label style="font-size: 14px;font-weight: bold;">科目目录</label>
        <ShrinkOutlined title="收起" class="exit-class" @click="showCatalog=!showCatalog"/>
      </div>
      <BasicTree
        v-show="showCatalog"
        defaultExpandAll
        ...
        @select="handleSelect"/>
      <div v-show="!showCatalog"
           style="width: 20px;text-align: center; height: 100%;background-color: rgb(216 216 216);padding-top: 10%">
        <ArrowsAltOutlined class="exit-class" style="padding: 3px" title="展开"
                           @click="showCatalog=!showCatalog"/>
        <label style="font-size: 14px;font-weight: bold;">科目目录</label>
      </div>-->
    </div>
    <div class="app-container-bottom"
         :style="{height: (windowHeight+60)+'px',display: 'inline',width: showCatalog?'calc( 100% - 90px )':'calc( 100% - 22px )',float: 'right',marginTop: '5px'}">
      <BasicTable
        ref="tableRef"
...
import {
  ...
  Drawer as ADrawer, Collapse, message, Divider, Button, Progress,Tabs
} from "ant-design-vue"
...
import {findYearMinDate} from "/@/api/record/generalLedger/data";
import {onBeforeRouteLeave} from "vue-router";
import {
  assemblyDynamicColumn,
  combineParameters,
  initDynamics
} from "/@/views/boozsoft/system/subjectInitialBalanceFuZhu/data";
...
const accountingInfoModel = reactive({
  definitions: null,
  details: null,
})
// 数量/外币 栏目是否显示
...
const assemblyChildren = (it, aList) => {
  ...
  if (tList.length > 0) {
    let list = [{
      'rowType': '1',
      text: '北京希哥科技;',
      children: [{'rowType': '2', text: '20220303;张靓颖;天下无双'}, {
        'rowType': '2',
        text: '2022034;周华健;江湖笑'
      }]
    }]
    // 得到辅助核算类型
