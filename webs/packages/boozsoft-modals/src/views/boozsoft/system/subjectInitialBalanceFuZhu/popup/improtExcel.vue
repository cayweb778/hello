...
<script setup="props, { content }" lang="ts">
  ...
  const headers = ref({
    ...
    datasource: JSON.stringify({
      databaseName: '',
      schemaName: useCompanyOperateStoreWidthOut().getSchemaName+'-2021',
    }),
  });
  ...
  const handleDownByData = async (val: any) => {
    ...
    const lastCodeList = await useRouteApi(findAllByBendAndIyearOrderByCcode,{schemaName:databasenum})({ bend: '1', iyear: iyear123.value });
    var arr = [];
