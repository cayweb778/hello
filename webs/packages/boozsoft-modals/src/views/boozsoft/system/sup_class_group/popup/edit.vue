  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height: 25px;font-size: 16px;"><FormOutlined style="font-size: 18px;font-weight: bold"/>&nbsp;&nbsp;供应商分类</span>
      </div>
    </template>
    <div
...
<script setup="props, { content }" lang="ts">
  ...
  import {
    ...
    verifyCusClass, verifyCusClassName,
  ...
  async function handleOk() {
    if(formItems.value.cusClass==='' || formItems.value.cusClass===undefined){
      return createErrorPop('请填写分类编码')
    }
    if(formItems.value.cusClass!==oldcusClass.value){
    ...
    if(formItems.value.cusCclassName==='' || formItems.value.cusCclassName===undefined){
      return createErrorPop('请填写分类名称')
    }else{
      if(formItems.value.cusCclassName!==oldcusCclassName.value){
        const sumName = await verifySuperCusClassName(
        ...
        const sumName2 = await verifyCusClassName(
          formItems.value.cusCclassName,
        );
        if (sumName2 > 0) {
          return  createErrorPop('同级别分类名称已存在')
