<template>
  ...
  >
    ...
    <div style="margin-top: 40px;margin-left: 30px;">
      <span style="font-size: 20px;">导入内容：</span><span style="font-weight: bold;font-size: 20px;">供应商信息</span>
      <p/>
...
import {importCus} from "/@/api/record/supplier_data/supplier";
...
const handleDownByData = async (val: any) => {
  ...
  const columns2 = ['供应商编码','供应商全称','供应商简称','税号','分类名称','母公司','对应客户','省','市','区','详细地址','电话','手机','网站','EMail','行业性质','开户银行','开户地','银行账户','银行行号','联系人','价格级次','税率']
  aoaToSheetXlsx({
    data: [],
    header: val === '1'?columns1:columns2,
    filename: databaseName.value+'-供应商信息导入模板.xlsx',
  });
}
  ...
  await useRouteApi(importCus,{schemaName: database})({params:formData,userid:useUserStoreWidthOut().getUserInfo.id}).then(async (info:any)=>{
    ...
    if(info[0].code==='200' || info[0].error==='error'){
      ...
      for (let i = 0; i < info[0].list.length; i++) {
        let log='操作内容【导入供应商信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,供应商编码【'+info[0].list[i].cusCode+'】,供应商全称【'+info[0].list[i].cusName+'】,供应商全称【'+info[0].list[i].cusAbbname+'】'
        /************** 记录操作日志 ****************/
        let map={
