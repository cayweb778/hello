    ...
    >
      <template #title>
        ...
        <span style="font-size: 20px;line-height:30px;">
          <DeleteOutlined style="font-size: 20px;font-weight: bold"/>&nbsp;&nbsp;回收站 － 供应商信息
        </span>
        </div>
      </template>
          ...
          <div style="background:#0096c7;padding:10px;border-radius: 2px;margin-bottom:10px;display: flex;justify-content : space-between;">
            <div class="a1">
              <a-select v-model:value="pageParameter.searchConditon.requirement" style="width: 120px;font-size: 12px;" class="special_select">
                ...
                <a-select-option style="font-size: 12px;" value="custName">供应商名称</a-select-option>
              </a-select>
              <a-input-search
                placeholder=""
...
import {delCusArr, findAllIsDel,recoverIsDel} from "/@/api/record/supplier_data/supplier";
    ...
    onOk: async() => {
      ...
      .then(async (a)=>{
          ...
          getSelectRows().forEach(async (v)=>{
            // 埋点-操作日志
            let logs='操作内容【还原供应商信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,供应商编码【'+v.custCode+'】,供应商全称【'+v.custName+'】,供应商简称【'+v.custAbbname+'】'
            /************** 记录操作日志 ****************/
            let map={
    ...
    onOk: async () => {
      await useRouteApi(delCusArr,{schemaName: database})({id:getSelectRows().map(a=>a.id).join(',')})
        .then(async (a)=>{
          ...
          getSelectRows().forEach(async (v)=>{
            // 埋点-操作日志
            let logs='操作内容【删除供应商信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,供应商编码【'+v.custCode+'】,供应商全称【'+v.custName+'】,供应商简称【'+v.custAbbname+'】'
            /************** 记录操作日志 ****************/
            let map={
              loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
              userId: useUserStoreWidthOut().getUserInfo.username,
...
const exportExcel123 = async () => {
  ...
  const columns = [
    {title: '供应商编码',key: 'custCode'},
    {title: '供应商全称',key: 'custName'},
    {title: '供应商简称',key: 'custAbbname'},
  ...
  aoaToSheetXlsx({
    ...
    filename: accountInfo.value.companyName+'-供应商信息.xlsx',
  });
}
