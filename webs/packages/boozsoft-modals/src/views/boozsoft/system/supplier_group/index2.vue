        ...
        <div style="width: calc(100% - 250px); float: right;margin-left: 5px;">
          ...
          >
            ...
            <template #successState="{ record }">
              <span v-if="record.successState!=='11'">
...
<script setup lang="ts">
  ...
  import {
    ...
    delCusGroup,findByOrgAssigAccountCtype,
  ...
  import {
    ...
    GroupCustomerSaveApi,findAllCustomerClass
  } from "/@/api/record/system/supplier_class_group";
  import CustomerAdd from '../sup_class_group/popup/save.vue';
...
const DynamicColumn=null
  ...
  const windowHeight = (window.innerHeight - (305))
...
async function closePage() {
  ...
  const rowDel = async () => {
    ...
    if(noDel.length>0){
      return createWarningModal({ content: noDel.join(',')+'供应商已分配组织,不能删除!' });
    }else{
      createConfirm({
        iconType: 'warning',
        title: '提示',
        ...
        onOk: async() => {
          okDel.forEach(async (v)=>{
            ...
            let map={
              ...
              optRange:'2',
              optAction:'删除',
              optContent:'操作内容【删除供应商信息】,供应商编码【'+temp[0].custCode+'】,供应商全称【'+temp[0].custName+'】,供应商简称【'+temp[0].custAbbname+'】',
            }
            await saveLog(map)
            /************** 记录操作日志 END ****************/
            reload()
  ...
  async function findByOrgPermission() {
    ...
    // 权限表：是否具有审批权限组织
    let temp=findByPermission.items.filter(a=>a.ctype=='1'&&a.baseName=='供应商'&&a.userId==useUserStoreWidthOut().getUserInfo.id)
    if(temp.length>0){
