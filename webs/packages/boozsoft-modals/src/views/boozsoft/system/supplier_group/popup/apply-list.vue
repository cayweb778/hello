    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <FormOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;生效审批 － 供应商信息
        </span>
        </div>
      </template>
      ...
      <a-modal v-model:visible="visibleModel2" :closable="false" @ok="modalSubmit" style="margin-top: 10px;" width="850px">
        <template #title>
          <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <FileSearchOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;集团供应商
        </span>
          </div>
        </template>
        <br>
...
import {findAllOrgCusClass, saveAll as orgCusClassSave} from "/@/api/org/sup_class/org_sup_class";
import {saveLog} from "/@/api/record/system/group-sys-login-log";
import Edit from '/@/views/boozsoft/system/supplier/popup/edit.vue';
...
async function modalSubmit() {
  ...
  data1.forEach(async (data)=>{
    ...
    await GroupCustomerAddApi(data).then(async (a)=>{
      ...
      let map={
        ...
        optAction:'新增',
        optContent:'操作内容【新增供应商信息】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】',
      }
      // 集团不存在的需要加日志
  ...
  await originCustomerSaveAll(data1).then(async (a)=>{
    ...
    let map={
      ...
      uniqueCode:orgUnique.value,
      optAction:'引入',
      optContent:'操作内容【引入供应商信息】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】',
    }
    await saveLog(map)
    /************** 记录操作日志 End ****************/
  })
...
async function applyNoSubmit() {
  ...
  await editGroupCusAll(getSelectRows()).then(async (a)=>{
    ...
    let map={
      ...
      optRange:'2',
      optAction:'生效审批',
      optContent:'操作内容【驳回】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】',
    }
    await saveLog(map)
    /************** 记录操作日志 END ****************/
  })
        ...
        if(temp.length==0){
          await editByIdSuccessState(getSelectRows()[i].id,useUserStoreWidthOut().getUserInfo.realName).then(async ()=>{
            ...
            let map={
              ...
              optRange:'2',
              optAction:'审批',
              optContent:'操作内容【审批同意】,供应商编码【'+getSelectRows()[i].custCode+'】,供应商全称【'+getSelectRows()[i].custName+'】,供应商简称【'+getSelectRows()[i].custAbbname+'】',
            }
            await saveLog(map)
            /************** 记录操作日志 END ****************/
            ...
            await groupCusAssignOrgSave([getSelectRows()[i]]).then(async (tx)=>{
              ...
              let map={
                ...
                optRange:'2',
                optAction:'分配',
                optContent:'操作内容【分配给组织供应商】,供应商编码【'+getSelectRows()[i].custCode+'】,供应商全称【'+getSelectRows()[i].custName+'】,供应商简称【'+getSelectRows()[i].custAbbname+'】',
              }
              await saveLog(map)
              /************** 记录操作日志 END ****************/
            })
            ...
            if(getSelectRows()[i].ctype=='2'){
              ...
              await originCustomerSaveAll([getSelectRows()[i]]).then(async ()=>{
                ...
                let map={
                  ...
                  optRange:'0',
                  optAction:'新增',
                  optContent:'操作内容【新增供应商信息】,供应商编码【'+getSelectRows()[i].custCode+'】,供应商全称【'+getSelectRows()[i].custName+'】,供应商简称【'+getSelectRows()[i].custAbbname+'】',
                }
                await saveLog(map)
                /************** 记录操作日志 END ****************/
              })
          ...
          createConfirm({
            iconType: 'warning',
            title: '提示',
            content: '供应商全称【' + getSelectRows()[i].custName + '】,供应商简称【' + getSelectRows()[i].custAbbname + '】。在集团中已存在,请选择引用',
            onOk: async () => {
              // 同组织供应商全称、供应商简称重复删除
...
async function openPageData(data) {
  ...
  let map={
    ...
    optRange:'2',
    optAction:'生效审批',
    optContent:'操作内容【点击生效审批-查看】,供应商编码【'+data.custCode+'】,供应商全称【'+data.custName+'】,供应商简称【'+data.custAbbname+'】',
  }
  await saveLog(map)
  /************** 记录操作日志 END ****************/
  openEditPage(true, {
    ...
    onOk: async () => {
      ...
      getSelectRows().forEach(async (a)=>{
        let map={
          ...
          optRange:'2',
          optAction:'生效审批',
          optContent:'操作内容【点击生效审批-删除】,供应商编码【'+a.custCode+'】,供应商全称【'+a.custName+'】,供应商简称【'+a.custAbbname+'】',
        }
        await saveLog(map)
      })
    }
