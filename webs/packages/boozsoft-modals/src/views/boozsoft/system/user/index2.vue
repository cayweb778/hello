    ...
    <div class="app-container">
      <div class="app-container-head">
        ...
        <div class="ant-btn-group" style="float: right">
          ...
          <button type="button" class="ant-btn ant-btn-me" @click="startUnLockBefore">解锁</button>
...
<!--          <a-button @click="activeKey = !activeKey">
          ...
          <a-popover placement="bottom">
            <template #content>
                 <span class="group-btn-span-special" @click="onChangeSwitch('')"
              ...
              <span class="group-btn-span-special" @click="onChangeSwitch('1')"
              ...
              <span class="group-btn-span-special" @click="onChangeSwitch('0')"
    ...
    <div class="app-container">
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        <Pwd @save="resetData" @register="registerPwdPage"/>
        <Excel
          @reload="pageReload"
        ...
        <div style="width: 100%;">
          ...
          >
            ...
            <template #flag="{ record }">
              <Tag :color="record.flag === '1'?'green':record.flag === '2'?'orange':'red'">{{
...
const [registerTable, {setTableData, getColumns, setColumns, reload, getDataSource}] = useTable({
  ...
  indexColumnProps: {width: 60, fixed: true},
  pagination: {
    pageSize: 200,
    simple: true
  },
...
async function saveDataBatch(data) {
  data.operName = useUserStoreWidthOut().getUserInfo.realName  || '未知'
...
const resetData = async (data) => {
  ...
  await changePass(parm).then((res) => {
    createInfoModal({title: '温馨提示', content: '操作员已完成更新！'})
    writeLog('修改', res, null)
  }).catch(() => {
    createInfoModal({title: '温馨提示', content: '操作员更新失败！'})
  }).finally(() => {
    reload()
...
const openDelBefore = async () => {
  if (tableSelectedRowKeys.value.length != 1) {
    ...
    if (f) {
      ...
      // 执行删除操作
      await updateFlag(tableSelectedRowObjs.value[0].id, '0').then((a)=>writeLog("停用", tableSelectedRowObjs.value[0], null));
      pageReload()
    }
  }
}
...
const startUnLockBefore = () => {
  if (tableSelectedRowKeys.value.length != 1 ||  tableSelectedRowObjs.value.filter(it=>it.flag == '2').length == 0) {
    createWarningModal({title: '温馨提醒', content: "请先选中一条要解锁的数据数据！"});
  }else {
    createConfirm({
      iconType: 'warning',
      title: '解除锁定',
      content: '是否将当前操作员解除锁定!',
      onOk: async () => {
        let id = tableSelectedRowObjs.value[0].id
        await updateFlag(id, '1').then((a)=>writeLog("解锁", tableSelectedRowObjs.value[0], null));
        pageReload()
      }
    });
  }
...
function resetDynamicColumnData() {
  // 先从数据查询是否已经设置
  lanMuData.accId = getCurrentAccountName(false)
  const statiList = initDynamics()['DATA']
...
async function writeLog(action, a, content) {
  ...
  old.beiyong5 = null
  // await writeAudit(action, old) 遗弃
}
