    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span style="font-size: 16px;">导入内容：</span><span style="font-weight: bold;font-size: 16px;">凭证类别导入</span><br/><br/>
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      if (voucherTypeCode == null || voucherTypeCode == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i+2) + '行类别字简称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (voucherTypeName == null || voucherTypeName == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i+2) + '行类别字名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i != j) {
          if (item1.voucherTypeCode == item2.voucherTypeCode) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行类别字简称与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.voucherTypeName == item2.voucherTypeName) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行类别字名称与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function checkExcel(){
  ...
  for (let i=0; i<list.value.length; i++){
    ...
    if (voucherTypeCode==null || voucherTypeCode==''){
      msg="第"+(i+2)+'行类别字简称为空,不能进行信息导入'
      return false
    }
    //判断类别字名称是否为空
    const voucherTypeName = item['voucherTypeName']
    if (voucherTypeName==null || voucherTypeName==''){
      msg="第"+(i+2)+'行类别字名称为空,不能进行信息导入'
      return false
    }
    //根据导入类型导入数据是否重复
    for (let j = 0; j < voucherTypeList.value.length; j++) {
      ...
      if (object['voucherTypeCode'] == voucherTypeCode) {
        msg = "第" + (i + 2) + '行类别字简称重复,不能进行信息导入'
        return false
      }
      if (object['voucherTypeName'] == voucherTypeName) {
        msg = "第" + (i + 2) + '行类别字名称重复,不能进行信息导入'
        return false
      }
    }
  }
