  ...
  <div class="app-container">
    ...
    <div class="app-container-bottom"/>
        ...
        >
          ...
          <template #jmoney="{ record }">
          <span style="float: right">
           {{ money(record.jmoney) }}
          </span>
          </template>

          <template #dmoney="{ record }">
          <span style="float: right">
           {{ money(record.dmoney) }}
          </span>
          </template>
...
<script setup lang="ts">
  ...
  import {
    findAllHexiaoHistory, backHx
  } from '/@/api/record/system/write-off'
  import { useHxhistoryStore } from '/@/store/modules/accvoucher-hxls'
  ...
  // 全局常量
  const accvoucherStore = useHxhistoryStore()
  // 页面变量
  ...
  const lanMuData = {'accId':'','menuName':'往来核销历史','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
  // 表格参数
  ...
  const CrudApi = reactive({
    list: useRouteApi(findAllHexiaoHistory,{schemaName:manipulateDbName.value}),
  ...
  onMounted( ()=>{
    ...
    if(route.query.isHxindex === '1'){
      pageParameter.biZhong  = route.query.biZhong
      pageParameter.bzName = route.query.bzName
      pageParameter.ifUnit = route.query.ifUnit
      pageParameter.km = route.query.km
      pageParameter.queryMark  = route.query.queryMark
      pageParameter.reloadMark = route.query.reloadMark
      pageParameter.showRulesSize = route.query.showRulesSize
      console.log(JSON.stringify(route.query.searchConditon))
      console.log(JSON.parse(JSON.stringify(route.query.condition)))
      console.log(route.query.filterConditon)
      //pageParameter.condition = route.query.condition
      //pageParameter.filterConditon = route.query.filterConditon
      //pageParameter.searchConditon  = route.query.searchConditon
  ...
  const backselectHx = async(data)=>{
    //反核销
    if (checkRow.value.length > 0) {
      let list = []
      checkRow.value.forEach(v=>{
        list.push(v.vouchUnCode)
      })
      pageParameter.list = list
      console.log(list)
      await useRouteApi(backHx, {schemaName: manipulateDbName.value})(pageParameter)
      pageReload()
    }else{
      createWarningModal({
        iconType: 'warning',
