      ...
      <div class="open-content-up" style="margin-top: 20px;">
        <div style="text-align: center;">
          ...
          <span style="margin-left: -196px;">
            ...
            <span style="margin-left: -196px;">
             <label>排序编号：</label>
            <a-select v-model:value="formItems.panelOrder" disabled style="width: 150px">
...
import {findAllByCurrency} from "/@/api/codekemu/codekemu";
...
const [register, { closeModal }] = useModalInner(async(data) => {
  ...
  if(data.data!==''){
    ...
    console.log(data.data.panelFormula)
  ...
  // 会计科目
  codelist.value=await useRouteApi(findCodeKmByPeriod,{schemaName: database})({ strDate: formItems.value.panelIyear+'-01', endDate: 'test', accId: database.value.substring(0,database.value.length-5),userId:userStore.getUserInfo.id });
})
