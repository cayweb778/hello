  ...
  <div class="app-container">
    <div class="app-container-top">
      <div class="app-container-top-one">
        <div class="act-one-d-left" ></div>
        <div class="act-one-d-title">
          <span class="acto-d-title-span">现金流量凭证查询</span>
        </div>
      ...
      <div class="app-container-top-two">
        <div class="act-two-d-left">
          <AccountPicker v-if="showThis" readonly="" theme="three" @reloadTable="pageReload2"/>
        </div>
...
const pageReload = async (currentPageNum) => {
  ...
  if (true /*|| pageParameter.condition.constant.queryType == '1'*/) {
    ...
    data.accId = manipulateDbName.value.split('-')[0]+'-'+manipulateDbName.value.split('-')[1]
    loadMark.value = true
    ...
    for (let accId of accIds) {
      tables[accId] = await useRouteApi(findAllAccvoucherXianJin, {schemaName: accId + '-' + manipulateDbName.value.split('-')[2]})(data)
...
const pageReload2 = async (obj) => {
  ...
  data.accId = manipulateDbName.value.split('-')[0]+'-'+manipulateDbName.value.split('-')[1]
  loadMark.value = true
  let res = await useRouteApi(findAllAccvoucherXianJin, {schemaName: obj.accId + '-' + obj.year})(data)
  if (res != null && res.items.length > 0) {
...
const loadPage = async (data) => {
  ...
  unitName.value = settings.basisMap.unitName || data.constant?.thisAdInfo?.accNameCn
