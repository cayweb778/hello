    ...
    <div class="app-container-top">
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 40%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">银行日记账</b>
...
function printData(obj) {
  ...
  if (showRules.value == 'J') {//金额式
    ...
    list.forEach((item,index) => {
      ...
      item1[0] = item.idate
      item1[1] = item.pzNum
      item1[2] = setString(item.zhaiyao,27)
      item1[3] = setString(item.dfkemu,15)
      item1[4] = setString(item.dfUnit,15)
      item1[5] = item.jieMoney
      item1[6] = item.daiMoney
      item1[7] = item.fangxiang
      item1[8] = item.yueMoney
      ...
      if (item.zhaiyao!='本日合计' && item.zhaiyao!='本月合计' && item.zhaiyao!='本年累计'){
        ...
        if ((index)>0 && (index)%26==0){
          ...
          item2[3] = ''
          item2[4] = ''
          item2[5] = toThousandFilter(jieDayMoney)
          item2[6] = toThousandFilter(daiDayMoney)
          item2[7] = item.fangxiang
          item2[8] = item.yueMoney
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '银行日记账', '', '', '', '', ''],
          ...
          ['日期', '凭证字号', '摘要', '对方科目', '对方单位', '借方', '贷方', '方向', '余额']],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
        margin: {
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '银行日记账', '', '', '', '', ''],
          ...
          ['日期', '凭证字号', '摘要', '对方科目', '对方单位', '外币借方', '外币贷方', '方向', '外币余额']],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
        margin: {
...
async function exportExcel() {
  ...
  if (showRules.value == 'J') {//金额式
    ...
    const bookType = 'xlsx'
    // 类型默认为xlsx
    // writeExcel(wb, bookType, pageParameter.companyName + '_' + thisInterval.value + '_银行日记账')
    writeExcel(wb, bookType, '银行日记账_'+pageParameter.companyName)
  } else if(showRules.value == 'SJ') {//外币式
    if (waibi==''){
      createErrorModal({
        iconType: 'warning',
    ...
    dataArr.value.filter(item=>item.zhaiyao!=null&&item.zhaiyao!='').forEach(item => {
      ...
      item1[5] = item.jieMoney
      item1[6] = item.jiewb
      item1[7] = item.daiMoney
      item1[8] = item.daiwb
      item1[9] = item.fangxiang
      item1[10] = item.yueMoney
      item1[11] = item.yuewb
      list.push(item1)
    })
    let qijian = thisInterval.value.replace(/-/g, '.').replace('~', ' - ')
    const sheet:any = [
      {
        ...
        merges: ['A2:C2', 'E2:G2', 'J2:L2','A3:A4','B3:B4','C3:C4','D3:D4','E3:E4','F3:G3','H3:I3','J3:L3'],
        sheetName: '银行日记账',
        cellStyle: [
          {
            cell: 'A1',
        ...
        colWidth: [8, 8, 25, 12, 12, 12, 12, 12, 12, 5, 12,12]
      }
    ]
    // 处理数据前
    if (!sheet || sheet.length <= 0) {
    ...
    sheet.forEach((item, index) => {
      ...
      if (foot || foot === '') {
        foot && data.push(foot);
        const str = ('A' + data.length) + (':L' + data.length)
        merges.push(str)
      }
      const ws = sheet_from_array_of_arrays(data,'');
      if (merges && merges.length > 0) {
      ...
      (function () {
        Object.keys(dataInfo).forEach(i => {
          if (i == '!ref' || i == '!merges' || i == '!cols') {
            ...
            } else if (i.substring(0, 1) == 'A' || i.substring(0, 1) == 'C' || i.substring(0, 1) == 'D' || i.substring(0, 1) == 'E') {
              dataInfo[i.toString()].s = {
                border: border === {} ? border : border || globalStyle.border,
                font: font || globalStyle.font,
                fill: fill || globalStyle.fill,
            ...
            } else if (i.substring(0, 1) == 'F' || i.substring(0, 1) == 'G' || i.substring(0, 1) == 'H' || i.substring(0, 1) == 'I' || i.substring(0, 1) == 'K' || i.substring(0, 1) == 'L') {
              dataInfo[i.toString()].s = {
                border: border === {} ? border : border || globalStyle.border,
                font: font || globalStyle.font,
                fill: fill || globalStyle.fill,
    ...
    const bookType = 'xlsx'
    // 类型默认为xlsx
    // writeExcel(wb, bookType, '银行日记账_'+pageParameter.companyName + '_' + thisInterval.value)
    writeExcel(wb, bookType, '银行日记账_'+pageParameter.companyName)
  }
}
