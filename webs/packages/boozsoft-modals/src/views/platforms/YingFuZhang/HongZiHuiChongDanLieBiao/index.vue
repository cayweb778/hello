...
<!--   <div><textarea>红字回冲单列表的页面</textarea>复制内容快速定位页面</div>-->
...
import {
  ...
  findArApHzhcdList, findArApHzhcdsByCcode
...
async function reloadTable(){
  ...
  let len = 0
  const res = await useRouteApi(findArApHzhcdList,{schemaName: dynamicTenantId})({billStyle:'FKD',iyear:pageParameter.year})
...
function formatBusStyle(busStyle){
  let str = busStyle
  //SKD收款单、YSD应收单、FKD付款单、YFD应付单
  if (busStyle=='SKD'){
    str = '收款单'
  } else if (busStyle=='YSD'){
    str = '应收单'
  } else if (busStyle=='FKD'){
    str = '付款单'
  } else if (busStyle=='YFD'){
    str = '应付单'
  }
  return str
}
...
function addPage(){
  ...
  router.push({
    path: '/YingFuZhang/ChongXiao/CaoZuo/CxHongZiHuiChong',
    query: pageParameter
  });
}
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'红字回冲单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            ...
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'红字回冲单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
          }
        }
        const list = await useRouteApi(findArApHzhcdsByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
        if (item.busStyle=='FKD') {
...
import {findArApYsyfByCcode, saveYsyf} from "/@/api/record/system/arApYsyf";
import {
