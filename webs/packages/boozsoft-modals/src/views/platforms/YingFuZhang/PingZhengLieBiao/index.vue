...
<!--   <div><textarea>凭证列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">凭证列表</b>
    ...
    <div class="app-container-bottom">
      ...
      >
        <template #ccode="{ record }">
          <a class="tableUStyle" v-if="record.billStyle!=''" @click="openPage(record)">{{ record.ccode }}</a>
          <span v-if="record.billStyle==''">{{ record.ccode }}</span>
        </template>
...
function openPage(data){
  ...
  } else if (data.billStyle=='XHD') {
    router.push({path: '/xs-arrive',query: {type:'info',ccode:data.ccode}});
  } else if (data.billStyle=='XSFP') {
    router.push({path: '/xs-bill',query: {type:'info',ccode:data.ccode}});
  }
}
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '凭证列表_'+pageParameter.companyName)
}
