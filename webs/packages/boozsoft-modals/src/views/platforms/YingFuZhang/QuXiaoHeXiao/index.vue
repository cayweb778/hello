...
<!--   <div><textarea>取消核销的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">撤销应付款核销</b>
...
<!--        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;font-weight: bold;">
              ...
              <TableSummaryCell :index="2" :align="'center'" style="background-color: #cccccc;font-weight: bold;">合&emsp;&emsp;计</TableSummaryCell>
              ...
              <TableSummaryCell :index="6" :align="'center'" style="background-color: #cccccc;font-weight: bold;"></TableSummaryCell>
              <TableSummaryCell :index="7" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{ toThousandFilter(hxIsum) }}</TableSummaryCell>
              <TableSummaryCell :index="8" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{ toThousandFilter(yue) }}</TableSummaryCell>
              ...
              <TableSummaryCell :index="11" :align="'center'" style="background-color: #cccccc;font-weight: bold;"></TableSummaryCell>
              <TableSummaryCell :index="12" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{ toThousandFilter(hxMoney) }}</TableSummaryCell>
              <TableSummaryCell :index="13" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{ toThousandFilter(idiscount) }}</TableSummaryCell>
            </TableSummaryRow>
          </TableSummary>
        </template>-->
        <template #summary>
...
async function reloadTable(){
  ...
  let len = 0
  const yueList = await useRouteApi(findQuxiaohexiaoYfdYueList,{schemaName: dynamicTenantId})(pageParameter.year)
  const xhdList = await useRouteApi(findQuxiaohexiaoYfdList,{schemaName: dynamicTenantId})(pageParameter.year)
  tableDataAll.value = []
...
const onSelectChange = (selectedRowKeys,row) => {
  ...
  checkRow.value = row.filter(item=>item.billStyle!=null)
};
...
function selectionChange(a) {
  editAndDelBtnShow.value=a.rows.filter(aa=>aa.billStyle=='1').length>0?true:false
}

function rowClick(a) {
  if(hasBlank(a.billStyle)){
  ...
  if (checkRow.value.length>0) {
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      if (item.ysdQichu == '1') {
        ...
        xhdyue.hxIsum = sub(xhdyue.hxIsum, add(item.hxMoney == null ? '' : item.hxMoney, item.idiscount == null ? '' : item.idiscount))
        xhdyue.hxFlag = '0'
        ...
        //修改销货单核销金额和核销状态
        const saleousing = await useRouteApi(findSaleousingByCcode, {schemaName: dynamicTenantId})(item.ccode)
      ...
      //删除核销单据
      const arApWriteOff = await useRouteApi(findArApWriteOffByCcodeAndHxCcode, {schemaName: dynamicTenantId})({
        ccode: item.hxCode,
        hxCcode: item.ccode
      })
      if (arApWriteOff.length > 0) {
        await useRouteApi(deleteArApWriteOffById, {schemaName: dynamicTenantId})(arApWriteOff[0].id)
      }
      if (item.skdQichu == '1') {
        //修改应收单核销金额和状态
        const ysdyue = await useRouteApi(findArBeginBalanceByCcode, {schemaName: dynamicTenantId})(item.hxCode)
        ysdyue.hxIsum = sub(ysdyue.hxIsum == null ? '' : ysdyue.hxIsum, item.hxMoney == null ? '' : item.hxMoney)
        ysdyue.hxIdiscount = sub(ysdyue.hxIdiscount == null ? '' : ysdyue.hxIdiscount, item.idiscount == null ? '' : item.idiscount)
        ysdyue.hxFlag = '0'
        await useRouteApi(saveBeginBalance, {schemaName: dynamicTenantId})(ysdyue)
      } else {
        //修改应收单核销金额和状态
        const arApYsyf = await useRouteApi(findArApYsyfByCcode, {schemaName: dynamicTenantId})(item.hxCode)
        if (arApYsyf.length > 0) {
          arApYsyf[0].hxIsum = sub(arApYsyf[0].hxIsum == null ? '' : arApYsyf[0].hxIsum, item.hxMoney == null ? '' : item.hxMoney)
          arApYsyf[0].hxIdiscount = sub(arApYsyf[0].hxIdiscount == null ? '' : arApYsyf[0].hxIdiscount, item.idiscount == null ? '' : item.idiscount)
          arApYsyf[0].writeOffStatus = '0'
          if (arApYsyf[0].hxIsum == 0) {
            arApYsyf[0].busStyle = 'YFK'
          }
          await useRouteApi(saveYsyf, {schemaName: dynamicTenantId})(arApYsyf[0])
        }
      }
      //修改应收单明细
      // const arApYsyfs = await useRouteApi(findArApYsyfsByCcode,{schemaName: dynamicTenantId})(item.hxCode)
    }
    await reloadTable()
  } else {
    createErrorModal({
...
const excelData = () => {
  ...
  const sheet:any = [
    {
      ...
      multiHeader: [
        ...
        ['单据类型', '单据日期','单据编码', '结算供应商名称', '供应商名称', '核销金额', '余额', '核销单据类型', '核销单据日期', '核销单据编号','核销金额','折扣金额']
      ],
      table: arrData,
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应付款核销明细表_'+pageParameter.companyName)
}
...
import {
  deleteArApWriteOffById,
  deleteXyCsouceById,
  findArApWriteOffByCcodeAndHxCcode, findArApYsyfByCcode,
  findQuxiaohexiaoYfdList, findQuxiaohexiaoYfdYueList,
  findSaleousingByCcode, findXyCsourceByCcodeAndXyccode,
  saveYsyf
