...
<!--   <div><textarea>付款单列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">付(退)款单列表</b>
...
<!--          <button
      ...
      <div style="margin-top: -30px;margin-left: 80px;">
        ...
        <div style="float: right; margin-left: 10px">
          ...
          <Popover placement="bottom">
            <template #content>
                <span class="group-btn-span-special2" @click="titleValue = '0';reloadTable()"
                      :style="titleValue==='0'?{backgroundColor: '#0096c7',color: 'white'}:''">
                &nbsp;全部&emsp;&emsp;&emsp;<CheckOutlined
                  v-if="titleValue==='0'"/></span><br/>
              <span class="group-btn-span-special2" @click="titleValue = '1';reloadTable()"
                    :style="titleValue==='1'?{backgroundColor: '#0096c7',color: 'white'}:''">
                &nbsp;付款单&emsp;&emsp;<CheckOutlined
                v-if="titleValue==='1'"/></span>
              <br/>
              <span class="group-btn-span-special2" @click="titleValue = '2';reloadTable()"
                    :style="titleValue==='2'?{backgroundColor: '#0096c7',color: 'white'}:''">
                &nbsp;退款单&emsp;&emsp;<CheckOutlined
    ...
    <div class="app-container">
      ...
      >
        ...
        <template #writeOffStatus="{ record }">
          <span v-if="record.writeOffStatus!='' && record.writeOffStatus!=null">
<!--            <Tag :color="record.writeOffStatus === '1' ? 'green' : 'volcano'">
              {{ record.writeOffStatus === '1' ? '已核销' : '未核销' }}
            </Tag>-->
            <Tag v-if="record.writeOffStatus == '1'" color="green">已核销</Tag>
            <Tag v-else-if="record.writeOffStatus != '1' && (record.hxIsum=='0'||record.hxIsum==''||record.hxIsum==null)" color="volcano">未核销</Tag>
            <Tag v-else color="warning">核销中</Tag>
          </span>
        </template>
        ...
        <template #hxIdiscount="{ record }">{{toThousandFilter(record.hxIdiscount=='0'?'':record.hxIdiscount)}}</template>
...
<!--        <template #footer>
          <div class="nc-summary">
            ...
            <div>
              <span>累计折扣金额:</span>
              <span>&ensp;{{ toThousandFilter(hxIdiscount) }}</span>
            </div>
          </div>
        </template>-->
<!--        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;font-weight: bold;">
              <TableSummaryCell :index="0" :colSpan="10" :align="'center'" style="background-color: #cccccc;font-weight: bold;">合&emsp;&emsp;计</TableSummaryCell>
              <TableSummaryCell :index="10" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(isum)}}</TableSummaryCell>
              <TableSummaryCell :index="11" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(idiscount)}}</TableSummaryCell>
              <TableSummaryCell :index="12" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(hxIsum)}}</TableSummaryCell>
              <TableSummaryCell :index="13" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(hxIdiscount)}}</TableSummaryCell>
              <TableSummaryCell :index="14" :align="'right'" style="background-color: #cccccc;font-weight: bold;"></TableSummaryCell>
            </TableSummaryRow>
          </TableSummary>
        </template>-->
        <template #summary>
...
async function reloadTable(){
  ...
  const res = await useRouteApi(findByTypeByDateList,{schemaName: dynamicTenantId})({type:'FKD',ddate1:date1,ddate2:date2})
  if (titleValue.value == '1') {
    tableDataAll.value = res.filter(item => item.bdocumStyle != '1')
  } else if (titleValue.value == '2') {
    tableDataAll.value = res.filter(item => item.bdocumStyle == '1')
  } else {
...
const CrudApi = {
  columns: [
    ...
    },
    {
      ...
      align: 'left',
      ellipsis: true,
      slots: { customRender: 'writeOffStatus' }
    },
    {
      title: '单据日期',
      dataIndex: 'ddate',
...
function addPage(){
  ...
  router.push({
    path: '/YingFuZhang/ShouKuan/CaoZuo/YingShouShouKuanDan',
    query: pageParameter
  });
}
...
async function editPage(){
  if (checkRow.value.length == 1) {
    ...
    if (num == 0) {
      ...
      if (checkRow.value[0].bdocumStyle!='1') {
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'付款单',method:'修改,删除,审核,弃审',recordNum:checkRow.value[0].ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
          }
        }
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'退款单',method:'修改,删除,审核,弃审',recordNum:checkRow.value[0].ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'退款单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
          }
        }
        pageParameter.isEdit = '2'
        router.push({
          path: '/YingFuZhang/ShouKuan/CaoZuo/YingShouTuiKuanDan',
          query: pageParameter
        });
      }
    }
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        if(item.bdocumStyle!='1'){
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'付款单',method:'修改,删除,审核,弃审',recordNum:item.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        } else {
          ...
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'退款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        }
        ...
        if(item.bdocumStyle!='1'){
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:item.ccode})
        } else {
          //删除任务锁定
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        if(item.bdocumStyle!='1'){
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'付款单',method:'修改,删除,审核,弃审',recordNum:item.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        } else {
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'退款单',method:'修改,删除,审核,弃审',recordNum:item.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'退款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        }
        item.bcheck = '1'
        item.bcheckTime = dateTime
        item.bcheckUser = useUserStore().getUserInfo['name']
        await useRouteApi(saveYsyf, {schemaName: dynamicTenantId})(item)
        const ysyfsList = await useRouteApi(findArApYsyfsByCcode,{schemaName: dynamicTenantId})(item.ccode)
        ysyfsList.map(ysyfs=>{
          ysyfs.bcheck = '1'
        ...
        if(item.bdocumStyle!='1'){
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:item.ccode})
        } else {
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'审核',recordNum:item.ccode})
        }
      }
    }
    state.selectedRowKeys = []
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let zdhx= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付自动核销',method:'核销'})
        if(zdhx.length){
          return createWarningModal({ content: zdhx[0]?.username+'正在进行应付自动核销,不能同时进行操作！' });
        }
        let ymjz= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付款月末结账',method:'月末结账'})
        if(ymjz.length){
        ...
        if(item.bdocumStyle!='1'){
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'付款单',method:'修改,删除,审核,弃审',recordNum:item.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        } else {
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'退款单',method:'修改,删除,审核,弃审',recordNum:item.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'弃审',recordNum:item.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'退款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'弃审',recordNum:item.ccode,caozuoModule:'ap'})
            }
          }
        }
        item.bcheck = '0'
        ...
        if(item.bdocumStyle!='1'){
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:item.ccode})
        } else {
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'退款单',method:'弃审',recordNum:item.ccode})
        }
      }
    }
    state.selectedRowKeys = []
...
const excelData = () => {
  ...
  arrData.forEach(item=>{
    ...
    item.hxStyle = item.hxStyle==null?'':'付款单'
...
const calculateTotal = () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    num2+=parseFloat(e['idiscount'] || 0)
    num3+=parseFloat(e['hxIsum'] || 0)
    num4+=parseFloat(e['hxIdiscount'] || 0)
  }
  summaryModel.value={
    isum: num1,
    idiscount: num2,
