...
<!--   <div><textarea>付款明细账的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">付款明细账</b>
    ...
    <div class="app-container-bottom">
      ...
      >
        ...
        <template #hxIdiscount="{ record }">
          <span v-if="record.hxIdiscount!=null && record.hxIdiscount!='' && record.hxIdiscount!=0">
...
<!--        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;font-weight: bold;">
              <TableSummaryCell :index="0" :colSpan="8" :align="'center'" style="background-color: #cccccc;font-weight: bold;">合&emsp;&emsp;计</TableSummaryCell>
              <TableSummaryCell :index="8" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(isum)}}</TableSummaryCell>
              <TableSummaryCell :index="9" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(hxIdiscount)}}</TableSummaryCell>
            </TableSummaryRow>
          </TableSummary>
        </template>-->
        <template #summary>
...
async function reloadTable(){
  ...
  if (cvencode.value!=null && cvencode.value!='') {
    ...
    const res = await useRouteApi(findFkdYueByCvencodeList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
      ddate1:ddate1.value,
      ddate2:ddate2.value,
      cvencode:cvencode.value
    })
    const res1 = await useRouteApi(findFkdmxzList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
      ddate1:ddate1.value,
      ddate2:ddate2.value,
      cvencode:cvencode.value
    })
    const res2 = await useRouteApi(findFkdmxzList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
    ...
    if (res>0) {
      tableDataAll.value.push(...res.map(item => {
        item.billStyle = '期初余额'
        item.cvenName = item.cvencode
        item.isum = item.isumBenbi
        let yue = 0
        res2.forEach(item1 => {
          yue = yue + sub(item1.isum, item1.hxIsum == null ? '0' : item1.hxIsum)
        })
        item.isum = add(item.isumBenbi, yue)
        return item
      }))
    } else {
      let yue = 0
    ...
    tableDataAll.value.push(...res1.map(item => {
      item.cvenName = item.cvencode
      return item
    }))
...
//单据日期、单据编号、结算供应商编码、结算供应商名称、业务部门、业务员、备注、付款金额、折扣金额
const CrudApi = {
  columns: [
    {
      title: '单据日期',
...
function printData(){
  ...
  arrData.forEach(item=>{
    let item1 = {}
    //['ddate', 'ccode', 'cvencode', 'cvenName', 'cdepcode', 'cpersoncode', 'cmemo', 'isum', 'hxIdiscount'],
    item1[0] = item.ddate
    item1[1] = item.ccode
    item1[2] = formatCvencode(item.cvencode)
    item1[3] = formatCvenName(item.cvenName)
    item1[4] = formatCdepcode(item.cdepcode)
    item1[5] = formatCpersoncode(item.cpersoncode)
    item1[6] = item.cmemo
    item1[7] = item.isum==null?'':item.isum==0?'':toThousandFilter(item.isum)
    item1[8] = item.hxIdiscount==null?'':item.hxIdiscount==0?'':toThousandFilter(item.hxIdiscount)
  ...
  for (let i=0; i<a; i++){
    if (i==a-1) {
      ...
      item1[8] = hxIdiscount.value==null?'':hxIdiscount.value==0?'':toThousandFilter(hxIdiscount.value)
      printList.push(item1)
    } else {
      let item1 = {}
      item1[0] = ''
...
const excelData = () => {
  ...
  arrData.forEach(item=>{
    ...
    item.isum = item.isum==null?'':item.isum==0?'':toThousandFilter(item.isum)
    item.hxIdiscount = item.hxIdiscount==null?'':item.hxIdiscount==0?'':toThousandFilter(item.hxIdiscount)
  ...
  const sheet:any = [
    {
      ...
      table: arrData,
      keys: ['ddate', 'ccode', 'cvencode', 'cvenName', 'cdepcode', 'cpersoncode', 'cmemo', 'isum', 'hxIdiscount'],
      merges: ['A2:C2','D2:G2'],
      sheetName: '付款明细账',
      cellStyle: [
        {
      ...
      colWidth: [10, 15, 15, 20, 15, 15, 15, 15, 15, 15]
    }
  ]
  // 处理数据前
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '付款明细账_'+pageParameter.companyName)
}
...
import {findFkdmxzList} from "/@/api/record/system/arApYsyf";
import {findFkdYueByCvencodeList} from "/@/api/record/system/arBeginBalance";
...
const calculateTotal = () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    num2+=parseFloat(e['hxIdiscount'] || 0)
  }
  summaryModel.value={
    isum: num1,
    hxIdiscount: num2,
