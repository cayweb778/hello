...
<!--   <div><textarea>付款执行表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付付款执行表</b>
...
<!--        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;font-weight: bold;">
              ...
              <TableSummaryCell :index="9" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(hxIsum)}}</TableSummaryCell>
              <TableSummaryCell :index="10" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(whxIsum)}}</TableSummaryCell>
            </TableSummaryRow>
          </TableSummary>
        </template>-->
        <template #summary>
...
async function reloadTable(){
  ...
  qijian.value = ddate1.value.replace(/-/g, '.') + " - " + ddate2.value.replace(/-/g, '.')
  let ysdList:any = []
  const res1 = await useRouteApi(findYfdmxbList, {schemaName: dynamicTenantId})({
    bcheck: bcheck.value,
    ddate1: ddate1.value,
    ddate2: ddate2.value,
  })
  if (apSourceFlag.value=='1'){
    ysdList = res1.filter(item => item.billStyle!='CGDHD')
  } else {
    ysdList = res1.filter(item => item.billStyle!='CGFP')
  }
  tableDataAll.value = []
  const groupDataList = groupData(ysdList)
  ...
  for (let i = 0; i < groupDataList.length; i++) {
    ...
    group.items.forEach(item => {
      ...
      isum1 = add(isum1,item.isum==null?'0':item.isum)
      hxIsum1 = add(hxIsum1,item.hxIsum==null?'0':item.hxIsum)
      whxIsum1 = add(whxIsum1,item.whxIsum==null?'0':item.whxIsum)
      list.push(item)
    })
    tableDataAll.value.push({
      cvencodeJs: group.cvencodeJs,
      ccode: '合计',
      isum: isum1,
      hxIsum: hxIsum1,
      whxIsum: whxIsum1,
    })
    tableDataAll.value.push(...list)
  }
...
function groupData(beforeData:any) {
  let tempArr: any = [];
  let afterData: any = []
  for (let i = 0; i < beforeData.length; i++) {
    if (tempArr.indexOf(beforeData[i].cvencodeJs) === -1) {
      afterData.push({
        cvencodeJs: beforeData[i].cvencodeJs,
        items: [beforeData[i]]
      });
      tempArr.push(beforeData[i].cvencodeJs);
    } else {
      for (let j = 0; j < afterData.length; j++) {
        if (afterData[j].cvencodeJs == beforeData[i].cvencodeJs) {
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['应付明细表','', '', '', '', '', '', '', '', ''],
        ...
        ['结算供应商名称', '日期','供应商', '业务员', '备注', '单据类型', '单据编号', '应付', '已付','余额']
      ],
      body: printList,
      // startY: 60,
      styles: tableStyle(),
...
const excelData = () => {
  ...
  const sheet:any = [
    {
      ...
      multiHeader: [
        ['单位：'+pageParameter.companyName, '','', '', '期间：'+qijian.value, '', '', '', '', '',''],
        ['结算供应商名称', '日期','供应商', '部门', '业务员', '备注', '单据类型', '单据编号', '应付', '已付','余额']
      ],
      table: arrData,
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应付明细表_'+pageParameter.companyName)
}
...
import { findYfdmxbList } from "/@/api/record/system/yfd";
