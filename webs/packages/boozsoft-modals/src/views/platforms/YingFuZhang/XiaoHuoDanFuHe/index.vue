...
<!--   <div><textarea>采购到货单复核的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">采购到货单复核</b>
...
async function reloadTable(){
  ...
  if(apCheckFlag.value!='1') {
    const res = await useRouteApi(findYshCgdhdByIyear, {schemaName: dynamicTenantId})({
      ddate1: date1,
      ddate2: date2
    })
...
async function approved(){
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        const item = checkRow.value[i]
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'采购到货单',method:'修改,删除,审核,弃审,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'复核',recordNum:item.ccode,caozuoModule:'stock'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'采购到货单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'复核',recordNum:item.ccode,caozuoModule:'stock'})
          }
        }
        item.bworkable = '1'
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'复核',recordNum:item.ccode})
      }
    }
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'采购到货单',method:'修改,删除,审核,弃审,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'弃复',recordNum:item.ccode,caozuoModule:'stock'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'采购到货单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'弃复',recordNum:item.ccode,caozuoModule:'stock'})
          }
        }
        item.bworkable = '0'
        item.bworkableTime = ''
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购到货单',method:'弃复',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
...
import {
  ...
  findYshCgdhdByIyear,
  saveWarehousing, saveWarehousingsList
