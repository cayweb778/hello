...
<!--   <div><textarea>应付冲应付的页面</textarea>复制内容快速定位页面</div>-->
...
import {
  deleteWarehousingById, deleteWarehousingsByCcode,
  findBukongCcode as ysdfindBukongCcode,
  findWarehousingByCcode,
  findWarehousingsByCcode, saveWarehousings,
...
const titleContents = ['应付冲应付', '应付冲应付', '应付冲应付']
...
function openPage(){
  ...
  router.push({
    path: '/YingFuZhang/ChongXiao/ChaXun/YingShouChongYingShouLieBiao',
    query: pageParameter
  });
}
...
async function cvencodeChange(){
  ...
  if (arSourceFlag.value!='1') {
    arBeginBalanceList.value = qcList.filter(item => item.busStyle == 'YFD' && item.ysIsumBenbi!='0')
  } else {
    arBeginBalanceList.value = qcList.filter(item => item.arStyle == 'YFD' && item.ysIsumBenbi!='0')
  }
  ...
  const saleList = await useRouteApi(findByFkWhxCgdhd, {schemaName: dynamicTenantId})({
    year: dynamicYear.value,
    cvencode: formItems.value.cvencodeZc
  })
  if (arSourceFlag.value=='1') {
    saleousingList.value = saleList.filter(item => item.busStyle !='CGDHD' && item.bworkable=='1' && item.isum!='0')
  } else {
    if(arCheckFlag=='1'){
      saleousingList.value = saleList.filter(item => item.busStyle !='CGFP' && item.bworkable=='1' && item.isum!='0')
    } else {
      saleousingList.value = saleList.filter(item => item.busStyle !='CGFP' && item.isum!='0')
    }
  }
  tableData.value.push(...saleousingList.value.map(item => {
    item.sourcecode = item.ccode
...
async function contentSwitch(action) {
  loadMark.value = true
  const res = await useRouteApi(findArApChongxiaoList,{schemaName: dynamicTenantId})({billStyle:'FF',iyear:dynamicYear.value})
  ...
  if(arApYsyfList.value.length>0){
    ...
    const res = await useRouteApi(findArApChongxiaosByCcode,{schemaName: dynamicTenantId})({ccode:formItems.value.ccode,billStyle:'FF'})
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'应付冲应付',method:'添加'})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'添加',caozuoModule:'ap'})
    } else {
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'添加',caozuoModule:'ap'})
    }
    status.value = 1
    formItems.value.id=null
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    //判断新生成的应付单是否做过核销
    let ysd = await useRouteApi(findWarehousingByCcode,{schemaName: dynamicTenantId})({ccode:formItems.value.targetCode,billStyle:'YFD'})
    if(ysd.hxIsum!=null && ysd.hxIsum!='' && ysd.hxIsum!='0'){
      createErrorModal({
        iconType: 'warning',
        title: '温馨提示',
        content: '生成的应付单已做过核销，不能删除！'
      })
      return false
    }
    //判断应付单没有其他下游单据（除应付冲应付）
    createConfirm({
      iconType: 'warning',
      title: '应付冲应付单删除',
      content: '您确定要进行应付冲应付删除吗!',
      onOk: async () => {
        //判断应付冲应付
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'应付冲应付',method:'删除',recordNum:formItems.value.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应付冲应付,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ap'})
          }
        }
        //判断应付单
        let ysdData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'应付单',method:'修改,删除,审核,弃审,复核,弃复',recordNum:ysd.ccode})
        if(ysdData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(ysdData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: ysdData[0]?.username+'正在'+ysdData[0]?.method+'应付单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:ysdData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode,caozuoModule:'ap'})
          }
        }
        //删除新生成的应付单
        await useRouteApi(deleteWarehousingById,{schemaName: dynamicTenantId})(ysd.id)
        await useRouteApi(deleteWarehousingsByCcode,{schemaName: dynamicTenantId})({ccode:ysd.ccode,billStyle:ysd.billStyle})
        //删除下游单据
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if(item.sourcetype.substring(0,2)=='QC'){
            ...
            //普通应付单
            const saleousing = await useRouteApi(findWarehousingByCcode, {schemaName: dynamicTenantId})({ccode:item.targetCode,billStyle:item.billStyle})
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:formItems.value.ccode})
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode})
...
async function saveData() {
  ...
  await useRouteApi(saveWarehousings, {schemaName: dynamicTenantId})(ysds)
  //保存新的应付单的下游单据
  let stockXyCsource1: any = {}
  stockXyCsource1.iyear = pageParameter.year
  stockXyCsource1.xyBillStyle = 'FF'
  stockXyCsource1.billStyle = 'YFD'
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if(!hasBlank(item.hxMoney) && item.hxMoney!=0) {
      ...
      if (item.type == 'YUE') {
        ...
        //普通应付单
        const item1 = await useRouteApi(findWarehousingByCcode,{schemaName: dynamicTenantId})({ccode:item.sourcecode,billStyle:item.sourcetype})
  ...
  //删除任务锁定
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'添加'})
  await balanceTaskDel()
