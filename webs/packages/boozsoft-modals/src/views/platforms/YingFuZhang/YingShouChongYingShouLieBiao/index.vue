...
<!--   <div><textarea>应付冲应付列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付冲应付列表</b>
...
async function reloadTable(){
  ...
  let len = 0
  const res = await useRouteApi(findArApChongxiaoList,{schemaName: dynamicTenantId})({billStyle:'FF',iyear:pageParameter.year})
...
function addPage(){
  ...
  router.push({
    path: '/YingFuZhang/ChongXiao/CaoZuo/YingShouChongYingShou',
    query: pageParameter
  });
}
...
async function delList(){
  if (checkRow.value.length > 0) {
    for (let x = 0; x < checkRow.value.length; x++) {
      ...
      //判断新生成的应付单是否做过核销
      let ysd = await useRouteApi(findWarehousingByCcode, {schemaName: dynamicTenantId})({
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let x = 0; x < checkRow.value.length; x++) {
          ...
          //判断应付冲应付
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付冲应付',method:'删除',recordNum:chongxiao.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              ...
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ap'})
            }
          }
          //判断应付单
          let ysdData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付单',method:'修改,删除,审核,弃审,复核,弃复',recordNum:ysd.ccode})
          if(ysdData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(ysdData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              ...
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:ysdData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode,caozuoModule:'ap'})
            }
          }
          //删除新生成的应付单
          await useRouteApi(deleteWarehousingById, {schemaName: dynamicTenantId})(ysd.id)
          await useRouteApi(deleteWarehousingsByCcode, {schemaName: dynamicTenantId})({
            ccode: ysd.ccode,
          ...
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付冲应付',method:'删除',recordNum:chongxiao.ccode})
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:ysd.ccode})
