...
<!--   <div><textarea>应付单列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付单列表</b>
    ...
    <div class="app-container">
      ...
      >
        <template #bworkable="{ record }">
          <span v-if="record.id!='' && record.id!=null">
...
async function reloadTable(){
  ...
  const res = await useRouteApi(findYfdmxbList,{schemaName: dynamicTenantId})({bcheck:'',ddate1:date1,ddate2:date2})
  if (titleValue.value == 1) {
    tableDataAll.value = res.filter(item => item.billStyle=='YSD' && item.bdocumStyle != '1')
  } else if (titleValue.value == 2) {
    tableDataAll.value = res.filter(item => item.billStyle=='YSD' && item.bdocumStyle == '1')
  } else {
    tableDataAll.value = res.filter(item => item.billStyle=='YSD')
  }
...
function addPage(){
  ...
  router.push({
    path: '/YingFuZhang/YingShou/DanJu/YingShouDan',
    query: pageParameter
  });
}
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      if (item.bcheck == '1') {
        ...
        createErrorModal({
          iconType: 'warning',
          title: '警告',
          content: '单据已复核，不能进行修改操作！'
        })
        return false
      }
    }
    if (num == 0) {
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付单',method:'修改,删除,复核,弃复',recordNum:checkRow.value[0].ccode})
      if(taskData.length==0){
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
      } else {
        // 任务不是当前操作员的
        if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
          ...
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ap'})
        }
      }
...
async function delList(){
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        const item = checkRow.value[i]
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付单',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应付单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:item.ccode,caozuoModule:'ap'})
          }
        }
        await useRouteApi(deleteWarehousingById,{schemaName: dynamicTenantId})(item.id)
        await useRouteApi(deleteWarehousingsByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'删除',recordNum:item.ccode})
      }
    }
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付单',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'复核',recordNum:item.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应付单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'复核',recordNum:item.ccode,caozuoModule:'ap'})
          }
        }
        item.bcheck = '1'
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'复核',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应付单',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'弃复',recordNum:item.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应付单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'弃复',recordNum:item.ccode,caozuoModule:'ap'})
          }
        }
        item.bcheck = '0'
        item.bcheckTime = ''
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付单',method:'弃复',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
...
import {
  deleteWarehousingById,
  deleteWarehousingsByCcode,
  findWarehousingList, findWarehousingsByCcode, findYfdmxbList, saveWarehousing, saveWarehousingsList
