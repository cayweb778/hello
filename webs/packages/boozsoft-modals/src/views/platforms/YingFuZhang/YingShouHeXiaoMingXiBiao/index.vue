...
<!--   <div><textarea>应付核销明细表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付款核销明细表</b>
...
async function reloadTable(){
  ...
  let yueList:any = []
  let xhdList:any = []
  const res = await useRouteApi(findYfkhxmxbYue,{schemaName: dynamicTenantId})(pageParameter.year)
  const res1 = await useRouteApi(findYfkhxmxb,{schemaName: dynamicTenantId})(pageParameter.year)
  if (apSourceFlag.value=='1'){
    yueList = res.filter(item => item.bill_style!='CGDHD')
    xhdList = res1.filter(item => item.bill_style!='CGDHD')
  } else {
    yueList = res.filter(item => item.bill_style!='CGFP')
    xhdList = res1.filter(item => item.bill_style!='CGFP')
  }
  ...
  const sheet:any = [
    {
      ...
      multiHeader: [
        ...
        ['单据类型', '单据日期','单据编码', '结算供应商名称', '供应商名称', '结算金额', '余额', '结算单据类型', '结算单据日期', '结算单据编号','结算金额','折扣金额']
      ],
      table: arrData,
...
import {findYfkhxmxb, findYfkhxmxbYue} from "/@/api/record/system/arApYsyf";
/*start栏目设置*/
