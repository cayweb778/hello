...
<!--   <div><textarea>应付款期初余额的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付款期初余额</b>
...
<!--          <button
          ...
          <Popover placement="bottom">
            ...
            <template #content>
              <span class="group-btn-span-special2 p_specifics" @click="openEdit('YFD')">应付单</span><br/>
              <span class="group-btn-span-special2 p_specifics" @click="openEdit('FKD')">付款单</span><br/>
              <span class="group-btn-span-special2 p_specifics" >采购发票</span><br/>
            </template>
          </Popover>
          <button
            type="button"
...
const openEdit = async (str) => {
  ...
  if(str=='YFD'){
    val.value.arStyle = 'YFD'
  } else {
    val.value.arStyle = 'PTFK'
  }
...
async function reloadTable(){
  ...
  const res = await useRouteApi(findByBillStyleAndIyearOrderByCcode,{schemaName: dynamicTenantId})({billStyle:'ap', iyear:pageParameter.year})
...
function formatArStyle(arStyle){
  let str = arStyle
  //CGFP采购发票、YFD应付单、DHD到货单、PTFK普通付款、YFK预付款
  //YFD应付单、FKD付款单
  if (arStyle=='YFD'){
    str = '应付单'
  } else if (arStyle=='FKD'){
    str = '付款单'
  } else if (arStyle=='CGFP'){
    str = '采购发票'
  } else if (arStyle=='CGDHD'){
    str = '到货单'
  } else if (arStyle=='PTFK'){
    str = '普通付款'
  } else if (arStyle=='YFK'){
    str = '预付款'
  }
...
async function balanceTaskDel() {
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付款期初余额',method:'编辑'})
}
async function balanceTaskSave() {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付款期初余额',method:'编辑',caozuoModule:'ap'})
}
...
const excelData = async () => {
  ...
  const sheet:any = [
    {
      ...
      multiHeader: [
        ...
        ['应付类型','单据编码','客户名称','结算客户名称','业务部门','业务员','币种','单据日期','金额','摘要']
      ],
      table: arrData,
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, pageParameter.year+'年_应付款期初余额_'+pageParameter.companyName)
}
