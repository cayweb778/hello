...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      ...
      item1.arStyle = item['应收类型']
      item1.ccode = item['单据编码']
      ...
      if (busStyle.value=='YSD') {
        if (item['应收类型'] == '应收单') {
          item1.arStyle = 'YSD'
        } else if (item['应收类型'] == '销售发票') {
          item1.arStyle = 'XSFP'
        } else if (item['应收类型'] == '销货单') {
          item1.arStyle = 'XHD'
        } else {
          item1.arStyle = 'YSD'
        }
...
function exportExcel() {
  ...
  if (busStyle.arStyle=='YSD'){
    arrHeader = ['应收类型','单据编码','客户名称','业务部门名称','业务员编码','业务员名称','币种','单据日期','收款金额','应收金额','期初余额','摘要']
  } else {
    arrHeader = ['应收类型','单据编码','结算客户名称','业务部门名称','业务员编码','业务员名称','币种','单据日期','金额','摘要']
  }
