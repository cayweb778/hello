...
<!--   <div><textarea>应付款统计表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付款统计表</b>
    ...
    <div class="app-container-bottom">
      ...
      >
        ...
        <template #hxIsum="{ record }">
          <a v-if="record.hxIsum!=null && record.hxIsum!=''" @click="openSkmx(record)">
            {{toThousandFilter(record.hxIsum)}}
          </a>
        </template>
        <template #whxIsum="{ record }">{{toThousandFilter(record.whxIsum)}}</template>
        <template #kpIsum="{ record }">{{toThousandFilter(record.kpIsum)}}</template>
...
<!--        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;font-weight: bold;">
              ...
              <TableSummaryCell :index="4" :align="'center'" style="background-color: #cccccc;font-weight: bold;">合&emsp;&emsp;计</TableSummaryCell>
              <TableSummaryCell :index="5" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(isum)}}</TableSummaryCell>
              ...
              <TableSummaryCell :index="7" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(whxIsum)}}</TableSummaryCell>
              <TableSummaryCell :index="8" :align="'right'" style="background-color: #cccccc;font-weight: bold;">{{toThousandFilter(kpIsum)}}</TableSummaryCell>
...
async function reloadTable(){
  ...
  qijian.value = ddate1.value.replace(/-/g, '.') + " - " + ddate2.value.replace(/-/g, '.')
  const res:any = await useRouteApi(findYfdtjbList,{schemaName: dynamicTenantId})({bcheck:bcheck.value,ddate1:ddate1.value,ddate2:ddate2.value,apSourceFlag:apSourceFlag.value})
  const res1:any = await useRouteApi(findFkdtjbList,{schemaName: dynamicTenantId})({bcheck:bcheck.value,ddate1:ddate1.value,ddate2:ddate2.value})
  ...
  tableDataAll.value = list.filter(item=>(item.isum!=null && item.isum!='' && item.isum!=0) || (item.hxIsum!=null && item.hxIsum!='' && item.hxIsum!=0))
  tableData.value = tableDataAll.value.map(item=>{
    item.whxIsum = sub(item.isum, item.hxIsum==null?'0':item.hxIsum)
    if (item.isum!=null && item.isum!='' && item.isum!=0) {
      item.bili = mul(div(item.hxIsum==null?'0':item.hxIsum, item.isum), 100).toFixed(2) + '%'
    }
    return item
  })
...
function printData(){
  ...
  for (let i=0; i<a; i++){
    if (i==a-1) {
      ...
      item1[7] = kpIsum.value==null?'':kpIsum.value==0?'':toThousandFilter(kpIsum.value)
      item1[8] = ''
      printList.push(item1)
    } else {
      let item1 = {}
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['应付款统计表','', '', '',  '', '', '', '', ''],
        ...
        ['销售供应商编码', '销售供应商名称','结算供应商编码', '结算供应商名称', '应付金额', '已付金额', '未付金额', '开票金额', '付款比例']
      ],
      body: printList,
      // startY: 60,
      styles: tableStyle(),
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应付款统计表_'+pageParameter.companyName)
}
...
import {findYfdtjbList} from "/@/api/record/system/yfd";
import {findFkdtjbList} from "/@/api/record/system/arApYsyf";
/*start栏目设置*/
