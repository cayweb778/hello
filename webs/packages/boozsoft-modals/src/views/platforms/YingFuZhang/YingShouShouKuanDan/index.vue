...
<!--   <div><textarea>应付付款单的页面</textarea>复制内容快速定位页面</div>-->
...
<!--          <DynamicForm :datasource="dynamicFormModel" :formDataFun="formFuns"
          ...
          <label>付款日期：</label>
          <DatePicker v-model:value="formItems.ddate" @change="generateCode(formItems.ddate)" :disabled-date="disabledDate" :locale="localeCn" placeholder="付款日期" format="YYYY-MM-DD" value-format="YYYY-MM-DD" style="width: 15%;"/>
          <label>单据编号：</label>
...
<!--          <template #pjCode="{ record }">
          ...
          <template #pjDate="{ record }">
            <template v-if="record.editSix">
              <DatePicker v-model:value="record.tempSix" class="pjDate" style="width: 85%;" :locale="localeCn" placeholder="" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
...
<!--          <template #pjDate="{ record }">
          ...
          <template #pjDhDate="{ record }">
            <template v-if="record.editSeven">
              <DatePicker v-model:value="record.tempSeven" class="pjDhDate" style="width: 85%;" :locale="localeCn" placeholder="" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
...
<!--          <template #pjDhDate="{ record }">
          ...
          <template #cmemo="{ record }">
            <template v-if="record.editEight">
              <Input v-model:value="record.tempEight" class="cmemo" style="width: 85%;" placeholder="" @keyup.enter="focusNext(record,'cmemo')"/>
...
<!--            <span v-if="record.hxIsumFlag!='1'">{{record.hxIsum}}<EditOutlined v-if="status == 1 || status == 2" @click="record.hxIsumFlag='1'"/></span>
          ...
          <template #hxMoney="{ record,index }">
            <template v-if="record.editOne">
              <InputNumber v-model:value="record.tempOne" class="hxMoney" style="width: 85%;" @blur="checkMoney(record,index,'hxMoney')" @keyup.enter="focusNext1(record,'hxMoney')"/>
...
<!--          <template #hxMoney="{ record }">
          ...
          <template #idiscount="{ record,index }">
            <template v-if="record.editTwo">
              <InputNumber v-model:value="record.tempTwo" class="idiscount" style="width: 85%;" @blur="checkMoney(record,index,'idiscount')" @keyup.enter="focusNext1(record,'idiscount')"/>
...
const titleContents = ['付款单', '付款单', '付款单']
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'付款单',method:'添加'})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'添加',caozuoModule:'ap'})
    } else {
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'添加',caozuoModule:'ap'})
    }
    status.value = 1
    formItems.value.id=null
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'付款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'修改',recordNum:formItems.value.ccode,caozuoModule:'ap'})
    } else {
      // 任务不是当前操作员的
      if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
        ...
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'修改',recordNum:formItems.value.ccode,caozuoModule:'ap'})
      }
    }
    status.value = 2
    let list = getDataSource()
  ...
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if (formItems.value.bcheck=='1') {
      ...
      createConfirm({
        iconType: 'warning',
        title: '付款单删除',
        content: '您确定要进行付款单删除吗!',
        onOk: async () => {
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'付款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ap'})
            }
          }
          ...
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'删除',recordNum:formItems.value.ccode})
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    if (b==true){
      ...
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'付款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
      if(taskData.length==0){
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:formItems.value.ccode,caozuoModule:'ap'})
      } else {
        // 任务不是当前操作员的
        if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
          return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
        }else{
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:formItems.value.ccode,caozuoModule:'ap'})
        }
      }
      ...
      //删除任务锁定
      await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'审核',recordNum:formItems.value.ccode})
    } else {
      // console.log('取消审核')
      if (formItems.value.writeOffStatus=='1' || formItems.value.hxIsum>0) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'付款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:formItems.value.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'付款单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:formItems.value.ccode,caozuoModule:'ap'})
          }
        }
        ...
        await useRouteApi(saveYsyfsList, {schemaName: dynamicTenantId})(ysyfsList)
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'弃审',recordNum:formItems.value.ccode})
      }
    }
  ...
  for (let i = 0; i < getDataSource().length; i++) {
    ...
    if(!hasBlank(item.cunitid)) {
      ...
      if (formItems.value.busStyle == 'PTFK') {
        ...
        ysyfs.bcheckTime = item.bcheckTime
        ysyfs.bcheckUser = item.bcheckUser
        ysyfs.writeOffStatus = ysyf.writeOffStatus
      } else {
        ysyfs.bcheck = '0'
        ysyfs.bcheckTime = item.bcheckTime
        ysyfs.bcheckUser = item.bcheckUser
...
async function balanceTaskDel() {
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'付款单',method:'添加'})
  ...
  if(tableSelectedRowObjs1.value.length>0){
    ...
    for (let i = 0; i < tableSelectedRowObjs1.value.length; i++) {
      ...
      if (item.whxIsum != null && item.whxIsum != '') {
        item.hxMoney = '0'
        let whxIsum = 0
        if (sub(formItems.value.idiscount==null?'0':formItems.value.idiscount, idiscount.value==null?'0':idiscount.value) > item.whxIsum) {
          item.idiscount = item.whxIsum
        } else {
          item.idiscount = sub(formItems.value.idiscount==null?'0':formItems.value.idiscount, idiscount.value==null?'0':idiscount.value)
        }
        if (item.idiscount != null && item.idiscount != '') {
          whxIsum = sub(item.whxIsum, item.idiscount==null?'0':item.idiscount)
        } else {
          whxIsum = item.whxIsum
        }
        if (sub(isum.value, hxMoney.value) > whxIsum) {
...
const tableDataChange =  (r,c) => {
  switch (c) {
    case 'isum':
      if(c=='isum'){
        r.tempThree=parseFloat(r.tempThree) < 0?(Math.abs(parseFloat(r.tempThree))):r.tempThree
        r.isum=r.tempThree
      }
      break
  }
