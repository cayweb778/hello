...
<!--   <div><textarea>应付总账的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应付总账</b>
    ...
    <div class="app-container-bottom">
      ...
      >
        ...
        <template #shoukuan="{ record }">
          <a v-if="record.shoukuan!=null && record.shoukuan!='' && record.shoukuan!=0" @click="openSkmx(record)">
...
async function reloadTable(){
  ...
  const res: any = await useRouteApi(findYfdtjbList, {schemaName: dynamicTenantId})({
    bcheck:bcheck.value,
    ddate1:ddate1.value,
    ddate2:ddate2.value,
    apSourceFlag:apSourceFlag.value
  })
  const res1: any = await useRouteApi(findFkdtjbList, {schemaName: dynamicTenantId})({
    bcheck:bcheck.value,
    ddate1:ddate1.value,
    ddate2:ddate2.value
  })
  ...
  if (apSourceFlag.value=='1'){
    res2 = yueList.filter(item => item.arStyle=='YFD')
  } else {
    res2 = yueList
  }
  let list: any = []
...
function openYsmx(data){
  // console.log(data)
  router.push({
    path: '/YingFuZhang/ZhangBiao/ZhangBiaoChaXun/YingShouMingXiZhang',
    query: {
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应付总账_'+pageParameter.companyName)
}
