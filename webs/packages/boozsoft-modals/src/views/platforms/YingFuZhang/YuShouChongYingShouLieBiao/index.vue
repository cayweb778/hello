...
<!--   <div><textarea>预付冲应付列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">预付冲应付列表</b>
...
async function reloadTable(){
  ...
  let len = 0
  const res = await useRouteApi(findArApChongxiaoList,{schemaName: dynamicTenantId})({billStyle:'YF',iyear:pageParameter.year})
...
function addPage(){
  ...
  router.push({
    path: '/YingFuZhang/ChongXiao/CaoZuo/YuShouChongYingShou',
    query: pageParameter
  });
}
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let x = 0; x < checkRow.value.length; x++) {
          ...
          //判断预付冲应付
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'预付冲应付',method:'删除',recordNum:chongxiao.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预付冲应付',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ap'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              ...
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预付冲应付',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ap'})
            }
          }
          const list = await useRouteApi(findArApChongxiaosByCcode,{schemaName: dynamicTenantId})({ccode:chongxiao.ccode,billStyle:chongxiao.busStyle})
          ...
          //删除核销单据
          await useRouteApi(deleteByCxCcode,{schemaName: dynamicTenantId})({ccode:chongxiao.ccode,billStyle:chongxiao.busStyle})
          //删除冲销表
          await useRouteApi(deleteArApChongxiaosById,{schemaName: dynamicTenantId})(chongxiao.id)
          await useRouteApi(deleteArApChongxiaosByCcode,{schemaName: dynamicTenantId})({ccode:chongxiao.ccode,billStyle:chongxiao.busStyle})
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'预付冲应付',method:'删除',recordNum:chongxiao.ccode})
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应付' +
    '款核销明细表_'+pageParameter.companyName)
}
