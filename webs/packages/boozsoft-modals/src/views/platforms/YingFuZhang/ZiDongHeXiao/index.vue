...
async function saveQuery(data) {
  ...
  //查询期初
  let res = await useRouteApi(findWhxList,{schemaName: dynamicTenantId})({billStyle:'ap',iyear:pageParameter.year})
  let skdQichuList = res.filter(item => item.busStyle=='FKD')
  let ysdQichuList = res.filter(item => item.busStyle=='YFD')
  //查询收款单
  let skdList = await useRouteApi(findWhxskdByEndDate,{schemaName: dynamicTenantId})({billStyle:'FKD',year:pageParameter.year,endDate:endDate.value})
  //查询销货单
  let ysdList:any = []
  let saleList = await useRouteApi(findFkWhxCgdhdByEndDate,{schemaName: dynamicTenantId})({year:pageParameter.year,endDate:endDate.value})
  if (apSourceFlag.value=='1') {
    ysdList = saleList.filter(item => item.busStyle !='CGDHD' && item.bworkable=='1')
  } else {
    if(apCheckFlag=='1'){
      ysdList = saleList.filter(item => item.busStyle !='CGFP' && item.bworkable=='1')
    } else {
      ysdList = saleList.filter(item => item.busStyle !='CGFP')
    }
  }
  //自动核销期初收款单
    ...
    for (let i = 0; i < ysdList.length; i++) {
      ...
      if (item1.cvencodeJs==item.cvencodeJs) {
        ...
        if (whxIsum!=0 && whxIsum1!=0) {
          ...
          //保存方法
          await useRouteApi(saveBeginBalance, {schemaName: dynamicTenantId})(item)
          await useRouteApi(saveWarehousing, {schemaName: dynamicTenantId})(item1)
          //修改销货单子表
  ...
  //删除任务锁定
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应付自动核销',method:'核销'})
  await reloadTable()
...
import {findFkWhxCgdhdByEndDate} from "/@/api/record/system/hexiao";
import {
  findQuxiaohexiaoYfdList, findQuxiaohexiaoYfdYueList,
  findWhxskdByEndDate, saveStockXyCsource, saveWriteOff, saveYsyf
} from "/@/api/record/system/arApYsyf";
import {findWhxList, saveArXySource,saveBeginBalance} from "/@/api/record/system/arBeginBalance";
