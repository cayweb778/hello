...
<!--   <div><textarea>费用单列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">费用单列表</b>
...
import {
  deleteArApExpenseById,
  deleteArApExpensesByCcode,
  findArApExpenseList, findArApExpensesByCcode, saveArApExpense, saveArApExpensesList
...
async function reloadTable(){
  loadMark.value = true
  const res = await useRouteApi(findArApExpenseList,{schemaName: dynamicTenantId})({billStyle:'ar',iyear:pageParameter.year})
...
function addPage(){
  ...
  router.push({
    path: '/YingShouZhang/YingShou/DanJu/FeiYongDan',
    query: pageParameter
  });
}
...
async function delList(){
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        const item = checkRow.value[i]
        await useRouteApi(deleteArApExpenseById,{schemaName: dynamicTenantId})(item.id)
        await useRouteApi(deleteArApExpensesByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
      }
    }
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        await useRouteApi(saveArApExpense, {schemaName: dynamicTenantId})(item)
        const ysyfsList = await useRouteApi(findArApExpensesByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
        ysyfsList.map(ysyfs=>{
          ysyfs.bcheck = '1'
