...
<!--   <div><textarea>收款明细账的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">收款明细账</b>
...
async function reloadTable(){
  ...
  if (cvencode.value!=null && cvencode.value!='') {
    ...
    const res = await useRouteApi(findSkdYueByCvencodeList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
      ddate1:ddate1.value,
      ddate2:ddate2.value,
      cvencode:cvencode.value
    })
    const res1 = await useRouteApi(findSkdmxzList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
      ddate1:ddate1.value,
      ddate2:ddate2.value,
      cvencode:cvencode.value
    })
    const res2 = await useRouteApi(findSkdmxzList, {schemaName: dynamicTenantId})({
      bcheck:bcheck.value,
...
//单据日期、单据编号、结算客户编码、结算客户名称、业务部门、业务员、备注、收款金额、折扣金额
const CrudApi = {
  columns: [
    {
      title: '单据日期',
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '收款明细账_'+pageParameter.companyName)
}
...
import {findSkdmxzList} from "/@/api/record/system/arApYsyf";
import {findSkdYueByCvencodeList} from "/@/api/record/system/arBeginBalance";
