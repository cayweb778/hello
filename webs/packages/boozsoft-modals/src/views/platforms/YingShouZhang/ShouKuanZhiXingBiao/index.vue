...
<!--   <div><textarea>收款执行表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应收收款执行表</b>
...
async function reloadTable(){
  ...
  qijian.value = ddate1.value.replace(/-/g, '.') + " - " + ddate2.value.replace(/-/g, '.')
  let ysdList:any = []
  const res1 = await useRouteApi(findYsdmxbList, {schemaName: dynamicTenantId})({
    bcheck: bcheck.value,
    ddate1: ddate1.value,
    ddate2: ddate2.value,
  })
  if (arSourceFlag.value=='1'){
    ysdList = res1.filter(item => item.billStyle!='XHD')
  } else {
    ysdList = res1.filter(item => item.billStyle!='XSFP')
  }
  tableDataAll.value = []
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['应收明细表','', '', '', '', '', '', '', '', ''],
        ...
        ['结算客户名称', '日期','客户', '业务员', '备注', '单据类型', '单据编号', '应收', '已收','余额']
      ],
      body: printList,
      // startY: 60,
      styles: tableStyle(),
...
const excelData = () => {
  ...
  const sheet:any = [
    {
      ...
      multiHeader: [
        ['单位：'+pageParameter.companyName, '','', '', '期间：'+qijian.value, '', '', '', '', '',''],
        ['结算客户名称', '日期','客户', '部门', '业务员', '备注', '单据类型', '单据编号', '应收', '已收','余额']
      ],
      table: arrData,
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应收明细表_'+pageParameter.companyName)
}
...
import {
  findYsdmxbList,
  findYsdmxzByDateAndCvencodeList,
  findYsdtjbList
