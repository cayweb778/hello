...
<!--   <div><textarea>销售发票复核的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">销售发票复核</b>
...
async function reloadTable(){
  ...
  const res = await useRouteApi(findXsfpByIyear,{schemaName: dynamicTenantId})({ddate1:date1,ddate2:date2})
...
async function approved(){
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        const item = checkRow.value[i]
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'销售发票',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'复核',recordNum:item.ccode,caozuoModule:'stock'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'销售发票,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'复核',recordNum:item.ccode,caozuoModule:'stock'})
          }
        }
        item.bworkable = '1'
        ...
        //应收审核自动核销
        if (arHexiaoAuto.value=='1' && arSourceFlag.value=='1'){
          let money = item.isum
          //查询期初收款单
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'复核',recordNum:item.ccode})
      }
    }
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'销售发票',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'弃复',recordNum:item.ccode,caozuoModule:'stock'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'销售发票,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'弃复',recordNum:item.ccode,caozuoModule:'stock'})
          }
        }
        item.bworkable = '0'
        item.bworkableTime = ''
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售发票',method:'弃复',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
...
import {
  ...
  findSaleousingList,
  findSaleousingsByCcode, findXsfpByIyear,
  findYsdmxbList,
  findYshXhdByIyear,
  saveSaleousing,
  saveSaleousingsList
