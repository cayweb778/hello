...
<!--   <div><textarea>应收明细账的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container-top">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应收明细账</b>
...
async function reloadTable(){
  ...
  tableData.value = []
  tableDataAll.value = []
  // const res = await useRouteApi(findYsdmxbList,{schemaName: dynamicTenantId})({iyear:pageParameter.year,bcheck:bcheck.value})
  if (cvencode.value!=null && cvencode.value!='') {
    ...
    if (arSourceFlag.value=='1'){
      res = yueList.filter(item => item.arStyle=='YSD')
    } else {
      res = yueList
    }
    const res2 = await useRouteApi(findYsdmxzByDateAndCvencodeList, {schemaName: dynamicTenantId})({
      bcheck: bcheck.value,
    ...
    const res1 = await useRouteApi(findYsdmxzByDateAndCvencodeList, {schemaName: dynamicTenantId})({
      bcheck: bcheck.value,
      ddate1: ddate1.value,
      ddate2: ddate2.value,
      cvencode: cvencode.value,
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '应收明细账_'+pageParameter.companyName)
}
