...
<!--   <div><textarea>预收冲应收的页面</textarea>复制内容快速定位页面</div>-->
...
import {
  ...
  findByTypeList, findXyCsourceByCcodeAndXyccode, saveSaleousing, saveStockXyCsource, saveWriteOff,
  saveWriteOffList,
  saveYsyf,
  saveYsyfsList
...
const titleContents = ['预收冲应收', '预收冲应收', '预收冲应收']
...
function openPage(){
  ...
  router.push({
    path: '/YingShouZhang/ChongXiao/ChaXun/YuShouChongYingShouLieBiao',
    query: pageParameter
  });
}
...
async function contentSwitch(action) {
  loadMark.value = true
  const res = await useRouteApi(findArApChongxiaoList,{schemaName: dynamicTenantId})({billStyle:'YS',iyear:dynamicYear.value})
  ...
  if(arApYsyfList.value.length>0){
    ...
    const res = await useRouteApi(findChongxiaosByCcode,{schemaName: dynamicTenantId})({ccode:formItems.value.ccode,billStyle:'YS'})
    tableData.value = res.filter(item => item.busStyle=='SKD' || item.busStyle=='QCSKD').map(item => {
    ...
    setTableData(tableData.value)
    tableData1.value = res.filter(item => item.busStyle!='SKD' && item.busStyle!='QCSKD').map(item => {
      item.whxIsum = sub(item.isum, item.ydhxIsum)
      item.hxMoney = item.hxIsum
      item.ljhxIsum = item.ydhxIsum
      return item
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'预收冲应收',method:'添加'})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'添加',caozuoModule:'ar'})
    } else {
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'添加',caozuoModule:'ar'})
    }
    status.value = 1
    formItems.value.id=null
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    createConfirm({
      iconType: 'warning',
      title: '预收冲应收删除',
      content: '您确定要进行预收冲应收删除吗!',
      onOk: async () => {
        //判断预收冲应收
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'预收冲应收',method:'删除',recordNum:formItems.value.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'预收冲应收,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
          }
        }
        ...
        const skdList = list.filter(item => item.busStyle=='SKD' || item.busStyle=='QCSKD')
        const ysdList = list.filter(item => item.busStyle!='SKD' && item.busStyle!='QCSKD')
        //修改应收单核销金额和核销状态
        ...
        for (let i = 0; i < ysdList.length; i++) {
          ...
          if(item.sourcetype.substring(0,2)=='QC'){
            ...
            //普通应收单
            const saleousing = await useRouteApi(findSaleousingByCcode, {schemaName: dynamicTenantId})({ccode:item.sourcecode,billStyle:item.billStyle})
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:formItems.value.ccode})
...
async function saveData() {
  ...
  for (let j = 0; j < list2.length; j++) {
    ...
    if (xhd.type=='XHD') {
      const item1 = await useRouteApi(findSaleousingByCcode, {schemaName: dynamicTenantId})({ccode:xhd.sourcecode,billStyle:xhd.sourcetype})
  ...
  //删除任务锁定
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'添加'})
  await balanceTaskDel()
