  ...
  <div class="dynamic-form">
    <BasicForm @register="register"  ref="formElRef">
      <template #cvencode="{ model, field }">
        <Select @change="cvencode(model)" placeholder="请选择" v-model:value="model[field]" ref="cvencodeRef" :show-search="true"
      ...
      <template #methodPay="{ model, field }">
        <Select placeholder="请选择" v-model:value="model[field]" ref="methodPayRef"
                :filter-option="filterOption" @keyup.enter.native="focusNext(field)"
                :options="[
...
import {DatePicker,Select, Input, message,InputNumber} from "ant-design-vue";
...
async function cvencode(a) {
  ...
  let sup=await useRouteApi(findBySupplierId, {schemaName: props.accId})(a['cvencode'])
  if(!hasBlank(sup.contacts)&&!hasBlank(sup.telephone)){
    a['cvencodeContact']=sup.contacts+','+sup.telephone
  }else{
    a['cvencodeContact']=''
  }
  cvencodeJs(a)
...
function createItem(it) {
  return {
    ...
    slot: it['component'] == 'Select' || it['field'] == 'rate'|| it['field']=='payDate'|| it['field']=='theDeposit' ? it['field'] : null,
    show: it['isShow']
  }
}
