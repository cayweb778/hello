        ...
        <div>
          ...
          <div>
            ...
            <Popover class="ant-btn-default" placement="bottom" v-model:visible="visible">
              <template #content>
                <DynamicColumn :defaultData="defaultData" :dynamicData="dynamicColumnModel" :lanmuInfo="lanMuData" @reload="reloadColumns"/>
    ...
    <div class="app-container">
        ...
        >
          ...
          <template #cnumber="{ record }"><span v-if="!isNaN(record.cnumber)" class="tableUStyle" @click="openNewPage(record)">
            {{ parseFloat(record.cnumber).toFixed(2) }}</span></template>
          <template #baseQuantity="{ record }"><span class="tableUStyle" @click="openNewPage(record)">{{ toThousandFilter(record.baseQuantity) }}</span></template>
...
async function saveQuery(e) {
  ...
  defaultData.value=data.constant.queryType=='1'?initDynamics()['1']:initDynamics()['2']
...
async function findByAll() {
  loadMark.value = true
  popData.value.selectClass=pageParameter.selectClass
...
const exportExcel = () =>{
  ...
  arrData.filter(a=>a.bcheck!=='11').forEach((a,index)=>{
    ...
    a.dingDanHao=a.dingDanHao==null?'':a.dingDanHao
    list.push(a)
  })
...
const printStock = (obj,printUser,thisDate) => {
  ...
  data.forEach((item,index) => {
    let item1 = {}
    item1[0] = index+1
    item1[1] = item.cinvode
    item1[2] = setString(item.stockName,13)
    item1[3] = item.baseQuantity
    item1[4] = item.subQuantity1
    item1[5] = item.subQuantity2
    item1[6] = item.icost
    item1[7] = item.itaxprice
    item1[8] = item.isum
    item1[9] = item.isumRuku
    item1[10] = item.isumTuihuo
    item1[11] = item.isumFapiao
    item1[12] = item.isumJieKuan

    baseQuantity+=parseFloat(hasBlank(item.baseQuantity.replace(/\$\s?|(,*)/g, ''))?'0':item.baseQuantity.replace(/\$\s?|(,*)/g, ''))
    subQuantity1+=parseFloat(hasBlank(item.subQuantity1.replace(/\$\s?|(,*)/g, ''))?'0':item.subQuantity1.replace(/\$\s?|(,*)/g, ''))
    subQuantity2+=parseFloat(hasBlank(item.subQuantity2.replace(/\$\s?|(,*)/g, ''))?'0':item.subQuantity2.replace(/\$\s?|(,*)/g, ''))
    itaxprice+=parseFloat(hasBlank(item.itaxprice.replace(/\$\s?|(,*)/g, ''))?'0':item.itaxprice.replace(/\$\s?|(,*)/g, ''))
    isum+=parseFloat(hasBlank(item.isum.replace(/\$\s?|(,*)/g, ''))?'0':item.isum.replace(/\$\s?|(,*)/g, ''))
    icost+=parseFloat(hasBlank(item.icost.replace(/\$\s?|(,*)/g, ''))?'0':item.icost.replace(/\$\s?|(,*)/g, ''))
    isumRuku+=parseFloat(hasBlank(item.isumRuku.replace(/\$\s?|(,*)/g, ''))?'0':item.isumRuku.replace(/\$\s?|(,*)/g, ''))
    isumTuihuo+=parseFloat(hasBlank(item.isumTuihuo.replace(/\$\s?|(,*)/g, ''))?'0':item.isumTuihuo.replace(/\$\s?|(,*)/g, ''))
    isumFapiao+=parseFloat(hasBlank(item.isumFapiao.replace(/\$\s?|(,*)/g, ''))?'0':item.isumFapiao.replace(/\$\s?|(,*)/g, ''))
    isumJieKuan+=parseFloat(hasBlank(item.isumJieKuan.replace(/\$\s?|(,*)/g, ''))?'0':item.isumJieKuan.replace(/\$\s?|(,*)/g, ''))
  ...
  for (let i=0; i<printList.length%27; i++){
    ...
    if(printList.length%27==26){
      item1[1] = '合计'
      item1[3] = toThousandFilter(baseQuantity)
      item1[4] = toThousandFilter(subQuantity1)
      item1[5] = toThousandFilter(subQuantity2)
      item1[6] = toThousandFilter(icost)
      item1[7] = toThousandFilter(itaxprice)
      item1[8] = toThousandFilter(isum)
      item1[9] = toThousandFilter(isumRuku)
      item1[10] = toThousandFilter(isumTuihuo)
      item1[11] = toThousandFilter(isumFapiao)
      item1[12] = toThousandFilter(isumJieKuan)
    }
    printList.push(item1)
  }
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    tableName=tableName+'采购排行榜'
    doc.autoTable({
      head: [['', '', '', '', '', '', tableName, '', '', '', '', '','',],
        ['核算单位：' + companyName.value, '', '','', '','', '期间：'+ qijianText.value , '', '', '','','', '单位：元'],
        ['序号', '存货编码', '存货名称', '主数量', '数量1', '数量2', '无税金额', '税额', '价税合计', '累计入库数量', '累计退货数量', '累计开票数量','累计结款金额']],
      body: printList,
      styles: tableStyle(),
      margin: {
        left: 30,
...
const printSupplier = (obj,printUser,thisDate) => {
  ...
  data.forEach((item,index) => {
    ...
    item1[2] = setString(item.custName,13)
    item1[3] = item.baseQuantity
    item1[4] = item.subQuantity1
    item1[5] = item.subQuantity2
    item1[6] = item.icost
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['', '', '', '', '', '', tableName, '', '', '', '', '','',],
        ['核算单位：' + companyName.value, '', '','', '','', '期间：'+ qijianText.value , '', '', '','','', '单位：元'],
        ['序号', '供应商编码', '供应商名称', '主数量', '数量1', '数量2', '无税金额', '税额', '价税合计', '累计入库数量', '累计退货数量', '累计开票数量','累计结款金额']],
      body: printList,
      styles: tableStyle(),
      margin: {
        left: 30,
