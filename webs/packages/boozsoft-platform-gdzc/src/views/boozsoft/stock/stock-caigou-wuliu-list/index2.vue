<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">物流送货单列表</b></div>
...
<!--              <Button @click="excelMxData">导出明细列表</Button>-->
<!--            </template>-->
        ...
        <div>
          <div>
            ...
            <Popover class="ant-btn-default" placement="bottom">
              <template #content>
                <span class="group-btn-span-special2" @click="dataType='0';reloadPage()" :style="dataType=='0'?{backgroundColor: '#0096c7',color: 'white'}:''" style="width: 130px;cursor: pointer;">
                  蓝字入库单&emsp;<CheckOutlined v-if="dataType=='0'"/>
                </span><br/>
                <span class="group-btn-span-special2" @click="dataType='1';reloadPage()" :style="dataType=='1'?{backgroundColor: '#0096c7',color: 'white'}:''" style="width: 130px;cursor: pointer;">
        ...
        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;">
              ...
              <TableSummaryCell class="nc-summary" style="border-right: none;" v-for="cell in getCurrSummary()"  :index="cell.ind" :align="cell.align"><span class="a-table-font-arial">{{null == summaryTotals[cell.dataIndex]?'':summaryTotals[cell.dataIndex].toFixed(2)}}</span></TableSummaryCell>
            </TableSummaryRow>
          </TableSummary>
        </template>
      </BasicTable>
...
import {
  ...
  findAllByStockWarehListCcode, findByStockWarehLinecode, findByStockWaresCcodeAndLineCode,
  findCangkuAllList,
...
import {findAllWuliusList} from "/@/api/record/stock/stock-wuliu";
...
async function tablesData1(){
  tableDataAll1.value = await useRouteApi(findAllWuliusList,{schemaName: dynamicTenantId})(popData.value)
  console.log(tableDataAll1.value)
...
const windowHeight1 = (window.innerHeight - (310))
...
function toRouter(data,type) {
  if(type=='list'){
    router.push({name: 'CaigouRk',params: {type:'info',ccode:data.ccode}});
...
function editFun() {
  ...
  router.push({name: 'CaigouRk',params: {type:'edit',ccode:data[0].ccode}});
}
...
async function delFun() {
  ...
  for (let i = 0; i < getSelectRows1().length; i++) {
    // 任务
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'采购入库单',method:'修改,审核,删除',recordNum:getSelectRows1()[i].ccode})
    if(!hasBlank(taskData)){
      // 任务不是当前操作员的
      if(taskData[0]?.caozuoUnique!==useUserStoreWidthOut().getUserInfo.id){
        return createWarningModal({ content: '【'+getSelectRows1()[i].ccode+'】'+taskData[0]?.username+'正在'+taskData[0]?.method+'采购到货单,不能同时进行操作！' });
      }
    }

    // 入库单明细
    let mx=await useRouteApi(findAllByStockWarehListCcode, {schemaName: dynamicTenantId})({ccode:getSelectRows1()[i].ccode,type:'CGRKD'})
    for (let j = 0; j < mx.length; j++) {
      // 判断可用量所需条件
      let tx=mx[j]
      let temp2:any={}
  ...
  createConfirm({
    ...
    onOk: async () => {
      ...
      for (let i = 0; i < getSelectRows1().length; i++) {
        ...
        // 上游单据 && 是否生成
        if(!hasBlank(getSelectRows1()[i].sourcetype)){
          // 删除下游单据
          await useRouteApi(delXyCsourceByxyCcode, {schemaName: dynamicTenantId})(getSelectRows1()[i].ccode)
          // 入库单明细
          let mx=await useRouteApi(findAllByStockWarehListCcode, {schemaName: dynamicTenantId})({ccode:getSelectRows1()[i].ccode,type:'CGRKD'})
          for (let j = 0; j < mx.length; j++) {
            // 上游单据明细
        ...
        await useRouteApi(deleteByMethodAndRecordNum, {schemaName: dynamicTenantId})({method:'删除',ccode:getSelectRows1()[i].ccode})
      }
