...
const titleContents = ['入库调整单', '入库调整单', '入库调整单']
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    // 任务
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'入库调整单',method:'修改,审核,删除',recordNum:formItems.value.ccode})
    if(taskData==''){
      tempTaskSave('修改')
    }else{
      for (let i = 0; i < taskData.length; i++) {
        ...
        if(taskData[i]?.caozuoUnique!==useUserStoreWidthOut().getUserInfo.username){
          return createWarningModal({ content: taskData[i]?.username+'正在'+taskData[i]?.method+'入库调整单,不能同时进行操作！' });
        }
      }
...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'入库调整单',method:method,recordNum:formItems.value.ccode,caozuoModule:'stock'})
    .then((a)=>{
      taskInfo.value=a
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    // 可用量不足 弹出框提示
    let currData=await useRouteApi(findByStockNumVerifyLack, { schemaName: dynamicTenantId })({list:JSON.stringify(verifylist),typeData:'RKTZD'})
...
async function saveLogData(optAction) {
  let logmap={
    ...
    optAction:optAction,
    accId:dynamicAccId.value,
    optContent:'操作内容【入库调整单】,单据编号【'+formItems.value?.ccode+'】',
  }
