  ...
  <div class="dynamic-form">
    <BasicForm @register="register"  ref="formElRef">
      <template #cvencode="{ model, field }">
        <Select @change="cvencodeJs(model)" placeholder="请选择" v-model:value="model[field]" ref="cvencodeRef" :show-search="true"
...
<!--            <SearchOutlined @click="exec(field)"/>-->
      ...
      <template #ddate="{ model, field }">
        <DatePicker v-model:value="model[field]" ref="ddateRef" @change="dateChange(model[field],model, field)" style="width: 100%;" />
      </template>
...
.dynamic-form{
  :deep(.ant-select-selector), :deep(.ant-picker), :deep(.ant-input-affix-wrapper),:deep(#form_item_rate),:deep(#form_item_billCode),:deep(#form_item_billNumber){
    border: none;
