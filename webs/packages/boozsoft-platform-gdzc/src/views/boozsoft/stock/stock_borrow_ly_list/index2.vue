  ...
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        ...
        <div><span style="font-size: 14px;font-weight: bold;color: #666666">期间：{{ strDate }} - {{
            endDate
          }}</span></div>
      </div>
      <div>
        <div>
          ...
          <button
            type="button"
            class="ant-btn ant-btn-me"
            @click="router.push({path: 'jy-iLend',query: {type:'add',ccode:''}})"
          ><span>新增</span></button>
          <button
            type="button"
            class="ant-btn ant-btn-me"
    ...
    <div class="app-container">
      ...
      >
        <template #bcheck="{ record }">
          <span v-if="record.bcheck!=undefined">
            <Tag :color="record.bcheck === '已审核' ? 'green' : 'volcano'">
              {{ record.bcheck }}
            </Tag>
          </span>
        </template>
        ...
        <template #cnumber="{ record }">
          {{ hasBlank(record.cnumber)?'':parseFloat(record.cnumber).toFixed(2) }}
        </template>
        <template #baseQuantity="{ record }">
          {{ hasBlank(record.baseQuantity)?'':parseFloat(record.baseQuantity).toFixed(2) }}
        </template>
        <template #ljhhSum="{ record }">
          {{ hasBlank(record.ljhhSum)?'':parseFloat(record.ljhhSum).toFixed(2) }}
        </template>
        <template #noLjhhNum="{ record }">
          {{ hasBlank(record.noLjhhNum)?'':parseFloat(record.noLjhhNum).toFixed(2) }}
        </template>
        <template #ljzhSum="{ record }">
          {{ hasBlank(record.ljzhSum)?'':parseFloat(record.ljzhSum).toFixed(2) }}
        </template>
        <template #summary>
          <TableSummary fixed>
...
async function tablesData1() {
  ...
  tableDataAll1.value = await useRouteApi(findByStockBorrowTableList, {schemaName: dynamicTenantId})(popData.value)
...
const getCurrSummary = () => {
  return (getColumns1().filter(it => it.title != '序号' && it.ifShow).map((it, ind) => {
    it['ind'] = ind + 2;
    return it;
  }))
}
const calculateTotal = () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    ljhhSum += parseFloat(e.ljhhSum || '0')
    noLjhhNum += parseFloat(e.noLjhhNum || '0')
    ljzhSum += parseFloat(e.ljzhSum || '0')
  }
  summaryTotals.value = {
    cnumber: parseFloat(cnumber)==0?'':parseFloat(cnumber).toFixed(2),
    baseQuantity: parseFloat(baseQuantity)==0?'':parseFloat(baseQuantity).toFixed(2),
    ljhhSum: parseFloat(ljhhSum)==0?'':parseFloat(ljhhSum).toFixed(2),
    noLjhhNum: parseFloat(noLjhhNum)==0?'':parseFloat(noLjhhNum).toFixed(2),
    ljzhSum: parseFloat(ljzhSum)==0?'':parseFloat(ljzhSum).toFixed(2),
  }
}
...
const CrudApi1 = {
  columns: [
    ...
    // },
    {
      ...
      width: 150,
      align: 'left',
      ellipsis: true,slots: {customRender: 'ljhhSum'}
    },{
      title: '未归还数量',
      dataIndex: 'noLjhhNum',
      width: 150,
      align: 'left',
      ellipsis: true,slots: {customRender: 'noLjhhNum'}
    },{
      title: '转换为赠送数量',
      dataIndex: 'zsNumber',
      width: 150,
      align: 'left',
      ellipsis: true,slots: {customRender: 'zsNumber'}
    },{
      title: '转换为采购数量',
      dataIndex: 'cgNumber',
      width: 150,
      align: 'left',
      ellipsis: true,slots: {customRender: 'cgNumber'}
    },{
      title: '入库数量',
      dataIndex: 'rukuSum',
      width: 150,
      align: 'left',
      ellipsis: true,slots: {customRender: 'rukuSum'}
    },{
      title: '制单人',
      dataIndex: 'cmaker',
      width: 150,
...
const excelThisData = () => {
  ...
  arrData.filter(a => a.bcheck !== '11').forEach((a, index) => {
    ...
    a.ljhhSum = a.ljhhSum == null ? '' : parseFloat(a.ljhhSum).toFixed(2)
    a.zsNumber = a.zsNumber == null ? '' : parseFloat(a.zsNumber).toFixed(2)
    a.cgNumber = a.cgNumber == null ? '' : parseFloat(a.cgNumber).toFixed(2)
    a.rukuSum = a.rukuSum == null ? '' : parseFloat(a.rukuSum).toFixed(2)
    list.push(a)
  })
  const sheet = [
    {
  ...
  exportExcel3(sheet, 'xlsx', '借入借用单列表_' + pageParameter.companyName)
}
...
const startReview = async (b) => {
  ...
  let map={
    ...
    bcheckUser:bcheckUser,
    ccode:list.map(a=>a.ccode)
  }
  await useRouteApi(updateStockBorrowBcheckByCcodeList, {schemaName: dynamicTenantId})(map)

  loadMark.value = true
  message.success(b ? '审核成功' : '弃审成功')
  loadMark.value = false
  await tablesData1()
