    ...
    <div class="app-container lcr-theme-div">
      <div>
        ...
        <div style="padding-left: 9%;">
          <label>业务类型：</label>
          <Select v-model:value="pageParameter.dataType"  style="width: 180px;font-size: 14px;font-weight: bold;" @change="changeType" >
            <SelectOption  value="XTZHRKD">形态转换入库</SelectOption>
            <SelectOption  value="XTZHCKD">形态转换出库</SelectOption>
          </Select>
        </div>
      </div>
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">形态转换出入库单列表</b></div>
...
import {findAllByCondition} from "/@/api/record/stock/stock-xtzhd";
...
async function reloadTable(){
  ...
  tableDataAll1.value = await useRouteApi(findByTypeList,{schemaName: dynamicTenantId})({type:'XTZHCKD',year:pageParameter.year})
  ...
  tableData1.value =  replenishTrs(tableDataAll1.value)
...
const excelDataCode = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '形态转换出库单列表_'+pageParameter.companyName)
}
