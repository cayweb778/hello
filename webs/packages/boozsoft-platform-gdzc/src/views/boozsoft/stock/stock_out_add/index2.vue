      ...
      <div>
        <div>
          ...
          <Popover placement="bottom" v-if="status == 1">
            <template #content>
              <template v-if="titleValue == 0">
                ...
                <Button @click="referData('XHD')">销货单</Button>
              </template>
              <template v-else>
          ...
          <Popover placement="bottom" v-if="status == 3 && formItems.bcheck == '1'" >
            <template #content>
              <Button @click="generate('fp')" v-if="titleValue == 0">销售发票</Button><br/>
              <Button @click="generate('xh')" v-if="titleValue == 0">销货单</Button><br/>
              <Button @click="generate('pz')" >财务凭证</Button><br/>
            </template>
          ...
          <Button class="actod-btn actod-btn-last" @click="outBefore">退出</Button></div>
<!--        <Button class="actod-btn actod-btn-last" @click="testOpenDeptModal">测试部门</Button>
        <Button class="actod-btn actod-btn-last" @click="testOpenPsnModal">测试人员</Button>
        ...
        <div class="acbgead-one">
          ...
          <div class="acbgead-one-changes" :class="status == 1 || status == 2 ?'status-look':''">
            ...
            <span v-if="status=='3'" class="anticon" style="margin-left: 1em">
              <Tag v-if="formItems.bcheck=='1'" :color="'volcano' ">已审核</Tag>
              <Tag :color="'volcano'" v-if="formItems.bcheck == '1' && formItems.jhzxStatus=='1'">已拣货</Tag>
              <Tag :color="'volcano'" v-if="formItems.bcheck == '1' && formItems.fapiaoStatus=='1'">已开票</Tag>
            </span>
          </div>
      ...
      <div class="acb-centent">
        ...
        >
          <template #cwhcode="{ record }">
            <template v-if="record.editOne && record.sourcecode==null">
              <Select
          ...
          <template #cinvode="{ record }">
            <template v-if="record.editTwo  && record.sourcecode==null">
              <Select
          ...
          <template #cinvodeName="{ record }">
            <template v-if="record.editEleven  && record.sourcecode==null">
              <Select
          ...
          <template #cinvodeBarcode="{ record }">
            <template v-if="record.editThree  && record.sourcecode==null">
              <Select
          ...
          <template #xsUnitId="{ record }">
            <template v-if="record?.editCunit && record.sourcecode==null">
          ...
          <template #baseQuantity="{ record }">
            <template v-if="record?.editSix  && record.sourcecode==null">
         ...
         <span class="a-table-font-arial">{{
          ...
          <template #taxprice="{ record }">
            <template v-if="record?.editTaxprice  && record.sourcecode==null">
          ...
          <template #itaxrate="{ record }">
            <template v-if="record?.editItaxrate  && record.sourcecode==null">
          ...
          <template #isum="{ record }">
            <template v-if="record?.editIsum  && record.sourcecode==null">
          ...
          <template #batchId="{ record }">
            <template v-if="record?.editTwelve && record.isBatch　 &&　( record.sourcecode==null)">
          ...
          <template #dpdate="{ record }">
            <template v-if="( record.sourcecode==null)&&record?.editDpdate">
          ...
          <template #dvdate="{ record }">
            <template v-if="record?.editFifteen　 &&　( record.sourcecode==null)">
...
import {useNcModals} from './otherServerReferences'
...
const titleContents = ['销售出库单', '销售出库单']
...
async function contentSwitch(action) {
  loadMark.value = true
  await flushPeriod()
  if (route.query?.bdocum != null && !isNaN(route.query?.bdocum))titleValue.value = route.query?.bdocum
...
async function generateCodeXhd(date) {
  return await useRouteApi(findBillCode, {schemaName: dynamicTenantId})({
    type: "XHD",
    date: date,
    key: '3-6'
...
function codeToName(arr) {
  let names = {'XSCKD':'销售出库单','JHZXD':'拣货装箱单','XSQRD':'销售出库确认单'}
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'销售出库单',method:'修改,审核,弃审,删除',recordNum:formItems.value.ccode})
    if(taskData==''){
    ...
    if (!b){ // 弃审前 检查
      /* let taskData2= await useRouteApi(getByStockBalanceBatchTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'拣货装箱单,销售出库确认单',method:'新增',recordNum:formItems.value.ccode})
       if(taskData2==''){
         ...
         createWarningModal({ content: taskData2[0].caozuoUnique+'正在新增'+taskData2[0].functionModule+',当前单据已参与新增,不能同时进行弃审核操作！' });
      tempTaskDel()
         return false;
       }*/
      let che = await useRouteApi(unAuditBefore, {schemaName: dynamicTenantId})({
        codes: [formItems.value.ccode],
        style: 'XSCKD', id: dynamicTenant.value.target.id
      })
      if (null == che  || che.type != '0'){
        if (che.type == '1'){
          createWarningModal({title: '温馨提示',content: '销售出库单号【'+che.codes+'】已经收款核销，不能取消审核，请取消收款核销关联后继续！'})
        }else {
          createWarningModal({title: '温馨提示',content: '销售出库单号【'+che.codes+'】已经发现下游【'+codeToName(che.codeType)+'】，不能取消审核，请删除关联单据后继续！'})
        }
        tempTaskDel()
        return false;
      }
    }else {
      if (list.length  == 0 || (!await stockCheck(list)))return false;
    }
    // 盘点存货校验
    let resa = await useRouteApi(findByTakingChCodes, {schemaName: dynamicTenantId})({})
    if (!hasBlank(resa)){
      let ds = [...new Set(list.map(it=>it.cinvode))].filter(s=>resa.indexOf(s) != -1)
      let err = ''
    ...
    await useRouteApi(reviewCkd, {schemaName: dynamicTenantId})({
    ...
    tempTaskDel()
    writeLog(`${b?'审核':'弃审'}`,null,`操作内容【${b ? '审核' : '弃审'}销售出库单】,` + '销售出库单号【' + formItems.value.ccode + '】,单据日期【' + formItems.value.ddate + '】')
    await pageReload()
  } else {
...
async function stockCheck(list) {
  ...
  if (err != ''){
    // createWarningModal({title: '温馨提示', content: err})
    writeLog(t,formItems.value,`操作内容【${t}销售出库单】,` + `${t}失败,现存量不足！详情:`+err)
...
async function saveData() {
  let obj = formFuns.value.getFormValue()
  ...
  if (saveCheck(list)) {
    ...
    let nly = list.filter(it=>it.sourcecode == null)
    if (nly.length > 0 && (!await stockCheck(JsonTool.parseProxy(nly)))){
      tempTaskDel()
      return false;
    }
    ...
    if(checks.length > 0){
      ...
      for (let i = 0; i < checks.length; i++) {
        ...
        if (null== r || ((parseFloat(c.baseQuantity)-parseFloat(c.baseZOld)) > (parseFloat(r.baseQuantity) - parseFloat(r.isumChuku || '0'))))rowArr.push((i+1)+'')
      }
      if (rowArr.length > 0)
        return createWarningModal({title: "温馨提示",content: `系统检测到序号【${rowArr.join()}】行销售出库单数量不能大于参照的来源单据数量，请检查后重试！`})
    }
    ...
    await useRouteApi(saveCkd, {schemaName: dynamicTenantId})(formItems.value)
...
const tableDataChange =  (r,c) => {
  ...
  if (hasBlank(r.itaxrate)){
    r.itaxrate = parseFloat((dynamicTenant.value.target.xsRate || 0)).toFixed(6)
  }
  switch (c) {
    case 'cwhcode':
    ...
    case 'quantity':
      ...
      if (c == 'xsUnitId' && index!=-1){
        ...
        if (!hasBlank(r.quantity) && !hasBlank(r.price)){// 切换单价
          ...
          // let o  =  parseFloat(r.baseOld || 0)
          if (oldIndex == 0){
            let c = parseFloat(r.unitList[index]?.conversionRate)
            t = t*c
            ...
            // o = index == 0?o*parseFloat(r.unitList[oldIndex]?.conversionRate):o/(parseFloat(r.unitList[index]?.conversionRate) / parseFloat(r.unitList[oldIndex]?.conversionRate))
          }else if(oldIndex == 2){
            ...
            // o = o*(parseFloat(r.unitList[oldIndex]?.conversionRate) / parseFloat(r.unitList[index]?.conversionRate))
          }
          r.price = t
          // r.quantity = b
          // r.tempQuantity = b
      ...
      if (formItems.value.id != null && c == 'quantity' && !hasBlank(r.sourcecode)  && (r.quantity == '' || 0 >= parseFloat(r.quantity) ) ) {
        message.warning(`支持修改的数量范围[0～${r.baseOld}],且不包含0！`)
        r.quantity = r.baseOld
        break;
      } else if(formItems.value.id != null && c == 'quantity' && !hasBlank(r.sourcecode) && parseFloat(r.quantity) > parseFloat(r.baseOld)){ // 蓝字
        r.checkLyNumber = true
      }
      if (c == 'xsUnitId' && hasBlank(r.quantity)) break;
      slChangeNew(r)
      if (c == 'quantity' && !hasBlank(r.quantity)) tableDataChange(r,'price')
      break;
    case 'price':
    ...
    case 'icost':
      if (!hasBlank(r.quantity) && !hasBlank(r.icost)) { //反算 单价
        ...
        r.itaxprice = (d*parseFloat((r.itaxrate/100))).toFixed(6)
        // 得到税额
...
const focusNext =  (r, c) => {
  ...
  // 查找下一个
  let list = getDataSource();
  let filters = ['isGive','bcheck','cinvodeType','cunitid','cinvodeName','cinvodeBarcode','cunitidF1','cunitidF2','baseQuantity','subQuantity1','subQuantity2', 'dvdate', 'dpdate', 'itaxprice','availability']
...
const getNextMark = (c,b) => {
  let model = {
    ...
    cmemo: 'Thirteen', taxprice: 'Taxprice', itaxrate: 'Itaxrate', isum: 'Isum',xsUnitId:'Cunit',quantity:'Quantity',dpdate: 'Dpdate'
  }
  if (b){
    // 获取下一个
...
function xySourcePop() {
  if (hasBlank(formItems.value.ccode)){
    ...
    openXySourcePage(true, {
      database: dynamicTenantId.value, type: 'XSCKD',code: formItems.value.ccode,datas: {ck:ckListOptions.value},isRed: (titleValue.value == 1)
    })
  }
}
// 查看下游数据
function lySourcePop() {
  if (hasBlank(formItems.value.ccode)){
    ...
    openLySourcePage(true, {
      database: dynamicTenantId.value,code: formItems.value.ccode,type: 'XSCKD',datas: {ck:ckListOptions.value},isRed: (titleValue.value == 1)
    })
  }
}
...
function referData(type) {
  ...
  openReferPage(true, {
    ...
    type: type,btype: titleValue.value
  })
}
...
const referThrowData = async (data) => {
  if (data.length > 0){
    // 锁任务
    await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:data[0].sourcetype=='XSDD'?'销售订单':'销货单',method:data[0].sourcetype=='XSDD'?'变更':'审核',recordNum:data[0].sourcecode,caozuoModule:'stock'})
...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售出库单',method:method,recordNum:formItems.value.ccode})
    .then((a)=>{
...
async function writeLog(action, a, content) {
  ...
  if (action == '修改') {
    ...
    let text = `操作内容【${action}销售出库单】,` + '销售出库单号【' + a.ccode + '】,单据日期【' + a.ddate + '】,'
  ...
  let map = {
    ...
    optContent: content || `操作内容【${action}销售出库单】,` + '销售出库单号【' + a.ccode + '】,单据日期【' + a.ddate + '】',
  }
  await saveLog(map)
}
...
const generate = async (t) => {
  ...
  // 有无 任务
  let xclTaskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'销售退货单',method:'生单'})
  if(!hasBlank(xclTaskData)){
    return message.error('当前单据正在被'+xclTaskData[0].username+'操作员进行'+xclTaskData[0].method+'业务处理，任务互斥，请销后再试！')
  }
  let busDate = useCompanyOperateStoreWidthOut().getLoginDate;
  let genDate = busDate
  ...
  if(t=='xh'){
    data.map(it=>n+=parseFloat(it.isumXiaohuo || '0'))
    if (entry.xiaohuoStatus == '1' || n>=b){
      createWarningModal({title: "温馨提示",content: '当前销售出库单已经全部完成销货，不能再次进行销货操作！'})
      return;
    }
    if (entry.sourcetype == 'XHD' || data.filter(it=>it.sourcetype == 'XHD').length > 0){
      createWarningModal({title: "温馨提示",content: '当前售出库单为参照销售单生成，不能进行销货单循环生单操作！'})
      return;
    }
    entry.ccode = await generateCodeXhd(genDate)
    entry.bdocumStyle = "0"
    entry.xiaohuoStatus = null;
    entry.billStyle = 'XHD';
    let list = data.map(it=>{
      ...
      it.baseQuantity = (parseFloat(it.baseQuantity || 0)-parseFloat(it.isumXiaohuo || 0)).toFixed(10)
      it.quantity = getNumber(it)
      slChangeNew(it) // 更改数量
      tableDataChange(it,'price') // 计算金额
      it.isumXiaohuo = 0
      it.isumXiaohuo = 0
    ...
    createConfirm({
      ...
      content: '您确定要生成销货单吗!',
      onOk: async () =>{
        await useRouteApi(generateXhd, {schemaName: dynamicTenantId})(entry)
...
async function testOpenDeptModal(){
  const {openDeptModal} =  useNcModals()
  const params={}
  const result=await openDeptModal(params)

}

async function testOpenCustomModal(){
  const {openCustomModal} =  useNcModals()
  const params={}
  const result=await openCustomModal(params)
}

async function testOpenGysModal(){
...
async function testOpenPsnModal(){
  const {openPsnModal} =  useNcModals()
  const params={}
