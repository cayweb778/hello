...
export const useMultipleTabStore = defineStore({
  ...
  actions: {
    setActiveKeyRef(activeKeyRef){
      this.activeKeyRef=activeKeyRef
    },
    /**
    ...
    async closeTab(tab: RouteLocationNormalized, router: Router) {
        ...
        window.viewProvider[tab.fullPath]=null
      // console.log(window.viewProvider)
    ...
    async closeToFullPathsTabs(path:string, router: Router) {
      const closePathList = this.tabList.map((item) => item.fullPath).filter(p => null != p && router.currentRoute._value.fullPath != p &&  p.indexOf(path)!= -1);
      this.bulkCloseTabs(closePathList);
      this.updateCacheTab();
      // handleGotoPage(router);
    },
