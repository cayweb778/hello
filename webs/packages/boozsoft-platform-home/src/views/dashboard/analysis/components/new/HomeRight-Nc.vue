<template>
  <div class="nc-hr">
      ...
      <div class="nc-hr-top-left">
        <div>
          <Avatar src="" :size="{ xs: 24, sm: 32, md: 40, lg: 64, xl: 80, xxl: 80 }">
            <HomeFilled style="font-size: 40px;color: #146dc1;margin-top: 40%;" />
          </Avatar>
        </div>
        <div>
          <span>欢迎使用</span><br/>
    ...
    <div class="nc-hr-middle" v-if="showAccountPicker">
      <AccountPicker theme="three" @reloadTable="dynamicAdReload"/>
    </div>
    <div class="nc-hrm-body" :style="{maxHeight: windowHeight+'px'}">
      <a-row type="flex">
        <a-col :span="8" v-for="(item,index) in platforms.filter(it=>ztTypes.indexOf(it.type) != -1)"
               @click="goGdzcPlatform(item.platformId)">

          <template v-if="item.platformId==1099 ||item.platformId==20001">
            <div>
              <div>
...
import {ACCOUNT_ALL_LIST_KEY, APP_SERVER_DATE_KEY} from "/@/enums/cacheEnum";
...
let platforms = ref([
  ...
  },
  {
    ...
    platformId: 10133200,  type: 'ALL'
  },
  {
    title: '应收款管理',
    legend: '应收业务单据的审核、收款核销、往来转账业务',
    platformId: 1007,  type: 'CH'
  },
  {
    title: '应付款管理',
    legend: '应付业务单据的审核、付款核销、往来转账业务',
    platformId: 20017, type: 'CH'
  },
])
...
const dynamicAdReload = (e) => {
  ...
  ztTypes.value = (e.target.ztStyle.split(',') || [])
  console.log(ztTypes.value)
  ztTypes.value.push('ALL')
}

function goGdzcPlatform(id){
  window?.$wujie.bus.$emit("goGuDingZiChan",id);
}
