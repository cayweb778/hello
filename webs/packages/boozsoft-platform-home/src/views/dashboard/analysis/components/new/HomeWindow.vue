<template>
  ...
  >
    ...
    <div class="nc-open-content">
      <div class="nc-open-top">
        <div class="nc-hr-top-left">
          <div>
            <Avatar src="" :size="{ xs: 24, sm: 32, md: 40, lg: 64, xl: 65, xxl: 65 }">
              <UserOutlined style="font-size: 30px;color: #146dc1;line-height: 60px;" />
            </Avatar>
          </div>
          <div>
            <span>您好,</span><br/>
            <span>{{formItems.realName}}</span>
          </div>
        </div>
      ...
      <Tabs type="card" :tabBarGutter="1" v-model:activeKey="activeKey" tabPosition="left">
        <TabPane :key="1"
          ...
          <div class="tabpane-top">
            <div>
              <Button style="border: none;"  v-if="isEdit[1]" @click="handleOk(1)"><EditOutlined />保存</Button>
              <Button style="border: none;"  v-if="!isEdit[1]" @click="isEdit[1] = true"><EditOutlined />编辑</Button>
            </div>
            <div>
              <Button type="primary" @click="openPwd">修改密码</Button>
            </div>
          </div>
          <div class="tabpane-bottom">
              <ul>
                ...
                <li>
                  <span>操作员角色：</span><Input :value="nameAndRole.roleName"
                                           style="pointer-events: none;"/>
                </li>
                <li>
                ...
                <li>
                  <span>手机号：</span><Input v-model:value="formItems.phone" @blur="checkPhone()"
                                          :style="!isEdit[1]?{pointerEvents: 'none'}:{borderBottom: '1px solid black'}"/>
                </li>
                <li>
                ...
                <li style="padding-left:2em;">
                  <span>最小长度：</span>
                  <Input :value="policyModel != null?policyModel.pwdLength:'无'" style="pointer-events: none;width: 30%;"/>
                </li>
                <li style="padding-left:2em;">
                  <span>强制包含大写字母：</span>
                  <Input :value="policyModel != null?policyModel.pwdUpperCase=='1'?'是':'否':'无'" style="pointer-events: none;width: 30%;"/>
                </li>
                <li style="padding-left:2em;">
                  <span>强制包含特殊字符：</span>
                  <Input :value="policyModel != null?policyModel.pwdTe=='1'?'是':'否':'无'" style="pointer-events: none;width: 30%;"/>
                </li>
              </ul>
          </div>
        </TabPane>
...
import {LockOutlined, EditOutlined,FormOutlined,UserOutlined} from '@ant-design/icons-vue';
...
// import Pwd from '/@/views/boozsoft/system/user//popup/password.vue';
const Pwd=null
