<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">组装拆卸单列表</b></div>
        ...
        <div>
          <div>
            <Select v-model:value="pageSearch.selectType" class="acttdrd-search-select">
              ...
              <SelectOption style="font-size: 12px;" value="bomName">BOM存货名称</SelectOption>
              <SelectOption style="font-size: 12px;" value="bomVer">BOM版本号</SelectOption>
            </Select>
            <InputSearch
    ...
    <div class="app-container">
      ...
      >
        ...
        <template #feiyongJe="{ record }">
          <span :style="record.feiyongJe<0?{color:'red'}:{}">
        ...
        <template #bomLevel="{ record }">
          <span v-if="record.bcheck!=='11'">
            {{ getBomLevel(record.bomLevel)}}
          </span>
        </template>
        <template #summary>
          <TableSummary fixed>
      ...
      >
        ...
        <template #rukuCode="{ record }">
          <a @click="toRouter(record,'list')">{{record.rukuCode}}</a>
        </template>
        <template #bomLevel="{ record }">
          <span v-if="record.bcheck!=='11'">
            {{ getBomLevel(record.bomLevel)}}
        ...
        <template #price="{ record }">
          <span v-if="record.bcheck!=='11'" :style="record.price<0?{color:'red'}:{}">
        ...
        <template #fyftIsum="{ record }">
          <span :style="record.fyftIsum<0?{color:'red'}:{}">
            {{toThousandFilter(record.fyftIsum)}}
          </span>
        </template>
        <template #icostTotal="{ record }">
          <span :style="record.icostTotal<0?{color:'red'}:{}">
            {{toThousandFilter(record.icostTotal)}}
          </span>
        </template>
        <template #sjshMoney="{ record }">
          <span :style="record.sjshMoney<0?{color:'red'}:{}">
            {{toThousandFilter(record.sjshMoney)}}
          </span>
        </template>
        <template #deshMoney="{ record }">
          <span :style="record.deshMoney<0?{color:'red'}:{}">
            {{toThousandFilter(record.deshMoney)}}
          </span>
        </template>
...
const bomLevelList=ref([{value: '1',label: '一级'},{value: '2',label: '二级'},{value: '3',label: '三级'},{value: '4',label: '四级'},{value: '5',label: '五级'},{value: '6',label: '六级'}
...
const getBomLevel = (data) => {
 return bomLevelList.value.filter(tx=>tx.value==data)[0].label
}
...
async function tablesData1(){
  ...
  tableDataAll1.value = await useRouteApi(findByStockAdTableList,{schemaName: dynamicTenantId})(popData.value)
...
const calculateTotal = () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    feiyongJe += parseFloat(e.feiyongJe || '0')
    sjshMoney += parseFloat(e.sjshMoney || '0')
  }
  summaryTotals.value={
    deshMoney: parseFloat(deshMoney).toFixed(2),
    sjshMoney: parseFloat(sjshMoney).toFixed(2),
    icostTotal: parseFloat(icostTotal).toFixed(2),
    fyftIsum: parseFloat(fyftIsum).toFixed(2),
    icost: parseFloat(icost).toFixed(2),
    price: parseFloat(price).toFixed(2),
    cnumber: parseFloat(cnumber).toFixed(2),
    feiyongJe: toThousandFilter(feiyongJe),
  }
}
...
const CrudApi1 = {
  columns: [
    ...
    },
    {
      ...
      width: 150,
      align: 'left',
      ellipsis: true,slots: { customRender: 'bomLevel'}
    },
    {
      title: '业务部门',
      key: 'deptName',
...
const excelThisData = () =>{
  ...
  exportExcel3(sheet, 'xlsx','组装拆卸单列表_'+pageParameter.companyName)
}
...
async function tempTaskSave(method,ccode) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'组装拆卸单',method:method,recordNum:ccode,caozuoModule:'stock'})
    .then((a)=>{
...
const reloadPageMX = async () => {
  loadMark.value = true
  let list=await useRouteApi(findByStockAdMXTableList,{schemaName: dynamicTenantId})(popData.value)
...
const calculateTotalMX = (data) => {
  ...
  summaryTotals.value={
    deshMoney: parseFloat(deshMoney)==0?'':toThousandFilter(deshMoney),
    sjshMoney: parseFloat(sjshMoney)==0?'':toThousandFilter(sjshMoney),
    icostTotal: toThousandFilter(icostTotal),
    fyftIsum: toThousandFilter(fyftIsum),
