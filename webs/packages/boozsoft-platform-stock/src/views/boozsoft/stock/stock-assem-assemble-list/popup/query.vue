    ...
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;组装拆卸单列表</span>
        </div>
...
const dynamicAdReload = async (obj) =>{
  ...
  let q = ['bom']
  let map = (await useRouteApi(findStockCaiGouList, {schemaName: obj.accountMode})([...new Set(q)].join(',')))
  stockList.value=map['bom']
...
function handleOk() {
  ...
  // if(hasBlank(strNum.value) && hasBlank(endNum.value)){
  //   return message.error('起始单号与截止单号不能为空')
  // }
