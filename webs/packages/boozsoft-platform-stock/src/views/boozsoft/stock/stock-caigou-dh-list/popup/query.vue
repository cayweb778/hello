    ...
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;采购到货单列表</span>
        </div>
                  ...
                  <li style="margin: 2% 0px;">
                    <div>
                      <a-radio-group style="width: 100%" v-model:value="radiovalue" @change="radiochange">
                        <a-radio value="1"><label style="text-align: left">业务日期：</label></a-radio>
                        <a-select
...
function handleOk() {
  ...
  let jssupName=jssupList.value.filter(a=>a['uniqueCode']==jssup.value)[0]?.custName
  let stockName=stockList.value.filter(a=>a['stockNum']==stock.value)[0]?.stockNum
  let log:any=['会计期间:【'+strTime+','+endTime+'】','单据编码:【'+strNum.value+','+endNum.value+'】','业务类型:【'+dataTypeName+'】','供应商:【'+supName+'】','结算供应商:【'+jssupName+'】','存货:【'+stockName+'】']
