      ...
      <div class="acb-centent">
        ...
        >
          ...
          <template #price="{ record }">
            <span class="a-table-font-arial">{{
                (record.price == null || record.price == 0 ? '' : parseFloat(record.price).toFixed(2) + '').replace(/\B(?=(\d{3})+(?!\d))/g, ',')
              }}
             </span>
          </template>
          <template #icost="{ record }">
           <span class="a-table-font-arial">{{
               (record.icost == null || record.icost == 0 ? '' : parseFloat(record.icost).toFixed(2) + '').replace(/\B(?=(\d{3})+(?!\d))/g, ',')
             }}
           </span>
          </template>
          <!-- 税率-->
...
async function findBySearch(val) {
  loadMark.value = true
  let bySearchData=await useRouteApi(findByWarSearch, {schemaName: dynamicTenantId})({billStyle: 'CGFP',ccode:val})
      ...
      onOk: async () => {
        ...
        if(canzhao.value){
          for (let i = 0; i < dataList.length; i++) {
            // 删除下游单据
            await useRouteApi(delXyCsourceByxyCcodeAndxyBillTypeAndBillTypeAndCcode, {schemaName: dynamicTenantId})({xyccode:formItems.value.ccode,xybillStyle:'CGFP',billStyle:dataList[i].sourcetype,ccode:dataList[i].sourcecode})
            // 上游单据明细
            let sourceData=await useRouteApi(findByStockWarehLinecode, {schemaName: dynamicTenantId})(dataList[i].sourcedetailId)
            // 修改累计入库数量
            sourceData.isumFapiao=parseFloat(sourceData.isumFapiao)-parseFloat(dataList[i].baseQuantity)
            sourceData.isumFapiaoMoney=parseFloat(sourceData.isumFapiaoMoney)-parseFloat(dataList[i].isum)
  ...
  if (saveCheck(list)) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if(!hasBlank(o.sourcedetailId)){
        ...
        if(canzhao.value&&status.value==1){
          sourceData.isumFapiao=hasBlank(sourceData.isumFapiao)?o.baseQuantity:parseFloat(sourceData.isumFapiao)+(parseFloat(o.baseQuantity))
          sourceData.isumFapiaoMoney=hasBlank(sourceData.isumFapiaoMoney)?o.isum:parseFloat(sourceData.isumFapiaoMoney)+(parseFloat(o.isum))
        }else{
          // 一元二次方程
          sourceData.isumFapiao=parseFloat(hasBlank(sourceData.isumFapiao)?0:sourceData.isumFapiao)+(parseFloat(o.baseQuantity)-parseFloat(o.oldBaseQuantity))
          sourceData.isumFapiaoMoney=parseFloat(hasBlank(sourceData.isumFapiaoMoney)?0:sourceData.isumFapiaoMoney)+(parseFloat(o.isum)-parseFloat(o.oldIsum))
        }
      ...
      if(rowDelData.value.length>0){
        rowDelData.value.forEach(async (a)=>{
          ...
          if(canzhao.value){
            ...
            sourceData.isumFapiao=parseFloat(hasBlank(sourceData.isumFapiao)?0:sourceData.isumFapiao)-parseFloat(baseQuantity)
            sourceData.isumFapiaoMoney=parseFloat(hasBlank(sourceData.isumFapiaoMoney)?0:sourceData.isumFapiaoMoney)-parseFloat(isum)
            await useRouteApi(stockWarehListSave, {schemaName: dynamicTenantId})([sourceData])
          }
        })
      }
    ...
    if(canzhao.value){
      ...
      Array.from(new Set(symxList.map(s=>s.ccode))).forEach(async (main)=>{
        ...
        // 添加下游表
        if(canzhao.value&&status.value==1){
          let xy={iyear:dynamicYear.value,billStyle:syMain.billStyle,ccode:main,ccodeDate:syMain.ddate,xyBillStyle:'CGFP',xyccode:formItems.value.ccode,xyccodeDate:formItems.value.ddate}
  ...
  switch (c) {
    ...
    case 'taxprice':// 含税单价
      if (!hasBlank(r.cnumber)) {
        ...
        r.price =r.tempNine
        // 税额=价税合计-无税金额
        r.itaxprice=parseFloat(String(r.isum-r.icost)).toFixed(4)
      }
      break;
    case 'isum':// 价税合计
...
const focusNext =  async (r, c) => {
  ...
  // 查找下一个
  let list = getDataSource();
  let filters:any = ['bcheck1','isGive','cinvodeType','cinvodeName','cunitid','cunitidF1','cunitidF2','itaxprice','baseQuantity','subQuantity1', 'subQuantity2','price','icost']
  // 要求填批号才填写
...
const copyFun = async () => {
  ...
  formFuns.value.setFormValue({
    ...
    billCode: formItems.value.billCode,
    billNumber: formItems.value.billNumber,
    billDate: formItems.value.billDate,
    rate: formItems.value.rate,
    cwhcode: formItems.value.cwhcode,
    cdepcode: formItems.value.cdepcode,
    cvencodeAddr: formItems.value.cvencodeAddr,
    cpersoncode: formItems.value.cpersoncode,
    cvencodePhone: formItems.value.cvencodePhone,
