  ...
  <BasicModal width="950px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">采购{{ pageParameter.type=='CGDHD'?'到货单':'订单'}}生单参照</span>
      </div>
    </template>
...
async function findByTable(){
  ...
  data=data.filter(a=>a.swsIsumRuku!=0&&a.sourcetype!='CGRKD')
  // 去除：到货来源单据是否入库单
  // if(pageParameter.type=='CGDHD'){
  //   data=data.filter(a=>hasBlank(a.sourcetype)||a.sourcetype!=='CGRKD')
  // }
  let data2:any=[]
  for (let i = 0; i < data.length; i++) {
    if(pageParameter.type=='CGDD'){
      ...
      console.log('入库单蓝字操作：5--->判断订单'+data[i].ccode+'下游表是否有到货单-->'+existCGDHD.length)
      if(existCGDHD.length==0){
        data2.push(data[i])
      }
    }else if(pageParameter.type=='CGDHD'){
      data2.push(data[i])
    }
  }
  let ddata:any=data2
...
async function handleOk() {
  let methodName=pageParameter.type=='CGDD'?'采购订单':pageParameter.type=='CGDHD'?'采购到货单':'采购发票'
