      ...
      <div>
        <div>
          ...
          <span v-else>
            ...
            <Popover placement="bottom">
              <template #content>
                <Button class="actod-btn" @click="referData('CGDHD')">采购到货单</Button>
              </template>
    ...
    <div class="app-container-bottom" :style="{height: (windowHeight+70)+'px'}">
      <div class="acb-head">
        <div class="acbgead-one">
          ...
          <span style="font-size: 22px;font-weight: bold;margin-right: 300px;" :style="{color: '#c0392b'}">{{ titleContents[titleValue] }}</span>
        </div>
        <div class="acbgead-two" :style="status == 3?{ pointerEvents: 'none'}:{}">
          <DynamicForm :datasource="dynamicFormModel" :formDataFun="formFuns" :accId="dynamicTenantId" @open="openHeadSelectContent" :canzhao="canzhao" :dynamicTenant="dynamicTenant"/>
        </div>
      </div>
      <div class="acb-centent">
        ...
        >
          ...
          <template v-if="record.editCgUnitId&&!canzhao">
            <Select
              ...
              @change="cgUnitIdChange(record);verifyRowXCLData(record,'keyong')"
            >
          ...
          <template #itaxprice="{ record }">
            <template v-if="status==2&&record?.editItaxprice&&(hasBlank(stockWareData.hxIsum)||parseFloat(stockWareData.hxIsum)==0)">
          ...
          <template #batchId="{ record }">
            <template v-if="record?.editTwelve/* && record.isBatch*/">
              <Input v-model:value="record.tempTwelve" readonly
...
import {GenerateDynamicColumn} from "/@/views/boozsoft/stock/stock-caigou-th/component/DynamicForm";
...
import {
  ...
  findBillByCondition,
  findBillCode,
  findByCodeSetXyCodeBdocumStyle,
  findByStockCurrLock,
...
import {
  ...
  saveZTRK_XCL,
  subtractZTRKSum, verifyStockRowXCL, verifyStockXCLList
} from "/@/api/record/stock/stock-currents";
import {
  findByStockWarehLinecode, getPYRKDAndNoBcheck1,
  getUnitRate,
  stockWarehListSave
...
const titleContents = ['采购退货单', '采购退货单', '采购回冲单']
...
const pageReload = async () => {
  ...
  if(routeData.type=='add'&&!hasBlank(routeData.ccode)){
    ...
    canzhao.value=!hasBlank(tempRouteData)
    status.value = 1
    stockWareData.value=''
    // 获取最新
    formItems.value = {
      ...
      invoiceStyle: 'zyfp',
      cvencode: tempRouteData.cvencode,
      cvencodeJs: tempRouteData.cvencodeJs,
      cwhcode: tempRouteData.cwhcode,
...
const startEdit = async (type) => {
  ...
  console.log('退货单判断：---是否有月末结账任务--'+JSON.stringify(jzMethod))
  ...
  console.log('退货单判断：---当前业务日期期间是否结账--'+JSON.stringify(temp))
  if(temp>0){
    status.value = 3
...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购退货单',method:method,recordNum:formItems.value.ccode,caozuoModule:'stock'})
    .then((a)=>{
      taskInfo.value=a
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    createConfirm({
      iconType: 'warning',
      title: '采购退货单删除',
      content: '您确定要进行采购退货单删除吗!',
      onOk: async () => {
        let sourcecode = Array.from(new Set(getDataSource().filter(it => !hasBlank(it.cwhcode)).map(a=>a.sourcecode)))
        // 判断是否 是上游生成红字
        let countRedXy=await useRouteApi(findByCodeSetXyCodeBdocumStyle, {schemaName: dynamicTenantId})({syCode:sourcecode[0],billStyle:'CGDHD'})
        if(countRedXy>0){
          for (let i = 0; i < dataList.length; i++) {
            if(!hasBlank(dataList[i].sourcedetailId)){
              // 上游单据子表数据
              let sourceData=await useRouteApi(findByStockWarehLinecode, {schemaName: dynamicTenantId})(dataList[i].sourcedetailId)
              sourceData.isumTuiHuo=parseFloat(sourceData.isumTuiHuo)-parseFloat(Math.abs(dataList[i].baseQuantity))
...
const startReview = async (b) => {
  ...
  if(!b){
    ...
    // 到货单审核生成入库单
    if(dynamicTenant.value.target.cgShDhd=='1'){
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id))  {
    ...
    if(b&&dynamicTenant.value.target.cgShDhd=='1'){
      createConfirm({
        ...
        cancelText:'放弃',
        okText:'生成',
        content: '单据审核完成，是否继续生成对应红字采购入库单?',
        onOk: async () => {
          // 入库单子表-修改本单据的 累计入库数量
          tableList.forEach(async (temp)=>{
            ...
            // 修改本单据的 累计入库数量
            temp.isumRuku=hasBlank(temp.baseQuantity)?'0':temp.baseQuantity
            temp.bcheck=bcheck
  ...
  if (saveCheck(list)) {
    for (let i = 0; i < list.length; i++) {
      ...
      if(!hasBlank(o.sourcedetailId)){
        ...
        // 排除期初到货单,期初到货单不修改累计入库数量
        if(sourceData.billStyle!=='QC'){
          if(canzhao.value&&status.value==1){
            sourceData.isumTuiHuo=hasBlank(sourceData.isumTuiHuo)?Math.abs(o.baseQuantity):parseFloat(sourceData.isumTuiHuo)+(parseFloat(hasBlank(o.baseQuantity)?0:Math.abs(o.baseQuantity)))
          }else{
            // 一元二次方程
            sourceData.isumTuiHuo=parseFloat(hasBlank(sourceData.isumTuiHuo)?0:sourceData.isumTuiHuo)+(parseFloat(hasBlank(o.baseQuantity)?0:Math.abs(o.baseQuantity))-parseFloat(hasBlank(o.oldBaseQuantity)?0:Math.abs(o.oldBaseQuantity)))
          }
          symxList.push(sourceData)
        }
      }
    ...
    if(symxList.length>0){
      ...
      Array.from(new Set(symxList.map(s=>s.ccode))).forEach(async (main)=>{
        // 参照并保存&添加下游表
        if(canzhao.value&&status.value==1){
          let xy={iyear:dynamicYear.value,billStyle:'CGDHD',ccode:main,ccodeDate:Array.from(new Set(symxList)).filter(a=>a.ccode==main)[0]?.ddate,xyBillStyle:'CGDHD',xyccode:formItems.value.ccode,xyccodeDate:formItems.value.ddate}
          await useRouteApi(xyCsourceSave, {schemaName: dynamicTenantId})(xy)
        }
      })
    }
...
const tableDataChange =  async (r,c) => {
  ...
  switch (c) {
    case 'cnumber':
      ...
      // 参照并且是新增,数量不能大于参照数量
      if(canzhao.value&&status.value==1&&parseFloat(r.baseQuantity)>r.canzhaoCnumber){
        r.cnumber=null
        r.tempCnumber=r.cnumber
       return message.error('数量必须小于等于参照单据数量!')
      }else if(canzhao.value&&status.value==2){
       // 变大
        if(parseFloat(r.baseQuantity)>parseFloat(r.oldBaseQuantity)){
          // 校验可用量
          verifyRowXCLData(r,'keyong')
       }
      }
      break;
    case 'dvdate':
    ...
    case 'taxprice':// 含税单价
      if (!hasBlank(r.cnumber)) {
        ...
        r.isum =isum>0? 0 - (Math.abs(isum)):isum
        r.tempIsum = r.isum
        ...
        r.price =r.tempNine
        // 税额
        let itaxprice:any=parseFloat(r.isum)-(parseFloat(r.icost))
        r.itaxprice=parseFloat(String(itaxprice)).toFixed(4)
      }
      break;
    case 'isum':// 价税合计
      if (!hasBlank(r.cnumber)) { //反算 单价
        let n:any = (parseFloat(r.cnumber)*-1).toFixed(10)
        let d:any = hasBlank(r.isum)?0:parseFloat(r.isum).toFixed(4)
        if (titleValue.value != 0 && d > 0) d = 0 - (Math.abs(d))
        r.taxprice = Math.abs(parseFloat(String(d/n))).toFixed(10) + ''
        ...
        r.icost =r.tempTen
        // 无税单价
        r.tempNine = Math.abs(parseFloat(String(r.icost/n))).toFixed(10)
        r.price =r.tempNine
        // 税额
        let itaxprice:any=parseFloat(r.isum)-(parseFloat(r.icost))
        r.itaxprice=parseFloat(String(itaxprice)).toFixed(4)
...
// 按行校验存货现存量
async function verifyRowXCLData(r,queryType) {
...
const chChange = async (record) => {
  ...
  // 校验现存量
  verifyRowXCLData(record,'keyong')
...
const focusNext =  async (r, c) => {
  ...
  findBySupPrice(r)
    // 查找下一个
    let list = getDataSource();
  let filters:any = ['bcheck1','isGive','cinvodeType','cinvodeName','cunitid','cunitidF1','cunitidF2','baseQuantity','subQuantity1', 'subQuantity2','price','icost','showNumber','dpdate','dvdate']
...
const getNextMark = (c,b) => {
  let model = {cnumber:'Cnumber',cgUnitId:'CgUnitId',cwhcode:'One',cinvode:'Two',price:'Nine',icost:'Ten',taxprice:'Taxprice',itaxrate:'Itaxrate',isum:'Isum',batchId:'Twelve',cmemo:'Thirteen',itaxprice:'Itaxprice'}
  if (b){
    // 获取下一个
...
async function findBySupPrice(data) {
  ...
  if(supPrice!==''){
    // 供应商价格表含税（默认值1是，0或空否）
    if(dynamicTenant.value.target.cgShDhd.cgPriceIsRate=='1'){
...
async function saveLogData(optAction) {
  let logmap={
    ...
    optAction:optAction,
    accId:dynamicAccId.value,
    optContent:'操作内容【采购退货单】,单据编号【'+formItems.value?.ccode+'】',
  }
