  ...
  <BasicModal width="950px"
    ...
    <template #title>
      <div style="display: flex;margin-left: 10px;margin-top: 5px;" class="vben-basic-title">
        ...
        <span style="line-height: 29px;font-size: 20px;margin-left: 10px;color: rgb(21, 142, 184)">采购退货单参照</span>
      </div>
    </template>
...
async function findByTable(){
  ...
  // swsIsumDaohuo 是 数量 减 累计到货单数量 不等于0
  data=data.filter(a=>a.swsIsumDaohuo!=0)
  let data2:any=[]
  for (let i = 0; i < data.length; i++) {
    if(pageParameter.type=='CGDD'){
      ...
      console.log('退货单操作：--->判断'+data[i].ccode+'下游表是否有入库单-->'+existCGDHD.length)
...
const onSelectChange = async (k, o) => {
  for (let i = 0; i < o.length; i++) {
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: database })({iyear:iyear.value,name:'采购到货单',method:'弃审',recordNum:o[i].ccode})
  ...
  if(tempObj.length>0){
    ...
    stockWarehList.forEach(async (list)=>{
      ...
      let isumDaohuo=hasBlank(list.isumTuiHuo)?0:Math.abs(parseFloat(list.isumTuiHuo)) // 转成正数
...
async function handleOk() {
  ...
  let taskList:any=[]
  for (let i = 0; i < table1.length; i++) {
    let task= await useRouteApi(stockBalanceTaskSave, { schemaName: database })({iyear:iyear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:methodName,method:'变更',recordNum:table1[i].ccode,caozuoModule:'stock'})
    taskList.push(task)
    console.log('退货单操作：6--->参照单据添加任务表-->'+JSON.stringify(task))
  }
  ...
  list.forEach(async (a)=>{
    ...
    a.newBaseQuantity=a.newBaseQuantity*-1
