<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">出库调整单</b></div>
      ...
      <div>
        <div> 
          ...
          <button
            type="button"
            class="ant-btn ant-btn-me"
            @click="router.push({path: '/cb-out-depot',query: {type:'add',ccode:''}})"
          ><span>新增</span></button>
          <button
            type="button"
            class="ant-btn ant-btn-me"
...
async function saveQuery(data) {
  ...
  let logmap={
    ...
    optContent:'操作内容【出库调整单】,账套代码【'+data.coCode+'】,账套名称【'+data.companyName+'】 下条件为：' +
...
async function delList() {
  if (checkRow.value.length > 0) {
    ...
    // 任务
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:iyear.value,name:'出库调整单',method:'修改,审核,删除',recordNum:formItems.value.ccode})
...
async function saveLogData(optAction,ccode) {
  let logmap={
    ...
    optAction:optAction,
    accId:dynamicTenantId.value,
    optContent:'操作内容【出库调整单】,单据编号【'+ccode+'】',
  }
  await saveLog(logmap)
}
</script>
