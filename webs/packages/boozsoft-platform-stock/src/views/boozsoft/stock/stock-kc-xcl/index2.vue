...
import {kcXCLStore} from "/@/views/boozsoft/stock/stock-kc-xcl/generalLedger";
...
const dynamicAdReload = async (obj) => {
  // 业务参数
  let ywcs = await useRouteApi(findOption, {schemaName: obj.accountMode})({
    type: '2',
    code: obj.coCode
  })
  cgNumWs.value = hasBlank(ywcs.basisMap.cgNumWs) ? '2' : ywcs.basisMap.cgNumWs
...
function toThousandFilter(num: any) {
  ...
  return (+num || 0).toFixed(cgNumWs.value).replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, '$&,')
}
...
function excelData() {
  ...
  exportExcel(sheet, 'xlsx', '存货现存量查询_' + pageParameter.companyName)
}
</script>
