  ...
  <div class="app-container">
    ...
    <div class="app-container">
      ...
      >
        ...
        <template #bq="{ record }">{{toThousandFilter(record.bq)}}</template>
        <template #price="{ record }">{{toThousandFilter(record.price)}}</template>
        <template #icost="{ record }">{{toThousandFilter(record.icost)}}</template>
        <template #bq1="{ record }">{{toThousandFilter(record.bq1)}}</template>
        <template #price1="{ record }">{{toThousandFilter(record.price1)}}</template>
        <template #icost1="{ record }">{{toThousandFilter(record.icost1)}}</template>
        <template #bq2="{ record }">{{toThousandFilter(record.bq2)}}</template>
        <template #price2="{ record }">{{toThousandFilter(record.price2)}}</template>
...
import {chpctzStore} from "/@/api/record/stock/generalLedger-pckctz";
...
const openQuery = async () => {
 /* await useRouteApi(getCkPrice,{schemaName: 'jmgyl-001-2022'})({
    stockNum :'20751',
    stockCangku :'1538428925863137280',
    rkBcheck:'0',
    ddate :'2022-08-29',
    year :'2022',
  })*/
...
async function saveQuery(data) {
  ...
  pageParameter.pcNum = data.pcNum
  ...
  useRouteApi(findOption,{schemaName: databaseCode})({type: '3',code: databaseCode.value}).then(res=>{
    console.log(res.basisMap.kcNumWs)
...
const excelData = () => {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '库存批次台账_'+pageParameter.coCode+'_'+pageParameter.companyName)
}
