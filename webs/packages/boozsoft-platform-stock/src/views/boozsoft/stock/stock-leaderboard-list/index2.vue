    ...
    <div class="app-container lcr-theme-div">
      <div>
        ...
        <div>
          ...
          <div>
             <span style="color: black;margin-left: 1em;">排序方式：</span>
            <Select v-model:value="pageParameter.sortMark" style="width: 150px;text-align-last: center;"  @change="selectChange">
              <SelectOption value="">默认</SelectOption>
              <SelectOption value="1">主数量</SelectOption>
              <SelectOption value="2">价税合计</SelectOption>
            </Select>
    ...
    <div class="app-container">
        ...
        >
          ...
          <template #isumFapiao="{ record }"><span class="tableUStyle" @click="openNewPage(record,'fp')">{{ toThousandFilter(record.isumFapiao) }}</span></template>
            ...
            <template #cunitid="{ record }">{{ formatCunitid(record.cinvode,record.xsUnitId) }}</template>
...
const assembleTotal = (list) => {
  ...
  for (let o of list) {
    ...
    ljtk += parseFloat(o.isumJijian || '0')
