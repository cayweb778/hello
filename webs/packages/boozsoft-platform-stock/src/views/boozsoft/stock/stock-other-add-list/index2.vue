<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">其他入库单列表</b></div>
      ...
      <div>
        <div>
          ...
          <button
            type="button"
            class="ant-btn ant-btn-me"
            @click="router.push({name: 'CaigouQTRk',params: {type:'add',ccode:''}})"
          ><span>新增</span></button>
          <button
            type="button"
            class="ant-btn ant-btn-me"
    ...
    <div class="app-container">
      ...
      >
        ...
        <template #feiyongMoney="{ record }">
          {{toThousandFilter(record.feiyongMoney)}}
        </template>
        <template #summary>
          <TableSummary fixed>
            <TableSummaryRow style="background-color: #cccccc;">
...
const CrudApi1 = {
  columns: [
    ...
    {
      title: '单据类型',
      dataIndex: 'unitTransName',
      width: 150,
      align: 'left',
      ellipsis: true,
    },{
      title: '业务单位',
      dataIndex: 'unitValueName',
      width: 150,
      align: 'left',
      ellipsis: true,
    },{
...
const excelThisData = () =>{
  ...
  arrData.filter(a=>a.bcheck!=='11').forEach((a,index)=>{
    ...
    a.cnumber=toThousandFilter(a.isum)
    a.feiyongMoney=a.feiyongMoney==null?'':toThousandFilter(a.feiyongMoney)
  ...
  exportExcel3(sheet, 'xlsx','其他入库单列表_'+pageParameter.companyName)
}
...
async function tempTaskSave(method,ccode) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'其他入库单',method:method,recordNum:ccode,caozuoModule:'stock'})
    .then((a)=>{
...
const reloadPageMX = async () => {
  ...
  for (let i = 0; i < list.length; i++) {
    list[i].cwhcode=await getCKName(list[i].cwhcode)
  }
