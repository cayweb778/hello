  ...
  <div>
    <div class="app-container lcr-theme-div">
      <div>
        ...
        <div> <AccountPicker theme="three" readonly style="background-color: #f5f5f5;" @reloadTable="dynamicAdReload"/>
        </div>
      </div>
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">调拨单列表</b></div>
      ...
      <div>
        ...
        <div>
          <div>
            <Select v-model:value="formItems.selectType" style="width: 150px;font-size: 12px;font-weight: bold;border-radius: 4px;text-align: left;" class="special_select">
          ...
          <div>
            <Button class="ant-btn-me">
              <SyncOutlined :style="{ fontSize: '14px' }" @click="tablesData1()"/>
            </Button>
...
const toAdd = async () => {
  ...
  compState.loading = false
  router.push({path: '/kc-transfer',query: {type:'add',ccode:''}})
}
...
async function saveQuery(data) {
  ...
  let logmap={
    ...
    optContent:'操作内容【调拨单】,账套代码【'+data.coCode+'】,账套名称【'+data.companyName+'】 下条件为：' +
...
function toRouter(data,type) {
  if(type=='list'){
    router.push({path: '/kc-transfer',query: {ccode:data.ccode}});
  }
}
...
const startReview = async (b) => {
  ...
  if(state.selectedRowKeys.length==0){
    loadMark.value=false
    message.error("请选择数据进行审核！")
    return false
  }
  let data=getDataSource1().filter(g=>g.id!=null&&state.selectedRowKeys.indexOf(g.key)!=-1)
  let bcheck=data.filter(f=>f.bcheck=='1');
  let bcheck1=data.filter(f=>f.bcheck=='0');
  if(b&&bcheck.length>0){
    loadMark.value=false
    message.error("包含已审核单据,请检查！")
    return false
  }
  if(!b&&bcheck1.length>0){
    loadMark.value=false
    message.error("包含未审核单据,请检查！")
    return false
  }
  // 有无 整理现存量 任务
  ...
  if(b){
    for (let i = 0; i < data.length; i++) {
      ...
      let res = await useRouteApi(findBillByCondition, {schemaName: dynamicTenantId})({
        type: "CGRKD",
        iyear: pageParameter.iyear || '2022',
        action: 'tail',
        ccode: data[i].ccode,
        bdocumStyle: '',
        curr: ''
      ...
      await useRouteApi(reviewSetCGRKGMx, {schemaName: dynamicTenantId})(listmx)

      // 默认审核修改现存量
      let map={list:JSON.stringify(listmx),status:'1',kcCgrkCheck:"1",rkdCheck:'1'}
      await useRouteApi(saveStockCurrentstockZTRK_Increase, {schemaName: dynamicTenantId})(map)
    }
  }
  else{
    for (let i = 0; i < data.length; i++){
      let rukuMainData=await useRouteApi(verifyXyCsourceByXyCode, {schemaName: dynamicTenantId})({year:data[i].iyear,code:data[i].ccode,xyCode:'CGRKD'})
      if(rukuMainData!=='' ){
        loadMark.value=false
        message.error(data[i].ccode+' 已经生成采购入库单【编码'+rukuMainData.xyccode+'】,不能弃审！')
        return false;
      }
    }
    for (let i = 0; i < data.length; i++) {
      ...
      // 默认审核修改现存量
      let map={list:JSON.stringify(listmx),status:'1',kcCgrkCheck:"1",rkdCheck:'1'}
      await useRouteApi(saveStockCurrentstockZTRK_Edit_XCL, {schemaName: dynamicTenantId})(map)
    }
  }
