<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">销货单列表</b></div>
...
<!--              <span v-show="typeFlag=='0'">
            ...
            <Popover class="ant-btn-default" placement="bottom">
              ...
              <span class="group-btn-span-special2" @click="pageParameter.documentType='';reloadTable()" :style="pageParameter.documentType==''?{backgroundColor: '#0096c7',color: 'white'}:''" style="width: 130px;">
                ...
                销货单&emsp;&emsp;&emsp;<CheckOutlined v-if="pageParameter.documentType=='0'"/>
              </span><br/>
...
<!--                <span class="group-btn-span-special2 p_specifics" style="width: 120px;text-align: left;padding: 5px 10%;">
    ...
    <div class="app-container">
        ...
        >
          ...
          <template #cunitid="{ record }">{{ formatCunitid(record.cinvode,record.cunitid) }}</template>
...
async function saveQuery(e) {
  ...
  reloadList(e.map)
  initTable()
  // console.log(testgetPaginationRef())
  // await reloadTable()
}
const initTable = ()=>{
  visible.value = true
  setTimeout(()=>{
      ...
      lanMuData1.value.changeNumber+=1
      visible.value = false
    }
    ,100)
}
...
/*    if (title || title === '') {
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '销货单列表_'+companyName.value+'_'+qijianText.value)
}
...
const dynamicColumnModel1 = ref({value: []});
  ...
  createConfirm({
    ...
    onOk: async () => {
      ...
      if (lanMuData.value.objects == '[]') {
        ...
        saveStockLanMuList(lanMuData.value).then(res => {
...
const confirm1 = (e: MouseEvent) => {
  ...
  createConfirm({
    ...
    onOk: async () => {
      // 调整数据库 列参数
      lanMuData1.value.accId = getCurrentAccountName(false)
      lanMuData1.value.objects = JSON.stringify(filterModifyData(dynamicColumnData1.value, dynamicColumnDataCopy1))
      if (lanMuData1.value.objects == '[]') {
        createWarningModal({content: '请先做修改后再进行确认同步数据库！'})
      } else {
        saveStockLanMuList(lanMuData1.value).then(res => {
          message.success("数据库同步成功！")
        })
        // 重新赋值
...
function resetDynamicColumnData1() {
  // 先从数据查询是否已经设置
  lanMuData1.value.accId = getCurrentAccountName(false)
  findStockDbLanMuList(lanMuData1.value).then(res => {
    // 栏目列
    let dbList = res.items
    if (dbList.length > 0) {
...
const startReview = async (b) => {
    ...
    if (!hasBlank(a)) {
        if (checkRow.value.length == 0) {
          ...
          let taskData= await useRouteApi(getByStockBalanceBatchTask, { schemaName: dynamicTenantId })({iyear: strDate.value.substring(0,4),name:'销货单',method:'修改,审核,弃审,变更,删除',recordNum:codes.join()})
          let taskData2= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear: strDate.value.substring(0,4),name:'应收自动核销',method:'核销'})
          if(taskData=='' && taskData2 == ''){
            tempTaskSave(b?'审核':'弃审',codes)
          }else{
            // 任务不是当前操作员的
          ...
          if (!b){ // 弃审前 检查
            let che = await useRouteApi(unAuditBefore, {schemaName: dynamicTenantId})({
              codes: codes,id: dynamicTenant.value.target.id, style: 'XHD'
            })
            ...
            let taskData2= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:strDate.value.substring(0,4),name:'盘点单',method:'盘点',recordNum: [...new Set(checkRow.value.map(it => it.cwhcode1))].join()})
            if(taskData2!='') {
              tempTaskDel()
...
async function checkPeriod(){
  let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:strDate.value.substring(0,4),name:'月末结账',method:'结账',recordNum:''})
...
async function availability(mode,list){
  ...
  return (await useRouteApi(findAvailability, {schemaName: dynamicTenantId})({year:strDate.value.substring(0,4),mode: (mode || 'all'),checks: JsonTool.json(checks),parameters: JsonTool.json(parameters)})) || []
}
async function stockXclCheck(list) {
  ...
  try {
    ...
    for (let s of setParm) {
      let parms = s.split('=')
      let tStock =  stockList.value.filter(it=>it.stockNum == parms[1])[0]
...
function codeToName(arr) {
  let names = {'XSCKD':'销售出库单','XSFP':'销售发票','XHD':'销售退货单'}
...
async function tempTaskSave(method,ccode) {
  await useRouteApi(stockBalanceTaskBatchSave, { schemaName: dynamicTenantId })({iyear:strDate.value.substring(0,4),userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销货单',method:method,recordNum:ccode.join(),caozuoModule:'stock'})
    .then((a)=>{
