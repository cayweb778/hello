...
import {LinkOutlined, PlusCircleOutlined,CaretDownOutlined,AppstoreOutlined} from '@ant-design/icons-vue'
...
import {getStockBalanceMaxIineId, stockBalanceSave} from "/@/api/record/stock/stock_balance";
...
const ASelectOption = ASelect.Option;
  ...
  const dataReadonly1 = ref(true);
  const dataReadonly2 = ref(true);
...
const unitName:any = ref('');
  ...
  const [register, { closeModal }] = useModalInner( async (data) => {
    unitName.value=''
    if(data.cangkuFlag=='1'){
      cangku.cangkuName=data.stockCangkuId
    }else{
      cangku.cangkuName=''
    }
    ...
    stockCangkuId.value=data.stockCangkuId
    formItems.value={}
  ...
  async function handleOk(type) {
    ...
    formItems.value.iineId=await useRouteApi(getStockBalanceMaxIineId, {schemaName: database})('')
    formItems.value.iyear=iyear.value
    formItems.value.cangkuDuli=cangkuFlag.value
    formItems.value.stockCangkuId=stockCangkuId.value
    formItems.value.stockCangkuRecordId=cangku.id
    formItems.value.cmaker=useUserStoreWidthOut().getUserInfo.id

    await useRouteApi(stockBalanceSave, {schemaName: database})(formItems.value)
    if(type=='0'){
      saveLogData('新增')
      emit('save', '');
      closeModal();
  ...
  async function throwStockInfoData(data) {
    ...
    formItems.value.stockId=data[0].stockNum
  ...
  function countPrimaryNumber(type) {
    ...
    if(type=='price'){
      ...
      // formItems.value.money4=money==0?'':money.toFixed(4)
    }
    if(type=='money'){
      formItems.value.price=''
...
async function handleChange(val,b) {
  if(hasBlank(val)){
    formItems.value.primaryUnit=null
    formItems.value.secondaryUnit1=null
    formItems.value.secondaryUnit2=null
...
function stockCangKuModalShow() {
  openStockCangKuModalPage(true,{
    database:database.value,
    stockCangkuId:stockCangkuId.value
  })
}
