<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">期初到货单列表</b></div>
        <div><span style="font-size: 14px;font-weight: bold;color: #666666">业务年度：{{ popData.stockyear }}</span></div>
      </div>
      <div>
        <div>
          <button
          ...
          <button
            type="button"
            class="ant-btn ant-btn-me"
            @click="router.push({name: 'StockBalanceDH',params: {type:'add',ccode:''}})"
          ><span>新增</span></button>
          <button
            type="button"
            class="ant-btn ant-btn-me"
...
async function tablesData1(){
  ...
  tableDataAll1.value = await useRouteApi(findByQCDHDList,{schemaName: dynamicTenantId})(popData.value)
...
const excelThisData = () =>{
  ...
  exportExcel3(sheet, 'xlsx','期初到货单列表_'+pageParameter.companyName)
}
...
async function toRouter(data,type) {
  ...
  await closeToFullPaths('/stock_balance_dh')
  setTimeout(()=>{
    router.push({path: 'stock_balance_dh',query: {type:'info',ccode:data.ccode,co: databaseCo.value}});
  },1000)
}
async function editFun() {
  ...
  for (let i = 0; i < data.length; i++) {
    // 任务
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:strDate.value.split('.')[0],name:'期初到货单',method:'修改,审核,删除',recordNum:data[i].ccode})
    if(!hasBlank(taskData)){
      for (let i = 0; i < taskData.length; i++) {
        // 任务不是当前操作员的
        if(taskData[i]?.caozuoUnique!==useUserStoreWidthOut().getUserInfo.id){
          return message.error(taskData[i]?.username+'正在'+taskData[i]?.method+'期初到货单,不能同时进行操作！')
        }
      }
    }
  }
  ...
  router.push({name: 'StockBalanceDH',params: {type:'edit',ccode:data[0].ccode}});
}
...
const startReview = async (b) => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    let mx=await useRouteApi(findAllByCcodeAndBillStyle, {schemaName: dynamicTenantId})({ccode:list[i].ccode,type:'QC'})
    mx.forEach(tx=>{
      tx.bcheck=bcheck
...
async function tempTaskSave(method,ccode) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'期初到货单',method:method,recordNum:ccode,caozuoModule:'stock'})
    .then((a)=>{
