<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">期初采购发票列表</b></div>
        <div><span style="font-size: 14px;font-weight: bold;color:#666666">日期：{{ strDate }} - {{ endDate }}</span></div>
      </div>
      <div>
        <div>
          <button
...
const excelThisData = () =>{
  ...
  exportExcel3(sheet, 'xlsx','采购发票列表_'+pageParameter.companyName)
}
...
async function toRouter(data,type) {
  ...
  await closeToFullPaths('/stock_balance_fp')
  setTimeout(()=>{
    router.push({path: 'stock_balance_fp',query: {type:'info',ccode:data.ccode,co: databaseCo.value}});
  },1000)
}
async function editFun() {
  ...
  for (let i = 0; i < data.length; i++) {
    // 任务
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:strDate.value.split('.')[0],name:'期初暂估入库单',method:'修改,审核,删除',recordNum:data[i].ccode})
  ...
  router.push({path: 'stock_balance_fp',query: {type:'edit',ccode:data[0].ccode}});
}
...
async function tempTaskSave(method,ccode) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'采购发票',method:method,recordNum:ccode,caozuoModule:'stock'})
    .then((a)=>{
