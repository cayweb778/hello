<template>
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">形态转换单</b></div>
...
const toAuditBack = async () => {
  ...
  if (checkRow.value.length == 1) {
    ...
    //弃审 4 校验下有单据是否存在审核
    let bcheck= await useRouteApi(auditCheckBcheck, { schemaName: dynamicTenantId })({ccode:checkRow.value[0].ccode})
    if(bcheck == true){
      compState.loading = false
