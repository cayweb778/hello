  ...
  <div>
    <div class="app-container lcr-theme-div">
      <div>
        ...
        <div> <AccountPicker theme="three" style="background-color: #f5f5f5;" readonly @reloadTable="dynamicAdReload"/></div>
      </div>
      <div>
        <div>  <b class="noneSpan" style="font-size: 24px;color: #0096c7;">形态转换单</b></div>
      ...
      <div>
        <div>
          ...
          <button
            type="button"
            class="ant-btn ant-btn-me"
            @click="router.push({path: '/kc-shape',query: {type:'add',ccode:''}})"
          ><span>新增</span></button>
          <button
            type="button"
            class="ant-btn ant-btn-me"
...
import {
  findAllMainList, audit, deleteSettModes, auditCheck,getDataInfo
...
import {findAll as findAllJiLang} from "/@/api/record/system/unit-mea";
...
async function saveQuery(data) {
  ...
  let logmap={
    ...
    optContent:'操作内容【形态转换单】,账套代码【'+data.coCode+'】,账套名称【'+data.companyName+'】 下条件为：' +
...
async function saveLogData(optAction,ccode) {
  let logmap={
    ...
    optAction:optAction,
    accId:dynamicTenantId.value,
    optContent:'操作内容【形态转换单】,单据编号【'+ccode+'】',
  }
  await saveLog(logmap)
}
</script>
