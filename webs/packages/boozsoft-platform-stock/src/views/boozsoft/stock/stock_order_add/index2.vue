...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售订单',method:method,recordNum:formItems.value.ccode,caozuoModule:'stock'})
    .then((a)=>{
  ...
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if(taskData==''){
      ...
      createConfirm({
        iconType: 'warning',
        title: '销售订单删除',
        content: '您确定要进行销售订单删除吗!',
        onOk: async () => {
          // 删除前校验
          if (formItems.value.chukuStatus == '1' || formItems.value.xiaohuoStatus  == '1') {
            createWarningModal({title: '温馨提示', content: `当前单据已经生成${formItems.value.chukuStatus == '1'?'销售出库单':'销货单'}不能进行删除操作！`})
            tempTaskDel()
          }else {
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    if ((b && !hasBlank(formItems.value.bcheckUser)) || (!b && hasBlank(formItems.value.bcheckUser))){
      ...
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'销售订单',method:'修改,审核,弃审,变更,删除',recordNum:formItems.value.ccode})
...
const tableDataChange =  (r,c) => {
  ...
  switch (c) {
    ...
    case 'quantity':
      ...
      if (status.value == 1 && r.defaultPrice != null){ // 当存在默认价格时
        let p = 0
        if (index == 1){
...
async function writeLog(action, a, content) {
  ...
  if (action == '修改') {
    ...
    let text = `操作内容【${action}销售订单】,` + '销售订单号【' + a.ccode + '】,单据日期【' + a.ddate + '】,'
  ...
  let map = {
    ...
    optContent: content || `操作内容【${action}销售订单】,` + '销售订单号【' + a.ccode + '】,单据日期【' + a.ddate + '】',
  }
  await saveLog(map)
}
/************ 日志 *************/
