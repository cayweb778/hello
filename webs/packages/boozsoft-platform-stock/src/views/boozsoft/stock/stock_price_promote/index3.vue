    ...
    <div class="app-container lcr-theme-div">
      <div>
        ...
        <div style="padding-left: 9%;">
          <span style="font-size: 14px;">活动名称：<span style="font-size: 15px;font-weight: 900">{{routemsg.activityName}}</span></span>
          <span style="font-size: 14px;padding-left: 5px;">活动范围：<span style="font-size: 15px;font-weight: 900">{{routemsg.activityScope=== '1' ? '全部客户' : '选定客户' }}</span></span>
          <span style="font-size: 14px;padding-left: 5px;">活动内容：<span style="font-size: 15px;font-weight: 900">{{routemsg.activityContent}}</span></span>
        </div>
      </div>
      <div>
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">客户促销价格表</b></div>
        <div class="act-one-d-title">
          <span style="font-size: 14px;">期间:<span style="font-weight: 900">{{routemsg.activityDate}}</span></span>
        </div>
      </div>
      <div>
        <div>
...
<!--          <Button type="button"
    ...
    <div class="bg-white" :style="{height: (windowHeight+53)+'px',display: 'inline',width: showCatalog?'300px':'0px',float: 'left',marginTop: '5px',overflow:showCatalog?'auto':'hidden',overflowY:'hidden'}" >
      <div style="width: 100%; height: 32px;background-color: #d9d9d9;padding-top:5px;font-weight: bold;text-align: center;">
        <label style="font-size: 14px;font-weight: bold;">客户名称</label> <ShrinkOutlined title="收起" class="exit-class" @click="showCatalog=!showCatalog"/>
        <DeleteTwoTone  v-if="routemsg.activityScope === '0'" style="float: right;padding: 6px;" @click="delCusts"/>
        <PlusCircleTwoTone v-if="routemsg.activityScope === '0'" style="float: right;padding: 6px;" @click="addCust"/>
        <SearchOutlined v-if="!showSearch && routemsg.activityScope === '0'" style="float: right;padding: 6px;color: #66b1ff;" @click="searchCust"/>
    ...
    <div class="app-container-bottom" :style="{height: (windowHeight+60)+'px',display: 'inline',width: '60%',float: 'left',marginTop: '5px',marginLeft: '5px'}">
      <BasicTable
        ref="tableRef"
...
<!--              <EditOutlined @click="record.stockNumEdit = true;record.oldstockNum=record.stockNum"/>-->
      ...
      <div class="pagination-text" style="position: absolute;top:97%; bottom: 6px; right: 25%;font-size: 13px; color: black;z-index: 99999999;">
      ...
      >
        ...
        <br/>
        <Button type="primary" style="float: right;" @click="filterSearch">
...
import {
  ...
  combineParameters,
  initDynamics
} from "/@/views/boozsoft/stock/stock_price_promote/data";
...
function exportexcel() {
  ...
  for (let i = 0; i < tableDataAll.value.length; i++) {
    if(tableDataAll.value[i].stockNum !=null) {     // 所有末级科目
      list.push({
  ...
  jsonToSheetXlsx({
    ...
    filename: '客户促销活动价格表_'+ routemsg.value.accName +'.xlsx',
  });
}
...
async function reloadProjects(data) {
  ...
  data.forEach(v=>{
    ...
    v.activityId = routemsg.value.activityId
  })
...
async function fetch() {
  const deptTree = await useRouteApi(getAllCustTree,{schemaName: databaseTrue})(routemsg.value.activityId)
  ...
  if(deptTree!=null){
    ...
    if(routemsg.value.activityScope === '1'){
      treeData.value.push({title: '全部',key:'0',children: []})
    }else{
...
const dbSave = async (o) => {
  ...
  o.activityId = routemsg.value.activityId
...
const add = (record, type) => {
  //弹出存货编码选择
  //如果不是全部 则不能选择全部添加
  if(routemsg.value.activityScope != '1' && selectedKeys2.value[0] === '0'){
...
.lcr-theme-div{
  ...
  >div:nth-of-type(1){
    ...
    >div:nth-of-type(2){
      width: calc( 100% - 346px);display: inline-block;
    }
    >div:nth-of-type(3){
