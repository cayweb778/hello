  ...
  <div>
    <div class="app-container lcr-theme-div">
      ...
      <div style="padding-top: 15px;">
        <div>  <b class="noneSpan" style="font-size: 26px;color: #0096c7;">采购价格表</b></div>
      </div>
      <div>
        <div>
          <Button type="button"
    ...
    <div class="bg-white" :style="{height: (windowHeight+60)+'px',marginLeft:'10px',display: 'inline',width: showCatalog?'280px':'20px',float: 'left',marginTop: '5px',overflow:showCatalog?'auto':'hidden',overflowY:'hidden'}" >
      <div style="width: 100%; height: 32px;background-color: #d9d9d9;padding-top:5px;font-weight: bold;text-align: center;">
        <label style="font-size: 14px;font-weight: bold;">供应商名称</label>
<!--        <DeleteTwoTone  style="float: right;padding: 6px;" @click="delCusts"/>
    ...
    <div class="app-container-bottom" :style="{height: (windowHeight+60)+'px',display: 'inline',width: showCatalog?'calc( 100% - 295px )':'calc( 100% - 22px )',float: 'right',marginTop: '5px'}">
      <BasicTable
        ref="tableRef"
...
import CustomerModalPop from "/@/views/boozsoft/global/popup/supplier/modalPop.vue";
...
import {
  findAll, savePrice, getAllCustTree, delPrice,delCust,addCusts,improtExcel,clearList,saveMx
...
import {
  ...
  combineParameters,
  initDynamics
} from "/@/views/boozsoft/stock/stock_price_sup/data";
...
function exportexcel() {
  ...
  jsonToSheetXlsx({
    ...
    filename: '采购价格表_'+ companyName.value +'.xlsx',
  });
}
