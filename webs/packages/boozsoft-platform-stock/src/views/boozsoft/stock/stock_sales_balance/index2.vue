      ...
      <div>
        <div>
          ...
          <Popover placement="bottom" v-if="status == 3 && formItems.bcheck == '1'">
            <template #content>
              <Button @click="generate('ck')" :disabled="ymPeriod">销售出库单</Button><br/>
              <Button @click="generate('th')" :disabled="ymPeriod">销售退货单</Button><br/>
              <Button >销售发票</Button>
            </template>
            <Button class="actod-btn">生单</Button>
          </Popover>
          <Button class="actod-btn" @click="startDel" v-if="status == 3 && formItems.bcheck != '1'" :disabled="ymPeriod">删除</Button>
          <Button class="actod-btn" @click="startReview(true)" :disabled="ymPeriod"
                  v-if="status == 3 && formItems.bcheck != '1'">审核</Button>
          <Button class="actod-btn" @click="startReview(false)" :disabled="ymPeriod"
          ...
          <div class="acttd-right-d-btns">
            ...
            <Popover placement="bottom">
              <template #content>
                ...
                &nbsp;蓝字期初销货单&ensp;<CheckOutlined v-if="titleValue===0"/></span><br/>
                ...
                &nbsp;红字期初销货单&ensp;<CheckOutlined v-if="titleValue===1"/></span>
              </template>
              <Button class="acttdrd-btn">
      ...
      <div class="acb-centent">
        ...
        >
          ...
          <template #isumChuku="{ record }">
            <template v-if="record?.editIsumChuku">
              <InputNumber v-model:value="record.tempIsumChuku"
                           class="isumChuku"
                           :formatter="value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                           :parser="value => value.replace(/\$\s?|(,*)/g, '')"
                           style="width: 82%;"
                           @keyup.enter="focusNext(record,'isumChuku')"/>
              <CheckOutlined @click="record.editIsumChuku = null;record.isumChuku=record.tempIsumChuku;tableDataChange(record,'isumChuku')"/>
            </template>
            <template v-else>
              <div :class="status == 1 || status == 2?'suspended-div':'status-look'"
                   @click="record.tempIsumChuku=record.isumChuku,record.editIsumChuku = true;">
                    <span class="a-table-font-arial">{{
                        (record.isumChuku == null ? '' : parseFloat(record.isumChuku).toFixed(2) + '').replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                      }}</span>
              </div>
            </template>
          </template>
          <template #isumFapiao="{ record }">
            <template v-if="record?.editIsumFapiao">
              ...
              <CheckOutlined @click="record.editIsumFapiao = null;record.isumFapiao=record.tempIsumFapiao;tableDataChange(record,'isumFapiao')"/>
            </template>
            <template v-else>
              <div :class="status == 1 || status == 2?'suspended-div':'status-look'"
         ...
         <span class="a-table-font-arial">{{
          ...
          <template #hxIsum="{ record }">
            <template v-if="record?.editHxIsum">
              <InputNumber v-model:value="record.tempHxIsum" class="hxIsum"
                           :formatter="value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                           :parser="value => value.replace(/\$\s?|(,*)/g, '')"
                           style="width: 82%;"
            ...
            <template v-else>
              <div :class="  status == 3?'status-look':'suspended-div'"
...
import Refer from '/@/views/boozsoft/stock/stock_sales_add/popup/refer.vue';
import {
  CheckOutlined,
  EditOutlined,
  FileSearchOutlined,
...
const titleContents = ['期初销货单', '期初销货单']
...
async function checkBusDate(date) {
  ...
  if (list.length == 0){
    createWarningModal({title: "温馨提示",content: '当年度暂未创建，不能新增期初销货单，请先创建该年度'})
    return false;
  }else {
    if (list.filter(it=>it.istock == '1').length == list.length){
      createWarningModal({title: "温馨提示",content: '当年度已经全部结账，不能新增期初销货单，请切换公司代码或年度'})
      return false;
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    if (null == che  || che.type != '0'){
      ...
      // 任务
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'期初销货单',method:'变更,弃审,整理现存量',recordNum:formItems.value.ccode})
      if(taskData==''){
        tempTaskSave('变更')
      }else{
        // 任务不是当前操作员的
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    if ((b && !hasBlank(formItems.value.bcheckUser)) || (!b && hasBlank(formItems.value.bcheckUser))){
      ...
      if (ymPeriod.value){
        createWarningModal({title: '温馨提示',content: '当前业务日期期间已经结账，不能进行单据审核操作，请取消结账后后重试！'})
        return  false;
      }
      let taskData1= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'月末结账',method:'结账',recordNum:''})
      if (taskData1 != ''){
        createWarningModal({title: '温馨提示',content: '操作员'+taskData1.caozuoUnique+'正在对当前账套进行月末结账处理，不能进行单据单据审核操作，请销后再试！'})
        return  false;
      }
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'期初销货单',method:'修改,审核,弃审,删除,整理现存量',recordNum:formItems.value.ccode})
      if(taskData==''){
      ...
      let res = await useRouteApi(reviewRuKu, {schemaName: dynamicTenantId})({
        ...
        keys:  [],//tableSelectedRowObjs.value.filter(it=>null != it?.lineCode).map(it=>it
        // .lineCode),
        userId: a,
        type: b,
      ...
      if(!hasBlank(taskInfo.value))
        tempTaskDel(taskInfo.value?.id)
       writeLog(`${b?'审核':'弃审'}`,null,`操作内容【${b ? '审核' : '弃审'}期初销货单】,` + '期初销货单号【' + formItems.value.ccode + '】,单据日期【' + formItems.value.ddate + '】')
...
async function stockCheck(list,model) {
  ...
  if (err != ''){
    ...
    if (ic) writeLog('新增',formItems.value,`操作内容【新增期初销货单】,` + '新增失败,可用量不足！详情:'+err)
...
async function saveData() {
  ...
  formItems.value.bdocumStyle =   bdocumStyle.value  // 制单人
  ...
  if (saveCheck(list)) {
   ...
   /* if (!(await stockCheck(list,formItems.value))) {
    ...
    await useRouteApi(saveXhdQc, {schemaName: dynamicTenantId})(formItems.value)
    message.success('保存成功！')
    if(!hasBlank(taskInfo.value))
      tempTaskDel(taskInfo.value?.id)
    writeLog('新增',formItems.value,null)
    await pageReload()
    status.value = 3
  }
}
...
async function writeLog(action, a, content) {
  ...
  if (action == '修改') {
    ...
    let text = `操作内容【${action}期初销货单】,` + '期初销货单号【' + a.ccode + '】,单据日期【' + a.ddate + '】,'
  ...
  let map = {
    ...
    optContent: content || `操作内容【${action}期初销货单】,` + '期初销货单号【' + a.ccode + '】,单据日期【' + a.ddate + '】',
  }
  await saveLog(map)
}
...
const tableDataChange =  (r,c) => {
  ...
  if (hasBlank(r.hxIsum))r.hxIsum = 0
  if (hasBlank(r.isumFapiao))r.isumFapiao = 0
  if (hasBlank(r.isumChuku))r.isumChuku = 0
  switch (c) {
    case 'hxIsum':
      if(bdocumStyle.value==0&&!hasBlank(r.hxIsum)){
        ...
        if(parseFloat(r.hxIsum)>0){
          r.hxIsum=0
          return message.error('累计核销不能大于0')
        }else if(parseFloat(r.hxIsum)>parseFloat(r.isum)){
          r.hxIsum=0
          return message.error('累计核销不能大于价税合计')
        }
    ...
    case 'isumFapiao':
      if(bdocumStyle.value==0&&!hasBlank(r.isumFapiao)){
        ...
        }else if(parseFloat(r.isumFapiao)>parseFloat(r.quantity)){
          r.isumFapiao=0
          return message.error('累计发票数量不能大于数量')
        }
      }else if(bdocumStyle.value==1){
        r.isumFapiao=parseFloat(r.isumFapiao)>0?parseFloat(r.isumFapiao)*-1:parseFloat(r.isumFapiao)
        if(parseFloat(r.isumFapiao)>0){
          r.isumFapiao=0
          return message.error('累计发票数量不能大于0')
        }else if(parseFloat(r.isumFapiao)>parseFloat(r.quantity)){
          r.isumFapiao=0
          return message.error('累计发票数量不能大于数量')
        }
      ...
      break;
    case 'isumChuku':
      if(bdocumStyle.value==0&&!hasBlank(r.isumChuku)){
        if(parseFloat(r.isumChuku)<0){
          r.isumChuku=0
          return message.error('累计出库数量不能小于0')
        }else if(parseFloat(r.isumChuku)>parseFloat(r.quantity)){
          r.isumChuku=0
          return message.error('累计出库数量不能大于数量')
        }
      }else if(bdocumStyle.value==1){
        r.isumChuku=parseFloat(r.isumChuku)>0?parseFloat(r.isumChuku)*-1:parseFloat(r.isumChuku)
        if(parseFloat(r.isumChuku)>0){
          r.isumChuku=0
          return message.error('累计出库数量不能大于0')
        }
        else if(parseFloat(r.isumChuku)>parseFloat(r.quantity)){
          r.isumChuku=0
          return message.error('累计出库数量不能大于数量')
...
const loadPrint = (obj) => {
  ...
  const data = getDataSource().filter(it=>it.cvencode!=null)
  let printList: any = []
  let printUser = ''
  if(obj.isMark){
  ...
  let printDate = ''
  printDate = '审核人：' +(obj.isReview? formatUniqueOperator(formItems.value.bcheckUser):'')
  data.forEach((item,index) => {
    let item1 = {}
    item1[0] = index+1
    item1[1] = setString(item.cwhcodeText,14)
    item1[2] = item.cinvode
    item1[3] = setString(item.cinvodeName,14)
    item1[4] = item.cunitid
    item1[5] = parseFloat(item.baseQuantity).toFixed(2)
    item1[6] = item.cunitidF1
    item1[7] = parseFloat(item.subQuantity1).toFixed(2)
    item1[8] = item.cunitidF2
    item1[9] = parseFloat(item.subQuantity2).toFixed(2)
    item1[10] = parseFloat(item.taxprice).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')
    item1[11] = parseFloat(item.isum).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')
    item1[12] = item.batchId
    item1[13] = item.dvdate
    item1[14] = item.cmemo
  ...
  for (let i=0; i<printList.length%25; i++){
    ...
    if(printList.length%25==24){
      item1[1] = '合计'
      item1[5] = toThousandFilter(formItems.value.squantity)
      item1[7] = toThousandFilter(formItems.value.squantity1)
      item1[9] = toThousandFilter(formItems.value.squantity2)
      item1[11] = toThousandFilter(formItems.value.isum)
    }
    printList.push(item1)
  }
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['期初销货单', '', '', '', '', '', '', '', '', '', '', '', '', '', ''],
        ['销货单号：' + formItems.value.ccode, '', '','单据日期：'+formItems.value.ddate,'', '',  '销售客户：'+getCustName(formItems.value.cvencode),'', '', '','结算客户：'+getCustName(formItems.value.cvencodeJs), '', '','', ''],
        ['仓库：' + (ckListOptions.value.filter(it => it.id == formItems.value.cwhcode)[0]?.ckName || ''), '', '','业务员：'+getUserName(formItems.value.cpersoncode),'', '',  '备注：'+(formItems.value.cmemo || ''),'', '', '','', '', '', '销售税率：'+formItems.value.xsRate+'%',''],
        ['序号', '仓储位置', '存货编码','存货名称', '主计量', '主数量', '计量1', '数量1','计量2', '数量2', '含税单价','价税合计',  '批号','失效日期','备注']],
      body: printList,
      styles: tableStyle(),
      margin: {
        left: 40,
      ...
      didParseCell(data) {
        ...
        if (data.section == 'head' && (data.row.index == 1)) {
          ...
          if (data.column.index == 0) {
            ...
            data.cell.styles.halign = 'center'
          } else if (data.column.index == 6 || data.column.index == 10) {
            data.cell.colSpan = 4
            // data.cell.styles.cellPadding = {left: 10,top: 3}
            data.cell.styles.halign = 'center'
          }else {
        ...
        if (data.section == 'head' && (data.row.index == 2)) {
          ...
          if (data.column.index == 0) {
            ...
            data.cell.colSpan = 3
            data.cell.styles.cellPadding = {left: 20,top: 3}
            data.cell.styles.halign = 'left'
...
const generate = async (t) => {
  ...
  // 有无 任务
  let xclTaskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'期初销货单',method:'生单'})
  ...
  if (t=='ck'){
    ...
    if (formItems.value.chukuStatus == '1'){
      createWarningModal({title: "温馨提示",content: '当前期初销货单已经全部生成销售出库单，不能进行销售出库单重复生单操作！'})
      return;
    }
    if (data.filter(it=>it.sourcetype == 'XSCKD').length > 0){
      createWarningModal({title: "温馨提示",content: '当前期初销货单为参照销售出库单生成，不能进行销售出库单循环生单操作！'})
      return;
    }
    ...
    createConfirm({
      ...
      onOk: async () =>{
        await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.username,functionModule:'期初销货单',method:'生单'})
        await useRouteApi(generateCkd, {schemaName: dynamicTenantId})(entry)
        message.success('生成成功！')
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.username,functionModule:'期初销货单',method:'生单'})
      }
    })
  }else if(t=='th'){
    // 查看退货数量
    ...
    if (n>=b){
      createWarningModal({title: "温馨提示",content: '当前期初销货单已经全部完成退货，不能再次进行退货操作！'})
      return;
    }
...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'期初销货单',method:method,recordNum:formItems.value.ccode,caozuoModule:'stock'})
    .then((a)=>{
      taskInfo.value=a
    })
}
