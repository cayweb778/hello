...
<!--          <Popover placement="bottom" v-if="status == 3 && formItems.bcheck == '1'">
          ...
          <Popover placement="bottom" v-if="status == 1">
            <template #content>
              <Button @click="referData('XHD')">销货单</Button><br/>
            </template>
          ...
          <Popover placement="bottom" v-if="status == 3 && formItems.bcheck == '1'">
            <template #content>
              <Button @click="generate('ck')" >红字销售出库单</Button><br/>
              <Button >红字销售发票</Button>
            </template>
          ...
          <Button class="actod-btn" @click="startReview(false)" v-if="status == 3 && formItems.bcheck == '1'">弃审
          </Button>
        ...
        <div class="acbgead-one">
          ...
          <div>
            <span style="font-size: 22px;font-weight: bold;"
                  :style="{color: titleValue===1?'#0096c7':'red'}">{{
      ...
      <div class="acb-centent">
        ...
        >
          <template #cwhcode="{ record }">
            <template v-if="record.editOne &&　(!isReference && record.sourcecode==null)">
              <Select
          ...
          <template #cinvode="{ record }">
            <template v-if="record.editTwo　 &&　(!isReference && record.sourcecode==null)">
              <Select
          ...
          <template #cinvodeName="{ record }">
            <template v-if="record.editEleven　 &&　(!isReference && record.sourcecode==null)">
              <Select
          ...
          <template #cinvodeBarcode="{ record }">
            <template v-if="record.editThree　&&　(!isReference && record.sourcecode==null)">
              <Select
          ...
          <template #xsUnitId="{ record }">
            <template v-if="record?.editCunit &&　(!isReference && record.sourcecode==null)">
...
<!--              <div :class="record.isGive ||  status == 3?'status-look':'suspended-div'"
          ...
          <template #isGive="{ record }">
            <Switch v-model:checked="record.isGive" :disabled="status == 3 || isReference" size="small"
                    @change="giveChange(record)"/>
          </template>
          <template #batchId="{ record }">
            <template v-if="record?.editTwelve && record.isBatch　 &&　(!isReference && record.sourcecode==null)">
          ...
          <template #dpdate="{ record }">
            <template v-if="(!isReference && record.sourcecode==null)&&record?.editDpdate">
          ...
          <template #dvdate="{ record }">
            <template v-if="record?.editFifteen　 &&　(!isReference && record.sourcecode==null)">
...
import {
  ...
  VerticalRightOutlined,CopyOutlined,VerticalAlignTopOutlined,VerticalAlignBottomOutlined
...
const dynamicAdReload = async (obj) => {
  ...
  isReference.value = obj.target?.xsThXhd == '1'
...
async function checkBusDate(date) {
  ...
  if (list.length == 0){
    createWarningModal({title: "温馨提示",content: '当年度暂未创建，不能新增销售退货单，请先创建该年度'})
    return false;
  }else {
    if (list.filter(it=>it.istock == '1').length == list.length){
      createWarningModal({title: "温馨提示",content: '当年度已经全部结账，不能新增销售退货单，请切换公司代码或年度'})
      return false;
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    if (isReference.value) {
      // createWarningModal({title: '温馨提示', content: '依据当前参数设置，请先选择客户后,进行参照添加流程！' });
      message.info( '依据当前参数设置，请先选择客户后,进行参照添加流程！');
    }
  }else {
    // 任务
    ...
    if(taskData==''){
      ...
      if(taskData[0].caozuoUnique!==useUserStoreWidthOut().getUserInfo.id){
        return createWarningModal({title: '温馨提示', content: taskData[0].username+'正在修改销售退货单,不能同时进行操作！' });
      }else{
...
const startDel = async () => {
  if (null == formItems.value || hasBlank(formItems.value.id)) {
    ...
    if(taskData==''){
      ...
      // 任务不是当前操作员的
      if(taskData[0].caozuoUnique!==useUserStoreWidthOut().getUserInfo.id){
        return createWarningModal({ content: taskData[0].username+'正在删除销售退货单,不能同时进行操作！' });
      }else{
        await useRouteApi(stockBalanceTaskEditNewTime, { schemaName: dynamicTenantId })(taskData[0].id)
      }
    }
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    if ((b && !hasBlank(formItems.value.bcheckUser)) || (!b && hasBlank(formItems.value.bcheckUser))){
      ...
      if (await checkPeriod())return false;
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'销售退货单',method:'修改,审核,弃审,删除',recordNum:formItems.value.ccode})
      if(taskData==''){
      ...
      if (!b){ // 弃审前 检查
        ...
        if (null == che  || che.type != '0'){
          switch (che.type) {
            ...
            case '5':
              ...
              break;
            case '6':
              createWarningModal({title: '温馨提示',content: '单据【'+che.codes+'】已进行过参照开发票，不能进行取消审核操作，请删除参照生成的退货单据后继续！'})
              break;
            case '7':
              createWarningModal({title: '温馨提示',content: '单据【'+che.codes+'】已进行过参照退货，不能进行取消审核操作，请删除参照生成的发票单据后继续！'})
              break;
          }
          tempTaskDel()
          return false;
        ...
        if (isAuto){
          ...
          if(taskData!=''){
            createWarningModal({ content:`操作员${ taskData[0].username}正在对当前仓库进行盘点处理，不能进行审核销货单操作，生成销售出库单会影响盘点数量，请销后再试` });
            tempTaskDel()
            return true
          }
        }
      ...
      if (b && isAuto){
         ...
         await new Promise((resolve, reject) => {
              createConfirm({
               ...
               cancelText:'放弃',
               okText:'生成',
               content: '检查到已勾选业务参数“销货单审核生成出库单”，是否继续生成红字销售出库单?',
               onOk:  () => {
                 resolve(true)
               },onCancel: () => {
                 reject()
...
async function stockCheck(list,model) {
  ...
  if (err != ''){
    ...
    if (ic) writeLog('新增',formItems.value,`操作内容【新增销售退货单】,` + '新增失败,可用量不足！详情:'+err)
...
async function saveData() {
  ...
  formItems.value.bdocumStyle =  "1"  // 制单人
  ...
  if (saveCheck(list)) {
    ...
    if(checks.length > 0){
      ...
      for (let i = 0; i < checks.length; i++) {
        ...
        if (null== r || ((parseFloat(c.baseZOld)-parseFloat(c.baseQuantity)) > (parseFloat(r.baseQuantity) - parseFloat(r.isumTuiHuo || '0'))))rowArr.push((i+1)+'')
      }
      if (rowArr.length > 0)
        return createWarningModal({title: "温馨提示",content: `系统检测到序号【${rowArr.join()}】行退货单数量不能大于参照的来源单据数量，请检查后重试！`})
    }else if (status.value==2 &&  hasBlank(formItems.value?.sourcecode) && list.filter(it=>(parseFloat(it.baseQuantity)-parseFloat(it.baseZOld))>0).length > 0){
      let nlist =  JsonTool.parseProxy(list.filter(it=>(parseFloat(it.baseQuantity)-parseFloat(it.baseZOld))>0))
      nlist = nlist.map(it=>{it.baseQuantity=(parseFloat(it.baseQuantity)-parseFloat(it.baseZOld)).toFixed(10);return it})
      if(!await stockXclCheck(nlist)){
        tempTaskDel()
        return createWarningModal({ content: '可用量不足，请检查存货可用量后再试！' });;
      }
    }
...
async function writeLog(action, a, content) {
  ...
  if (action == '修改') {
    ...
    let text = `操作内容【${action}销售退货单】,` + '销售退货单号【' + a.ccode + '】,单据日期【' + a.ddate + '】,'
  ...
  let map = {
    ...
    optContent: content || `操作内容【${action}销售退货单】,` + '销售退货单号【' + a.ccode + '】,单据日期【' + a.ddate + '】',
  }
  await saveLog(map)
}
...
const tableDataChange =  (r,c) => {
  ...
  switch (c) {
    ...
    case 'quantity':
      ...
      r.quantity = (parseFloat(r.quantity)>0?(parseFloat(r.quantity)*-1):parseFloat(r.quantity)).toFixed(10)
      ...
      if (c == 'quantity' && !hasBlank(r.sourcecode)  && (r.quantity == '' || 0 <= parseFloat(r.quantity) ) ) {
        message.warning(`支持修改的数量范围[0～${r.baseOld}],且不包含0！`)
        r.quantity = r.baseOld
        break;
      } else if(c == 'quantity' && !hasBlank(r.sourcecode) && parseFloat(r.quantity) < parseFloat(r.baseOld)){ // 蓝字
      ...
      if (c == 'quantity' && !hasBlank(r.quantity) && !hasBlank(r.taxprice)) tableDataChange(r,'taxprice')
      break;
    // case 'baseQuantity':
    case 'price':
      if (!hasBlank(r.quantity) && !hasBlank(r.price)) {
        ...
        let d = Math.abs(parseFloat(r.price)).toFixed(10)
    ...
    case 'isum':// 价税合计
      if (!hasBlank(r.quantity) && !hasBlank(r.isum)) { //反算 单价
        let n:any = parseFloat(r.quantity).toFixed(10)
        if (titleValue.value != 0 && n > 0) n = 0 - (Math.abs(n))
        let d:any = Math.abs(parseFloat(r.isum))*-1
        r.isum =  d + ''
        ...
        let d:any = parseFloat(r.isum)*-1
        r.taxprice = n + ''
        r.isum = d + ''
        r.quantity = (d / n).toFixed(10) + ''
        r.icost = (parseFloat(r.isum)/(1+parseFloat((r.itaxrate/100)))).toFixed(4)
        r.price = (parseFloat(r.icost)/n) .toFixed(4)
        r.itaxprice =( parseFloat(r.icost)*parseFloat((r.itaxrate/100))).toFixed(6)
        slChangeNew(r)
      }
      break;
    case 'dvdate':
...
/*  let p = stockPriceList.value.filter(it => it.stockNum == o?.stockNum)[0]
  ...
  if (null != p && null != k && null != p[k] && '0' !=  p[k]) record.defaultPrice = parseFloat(p[k]).toFixed(4)*/
  // 计量类型
  record.cunitType = o.stockMeasurementType == '单计量' ? '1' : '0'
...
const focusNext =  (r, c) => {
  ...
  // 查找下一个
  let list = getDataSource();
  let filters = ['isGive', 'bcheck', 'cinvodeType', 'cunitid', 'cinvodeName','cinvodeBarcode', 'cunitidF1', 'cunitidF2', 'baseQuantity','subQuantity1', 'subQuantity2',  'itaxprice','dvdate', 'dpdate','price','icost']
...
function xySourcePop() {
  if (hasBlank(formItems.value.ccode)){
    ...
    openXySourcePage(true, {
      database: dynamicTenantId.value, type: 'XHD',code: formItems.value.ccode,datas: {ck:ckListOptions.value},isRed:true,schemaCode: dynamicTenant.value?.coCode
    })
  }
}
// 查看下游数据
function lySourcePop() {
  if (hasBlank(formItems.value.ccode)){
    ...
    openLySourcePage(true, {
      database: dynamicTenantId.value,code: formItems.value.ccode,type: 'XHD',isRed: true,datas: {ck:ckListOptions.value,ji:jiList.value,many:manyJiList.value},schemaCode: dynamicTenant.value?.coCode
    })
  }
}
function openItems() {
...
const generate = async (t) => {
  ...
  if (t=='ck'){
    ...
    if (formItems.value.chukuStatus == '1' || n >= b){
      createWarningModal({title: "温馨提示",content: '当前销售退货单已经全部生成销售出库单，不能进行销售出库单重复生单操作！'})
      return;
    }
   /* if (data.filter(it=>it.sourcetype == 'XSCKD').length > 0){
      createWarningModal({title: "温馨提示",content: '当前销售退货单为参照销售出库单生成，不能进行销售出库单循环生单操作！'})
      return;
    }*/
    ...
    let list = data.map(it=>{
      ...
      it.baseQuantity = ((Math.abs(parseFloat(it.baseQuantity || 0))-parseFloat(it.isumChuku || 0))*-1).toFixed(10)
...
const checkBasequantityThl =  async (r) => {
  let b = parseFloat(r.baseQuantity || 0)*-1
...
async function tempTaskSave(method) {
  await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售退货单',method:method,recordNum:formItems.value.ccode,caozuoModule:'stock'})
    .then((a)=>{
...
const referThrowData = async (data) => {
  if (data.length > 0){
    // 锁任务
    await useRouteApi(stockBalanceTaskSave, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'销售出库单',method:'审核',recordNum:data[0].sourcecode,caozuoModule:'stock'})
    ...
    getDataSource().forEach(async (b)=>{
      b = resetRow(b)
/*      // 得到存货单价
      let p = stockPriceList.value.filter(it => it.stockNum == b?.cinvode)[0]
      if (null != p && null != k && null != p[k] && '0' !=  p[k]) b.defaultPrice = parseFloat(p[k]).toFixed(4)*/
      b.quantity = (getNumber(b))*-1
      // 默认数量 不能大于该数量
