...
<!--   <div><textarea>应收单列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">应收单列表</b>
...
async function reloadTable(){
  ...
  // const res = await useRouteApi(findSaleousingList,{schemaName: dynamicTenantId})({billStyle:'YSD',iyear:pageParameter.year})
  const res = await useRouteApi(findYsdmxbList,{schemaName: dynamicTenantId})({bcheck:'',ddate1:date1,ddate2:date2})
...
async function editPage(){
  if (checkRow.value.length == 1) {
    ...
    if (num == 0) {
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应收单',method:'修改,删除,复核,弃复',recordNum:checkRow.value[0].ccode})
      if(taskData.length==0){
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ar'})
      } else {
        // 任务不是当前操作员的
        if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
          ...
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'修改',recordNum:checkRow.value[0].ccode,caozuoModule:'ar'})
        }
      }
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      ...
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'复核',recordNum:item.ccode,caozuoModule:'ar'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应收单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'复核',recordNum:item.ccode,caozuoModule:'ar'})
          }
        }
        item.bcheck = '1'
        ...
        const saleousingsList = await useRouteApi(findSaleousingsByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
        ...
        if (arHexiaoAuto.value=='1'){
          ...
          await useRouteApi(saveSaleousing, {schemaName: dynamicTenantId})(item)
          //修改销货单子表
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'复核',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
  ...
  if (checkRow.value.length > 0) {
    ...
    if (num == 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'应收单',method:'修改,删除,复核,弃复',recordNum:item.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'弃复',recordNum:item.ccode,caozuoModule:'ar'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'应收单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'弃复',recordNum:item.ccode,caozuoModule:'ar'})
          }
        }
        item.bcheck = '0'
        item.bcheckTime = ''
        ...
        item.bworkableUser = ''
        await useRouteApi(saveSaleousing, {schemaName: dynamicTenantId})(item)
        const ysyfsList = await useRouteApi(findSaleousingsByCcode,{schemaName: dynamicTenantId})({ccode:item.ccode,billStyle:item.billStyle})
        ysyfsList.map(ysyfs=>{
          ysyfs.bcheck = '0'
          ysyfs.bcheckTime = ''
          ysyfs.bcheckUser = ''
        ...
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收单',method:'弃复',recordNum:item.ccode})
      }
    }
    state.selectedRowKeys = []
    checkRow.value = []
