    ...
    <template #title>
      <div style="display: flex;margin-top: 10px;margin-left: 10px;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;" v-if="isState=='0'">
          <PlusCircleOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;应收款期初余额
        </span>
        <span style="line-height:40px;font-size: 28px;" v-if="isState=='1'">
          <FormOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;应收款期初余额
        </span>
        <span style="line-height:40px;font-size: 28px;" v-if="isState=='2'">
          <FileSearchOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;应收款期初余额
        </span>
      </div>
...
<!--        <a-radio-group v-model:value="formItems.busStyle" @change="changeBusStyle()" :disabled="isState=='1'" style="margin-left: -20px;">
          <a-radio value="YSD" style="width:100px;">
            <span style="color: #000000;font-weight: bold;font-size: 12px;">应收单</span>
          </a-radio>
          <a-radio value="SKD" style="width:100px;margin-left: -10px;">
            <span style="color: #000000;font-weight: bold;font-size: 12px;">收款单</span>
          </a-radio>
        </a-radio-group>-->
        <br/><br/>

          <div v-if="formItems.busStyle=='YSD'">
            <span class="red_span">{{isState=='2'?'':'*'}}</span>
            <label style="font-size: 18px;margin-left: 0;width:150px;">客户名称：</label>
          ...
          <div v-if="formItems.busStyle=='SKD'">
            <span class="red_span">{{isState=='2'?'':'*'}}</span>
            <label style="font-size: 18px;margin-left: 0;width:150px;">结算客户名称：</label>
...
<!--          <a-select-option v-if="formItems.busStyle=='YSD'" value="XSFP">销售发票</a-select-option>-->
        ...
        <a-select v-model:value="formItems.citemId" style="width: 30%">
          <template #suffixIcon><CaretDownOutlined v-if="isState!='2'" style="color:#666666;" /></template>
          <a-select-option v-for="item in projectList" :value="item.uniqueCode" >
...
<script setup="props, { content }" lang="ts">
  ...
  function changeBusStyle(){
    if (formItems.value.busStyle=='YSD'){
      formItems.value.arStyle = 'YSD'
    } else {
      formItems.value.arStyle = 'PTSK'
    }
    generateCode()
  }
  ...
  async function generateCode() {
    let code=''
    let qz = ''
    if (formItems.value.arStyle=='YSD'){
      qz = 'YS-'
    } else if (formItems.value.arStyle=='XHD'){
      qz = 'SA-'
    } else if (formItems.value.arStyle=='XSFP'){
      qz = 'SB-'
    } else if (formItems.value.arStyle=='PTSK'){
      qz = 'SK-'
    } else if (formItems.value.arStyle=='YSK'){
      qz = 'SK-'
    }
    //日期(年月)
  ...
  async function handleOk() {
    ...
    if (formItems.value.busStyle=='YSD' && (formItems.value.cvencodeXs=='' || formItems.value.cvencodeXs==null)){
      createErrorModal({
        iconType: 'warning',
        title: '提示',
        content: '客户名称不能为空！'
    ...
    if (formItems.value.busStyle=='SKD' && (formItems.value.cvencodeJs=='' || formItems.value.cvencodeJs==null)){
      createErrorModal({
        iconType: 'warning',
        title: '提示',
        content: '结算客户名称不能为空！'
