...
function openPage(){
  ...
  router.push({
    path: '/YingShouZhang/ShouKuan/ChaXun/ShouKuanDanLieBiao',
    query: pageParameter
  });
}
...
const startEdit = async (type) => {
  ...
  if (type === 'add') {
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'收款单',method:'添加'})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'添加',caozuoModule:'ar'})
    } else {
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'添加',caozuoModule:'ar'})
    }
    status.value = 1
    formItems.value.id=null
    ...
    let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'收款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'修改',recordNum:formItems.value.ccode,caozuoModule:'ar'})
    } else {
      // 任务不是当前操作员的
      if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
        ...
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'修改',recordNum:formItems.value.ccode,caozuoModule:'ar'})
      }
    }
    status.value = 2
    let list = getDataSource()
    ...
    if (formItems.value.bcheck=='1') {
      ...
      createConfirm({
        ...
        onOk: async () => {
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'收款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'收款单,不能同时进行操作！' });
            }else{
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'删除',recordNum:formItems.value.ccode,caozuoModule:'ar'})
            }
          }
          ...
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'删除',recordNum:formItems.value.ccode})
...
const startReview = async (b) => {
  ...
  if (!hasBlank(a) && !hasBlank(formItems.value.id)) {
    if (b==true){
      ...
      let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'收款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
      if(taskData.length==0){
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'审核',recordNum:formItems.value.ccode,caozuoModule:'ar'})
      } else {
        // 任务不是当前操作员的
        if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
          return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'收款单,不能同时进行操作！' });
        }else{
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'审核',recordNum:formItems.value.ccode,caozuoModule:'ar'})
        }
      }
      ...
      //删除任务锁定
      await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'审核',recordNum:formItems.value.ccode})
    } else {
      // console.log('取消审核')
      if (formItems.value.writeOffStatus=='1' || formItems.value.hxIsum>0) {
        ...
        let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,name:'收款单',method:'修改,删除,审核,弃审',recordNum:formItems.value.ccode})
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'弃审',recordNum:formItems.value.ccode,caozuoModule:'ar'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            return createWarningModal({ content: taskData[0]?.username+'正在'+taskData[0]?.method+'收款单,不能同时进行操作！' });
          }else{
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'弃审',recordNum:formItems.value.ccode,caozuoModule:'ar'})
          }
        }
        ...
        await useRouteApi(saveYsyfsList, {schemaName: dynamicTenantId})(ysyfsList)
        //删除任务锁定
        await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'弃审',recordNum:formItems.value.ccode})
      }
    }
...
async function balanceTaskDel() {
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'添加'})
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'收款单',method:'修改',recordNum:formItems.value.ccode})
}
...
import {findSaleousingsByCcode, saveSaleousingsList} from "/@/api/record/system/ysd";
