...
<!--   <div><textarea>预收冲应收列表的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">预收冲应收列表</b>
...
async function reloadTable(){
  ...
  let len = 0
  const res = await useRouteApi(findArApChongxiaoList,{schemaName: dynamicTenantId})({billStyle:'YS',iyear:pageParameter.year})
...
function addPage(){
  ...
  router.push({
    path: '/YingShouZhang/ChongXiao/CaoZuo/YuShouChongYingShou',
    query: pageParameter
  });
}
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let x = 0; x < checkRow.value.length; x++) {
          ...
          //判断预收冲应收
          let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:pageParameter.year,name:'预收冲应收',method:'删除',recordNum:chongxiao.ccode})
          if(taskData.length==0){
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ar'})
          } else {
            // 任务不是当前操作员的
            if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
              ...
              await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:chongxiao.ccode,caozuoModule:'ar'})
            }
          }
          ...
          //删除任务锁定
          await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'预收冲应收',method:'删除',recordNum:chongxiao.ccode})
