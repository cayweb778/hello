...
<!--   <div><textarea>自动核销的页面</textarea>复制内容快速定位页面</div>-->
    <div class="app-container">
      ...
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">自动核销</b>
    ...
    <div class="app-container">
      <a-spin :spinning="spinning" tip="正在核销中...">
      <BasicTable
        ref="tableRef"
...
async function saveQuery(data) {
  ...
  //查询期初
  let res = await useRouteApi(findWhxList,{schemaName: dynamicTenantId})({billStyle:'ar',iyear:pageParameter.year})
  let skdQichuList = res.filter(item => item.busStyle=='SKD')
  let ysdQichuList = res.filter(item => item.busStyle=='YSD')
  //查询收款单
  let skdList = await useRouteApi(findWhxskdByEndDate,{schemaName: dynamicTenantId})({billStyle:'SKD',year:pageParameter.year,endDate:endDate.value})
  //查询销货单
  let ysdList:any = []
  let saleList = await useRouteApi(findSkWhxskdByEndDate,{schemaName: dynamicTenantId})({year:pageParameter.year,endDate:endDate.value})
  if (arSourceFlag.value=='1') {
    ysdList = saleList.filter(item => item.busStyle !='XHD' && item.bworkable=='1')
  } else {
    if(arCheckFlag=='1'){
      ysdList = saleList.filter(item => item.busStyle !='XSFP' && item.bworkable=='1')
    } else {
      ysdList = saleList.filter(item => item.busStyle !='XSFP')
    }
  }
  //自动核销期初收款单
  for (let i = 0; i < skdQichuList.length; i++) {
    let item = skdQichuList[i]
    let whxIsum = sub(item.ysIsumBenbi,item.hxIsum==null?0:item.hxIsum)
    //核销期初应收单
    for (let i = 0; i < ysdQichuList.length; i++) {
      let item1 = ysdQichuList[i]
      if (item1.cvencodeJs==item.cvencodeJs) {
        let whxIsum1 = sub(item1.arIsumBenbi, item1.hxIsum == null ? 0 : item1.hxIsum)
        let money = 0
        if (whxIsum!=0 && whxIsum1!=0) {
          if (whxIsum > 0 && whxIsum1 > 0) {
            if (sub(whxIsum, whxIsum1) == 0) {
              item.hxIsum = item.ysIsumBenbi
              item.hxFlag = '1'
              item1.hxIsum = item1.arIsumBenbi
              item1.hxFlag = '1'
              money = whxIsum
            } else if (sub(whxIsum, whxIsum1) > 0) {
              item.hxIsum = add(item.hxIsum == null ? '0' : item.hxIsum, whxIsum1)
              item.hxFlag = '0'
              item1.hxIsum = item1.arIsumBenbi
              item1.hxFlag = '1'
              money = whxIsum1
              ...
              item.hxIsum = item.ysIsumBenbi
              item.hxFlag = '1'
              item1.hxIsum = add(item1.hxIsum == null ? '0' : item1.hxIsum, whxIsum)
              item1.hxFlag = '0'
              money = whxIsum
            }
          } else {
          ...
          let writeOff: any = {
            ...
            ddate: item.ddate,
            billStyle: 'SKD',
            hxStyle: item1.billStyle,
            cvencode: item.cvencodeJs,
            hxCcode: item1.ccode,
            hxIsum: item1.arIsumBenbi,
            hxMoney: money,
            citemcode: item1.citemcode,
            citemClass: item1.citemClass,
          ...
          let stockXyCsource: any = {
            ...
            billStyle: 'YUE',
            ccode: item1.ccode,
            ccodeDate: item1.ddate,
            xyccode: item.ccode,
            xyccodeDate: item.ddate,
          }
    ...
    for (let i = 0; i < ysdList.length; i++) {
      let item1 = ysdList[i]
      if (item1.cvencodeJs==item.cvencodeJs) {
        let whxIsum1 = sub(item1.isum, item1.hxIsum == null ? 0 : item1.hxIsum)
        let money:any = 0
        if (whxIsum!=0 && whxIsum1!=0) {
          if (whxIsum > 0 && whxIsum1 > 0) {
            if (sub(whxIsum, whxIsum1) == 0) {
          ...
          //保存方法
          await useRouteApi(saveBeginBalance, {schemaName: dynamicTenantId})(item)
          await useRouteApi(saveSaleousing, {schemaName: dynamicTenantId})(item1)
          //修改销货单子表
          const sales = await useRouteApi(findSaleousingsByCcode, {schemaName: dynamicTenantId})({
            ccode: item1.ccode,
            billStyle: item1.billStyle
          })
          let salesList: any = []
  ...
  for (let i = 0; i < skdList.length; i++) {
    let item = skdList[0]
    let whxIsum = sub(item.isum,item.hxIsum==null?0:item.hxIsum)
    //核销期初应收单
    for (let i = 0; i < ysdQichuList.length; i++) {
      let item1 = ysdQichuList[i]
      if (item1.cvencodeJs==item.cvencodeJs) {
    ...
    for (let i = 0; i < ysdList.length; i++) {
      let item1 = ysdList[i]
      if (item1.cvencode==item.cvencodeJs) {
        let whxIsum1 = sub(item1.isum, item1.hxIsum == null ? 0 : item1.hxIsum)
        let money:any = 0
        if (whxIsum!=0 && whxIsum1!=0) {
          if (whxIsum > 0 && whxIsum1 > 0) {
  ...
  //删除任务锁定
  await useRouteApi(stockBalanceTaskDelByUserName, { schemaName: dynamicTenantId })({iyear:pageParameter.year,userName:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收自动核销',method:'核销'})
  await reloadTable()
...
import {findByXhd, findSkWhxskdByEndDate} from "/@/api/record/system/hexiao";
import {
  ...
  findSaleousingByCcode, findWhxskdByEndDate, findXyCsourceByCcodeAndXyccode,
  findYskhxmxb, saveSaleousing, saveStockXyCsource, saveWriteOff, saveYsyf
} from "/@/api/record/system/arApYsyf";
import {
  deleteArXySouceById,
  findArBeginBalanceByCcode,
  findArXySourceByCcodeAndXyccode, findWhxList, saveArXySource,
  saveBeginBalance
} from "/@/api/record/system/arBeginBalance";
import {findSaleousingsByCcode, saveSaleousingsList} from "/@/api/record/system/ysd";
import {stockBalanceTaskDelByUserName} from "/@/api/record/stock/stock_balance";
/*start栏目设置*/
