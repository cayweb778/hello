import {createColumnDefine} from "/@/components/pingzheng-fillin/hooks/funs/columnModelsFuns";
import {ref} from "vue";

export function createFuZhuHeSuan(rowDefines, tableData){
  return createColumnDefine('fuZhuHeSuan', {
    columns: [
      {
        key: 'fuZhuHeSuan',
        label: '辅助项',
        width: '200',
        // component: shallowRef(fuZhuHeSuan),
        register({ target, row, rowIndex }) {
          ...
          target.next = () => {
            ...
            } else if (rowDefines.value[rowIndex].target.huiLv != null) {
          ...
          rowDefines.value[rowIndex].target.fuZhuHeSuan = target;
        },
      },
    ],
    defineData: () => ({
    ...
    resultMergeEvent(row) {
      ...
      if (res.isError) {
        ...
        const data = [
          // 结算方式编码
          ['pjCsettle', row.fuZhuHeSuan.jieSuanDanWei],
          // 票据号
          ['pjId', row.fuZhuHeSuan.piaoJuNumber],
          // 票据日期
          ['pjDate', row.fuZhuHeSuan.piaoJuDate],
          // 结算单位全称
          ['pjUnitName', row.fuZhuHeSuan.jieSuanDanWei],
        ];
        data.push(
          ...[
            // 人员辅助核算
