    ...
    <div class="pingzhengDefaultButtonFlex">
      ...
      <Popover placement="bottom">
        ...
        <Button
          ...
          @click="thisSaveFun"
          v-if="tableData.settings.typeLabel != '插入'"
          :loading="showSaveLoading"
        >
          <span style="color: rgb(0, 150, 199); font-weight: 400; margin-right: 5px">保存</span>
        </Button>
      </Popover>

      <template v-if="!pingzhengData.settings.enableModelSave">
        <Tooltip title="Alt+Z" placement="bottom">
          <Button
            class="hello-btn"
            style="margin-right: -2px; font-size: 15px"
            @click="thisTempSave()"
            v-if="tableData.settings.typeLabel != '插入'"
            ><span style="color: rgb(0, 150, 199); font-weight: 400; margin-right: 5px">暂存</span>
          </Button>
        </Tooltip>
        <Tooltip title="Alt+L" placement="bottom">
          <Button class="hello-btn" style="margin-right: -2px; font-size: 15px" @click="abc()"
            ><span style="color: rgb(0, 150, 199); font-weight: 400; margin-right: 5px">流量</span>
          </Button>
        </Tooltip>
        <Button
          v-if="isZhengFuKuaiJiZhiDu"
          style="margin-right: -2px; font-size: 15px"
          @click="emit('parallel')"
          ><span style="color: rgb(0, 150, 199); font-weight: 400; margin-right: 5px"
            >平行记账</span
          >
        </Button>
        ...
        >
        <!--          <Button style="font-size:15px" @click="tempSave()">临时保存</Button>-->

        <!--          <Button style="font-size:15px" @click="emit('ok')">审核</Button>-->
        <!--          <Button style="font-size:15px" @click="emit('ok')">记账</Button>-->

        <Button style="font-size: 15px" @click="exit"
          ><span style="color: rgb(0, 150, 199); font-weight: 400">退出</span></Button
        >
      </template>
      <Button
        v-if="pingzhengData.settings.enableModelSave"
        style="font-size: 15px"
        @click="exitPingzhengModal"
        >放弃
      </Button>
      <div style="float: right" v-if="false">
        <div
          ...
          v-if="
            tableData.settings.titleName === '查看凭证' ||
            tableData.settings.titleName === '复制凭证' ||
            tableData.settings.titleName === '冲销凭证'
          "
        >
          <button
            type="button"
          ...
          <Popover placement="bottom">
            <template #content>
              <span class="group-btn-span" @click="startChangeStatus(true, 1)"
                ><LockTwoTone />&nbsp;作废凭证</span
              ><br />
              <span class="group-btn-span" @click="startChangeStatus(false, 1)"
                ><UnlockTwoTone />&nbsp;取消作废</span
              >
            </template>
          ...
          <Popover placement="bottom">
            <template #content>
              <span class="group-btn-span" @click="startChangeStatus(true, 3)"
                ><LockTwoTone />&nbsp;标记错误</span
              ><br />
              <span class="group-btn-span" @click="startChangeStatus(false, 3)"
                ><UnlockTwoTone />&nbsp;取消标错</span
              >
            </template>
            <button type="button" class="ant-btn ant-btn-me"><span>记账</span></button>
          </Popover>
        </div>
      </div>
    </div>
