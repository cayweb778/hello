...
<!--      <div style="float: left;font-size: 16px;margin-left: 30px;margin-left: 20px;">
        自定义项设置：
        <a-select v-model:value="definedCode" @change="changCode()" class="head-index-select"
                  placeholder="请选择档案" style="min-width: 200px;">
          <a-select-option v-for="item in definedFileList" :key="item.definedCode"
...
import {excelDefinedFile, getDefinedFileList} from "/@/api/record/system/group-defined-file";
...
import {findByUniqueCode} from "/@/api/record/system/group-defined-record-account";
...
async function reloadDefinedFile(){
  definedFileList.value = await getDefinedFileList()
...
async function changCode() {
  ...
  tableAllData.value = await findAllDefinedRecordList(definedCode.value)
...
async function maxCode(){
  let str = ''
  const item1 = await findBukongRecordCode(definedCode.value)
  if (item1!=null && item1.recordCode!=null && item1.recordCode!=''){
    str = item1.recordCode
  } else {
    const item2 = await findMaxRecordCode(definedCode.value)
  ...
  if (checkRow.value.length > 0) {
    if (thisCheckKey.value.shuxing!='5') {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        const res = await findByUniqueCode(item.recordCode,item.definedCode)
        if (res.length > 0){
          createWarningModal({
            iconType: 'warning',
            title: '删除',
            content: '自定义项档案已分配不允许删除！'
          })
          return false
        }
      }
