    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;集团授权分配 － 自定义档案
        </span>
        </div>
      </template>
...
import {getDefinedRecordList} from "/@/api/record/system/group-defined-record";
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await getDefinedRecordList(definedCode.value)
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await save(item1)
      const res = await findByOriginIdAndUniqueCode(originId.value,item.definedCode)
      if (res.length==0) {
        const item2 = {
          id: null,
