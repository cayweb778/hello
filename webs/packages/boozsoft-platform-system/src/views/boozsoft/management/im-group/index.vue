...
<!--    <div class="app-container-head">
    ...
    <div class="nc-open-content" :class="isLook?'nc-open-content-look':''"
      ...
      &lt;!&ndash;      <label>代码：</label>
            ...
            <span class="red_span"><template v-if="!isLook"> ⋆</template></span>&ndash;&gt;
      <label>成立日期：</label>
  ...
  <div>
    <div class="nc-hr">
      <div class="nc-hr-top">
        <div class="nc-hr-top-left">
          ...
          <div>
            <div>
              <ul>
                <li><span>激活状态：</span><span>已激活</span></li>
                <li><span>激活单位名称：</span><span>湖北万亚软件技术有限公司</span></li>
                <li><span>授权到期日期：</span><span>2023年12月31日</span></li>
              </ul>
            </div>
            <div>
              <ul>
                <li><span>激活日期：</span><span>2022年10月15日</span></li>
                <li><span>税号：</span><span>6948596875632587</span></li>
                <li><span>授权操作员数量：</span><span>10</span></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="nc-hr-top-right">
          <div>
            <Rate v-model:value="rateValue" allow-half/><br/>
            <span>享受正版一对一客户在线支持</span>
          </div>
          <div>
            <span>软件激活</span>
          </div>
      ...
      <div class="nc-hr-middle">
            <div>
                  <span>集团（单位）信息</span>
                  <span @click="openEdit"><SettingOutlined />编辑</span>
            </div>
            <Divider style="height: 2px; background-color: #7cb305;margin: 12px 0;" />
            <div>
              <ul>
                <li><span>单位名称：</span><span>{{formItems.groupName || ''}}</span></li>
                <li><span>税号：</span><span>6948596875632587</span></li>
                <li><span>品牌：</span><span>{{formItems.brand || ''}}</span></li>
                <li><span>所属行业：</span><span>{{formatIndustry(formItems.industryclassCode) || ''}}</span></li>
                <li><span>国家（地区）：</span><span>{{formatCountry(formItems.countryId) || ''}}</span></li>
                <li><span>行政区划：</span><span>{{formatZoning(formItems.uniqueCodeZone) || ''}}</span></li>
                <li><span>通信地址：</span><span>{{formItems.address || ''}}</span></li>
                <li><span>联系电话：</span><span>{{formItems.telephone || ''}}</span></li>
                <li><span>联系人：</span><span>{{formItems.contacts || ''}}</span></li>
                <li><span>简介：</span><span>{{formItems.remarks || ''}}</span></li>
              </ul>
            </div>
      </div>
    </div>
    <EditModule @save="saveData" @register="registerEditModule" />
  </div>
</template>
...
import EditModule from "./popup/edit.vue";
...
const [registerEditModule, {openModal: openEditPage}] = useModal()
onMounted(async () => {
...
import {
  LoadingOutlined, PlusOutlined, AppstoreOutlined,CarryOutOutlined,SettingOutlined
...
import {
  ...
  Cascader,
  Upload, message,Avatar,Rate,Divider
} from 'ant-design-vue'
...
function formatCountry(uniquePsnType: any){
...
function formatZoning(arr: any){
  let str = ''
  if (!hasBlank(arr)) {
    let list = JsonTool.parseProxy(zoningList.value)
     str= arr.map(s=>{
      let a = list.filter(it=>it.value==s)[0]
      if (null != a.children && a.children.length > 0)list = a.children
      return  a?.label
    }).join()
  }
  return str
...
function formatIndustry(arr: any){
  let str = ''
  if (!hasBlank(arr)) {
    let list = JsonTool.parseProxy(industryList.value)
     str= arr.map(s=>{
      let a = list.filter(it=>it.value==s)[0]
      if (null != a.children && a.children.length > 0)list = a.children
      return  a?.label
