<template>
  <div>
    <div class="app-container">
      <div style="width: 50%;height: 100%;display: inline-block;float: left;font-size: 22px;line-height: 70px;font-weight: bold;color: #0096c7;">
        ...
        <div class="container-head-title"  style="margin-top: 14px;">
          <b class="noneSpan">存货管理账套</b>
        </div>
        <span style="font-size: 12px;color: black;position: absolute;top: 50px;left: 25px">共{{totalNumber}}条记录</span>
      </div>
    ...
    <div class="app-container">
      ...
      <BasicTable
        ...
        <template #stockAccName="{ record }"><u class="tableUStyle" @click="openEdit(record,true)">{{
            record.stockAccName
          }}</u></template>
        <template #uniqueCode="{ record }"> {{ formatOrgnInCharge(record.uniqueCode) }}</template>
        <template #currency="{ record }"> {{
            formatCurrencyInCharge(record.currency)
          }}
        </template>
        <template #country="{ record }"> {{ formatDqInCharge(record.country) }}</template>
        <template #jjfs="{ record }"> {{ record.jjfs == '1' ? '实时' : '定时' }}计价</template>
...
import {encodingInit} from "/@/api/record/encoding_rules/FileEncodingRules";
import {fileInit} from "/@/api/record/encoding_rules_report/FileEncodingRules";
...
const CrudApi = {
  ...
  columns: [
    ...
    },
    {
      ...
      ellipsis: true,
      fixed: true,
      slots: {customRender: 'stockAccName'}
    },{
      title: '关联账套',
...
async function saveData(obj: any) {
  ...
  if ( null == data.id && !hasBlank(res.beiyong1)){ // 初始化 其他参数
     let tid = res.beiyong1 // 租户ID
     await useRouteApi(encodingInit, {schemaName: tid})({})
     await useRouteApi(fileInit, {schemaName: tid})({})
  }
  obj.closeOpen()
...
function onSearch() {
  ...
  if (hasBlank(pageParameter.searchConditon.value.trim())) {
    ...
    let searchAfter = searchDataSource.filter(item => item['coCode'].indexOf(v) != -1 || item['associateCoCode'].indexOf(v) != -1 || item['stockAccName'].indexOf(v) != -1 || (formatOrgnInCharge(item['uniqueCode'])).indexOf(v) != -1)
...
const openRecycleDel = async () => {
  // 获取回收站数据
  recycleList.value = await getStockDelAll()
  if (recycleList.value.length == 0){
    await pointMessage({title: '温馨提起', content: '当前回收站为空！', delay: true})
  }else {
    openRecyclePage(true, {
      dynamicTenantId: '',
      dataList: recycleList.value.map(it=>({id: it.id,code: it.coCode,name: it.stockAccName,delName: it.delName,delDate: it.delDate}))
    })
  }
}
const recycleModify = async (e) => {
  if (e.type == 'reduce'){
    ...
    const data = e.data
    let model = recycleList.value.filter(it => it.id == e.data.id)[0]
    await deleteStockTrue(data).then(async (res) => {
      if (null != res && res.isOk == false) {
        await pointMessage({
          title: '处理结果',
...
async function writeLog(action, a, content) {
  ...
  if (action == '修改') {
    ...
    let text = `操作内容【修改存货管理账套】,`+'账套代码【' + a.coCode + '】,账套名称【' + a.stockAccName + '】,'
  ...
  let map = {
    ...
    uniqueCode: '',
    optAction: action,
    optContent: content || `操作内容【${action}存货管理账套】,` + '账套代码【' + a.coCode + '】,账套名称【' + a.stockAccName + '】',
  }
  await saveLog(map)
}
