    ...
    <div class="app-container">
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="margin-left: 20px;">
          <b class="noneSpan"><UnorderedListOutlined />&nbsp;&nbsp;集团-项目分类</b>
        </div>
...
import {
  ...
  saveProClass,
  projectClassFindAll
} from '/@/api/record/system/group_project_class'
...
import {findByUniqueCode} from "/@/api/record/system/group-project-class-account";
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      if (res.length > 0){
        createWarningModal({
          iconType: 'warning',
          title: '删除',
          content: '<span style="font-weight: bold;color: red;">[' + item.projectClassName + ']</span>项目分类已分配不允许删除！'
        })
        return false
      }
    }
    const projectList: any = await findAllList()
    for (let i = 0; i < checkRow.value.length; i++) {
      const item = checkRow.value[i]
