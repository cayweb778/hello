...
import {findByCode, findByName, saveProjectItem} from "/@/api/record/system/group-project-item";
...
async function checkCode() {
  ...
  const res = await findByCode(formItems.value.itemCode)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
...
async function checkName() {
  ...
  const res = await findByName(formItems.value.itemName)
  if (res.length != 0) {
    createErrorModal({
      iconType: 'warning',
      title: '输入失败',
