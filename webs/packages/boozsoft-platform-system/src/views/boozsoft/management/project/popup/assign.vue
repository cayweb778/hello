      ...
      <template #title>
        <div style="display: flex;margin-top: 10px;" class="vben-basic-title">
          <span style="font-size: 24px;line-height:30px;">
            <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;集团授权分配 － 项目信息
          </span>
        </div>
      </template>
...
import {findAllList} from "/@/api/record/system/group-project";
import {findProClassByCode} from "/@/api/record/system/group_project_class";
import {findByOriginIdAndUniqueCode,save as saveProClassAccount} from "/@/api/record/system/group-project-class-account";
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await findAllList()
...
async function reloadProject(){
  ...
  const projectAccount = await findByOriginId(originId.value)
  projectAccountList.value = projectAccount.items
  if(activeKey.value==1){
    tableDataAll.value = projectAccountList.value
  } else {
    tableDataAll.value = projectList.value.filter(item=>{
      return projectAccount.items.map(aa=>aa.uniqueCode).indexOf(item.uniqueCode) ==-1
    })
  }
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await save(item1)
      const projectClass = await findProClassByCode(item.projectClassCode)
      if (projectClass.length>0){
        const res = await findByOriginIdAndUniqueCode(originId.value,projectClass[0].uniqueCode)
        if (res.length == 0){
          const item2 = {
            id: null,
            uniqueCode: projectClass[0].uniqueCode,
            ctype: '1',
            originId: originId.value,
            assignUser: useUserStoreWidthOut().getUserInfo.id,
            assignDate: dateTime,
          ...
          await saveProClassAccount(item2)
        }
      }
    }
