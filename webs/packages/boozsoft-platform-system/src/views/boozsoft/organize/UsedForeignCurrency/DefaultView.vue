<template>
  <div class="app-container">
    ...
    <BasicTable
      ...
      <template #orgUniqueCode="{ record }">
          {{formatOrg(record.orgUniqueCode)}}
      </template>
...
function createColumn({label,field,slots}){
  return    {
    key: '0',
    title:label,
    dataIndex: field,
...
const CrudModel=createCurdModel({
  ...
  columns: [
    ...recordData.value.columns.map(it=>{
      return createColumn({label:it.label,field:it.field,slots: it.slots})
    })
  ]
})
...
function formatOrg(psnInCharge: any) {
  let str = psnInCharge
  orgList.value.forEach(
    function (psn: any) {
