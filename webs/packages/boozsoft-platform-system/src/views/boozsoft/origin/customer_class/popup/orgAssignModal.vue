...
import {findAllOrgCusClass, saveAll} from "/@/api/org/cus_class/org_cus_class";
import {
...
import {customerSaveApi, customerSaveCusClass} from "/@/api/record/system/customer_class";
...
async function findByOrgPermission() {
 let findByPermission= await findByUserIdAndTenantId(useUserStoreWidthOut().getUserInfo.id,database.value,'customer_class')
...
async function handleOk() {
  ...
  for (let i = 0; i < [...new Set(arr)].map(a=>a.cusCclassName).length; i++) {
    let log='操作内容【组织客户分类分配】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类名称【'+[...new Set(arr)].map(a=>a.cusCclassName)[i]+'】'
    /************** 记录操作日志 ****************/
    let logmap={
  ...
  if(orgSaveFlag.value==='1'){
    [...new Set(arr)].forEach(async (a)=>{
      let log='操作内容【分配客户分类】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类编码【'+a.cusClass+'】,分类名称【'+a.cusCclassName+'】'
      /************** 记录操作日志 ****************/
      let logmap={
        loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
        userId: useUserStoreWidthOut().getUserInfo.username,
