...
<!--            <span class="p_specifics group-btn-span-special2" @click="openAddPage()"><PlusOutlined/> 添加</span><br/>
            ...
            <span class="p_specifics group-btn-span-special2" @click="openInto()"><ShareAltOutlined/> 引用</span><br/>
...
import {
  ...
  saveDefinedFile
} from "/@/api/record/system/origin-defined-file";
import {
  deleteDefinedRecord,
...
async function fetch() {
  let classTree = await findAllDefinedFileList(originId.value)
  ...
  a(classTree)
  let classTree0 = await findByModelList({scope:'1',model:'',originId:originId.value})
  let classTree1 = await findByModelList({scope:'0',model:'ZW',originId:originId.value})
  let classTree2 = await findByModelList({scope:'0',model:'FA',originId:originId.value})
  let classTree3 = await findByModelList({scope:'0',model:'STOCK',originId:originId.value})
  a(classTree0)
  a(classTree1)
  a(classTree2)
  a(classTree3)
...
async function del() {
  if (selectedKeys.value.length > 0 && selectedKeys.value[0] != '0' && selectedKeys.value[0] != 'ZW' && selectedKeys.value[0] != 'FA' && selectedKeys.value[0] != 'STOCK') {
    ...
    if (items[0].shuxing!='5') {
      const res = await findAllDefinedRecordList(items[0].definedCode,originId.value)
      fileList.value = res
      for (let i = 0; i < fileList.value.length; i++) {
        const file: any = fileList.value[i];
        // console.log(file)
        const item1 = await findByOriginIdAndUniqueCode(originId.value,items[0].definedCode)
        if (item1.length>0) {
          item1[0].flag = '0'
          item1[0].acceptUser = ''
          item1[0].acceptDate = ''
