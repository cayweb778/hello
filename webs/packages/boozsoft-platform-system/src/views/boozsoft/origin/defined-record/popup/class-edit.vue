...
async function handleOkAdd() {
  ...
  if (isChanged) {
    emit('save', unref(formItems))
    // closeModal()
    // await saveDefinedFile(formItems.value)
...
async function checkName() {
  ...
  const res = await findAllDefinedFileList(formItems.value.originId)
...
async function maxCode(){
  const item1 = await findBukongDefinedCode(formItems.value.originId)
  if (item1!=null && item1.definedCode!=null && item1.definedCode!=''){
    formItems.value.definedCode = pad(item1.definedCode,3)
  } else {
    const item2 = await findMaxDefinedCode(formItems.value.originId)
