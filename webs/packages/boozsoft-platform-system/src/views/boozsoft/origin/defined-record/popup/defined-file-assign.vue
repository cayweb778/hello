    ...
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权分配 － 自定义项
        </span>
      </div>
    </template>
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await getDefinedFileList(data.originId)
...
async function reloadDefinedFile(){
  ...
  const definedFileAccount = await findByTenantId(tenantId.value)
