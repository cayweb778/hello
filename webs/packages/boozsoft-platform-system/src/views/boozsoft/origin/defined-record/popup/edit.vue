...
import {findByCode as findFileByCode} from "/@/api/record/system/group-defined-file"
...
const [register, {closeModal}] = useModalInner(async (data: any) => {
  ...
  const res = await findFileByCode(data.data.definedCode)
...
async function maxCode(){
  let str = ''
  const item1 = await findBukongRecordCode(definedfile.value.definedCode,formItems.value.originId)
  if (item1!=null && item1.recordCode!=null && item1.recordCode!=''){
    str = item1.recordCode
  } else {
    const item2 = await findMaxRecordCode(definedfile.value.definedCode,formItems.value.originId)
