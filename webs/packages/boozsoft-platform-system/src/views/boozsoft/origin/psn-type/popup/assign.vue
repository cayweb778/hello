    ...
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权分配 － 人员分类
        </span>
      </div>
    </template>
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await psnTypeFindAll(data.originId)
...
async function reloadPsnType(){
  ...
  const psnTypeAccount = await findByTenantId(tenantId.value)
  psnTypeAccountList.value = psnTypeAccount.items
  // const res:any = await psnTypeFindAll()
  // psnTypeList.value = res.items
  if(activeKey.value==1){
    tableDataAll.value = psnTypeAccountList.value
  } else {
    tableDataAll.value = psnTypeList.value.filter(item=>{
      return psnTypeAccount.items.map(aa=>aa.uniqueCode).indexOf(item.uniqueCode) ==-1
    })
  }
  /*tableDataAll.value = res.items.filter(item=>{
    return item
  })*/
