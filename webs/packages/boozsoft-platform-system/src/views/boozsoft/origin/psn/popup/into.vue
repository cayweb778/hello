...
import {savePsn} from "/@/api/record/system/origin-psn";
import {psnTypeFindAll} from "/@/api/record/system/group-psn-type";
import {findByUniqueCode} from "/@/api/record/system/group-psn";
import {
  findByOriginIdAndUniqueCode,
  save as savePsnTypeAccount
} from "/@/api/record/system/group-psn-type-account";
import {savePsnType} from "/@/api/record/system/origin-psn-type";
...
async function reloadPsnType(){
  checkRow.value = []
  state.selectedRowKeys = []
  const psnTypeAccount = await findByOriginId(originId.value)
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      const item1 = await findByUniqueCode(item.uniqueCode)
      item1.id = null
