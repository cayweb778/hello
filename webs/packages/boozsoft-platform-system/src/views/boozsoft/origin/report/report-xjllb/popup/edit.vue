  ...
  <div v-show="showEdit" style="position: absolute;left:0;top:0;z-index:100;width: 100%;height:100%;background:#b4c8e3;overflow:auto;">
    <div class="app-container" style="padding:10px;height: auto">
      ...
      <div class="nc-open-content">
        <div class="open-content-up" style="margin-left: 5%;margin-right: 5%;">

          <label>模板名称:</label>
          <a-input v-model:value="itemConfig.base.templateName" placeholder="模板名称" style="width: 20%" />
          <label>会计准则:</label>
          <a-select v-model:value="itemConfig.base.accStandard" @change="standardCheck()" placeholder="会计准则" style="width: 20%">
...
const openAddPage = () => {
  ...
  openSelectKemuPage(true, {
    data: {
      ...
      formulaCount: '',
      formulaTable: [],
      originId: itemConfig.value.base.originId
    },
    table: table.value,
    hang: hang.value
  })
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'xjllb',titleName:itemConfig.value.base.titleName,originId:itemConfig.value.base.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
