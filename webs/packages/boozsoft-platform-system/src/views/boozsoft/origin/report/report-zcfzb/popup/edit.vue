...
<!--            <a-button @click="openAddFzPage()" style="padding: 0px 4px; height: 20px;background-color: rgb(1, 129, 226);color: #fff">
      ...
      <div style="margin-top: 20px;width: 100%; float: left;">
        ...
        <p>6、取值方式有三种：余额、借方余额、贷方余额，默认值取余额；</p>
      </div>
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'zcfzb',titleName:itemConfig.value.base.titleName,originId:itemConfig.value.base.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
