<template>
  ...
  >
      ...
      <div class="open-content-up">
        <div style="margin-left: 20px;margin-top: 53px;">
          ...
          <span style="margin-left: 23px;">
            ...
            <a-input v-model:value.trim="formItems.ccodeName" @blur="findByCodeName(formItems.ccodeName)" placeholder="科目名称" autocomplete="off" class="abc" style="font-size: 18px;width: 60%;" />
    ...
    <template #footer>
      ...
      <a-button :disabled="saveClick" @click="handleOk('add')">保存</a-button>
      <a-button @click="handleOk('jx')" :disabled="saveClick" type="primary">保存并新增</a-button>
    </template>
  </BasicModal>
</template>
<script setup="props, { content }" lang="ts">
  ...
  import { PlusCircleOutlined,CaretDownOutlined,LinkOutlined,PlusCircleTwoTone,DeleteTwoTone} from '@ant-design/icons-vue'
  import {
  ...
  const keLevel: any = ref('');                           // 0是一级科目，1是下级科目
  ...
  const [register, { closeModal }] = useModalInner(async (data) => {
    ...
    getSingleUnit()
    if(data.keLevel=='1'){
      superiorCcode.value=JSON.parse(data.superiorData).ccode
      findByCodePzQc(JSON.parse(data.superiorData))
    }
  });
  ...
  // 新下级科目编码
  const getNewCcode =async (code,val) => {
    ...
    if(!independent.value){
      ...
      if (code.lowerControl === '0') {
        ...
        return message.error('新增下级科目控制已开启,不能增加下级科目！')
      }
    }

    superCodeBend.value = code.bend;
    formItems.value.cclass = code.cclass;
    formItems.value.bprogerty = code.bprogerty;
    formItems.value.lowerControl = code.lowerControl;
    formItems.value.fuzhuControl = code.fuzhuControl;
    ...
    if(val!=undefined){
      ...
      formItems.value.ccode = newccode;
      // 获取上级科目级次
      jici2.value =  igrade;
    }
  }
  // 获取上级科目信息
  const getSuperData = async (superData) => {
    formItems.value=superData
    formItems.value.id=null
    formItems.value.ccodeName=''
    formItems.value.uniqueCode=''

    const fuzhu=await useRouteApi(findFuzhuHesuanList,{schemaName:database.value})('')
    // 获取上级辅助信息
    if (superData.fuzhu!==null && superData.fuzhu !== '') {
      fuzhuList.value=[]
      for (let i = 0; i < superData.fuzhu.split(',').length; i++) {
        let temp=fuzhu.items.filter(a=>a.cname==superData.fuzhu.split(',')[i])
        if(temp.length>0){
    ...
    // 新下级科目编码
    getNewCcode(superData,superData.ccode)
  }
  const handleOk = (type) => {
    // 获取是否预算科目标志
