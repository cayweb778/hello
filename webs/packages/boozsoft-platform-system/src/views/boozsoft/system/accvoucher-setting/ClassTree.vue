...
<!--    <div style="width: 100%;height: 50px;">
      <label style="font-size: 20px;line-height: 50px;margin-left: 20px;">凭证分类</label>
...
import {findAllApi as findSettingAllApi} from '/@/api/boozsoft/account/AccvoucherSetting'
...
async function del() {
  ...
  if (selectedKeys.value.length > 0 && selectedKeys.value[0] != '0') {
    let items: any = list.value.filter((item: any) => item.classCode == selectedKeys.value[0])
    const res = await useRouteApi(findSettingAllApi, {schemaName: dynamicCompany.value})({})
    ...
    for (let i = 0; i < accList.value.length; i++) {
      ...
      if (acc.classCode == items[0].classCode) {
        createWarningModal({
          iconType: 'warning',
          title: '删除',
          content: '<span style="font-weight: bold;color: red;">[' + items[0].className + ']</span>凭证分类中经已存在常用凭证，不能进行删除操作！'
        })
        return false
      }
    }
