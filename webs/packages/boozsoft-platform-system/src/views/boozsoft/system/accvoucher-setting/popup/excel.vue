...
import {findAllApi} from "/@/api/boozsoft/account/AccvoucherSetting";
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length > 0) {
    for (let i = 0; i < items.length; i++) {
      ...
      const item1: any = {}
      item1.ccode = item['编码']
      item1.remarks = item['凭证说明']
      item1.ctype = item['凭证类别']
      item1.cnum = item['附单据数']
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      if (item1.ccode == null || item1.ccode == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行编码为空,不能进行常用凭证信息导入！'
        })
        list.value = []
        return false
      }
      if (item1.remarks == null || item1.remarks == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行凭证说明为空,不能进行常用凭证信息导入！'
        })
        list.value = []
        return false
      }
      for (let j = 0; j < list.value.length; j++) {
        ...
        if (i != j) {
          ...
          if (item1.remarks != '' && item1.remarks != null && item1.remarks == item2.remarks) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行凭证说明信息与第' + (j+2) + '行的凭证说明重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      for (let j = 0; j < accList.value.length; j++) {
        ...
        if (item.ccode != '' && item.ccode != null && acc.ccode == item.ccode) {
          msg = '第' + (i + 2) + '行编码重复,不能进行常用凭证信息导入！'
          return false
        }
        if (item.remarks != '' && item.remarks != null && acc.remarks == item.remarks) {
          msg = '第' + (i + 2) + '行凭证说明重复,不能进行常用凭证信息导入！'
          return false
        }
      }
    }
...
function exportExcel() {
  const arrHeader = ['编码', '凭证说明', '凭证类别', '附单据数', '所属分类编码'];
  console.log(arrHeader)
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '常用凭证模板.xlsx',
  });
}
