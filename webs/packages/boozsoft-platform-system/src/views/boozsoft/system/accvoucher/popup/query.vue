<template>
  ...
  >
    ...
    <div
      ...
      style="height: 100%"
    >
      <SecondaryTitle :iconType="'query'">凭证列表</SecondaryTitle>
        ...
        <a-tabs type="card" v-model:activeKey="defaultTabsKey" @change="tabsChange"
            ...
            <ul>
              ...
              <li><span>外币币种：</span>
                <a-select show-search style="width: 200px"
                          v-model:value="modelList['1'].constant.currency"
                          :allowClear="true">
              ...
              <li v-if="startRendering" v-for="hsKey in targetKeys2 ">
                ...
                <a-input v-model:value="modelList['1'].variable.accounting[hsKey]"
                         class="addonAfter-input">
                  <template #addonAfter>
                    <PlusSquareOutlined
                      @click="openAssistPage(hsKey,modelList['1'].variable.accounting[hsKey])"/>
                  </template>
                </a-input>
              </li>
            </ul>
          </a-tab-pane>
          <!--          <a-tab-pane key="2" tab="跨年模式">
                      <ul>
                        <li>
                          <AccountPicker theme="three" @reloadTable="codeChange"/>
                        </li>
                        ...
                        <li>
                          <template v-if="targetKeys.indexOf('subjectNumber') != -1">
                            ...
                            <PaperClipOutlined style="color: #0a84ff;margin-left: 5px;cursor: pointer;" @click="openKemu"/>
                          </template>
                          <template v-if="targetKeys.indexOf('direction') != -1">
                            &emsp;&emsp;<span class="right_span">科目方向：</span>
                            <a-select
                        ...
                        <li v-if="startRendering" v-for="hsKey in targetKeys2 ">
                          <span>{{ getAssistNameByKey(hsKey, filterDatas2) }}核算：&emsp;</span>
                          <a-input v-model:value="modelList['2'].variable.accounting[hsKey]"
                                   class="addonAfter-input">
                            <template #addonAfter>
                              <PlusSquareOutlined
                                @click="openAssistPage(hsKey,modelList['2'].variable.accounting[hsKey])"/>
                            </template>
                          </a-input>
                        </li>
                      </ul>
          ...
          <a-tab-pane key="3" tab="个人标准模式">
            <ul>
              ...
              <li v-if="startRendering" v-for="hsKey in targetKeys2 ">
                <span>{{ getAssistNameByKey(hsKey, filterDatas2) }}核算：&emsp;</span>
                <a-input v-model:value="modelList['3'].variable.accounting[hsKey]"
                         class="addonAfter-input">
                  <template #addonAfter>
                    <PlusSquareOutlined
                      @click="openAssistPage(hsKey,modelList['3'].variable.accounting[hsKey])"/>
                  </template>
                </a-input>
              </li>
            </ul>
...
import {
  ...
  FileSearchOutlined,
  SearchOutlined,
  PicLeftOutlined, PaperClipOutlined,AppstoreOutlined
