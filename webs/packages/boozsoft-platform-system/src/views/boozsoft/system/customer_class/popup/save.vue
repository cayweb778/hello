...
<script setup="props, { content }" lang="ts">
  ...
  import {findAllByUniqueCustclass} from "/@/api/record/costomer_data/customer";
  ...
  async function handleOk() {
    ...
    // 埋点-操作日志
    let log='操作内容【新增客户分类】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,分类编码【'+formItems.value.cusClass+'】,分类名称【'+formItems.value.cusCclassName+'】'
    /************** 记录操作日志 ****************/
    let logmap={
    ...
    if(!hasBlank(val)){
      ...
      if(data.length>0){
        createConfirm({
          iconType: 'warning',
          title: '警告',
          content: '上级分类已有客户档案,继续添加将自动修改成新分类下，确认要添加吗？',
          onOk: async () => {},
          onCancel: () => {
            formItems.value.parentId=""
            return false
