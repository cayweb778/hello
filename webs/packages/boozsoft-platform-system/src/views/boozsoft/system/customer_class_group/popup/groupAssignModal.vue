...
import {
  ...
  findAllCustomerClass, findByLikeCusGradeCode,
  findByOrgAssigAccount, findByRepeat
} from "/@/api/record/system/customer_class_group";
import {getOrganizeList} from "/@/api/record/group/im-organize";
import {useUserStoreWidthOut} from "/@/store/modules/user";
import {saveAll} from "/@/api/org/cus_class/org_cus_class";
...
async function findByOrgPermission() {
  // 集团档案权限分配表-集团客户分类
  let findByPermission=await findByUserIdAndOriginId(useUserStoreWidthOut().getUserInfo.id,orgUnique.value,'customer_class')
  ...
  createConfirm({
    ...
    onOk: async () => {
      ...
      for (let i = 0; i < [...new Set(arr)].map(a=>a.cusCclassName).length; i++) {
        let log='【集团客户分类分配】,分类名称【'+[...new Set(arr)].map(a=>a.cusCclassName)[i]+'】'
        /************** 记录操作日志 ****************/
        let logmap={
      ...
      if(orgSaveFlag.value==='1'){
        ...
        // 埋点-操作日志
        for (let i = 0; i < [...new Set(arr)].map(a=>a.cusCclassName).length; i++) {
          let log='【集团设置自动分配-组织客户分类分配】,分类名称【'+[...new Set(arr)].map(a=>a.cusCclassName)[i]+'】'
          /************** 记录操作日志 ****************/
          let logmap={
            loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
            userId: useUserStoreWidthOut().getUserInfo.username,
