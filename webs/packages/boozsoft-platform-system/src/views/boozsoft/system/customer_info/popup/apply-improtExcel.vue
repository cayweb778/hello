    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">客户信息</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
import {applyImportCus, duLiSaveApi, importCus} from "/@/api/record/costomer_data/customer";
...
const handleDownByData = async (val: any) => {
  ...
  const columns2 = ['客户编码','客户全称','客户简称','税号','分类名称','母公司','对应供应商','省','市','区','详细地址','电话','手机','网站','EMail','行业性质','开户银行','开户地','银行账户','银行行号']
  aoaToSheetXlsx({
    data: [],
      ...
      if (info[0].code === '200' || info[0].error === 'error') {
        ...
        info[0].list.forEach(async (tx)=>{
          ...
          let map={
            ...
            optAction:'新增申请',
            optContent:'操作内容【导入客户信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,客户编码【'+tx.custCode+'】,客户全称【'+tx.custName+'】,客户简称【'+tx.custAbbname+'】',
          }
          await saveLog(map)
          // 自动审批开启，组织客户表、集团客户分配表插入
          if(orgSaveFlag.value=='1'){
            ...
            .then(async (a)=>{
              ...
              let map={
                ...
                optAction:'新增',
                optContent:'操作内容【账套导入自动审批客户信息】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,客户编码【'+tx.custCode+'】,客户全称【'+tx.custName+'】,客户简称【'+tx.custAbbname+'】',
              }
              await saveLog(map)
            })
            // 往账套客户信息表增加
            await useRouteApi(duLiSaveApi,{schemaName: tenantId})(tx)
              .then(async (a)=>{
                ...
                let map={
                  ...
                  uniqueCode:tenantId.value,
                  optAction:'新增',
                  optContent:'操作内容【账套导入自动审批客户信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,客户编码【'+tx.custCode+'】,客户全称【'+tx.custName+'】,客户简称【'+tx.custAbbname+'】',
                }
                await saveLog(map)
              })
