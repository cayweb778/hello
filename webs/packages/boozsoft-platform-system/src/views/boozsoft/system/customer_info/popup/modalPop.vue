<template>
  ...
  >
    ...
    <div style="background-color: #158eb8;height: 50px;padding-top: 10px;padding-left:10px;border-radius: 5px;text-align: left;">
    ...
    <div style="display: inline;width: 69.5%;float: right;margin-top: 5px;">
      <BasicTable @register="registerTable" :scroll="{ y: 400 }" :class="'a-table-font-size-12'"
                  :row-selection="{ type: multipleFlag?'checkbox':'radio',selectedRowKeys: selectedRowKeys, onChange: onSelectChange}"
...
import {customerAddApi, duLiSaveApi, findAll} from "/@/api/record/costomer_data/customer";
...
async function findByCusClass() {
  ...
  if(temp!==''){
    let cusClassAuthorData=await getCusClassAuthorData({tableName:'customer_class',userId:useUserStoreWidthOut().getUserInfo.id,accId:database.value})
    if(cusClassAuthorData.length==1){
      ...
      let deptTree=await getCusClassAuthorData({tableName:'customer_class',userId:useUserStoreWidthOut().getUserInfo.id,accId:database.value})
...
async function findByCusAuthData() {
  ...
  if(temp!==''){
    let temp=await getCusDataAuthorData({tableName:'customer',userId:useUserStoreWidthOut().getUserInfo.id,accId:database.value,classVal:pageParameter.uniqueCustclass,searchConditon: pageParameter.searchConditon,})
    setTableData(temp)
    // 底部分页信息
