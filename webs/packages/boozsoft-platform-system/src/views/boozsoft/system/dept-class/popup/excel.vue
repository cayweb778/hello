    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">部门支出经济分类</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
import {getDeptList, GetZfTreeByFlag} from "/@/api/record/system/dept-class";
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.ecCode = item['分类编码']
      item1.ecName = item['分类名称']
      item1.pname = item['上级名称']
      item1.zfname = item['政府分类']
      item1.flag = item['状态']
      //匹配对应参数
      item1.bend = '1'
      //上级名称
      ...
      //政府名称
      const zfname = item1.zfname
      if (zfname!=null || zfname!=''){
        console.log(zfList.value)
        const zd = zfList.value.filter(v=> v.ecName === zfname)
        console.log(zd)
        if(zd){
          item1.zfCode = zd[0].id
        }else{
          createErrorModal({
            iconType: 'warning',
            title: '提示',
            content: '第'+(i+1)+'行政府名称分类系统不存在,不能进行信息导入'
          })
          list.value = []
          return false
        }
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行部门分类编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行部门分类名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.ecCode==item2.ecCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行部门分类编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.ecName==item2.ecName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行部门分类名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      for (let j = 0; j < deptList.value.length; j++) {
        ...
        if (dept.ecCode == item.ecCode) {
          msg = '第' + (i + 1) + '行部门分类编码重复,不能进行部门信息导入'
          return false
        }
        if (dept.ecName == item.ecName) {
          msg = '第' + (i + 1) + '行部门分类名称重复,不能进行部门信息导入'
          return false
        }
      }
    }
...
const [register, { closeModal }] = useModalInner((data) => {
  ...
  useRouteApi(GetZfTreeByFlag,{schemaName: dynamicTenantId})().then(res=>{
    zfList.value = res
  })
...
function exportExcel() {
  const arrHeader = ['分类编码','分类名称','政府分类','上级名称'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '部门支出经济分类导入模板.xlsx',
  });
}
const handleClose = () => {
  closeModal()
