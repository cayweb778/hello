  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;">
          <PlusCircleOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;支出经济分类
        </span>
      </div>
    ...
    >
      <div class="open-content-up" style="text-align: center;margin-top: 13%;">
        ...
        <a-input v-model:value="formItems.ecCode" @blur="changeCode" style="width: 45%;" :maxlength="codeLength" placeholder="编码规范：3-2-2"/>
        <span class="red_span">*</span>
        <br>
<!--
        <br><br>
        <label>分类名称：</label>
        <a-input v-model:value="formItems.ecName" placeholder=""/>
-->

        <label>政府分类：</label>
        <a-select
          v-model:value="formItems.zfCode"
          style="width: 50%;text-align: center;"
          placeholder="请选择政府分类"
          allow-clear
        >
          <a-select-option
            v-for="item in treeDataZf"
            :key="item.id"
            :value="item.id"
          >
            {{ item.title }}
...
import {getDeptList, GetDeptTreeByFlag, GetZfTreeByFlag,getMaxCode} from '/@/api/record/system/dept-class'
