  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;margin-top: 10px;margin-left: 10px;">
          <FormOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;部门信息
        </span>
      </div>
    ...
    <div class="nc-open-content" style="height: 100%" >
      <div class="open-content-up" style="text-align: center;margin-top: 60px;">
        <label style="font-size: 18px;margin-left: 0;width:110px;">部门名称：</label>
        ...
        >
          <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template>
          <a-select-option v-for="psn in psnList" :key="psn.uniqueCode" :value="psn.uniqueCode">
            {{ psn.psnCode }}-{{ psn.psnName }}
          </a-select-option>
        </a-select>
        <a style="font-weight: bold;font-size: 18px;"><LinkOutlined @click="openSelectPsn" /></a>
...
import {Input as AInput,Select as ASelect,Popover as APopover,TreeSelect} from "ant-design-vue";
...
function handleSearch(value) {
  psnList.value = psnLists.value.filter(item=>{
    if (value!=null && value!='' && value.length>0) {
      return item.psnCode.slice(0, value.length) == value || item.psnName.indexOf(value) != -1 || item.uniqueCode == value
    }
    return item
  })
}
function handleChange(value) {
  formItems.value.uniqueCodeUser = value
  psnList.value = psnLists.value.filter(item=>{
    if (value!=null && value!='' && value.length>0) {
      return item.psnCode.slice(0, value.length) == value || item.psnName.indexOf(value) != -1 || item.uniqueCode == value
    }
...
import SelectPsn from '../../department/popup/select-psn.vue'
...
function saveSelectPsn(data){
  psn.value = data
  formItems.value.uniqueCodeUser = data.uniqueCode
}
