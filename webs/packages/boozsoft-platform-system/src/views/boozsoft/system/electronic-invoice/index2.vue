<template>
  <div class="app-container">
          ...
          <div class="ant-btn-group">
            ...
            <a-popover placement="bottom">
              <template #content>
                ...
                <span class="group-btn-span-special" @click="openExcel()">批量导入</span><br/>
              </template>
              <button
                type="button"
            ...
            <button type="button" class="ant-btn ant-btn-me" ant-click-animating-without-extra-node="false" @click="openSelectPage"><span>凭证关联</span></button>
          ...
          <div style="display: inline-block;margin-left: 1%;font-size: 14px;width: 30%">
            <div style="display: inline-block;font-size: 14px;padding-top: 8px">
              ...
              >
                ...
                <a-select-option   style="width: 180px;font-size: 14px;"  value="0">
                   销项发票
                </a-select-option>
              </a-select>
            </div>
    ...
    <div class="app-container-bottom" :style="{height: (windowHeight+60)+'px',marginTop: '5px'}">
      <!--  针对过滤框显示添加的内容高度  -->
      <VoucherTree v-show="showTree" :treeInterval="treeInterval" :database="databasesName"
                   @select="treeFilter"/>
      ...
      >
        <template #number="{record,index }">
          ...
          <template #accuniqueCode="{record}">
            <span >{{ getData(record.csign,record.inoid) }}</span>
          </template>
...
<script setup lang="ts">
  ...
  import Query from "/@/views/boozsoft/system/electronic-invoice/popup/query.vue";
  import Identify from "/@/views/boozsoft/system/electronic-invoice/popup/identify.vue";
  import VoucherTree from "./popup/VoucherTree.vue";
  import Pzselect from "./popup/select.vue";
  ...
  import {
    ...
    eletronicInvoiceSaveApi, delInvoice, djStatus, rzStatus, delByIds, pzAssociationFp
  ...
  const CrudApi = {
    list: useRouteApi(findAllElectronicInvoice,{schemaName:databasesName}),
  ...
  const [registerSelectPage, { openModal: selectPage }] = useModal()
  ...
  const fpIdentify = () => {
    val.openOne = 0
    fpIdentifyPage(true, {
      databasesName: databasesName.value
    })
  }
  ...
  const openSelectPage = () => {
    ...
    if (checkRow.value.length > 0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        ...
        if(item.accuniqueCode){
          message.error('不能选择已被关联发票！')
          return false
        }
      }
      val.openOne = 0
  ...
  const saveGl= (data) => {
    ...
    let arr = []
    for (let i = 0; i < checkRow.value.length; i++) {
      const item = checkRow.value[i].id
      arr.push(item)
    }
    //保存数据 ceshi
    useRouteApi(pzAssociationFp,{schemaName:databasesName})({
      pzId: data,
      ids: arr
    })
  }
  ...
  /*栏目设置end */
  const loadPage = (data)=>{
    databasesName.value = data.databaseTrue
