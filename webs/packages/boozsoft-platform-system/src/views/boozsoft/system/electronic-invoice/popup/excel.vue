    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">发票</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
import {
  getDeptList,
  getFaPropertyList,
  getfaZhejiuMethodList, getUnitmeaList
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.ecCode = item['资产类别编码']
      item1.ecName = item['资产类别名称']
      item1.pname = item['上级名称']
      item1.aname = item['资产属性']
      item1.uname = item['计量单位']
      item1.serviceLife = item['使用年限(月)']
      item1.netSalvage = item['净残值率']
      item1.mname = item['折旧方法']
      item1.cardStyle = item['卡片样式']
      //匹配对应参数
      item1.bend = '1'
      //上级名称
      const pname = item1.pname
      if (pname!=null && pname!='' && typeof(pname) != undefined && typeof(pname) != 'undefined'){
        const d = deptList.value.filter(v=> v.ecName === pname)
        if(d){
          item1.parentId = d[0].id
        }else{
          createErrorModal({
            iconType: 'warning',
            title: '提示',
            content: '第'+(i+1)+'行上级名称系统不存在,不能进行信息导入'
          })
          list.value = []
          return false
        }
      ...
      //资产属性
      const aname = item1.aname
      if (aname!=null && aname!='' && typeof(aname) != undefined && typeof(aname) != 'undefined'){
        const d = zcsxList.value.filter(v=> v.peopertyName === aname)
        if(d){
          item1.assetAttId = d[0].id
        }
      }
      //计量单位
      const uname = item1.uname
      if (uname!=null && uname!='' && typeof(uname) != undefined && typeof(uname) != 'undefined'){
        const d = jldwList.value.filter(v=> v.unitName === uname)
        if(d){
          item1.unitId = d[0].id
        }
      }
      //折旧方法
      const mname = item1.mname
      if (mname!=null && mname!='' && typeof(mname) != undefined && typeof(mname) != 'undefined'){
        const d = zjffList.value.filter(v=> v.zjName === mname)
        if(d){
          item1.depMethodId = d[0].id
        }
      }
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行资产类别编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行资产类别名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.ecCode==item2.ecCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行资产类别编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.ecName==item2.ecName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行资产类别名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      for (let j = 0; j < deptList.value.length; j++) {
        ...
        if (dept.ecCode == item.ecCode) {
          msg = '第' + (i + 1) + '行资产类别编码重复,不能进行部门信息导入'
          return false
        }
        if (dept.ecName == item.ecName) {
          msg = '第' + (i + 1) + '行资产类别名称重复,不能进行部门信息导入'
          return false
        }
      }
    }
...
const jldwList:any = ref([])
const zjffList:any = ref([])
const zcsxList:any = ref([])
...
const [register, { closeModal }] = useModalInner((data) => {
  ...
  useRouteApi(getUnitmeaList,{schemaName: dynamicTenantId})().then(res=>{
    jldwList.value = res.items
  })
  useRouteApi(getfaZhejiuMethodList,{schemaName: dynamicTenantId})().then(res=>{
    zjffList.value = res.items
  })
  useRouteApi(getFaPropertyList,{schemaName: dynamicTenantId})().then(res=>{
    zcsxList.value = res.items
  })
...
function exportExcel() {
  const arrHeader = ['发票类型','发票日期','发票代码','发票号码','发票单位名称','价税合计','发票金额','发票税额'
    ,'校验码','发票种类','发票状态'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '发票导入模板.xlsx',
  });
}
