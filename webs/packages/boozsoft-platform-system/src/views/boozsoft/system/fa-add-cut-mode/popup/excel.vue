    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">增减方式</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
import {getDeptList,excelVoucherType} from '/@/api/record/system/fa-ac'
const {
  createErrorModal
} = useMessage()
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.ecCode = item['增减方式编码']
      item1.ecName = item['增减方式名称']
      item1.pname = item['上级名称']
      item1.isAccrual = item['方式类别']
      if(item1.isAccrual){
        item1.isAccrual = item1.isAccrual === '增'?'1':'2'
      }
      item1.flag = item['状态']
      //匹配对应参数
      item1.bend = '1'
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行增减方式编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      ...
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行增减方式名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.ecCode==item2.ecCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行增减方式编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.ecName==item2.ecName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行增减方式名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      for (let j = 0; j < deptList.value.length; j++) {
        ...
        if (dept.ecCode == item.ecCode) {
          msg = '第' + (i + 1) + '行增减方式编码重复,不能进行部门信息导入'
          return false
        }
        if (dept.ecName == item.ecName) {
          msg = '第' + (i + 1) + '行增减方式名称重复,不能进行部门信息导入'
          return false
        }
      }
    }
...
function exportExcel() {
  const arrHeader = ['增减方式编码','增减方式名称','上级名称','方式类别'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '增减方式导入模板.xlsx',
  });
}
const handleClose = () => {
  closeModal()
