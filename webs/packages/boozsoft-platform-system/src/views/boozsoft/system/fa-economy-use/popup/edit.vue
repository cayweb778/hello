  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;">
          ...
          <FormOutlined v-if="isEdit" style="font-size: 34px;font-weight: bold"/> &nbsp;&nbsp;经济用途
        </span>
      </div>
    ...
    >
      <div class="open-content-up" style="text-align: center;margin-top: 30px;">
        ...
        <label style="font-size: 18px;margin-left: 0;">用途名称：</label>
        <a-input v-model:value="formItems.bsName" placeholder="经济用途名称" class="abc" style="width: 65%;" />
        <span class="red_span">*</span>
        <br/><br/><br/>

        <label>用途编码:</label>
        <a-input v-model:value="formItems.bsCode" @blur="checkCode()" placeholder="经济用途编码" />
...
import {findBySettModes,findByBsName,editFlag} from "/@/api/record/system/fa-economy-use";
...
async function checkCode(){
  ...
  if(res.length!=0){
    createErrorModal({
      iconType: 'error',
      title: '提示',
      content: '经济用途编码已存在，请重新输入！'
    })
    // alert('经济用途编码已存在，请重新输入！')
