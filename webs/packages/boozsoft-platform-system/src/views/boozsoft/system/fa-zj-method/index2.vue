      ...
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        <div style="width: calc(100% - 250px); float: right;margin-left: 5px;">
          <BasicTable
            ...
            <template #zjName="{ record }">
              <span style="float: left" v-if="record.zjName!='' && record.zjName">{{ record.zjName }}</span>
            </template>
            <template #lformula="{ record }">
              <span style="float: left" v-if="record.lformula!='' && record.lformula">{{ record.lformula }}</span>
            </template>
            <template #eformula="{ record }">
              <span style="float: left" v-if="record.eformula!='' && record.eformula">{{ record.eformula }}</span>
            </template>
...
import {deleteDept, saveDept, editFlag, getDeptListById, GetDeptTree,excelVoucherType,getTotalData} from '/@/api/record/system/fa-zj'
...
const exportExcelNow = async () => {
  ...
  // debugger
  const arrHeader = ['折旧方法','月折旧率公式','月折旧额公式','类型','状态'];
  const columnList = ['zjName','lformula','eformula','zjType','flag']
  data.forEach(v=>{
    v.zjType = v.zjType === '1'?'系统':'自定义'
...
const loadPrint = (obj) => {
  ...
  data.forEach((item,index) => {
    let item1 = {}
    item1[0] = item.zjName
    item1[1] = item.lformula
    item1[2] = item.eformula
    item1[3] = item.zjType === '1'?'系统':'自定义'
