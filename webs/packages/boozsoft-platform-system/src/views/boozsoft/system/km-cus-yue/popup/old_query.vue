...
<!--            </a-checkbox-group>-->
          ...
          <a-tab-pane v-if="queryPlan!=null" key="1" tab="个人标准模式">
            <ul>
              ...
              <li>
                <span>币种：</span>
                <a-select v-model:value="bz" style="width: 198px;">
                  <a-select-option v-for="d in bzList" :value="d.id">
                    {{ d.currChName }}
                  </a-select-option>
                <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template></a-select>
                &nbsp;&nbsp;&nbsp;&nbsp;
                <span>字号：</span>
                <a-select v-model:value="fontSize" style="width: 174px;">
                  <a-select-option value="MAX"> 大号字体 </a-select-option>
                  <a-select-option value="MIN"> 小号字体 </a-select-option>
                <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template></a-select>
              </li>
...
async function handleOk() {
  if(kmList.value.length===0){
    return createConfirmPop('请先设置客户辅助科目')
  }
  if(deptList.value.length===0){
    return createConfirmPop('请先设置客户信息')
  }
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'科目客户余额表')
