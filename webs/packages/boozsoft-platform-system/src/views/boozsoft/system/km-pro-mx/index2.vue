<template>
  <div>
      ...
      <div class="app-container-top">
        <div style="width: 100%;margin-top: 5px;height: 100px;">
          ...
          <div style="width: 60%;float: right;margin-top: -40px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目项目明细账</span>
    ...
    <Query @save="loadPage" queryType="科目项目明细表" @register="registerQueryPage" />
...
import Query from "/@/views/boozsoft/system/km-pro-mx/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-pro-mx/popup/print.vue";
import { findAll,exportAll,exportAll2 } from '/@/api/record/km-pro-mx/data';
import {kmProMxGlStore} from "/@/api/record/km-pro-mx/proKmMxColumn";
//////////////////////////////// END /////////////////////////////////////////
...
const loadPage = async (data)=>{
  database.value=data.pageParameter.accId
...
function thisPageImport() {
  ...
  exportExcel(
    sheet,
    'xlsx',
    accNameAll.value + '-科目项目明细账-' + time.strDate + '-' + time.endDate,
  );
...
const loadPrint = async (obj) => {
  ...
  if (pageParameter.queryMark === 'J') {
    ...
    data.forEach((item, index) => {
      ...
      item1[4] = item.cdigest == null ? '' : setString(item.cdigest, 35);
    ...
    useNewPrint({ data: ['l', 'px', 'a4', true] }, (doc) => {
      ...
      doc.autoTable({
        head: [
          ...
          ['期间', '凭证号', '项目编码', '项目名称', '摘要', '借方', '贷方', '方向', '余额'],
        ],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
  ...
  if (pageParameter.queryMark === 'SJ') {
    ...
    data.forEach((item, index) => {
      ...
      item1[3] = item.ccodeName == null ? '' : setString(item.ccodeName, 13);
