        ...
        <a-tabs type="card" tabPosition="left" style="height: 400px" v-model:activeKey="currentTabsKey" @change="tabChange">
          <a-tab-pane key="" tab="标准模式">
            <ul>
              ...
              <li>
                <span>科目类型：</span>
                <a-select style="width: 170px" v-model:value="styleValue" @change="styleChange">
...
import {
  ...
  findBzAll,
  findCodeKmByPeriod, findDeptAll, findProjectCategory, findProjectClass, findProject,
...
async function handleChangeProjectCate() {
  ...
  maxDept.value=''
  //项目分类
  projectClassList.value = await useRouteApi(findProjectClass,{schemaName:database.value})({projectCate:projectCate.value,accId:database.value});
  // 项目
  deptList.value = await useRouteApi(findProject,{schemaName:database.value})({projectCate: projectCate.value,accId: database.value});
}

let isChanged = false;
async function handleOk() {
  if(kmList.value.length===0){
    message.error('请先设置个人辅助科目!');
    return;
  }
  if(projectCate.value===''){
  ...
  formItems.value.minDept = minDept.value===undefined?'':minDept.value;
  formItems.value.maxDept = maxDept.value===undefined?'':maxDept.value;
  formItems.value.km = minKm.value===undefined?kmList.value[0].ccode:minKm.value;
...
async function newfindAllCode(s,start) {
  ...
  return list.filter(ck=>ck.bitem==='1' && ck.bend==='1');
}
...
async function AllCondition() {
  ...
  //项目大类
  projectCateList.value = await useRouteApi(findProjectCategory,{schemaName:database.value})('');
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'科目项目明细账')
