    ...
    <div class="app-container-top">
      <div style="width: 100%;margin-top: 5px;height: 100px;">
        ...
        <div style="width: 60%;float: right;margin-top: -77px;">
          <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目个人余额表</span>
      ...
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        >
          ...
          <template #qcMd="{ record }">
            <span v-if="record.qcMd != 0">
              <a style="cursor: pointer" @click="goRouter(record.uniqueCode)">
        ...
        >
          <ul>
            <li>
              <span>---期初余额--------------------</span>
              <span style="color: black; font-weight: bold"> 期初借方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinQcMd"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMaxQcMd"
                  placeholder=""
            ...
            <li style="margin-top: 5%">
              <span style="color: black; font-weight: bold"> 期初贷方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinQcMc"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMaxQcMc"
                  placeholder=""
                  style="width: 95px; font-size: 10px; float: right"
                />
              </div>
            ...
            <li>
              <span>---本期发生--------------------</span>
              <span style="color: black; font-weight: bold"> 本期借方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinPzMd"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMaxPzMd"
                  placeholder=""
                  style="width: 95px; font-size: 10px; float: right"
                />
              </div>
            ...
            <li style="margin-top: 5%">
              <span style="color: black; font-weight: bold"> 本期贷方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinPzMc"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
            ...
            <li>
              <span>---期末余额--------------------</span>
              <span style="color: black; font-weight: bold"> 期末借方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinQmMd"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMaxQmMd"
                  placeholder=""
                  style="width: 95px; font-size: 10px; float: right"
                />
              </div>
            ...
            <li style="margin-top: 5%">
              <span style="color: black; font-weight: bold"> 期末贷方本币： </span>
              <div>
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMinQmMc"
                  placeholder=""
                  style="width: 95px; font-size: 10px"
                />
                ~
                <a-input
                  v-model:value="pageParameter.filterConditon.amountMaxQmMc"
                  placeholder=""
                  style="width: 95px; font-size: 10px; float: right"
                />
              </div>
...
<script setup lang="ts">
  ...
  import { findAll, exportAll, exportAll2 } from '/@/api/record/km-psn-yue/data';
  import Query from '/@/views/boozsoft/system/km-psn-yue/popup/query.vue';
  import Print from '/@/views/boozsoft/system/km-psn-yue/popup/print.vue';
  import { kmPsnYueGlStore } from '/@/api/record/km-psn-yue/kmPsnYueColumn';
  //////////////////////////////// END /////////////////////////////////////////
  ...
  // 跳转科目个人明细账
  async function goRouter(uniqueCode) {
    pageParameter.minDept = uniqueCode;
    pageParameter.bzName = pageParameter.bz;
    pageParameter.openOne = '2';
    pageParameter.dynamicTenantId = dynamicTenantId.value;
    router.push({
      path: '/zhongZhang/account-book/abf-personal-contacts/abfgc-kemu-mxtable',
      query: pageParameter,
    });
  }
  onMounted(async () => {
  ...
  const dynamicAdReload = async (obj) => {
    ...
    if (pageParameter.strDate !== '') {
      ...
      if (res != null && res.total > 0) {
        ...
        deptList.value = await useRouteApi(findPsnAll, { schemaName: obj.accId + '-' + obj.year })(
          '',
        );
      } else {
  ...
  function tableRow(row: any, num: any) {
    return row.number % 2 == 0 ? 'table-odd-striped' : null;
  }
  ...
  const filterSearch = async () => {
    ...
    if (qcmdmax != '' || qcmdmin != '') {
      if (
        ...
        (!isRealNum(qcmdmin) ||
          !isRealNum(qcmdmax) ||
          parseFloat(qcmdmin) == 0 ||
          parseFloat(qcmdmax) == 0)
      ) {
    ...
    if (qcmcmax != '' || qcmcmin != '') {
      if (
        ...
        (!isRealNum(qcmcmin) ||
          !isRealNum(qcmcmax) ||
          parseFloat(qcmcmin) == 0 ||
          parseFloat(qcmcmax) == 0)
      ) {
        message.warn('请输入数值类型值并且值不能为0！');
        closeFilterV();
        return false;
    ...
    if (pzmdmax != '' || pzmdmin != '') {
      if (
        ...
        (!isRealNum(pzmdmin) ||
          !isRealNum(pzmdmax) ||
          parseFloat(pzmdmin) == 0 ||
          parseFloat(pzmdmax) == 0)
      ) {
        message.warn('请输入数值类型值并且值不能为0！');
        closeFilterV();
        return false;
    ...
    if (pzmcmax != '' || pzmcmin != '') {
      if (
        ...
        (!isRealNum(pzmcmin) ||
          !isRealNum(pzmcmax) ||
          parseFloat(pzmcmin) == 0 ||
          parseFloat(pzmcmax) == 0)
      ) {
        message.warn('请输入数值类型值并且值不能为0！');
        closeFilterV();
        return false;
    ...
    if (qmmdmax != '' || qmmdmin != '') {
      if (
        ...
        (!isRealNum(qmmdmin) ||
          !isRealNum(qmmdmax) ||
          parseFloat(qmmdmin) == 0 ||
          parseFloat(qmmdmax) == 0)
      ) {
        message.warn('请输入数值类型值并且值不能为0！');
        closeFilterV();
        return false;
    ...
    if (qmmcmax != '' || qmmcmin != '') {
      if (
        ...
        (!isRealNum(qmmcmin) ||
          !isRealNum(qmmcmax) ||
          parseFloat(qmmcmin) == 0 ||
          parseFloat(qmmcmax) == 0)
      ) {
        message.warn('请输入数值类型值并且值不能为0！');
        closeFilterV();
        return false;
  ...
  function thisPageImport() {
    ...
    let columns: any = getColumns().filter((v) => v.title !== '累计发生');
    ...
    if (pageParameter.queryMark === 'J') {
      ...
      leftrow = ['A', 'B', 'C'];
      rightrow = ['D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'];
    } else {
      leftrow = ['A', 'B', 'C', 'D'];
      rightrow = ['E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P'];
    }
    const sheet = [
      {
        title: '科目个人余额表',
    ...
    exportExcel(
      sheet,
      'xlsx',
      accNameAll.value + '-科目个人余额表-' + time.strDate + '-' + time.endDate,
    );
  ...
  async function companyExportView() {
    ...
    let flg = 0;
    const columns: any = getColumns().filter((v) => v.title !== '累计发生');
    columns.forEach((v, index) => {
      //多级表头
      if (v.children) {
        multiHeader[0].push(v.title);
    ...
    exportExcel(
      sheet,
      'xlsx',
      accNameAll.value + '-科目供应商余额表-' + time.strDate + '-' + time.endDate,
    );
    compState.loading = false;
  }
  async function onlySheetExport() {
    ...
    let temp2 = getColumns().filter((v) => v.title !== '累计发生');
    ...
    let rightrow: any = [];
    //样式靠左列
    let leftrow: any = ['A', 'B', 'C', 'D'];
    if (pageParameter.queryMark === 'J') {
      rightrow = ['E', 'F', 'G', 'H', 'I', 'J'];
    } else if (pageParameter.queryMark === 'SJ' || pageParameter.queryMark === 'WJ') {
      leftrow = ['A', 'B', 'C', 'D', 'E'];
      rightrow = ['F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N'];
    } else {
      leftrow = ['A', 'B', 'C', 'D', 'E', 'F'];
      rightrow = ['G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R'];
    }
  ...
  const loadPrint = async (obj) => {
    ...
    if (pageParameter.queryMark === 'J') {
      ...
      data.forEach((item, index) => {
        let item1 = {};
        item1[0] = item.ccode == null ? '' : setString(item.ccode, 11);
        ...
        if (item.ccode != '合计') {
          ...
          if (index > 0 && index % 25 == 0) {
            ...
            item2[0] = '';
            item2[1] = '承前页';
            item2[2] = toThousandFilter(qcMdMoney);
            item2[3] = toThousandFilter(qcMcMoney);
            item2[4] = toThousandFilter(pzMdMoney);
            item2[5] = toThousandFilter(pzMcMoney);
            item2[6] = toThousandFilter(qmMdMoney);
            item2[7] = toThousandFilter(qmMcMoney);
      ...
      data.forEach((item, index) => {
        ...
        if (item.ccode != '合计') {
          ...
          if (index > 0 && index % 25 == 0) {
            ...
            item2[5] = toThousandFilter(a);
            item2[6] = pzNum == 0 ? '' : pzNum;
            item2[7] = toThousandFilter(pzMdMoney);
            item2[8] = toThousandFilter(pzMcMoney);
            item2[9] = '';
            item2[10] = qmNum == 0 ? '' : qmNum;
            item2[11] = toThousandFilter(b);
            printList.push(item2);
          } else {
            qcMdMoney = 0;
            qcMcMoney = 0;
      ...
      data.forEach((item, index) => {
        ...
        if (item.ccode != '合计') {
          ...
          if (index > 0 && index % 25 == 0) {
            ...
            item2[4] = toThousandFilter(a);
            item2[5] = pzNfrat == 0 ? '' : pzNfrat;
            item2[6] = toThousandFilter(pzMdMoney);
            item2[7] = toThousandFilter(pzMcMoney);
            item2[8] = '';
            item2[9] = qmNfrat == 0 ? '' : qmNfrat;
            item2[10] = toThousandFilter(b);
            printList.push(item2);
          } else {
            qcMdMoney = 0;
            qcMcMoney = 0;
