          ...
          <a-tab-pane key="" tab="标准模式">
            <ul>
              ...
              <li><span>部&emsp;&emsp;门：</span>
                ...
                >
                  <a-select-option v-for="d in deptList" :value="d.deptCode">
                    {{ d.label }}
                  </a-select-option>
                </a-select>
                <span>&emsp;~&emsp;</span>
...
<script setup="props, {content}" lang="ts">
  ...
  async function handleOk() {
    ...
    formItems.value.minDept = minDept.value
    formItems.value.maxDept = maxDept.value
    if(typeof(minDept.value) == 'undefined' || typeof(minDept.value) == undefined){
      formItems.value.minDept = deptList.value[0].deptCode
    }
    if(typeof(maxDept.value) == 'undefined' || typeof(maxDept.value) == undefined){
      formItems.value.maxDept =  deptList.value[deptList.value.length-1].deptCode
    }
    //科目 类别权限
    ...
    if(maxDept.value){
      deptlist = deptlist.filter(o =>  o.deptCode <= maxDept.value)
    }
    if(minDept.value){
      deptlist = deptlist.filter(o =>  o.deptCode >= minDept.value)
    }
  ...
  const handleFocusMaxDept = () => {
    ...
    if(maxDept.value){
      //
      deptList.value = deptList.value.filter( o => o.deptCode >= minDept.value)
    }else{
  ...
  async function getQueryPlan() {
    let queryPlanIngo= await findByQueryPlan(database.value,'项目部门表')
