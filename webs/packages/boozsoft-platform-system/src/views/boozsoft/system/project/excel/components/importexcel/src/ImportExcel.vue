<template>
  <div>
    ...
    <a-upload @change="handleInputClick" :file-list="fileList" :before-upload="()=>false">
      <a-button @click="clickQk()" style="background: #0096c7;color: #ffffff;border-radius: 5px;"  :loading="loadingRef">
        请选择导入的文件
      </a-button>
    </a-upload>
...
const loadMark = ref<Boolean>(false);
 ...
 */
async function handleInputClick({file}) {
  fileList.value=[]
  loadingRef.value = true;
  if(hasBlank(file.name)){loadingRef.value = false;return}
