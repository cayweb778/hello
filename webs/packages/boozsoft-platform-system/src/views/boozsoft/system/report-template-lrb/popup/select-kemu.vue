  ...
  >
    <div class="nc-open-content" style="min-height: 500px;">
      <div class="open-content-up">
        ...
        <label>栏目名称</label>
        <a-input v-model:value="formItems.base.columnShowName" placeholder="栏目名称" style="width: 35%" />
        <label>显示级次</label>
        <a-select v-model:value="formItems.base.jici" style="width: 35%">
        ...
        <label>行次</label>
        <a-input v-model:value="formItems.base.hangci" @blur="checkHangci()" placeholder="行次" style="width: 35%" />
        <label>计算方式</label>
        <a-select v-model:value="formItems.base.formulaMethod" @change="checkMethod()" style="width: 35%">
          <a-select-option value="">请选择</a-select-option>
          <a-select-option value="1">行次</a-select-option>
          <a-select-option value="2">科目</a-select-option>
        </a-select>
        <label>取值方式</label>
        <a-select v-model:value="formItems.base.valueMethod" :disabled="!showKemu" style="width: 35%">
          <a-select-option value="">请选择</a-select-option>
      ...
      <div>
        <div v-if="formItems.base.sysFlag!='1'">
          <a-button @click="createRow()" style="padding: 0px 4px; height: 20px;background-color: rgb(1,129,226);color: #ffffff">
            <PlusOutlined />
          </a-button>
        </div>
        ...
        <table v-show="showHangci" class="layui-table" id="tableId1" style="height: calc( 100% - 20px );margin-top: 20px;background-color:#f2f4f6;">
          ...
          <tr class="dy_table_thead_tr even">
            ...
            <th v-if="formItems.base.sysFlag!='1'" style="width: 100px;">操作</th>
          </tr>
          </thead>
          ...
          <tr class="odd" v-for="(row,rowIndex) in formItems.base.formulaTable">
            ...
            <td v-if="formItems.base.sysFlag!='1'" style="width: 100px;">
      ...
      <table v-show="showKemu" class="layui-table" id="tableId" style="height: calc( 100% - 20px );margin-top: 20px;background-color:#f2f4f6;">
        ...
        <tr class="odd" v-for="(row,rowIndex) in formItems.base.formulaTable">
          <td style="text-align: left">
            <a-select v-model:value="row.ccode" @change="chengeKemu(row)" placeholder="会计科目" style="width: 100%;">
              <a-select-option v-for="kemu in kemuList" :key="kemu.ccode" :value="kemu.ccode">
...
// import {deleteFormula, findKemuByAccStandard} from "/@/api/record/system/report-template";
...
const [register, { closeModal }] = useModalInner( (data) => {
  ...
  //根据会计准则获取科目信息
  /*await useRouteApi(company_findByTemplateIdCode,{schemaName:defaultAdName})(data.kemuTemplateId)
    .then((res:any)=>{
    kemuList.value = res
  })*/
  company_findByTemplateIdCode(data.kemuTemplateId)
    .then((res:any)=>{
      kemuList.value = res
    })
