    ...
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;生成资产负债表</span>
        </div>
    ...
    columnList.forEach(column => {
      ...
      accvoucherList.forEach(item => {
        if (column.id == item.columnId) {
          ...
          if (column.formulaMethod == '1') {//行次计算
            if (item.fuhao == '+') {
              nianchu = add(nianchu, columnList.filter(item1 => {
                return item.ccode == item1.hangci
              }).map(item1 => item1.nianchuMoney))
            } else {
              nianchu = sub(nianchu, columnList.filter(item1 => {
                return item.ccode == item1.hangci
              }).map(item1 => item1.nianchuMoney))
            }
          }
      ...
      qimoList.forEach(item => {
        if (column.id == item.columnId) {
          ...
          if (column.formulaMethod == '1') {//行次计算
            if (item.fuhao == '+') {
              qimo = add(qimo, columnList.filter(item1 => {
                return item.ccode == item1.hangci
              }).map(item1 => item1.qimoMoney))
            } else {
              qimo = sub(qimo, columnList.filter(item1 => {
                return item.ccode == item1.hangci
              }).map(item1 => item1.qimoMoney))
            }
          }
