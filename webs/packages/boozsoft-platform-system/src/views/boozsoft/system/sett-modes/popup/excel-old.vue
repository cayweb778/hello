    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span style="font-size: 16px;">导入内容：</span><span style="font-weight: bold;font-size: 16px;">结算方式导入</span><br/><br/>
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length > 0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.settModesCode = item['结算方式编码']
      item1.settModesName = item['结算方式名称']
      item1.flag = '1'
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      //判断结算方式编码是否为空
      const settModesCode = item1.settModesCode
      if (settModesCode == null || settModesCode == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行结算方式编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      //判断结算方式名称是否为空
      const settModesName = item1.settModesName
      if (settModesName == null || settModesName == '') {
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第' + (i + 2) + '行结算方式名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      for (let j = 0; j < list.value.length; j++) {
        ...
        if (i != j) {
          if (item1.settModesCode == item2.settModesCode) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行结算方式编码与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.settModesName == item2.settModesName) {
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第' + (i+2) + '行结算方式名称与第' + (j+2) + '行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function checkExcel(){
  ...
  for (let i=0; i<list.value.length; i++){
    ...
    //判断结算方式编码是否为空
    const settModesCode = item['settModesCode']
    if (settModesCode==null || settModesCode==''){
      msg="第"+(i+2)+'行结算方式编码为空,不能进行信息导入'
      return false
    }
    //判断结算方式名称是否为空
    const settModesName = item['settModesName']
    if (settModesName==null || settModesName==''){
      msg="第"+(i+2)+'行结算方式名称为空,不能进行信息导入'
      return false
    }
    //根据导入类型导入数据是否重复
    for (let j = 0; j < SettModesList.value.length; j++) {
      const object = SettModesList.value[j];
      if (object['settModesCode'] == settModesCode) {
        msg = "第" + (i + 2) + '行结算方式编码重复,不能进行信息导入'
        return false
      }
      if (object['settModesName'] == settModesName) {
        msg = "第" + (i + 2) + '行结算方式名称重复,不能进行信息导入'
        return false
      }
    }
  }
