    ...
    <div class="app-container">
      <div class="app-container-top">
        <div class="app-container-top-one">
          ...
          <div class="act-one-d-title">
            <span class="acto-d-title-span">科目期初余额</span>
          </div>
      ...
      <div class="app-container-bottom" :style="{height: (windowHeight+60)+'px',display: 'inline',width: showCatalog?'calc( 100% - 305px )':'calc( 100% - 22px )',float: 'right',marginTop: '5px'}">
        <BasicTable
          ...
          :indexColumnProps="{width: 30, fixed: 'left', title: '序号'}"
          :pagination="{
          pageSize: 200,
        ...
        >
          <template #caozuo="{ record }">
            <!--          只有未停用的科目能编辑-->
            <span
              v-if="record.flag=='1' &&　(record.bnum=='1' || (null != record.currencyType && record.currencyType != '') || record.fuzhu.length > 0)">
             <Button :disabled="!roweditflg" style="border-color: white;" size="small"
                     v-if="record.bend === '1'"
                     type="link"
          ...
          <span class="a-table-font-arial"
                :style="(record.ndS < 0 && record.ncS == 0) || record.ncS < 0 && record.ndS == 0?{color: 'red'}:{}">{{
          ...
          <span class="a-table-font-arial"
                :style="(record.nfratMd < 0 && record.nfratMc == 0) || record.nfratMc < 0 && record.nfratMd == 0?{color: 'red'}:{}">{{
...
import {
  ...
  qcjzsum,
  pzjzibook,
  emptyAll, findAllSubjectInitialBalanceFuZhuList, saveTaskInfo,
...
import {findCodeKmByPeriod, findYearMinDate} from "/@/api/record/generalLedger/data";
    ...
    if (accvoucheribook === 0) {
      ...
      if (task1.length === 0) {
        ...
        for (let i = 0; i < task1.length; i++) {
          if (task1[i].functionModule === '凭证记帐' && task1[i].caozuoUnique != null) {
            if (task1[i].state === '0') {
              aa = false;
              message.error(
                '发现操作员【' +
