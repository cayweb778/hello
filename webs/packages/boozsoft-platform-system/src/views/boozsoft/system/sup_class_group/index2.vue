...
<!--        <div style="display: inline-block;position:absolute;top:85px;left:110px;font-size: 10px;color:#999999;">共 {{tableTotal}} 条记录</div>-->
        <div style="float: right; margin-left: 10px;">
          <a-button style="padding: 0px 12px !important; margin-right: 10px" @click="pageReload">
    ...
    <div class="app-container">
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        ...
        <div style="width: calc(100% - 250px); float: right;margin-left: 5px;">
          <BasicTable @fetch-success="fetchSuccess" @register="registerTable" :scroll="{ y: windowHeight }" :class="'a-table-font-size-12'" :bordered="true" v-if="tableshow">
      ...
      onOk: async () => {
        await delGroupCustomerClass(getSelectRows()).then(async (data)=>{
          ...
          getSelectRows().forEach(async (a)=>{
            let log='【删除供应商分类】,分类编码【'+a.cusClass+'】,分类名称【'+a.cusCclassName+'】'
            /************** 记录操作日志 ****************/
            let logmap={
