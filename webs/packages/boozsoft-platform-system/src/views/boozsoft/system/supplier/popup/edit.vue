<template>
  ...
  >
    ...
    <div :class="isEdit?'nc-open-content':'nc-open-show-content'" style="height: 100%; overflow: hidden;margin-top: 20px;">
      ...
      <div class="open-content-down" style="margin-top: 25px;">
        <a-tabs v-model:activeKey="activeKey" type="card">
            ...
            <div class="content-down-tab">
              <div class="down-tab-content">
                ...
                >
                  ...
                  <a-select-option value="最高价">最高价</a-select-option>
                  <a-select-option value="最低价">最低价</a-select-option>
                  <a-select-option value="一级采购价">一级采购价</a-select-option>
                  <a-select-option value="二级采购价">二级采购价</a-select-option>
                  <a-select-option value="三级采购价">三级采购价</a-select-option>
                  <a-select-option value="四级采购价">四级采购价</a-select-option>
                  <a-select-option value="五级采购价">五级采购价</a-select-option>
                  <a-select-option value="六级采购价">六级采购价</a-select-option>
                  <a-select-option value="七级采购价">七级采购价</a-select-option>
                  <a-select-option value="八级采购价">八级采购价</a-select-option>
                  <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template>
                </a-select>
                <label>信用控制：</label>
                <a-radio-group v-model:value="formItems.creditMommand" >
                  <a-radio value="0" style="width:100px;">
                    <span style="color: #000000;font-weight: bold;font-size: 10px;">不控制</span>
                  </a-radio>
                  <a-radio value="11" style="margin-left: -10px;">
          ...
          <a-tab-pane key="7" tab="收款方式">
            <div class="content-down-tab">
              <div class="down-tab-content">
                <a-radio-group v-model:value="formItems.payType" @change="payChange">
                  <a-radio value="QT">其他</a-radio>
                  <a-radio value="QKXJ">全款现结</a-radio>
                  <a-radio value="DJ">订金</a-radio><br>
                  <a-radio value="XQSK" style="display: inline">
                    限期收款
                    &emsp;&emsp;&emsp;
                    销货日期后第多少天：<a-input v-model:value="formItems.payNumber1" onkeyup="value=value.replace(/\D/g,'')" onafterpaste="value=value.replace(/\D/g,'')" style="width:100px;" autocomplete="off" />天
                  </a-radio><br>
                  <a-radio value="YJ" style="display: inline">
                    月结
                    &emsp;&emsp;&emsp;&emsp;&emsp;
                    销货月后第多少月：&emsp;<a-input v-model:value="formItems.payNumber2" onkeyup="value=value.replace(/\D/g,'')" onafterpaste="value=value.replace(/\D/g,'')" style="width:100px;" autocomplete="off" />月
                  </a-radio>
                </a-radio-group>
              </div>
            </div>
...
import {
  ...
  findAllByFlag,
  findAllCountry,
  findAllSysTradeNature, findBySupMaxNum,
  verifyByCustAbbname,
...
import {
  findAllByFlag as custfindAllByFlag,
  findByCustMaxNum
...
const psnList = ref([]);
  ...
  const [register, { closeModal }] = useModalInner(async (data) => {
    ...
    formItems.value.payNumber1=data.data.payType=='XQSK'?formItems.value.payNumber:''
    formItems.value.payNumber2=data.data.payType=='YJ'?formItems.value.payNumber:''
   ...
   await useRouteApi(findAllByCusBendEq1,{schemaName: database})('').then((res) => {
    ...
    oldformItems.value=formItems.value
...
const loadingRef = ref(false);
  ...
  async function handleOk(closeflag) {
    ...
    if(!hasBlank(formItems.value.payType)&&formItems.value.payType=='XQSK'&&hasBlank(formItems.value.payNumber1)){
      return createWarningModal({ content: '销货日期后第多少天,不能为空！' });
    }
    if(!hasBlank(formItems.value.payType)&&formItems.value.payType=='YJ'&&hasBlank(formItems.value.payNumber2)){
      return createWarningModal({ content: '销货月后第多少月,不能为空！' });
    }

    formItems.value.uniqueCustclass = uniqueCustclass.value;
    formItems.value.flag = custflag.value;
    formItems.value.payNumber = formItems.value.payType=='XQSK'?formItems.value.payNumber1:formItems.value.payType=='YJ'?formItems.value.payNumber2:''
...
async function getCodingRuleNum() {
  // 读取编码规则-存货档案
  let temp=await useRouteApi(load,{schemaName: database})('1-3')
  if(temp!==undefined||!hasBlank(temp)||temp.id!=null){
    // 自动编码
    if(temp.codeWay=='0'){
      let temp2=await useRouteApi(findBySupMaxNum,{schemaName: database})(temp)
...
function payChange(val) {
  if(val.target.value=='XQSK'){
    formItems.value.payNumber2=''
  }else if(val.target.value=='YJ'){
    formItems.value.payNumber1=''
  }else{
    formItems.value.payNumber1=''
    formItems.value.payNumber2=''
  }
