  ...
  >
    <template #title>
      <div style="display: flex;margin-top: 5px;" class="vben-basic-title">
        <PlusCircleOutlined v-if="!isEdit && !isLook" style="font-size: 30px;color: #0096c7;padding: 10px 0 10px 10px;"/>
        <FormOutlined v-else-if="isEdit && !isLook" style="font-size: 30px;color: #0096c7;padding: 10px 0 10px 10px;"/>
        <FileSearchOutlined v-else style="font-size: 30px;color: #0096c7;padding: 10px 0 10px 10px;"/>
...
<!--    v-if="null != policyModel && policyModel.pwdLock == 1"    -->
        <span class="red_span"><span>⋆</span></span>
...
const [register, {closeModal,setModalProps}] = useModalInner(async (data) => {
  ...
  formItems.value.createTime = data.data.createTime
  formItems.value.passwordOld = data.data.passwordOld
  formItems.value.passwordOldTime = data.data.passwordOldTime
  formItems.value.passwordTime = data.data.passwordTime
...
async function handleOk() {
  ...
  if (hasBlank(formItems.value.realName)) {
    ...
    showNext.value = true
  } else if((hasBlank(formItems.value.phone) && hasBlank(formItems.value.email)) && (await checkName())){
    showNext.value = true
  } else if (hasBlank(formItems.value.pwdPolicy)) {
    createWarningModal({title: '温馨提醒', content: "操作员密码策略为必选项！"});
    showNext.value = true
  } else if (hasBlank(formItems.value.effectiveDate)) {
    createWarningModal({title: '温馨提醒', content: "操作员生效日期为必选项！"});
    showNext.value = true
  } else if (hasBlank(formItems.value.invalidDate)) {
    createWarningModal({title: '温馨提醒', content: "操作员失效日期为必选项！"});
    showNext.value = true
...
async function checkName() {
  let value = formItems.value.realName
  // 检查数据库
  let data = {
    id: formItems.value.id || null,
    realName: value,
  ...
  return  await checkOperator(data)
}
