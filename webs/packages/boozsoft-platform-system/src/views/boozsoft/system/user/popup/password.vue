  ...
  >
    <template #title>
      <div style="display: flex;margin-top: 5px;" class="vben-basic-title">
        <KeyOutlined  style="font-size: 30px;color: #0096c7;padding: 10px 0 10px 10px;"/>
        <span style="line-height:50px;font-size: 30px;color: #0096c7;">&ensp;密码修改</span>
      </div>
...
<!--        <img src="/@/assets/images/cz.png" draggable="false" style="height:100px;margin-right: 10px;"/>-->
    ...
    <div
      ...
      style="height: 100%"
    >
      <div class="open-content-up" style="    padding-top: 25px;">
        <label>密码策略：</label>
        <span style="font-size: 20px;color: red;">{{policyModel?.pwdName || '未设置'}}</span>
<!--        <label>旧密码：</label>
        ...
        <a-input-password v-model:value="formItems.password1" ref="focus7"
                          ...
                          @keydown.enter.native="$refs.focus8.focus()" placeholder=""  @paste.native.capture.prevent="()=>{return false}" @copy.native.capture.prevent="()=>{return false}" @cut.native.capture.prevent="()=>{return false}"
                          style="width: 60%"/>
        ...
        <br/>
        <label>确认新密码：</label>
        <a-input-password v-model:value="formItems.password" ref="focus8"  @paste.native.capture.prevent="()=>{return false}" @copy.native.capture.prevent="()=>{return false}" @cut.native.capture.prevent="()=>{return false}"
...
const checkPwd = (value, type) => {
  ...
  if (null == policyModel.value) {
    createWarningModal({title: '温馨提醒', content: "请先设置操作员密码策略！"});
