<template>
  <BasicModal
    ...
    @register="register"
  >
    <div class="nc-open-content" style="height: 350px;">
...
<!--        <a-input v-model:value="formItems.fangxiang" placeholder="方向" />-->
        ...
        <a-select v-model:value="formItems.yewuUser" placeholder="业务员" style="width: 20%">
          <a-select-option v-for="item in psnList" :key="item.label" :value="item.label" >
            {{ item.label }}
          </a-select-option>
        </a-select>
        <br/>
...
<!--        <a-input v-model:value="formItems.chupiaoUnit" placeholder="出票单位" />-->
        <a-select v-model:value="formItems.chupiaoUnit" :disabled="formItems.fangxiang=='1'" placeholder="出票单位" style="width: 20%">
          <a-select-option v-for="item in cusList" :key="item.key" :value="item.key" >
            {{ item.label }}
          </a-select-option>
        </a-select>
...
<!--        <a-input v-model:value="formItems.paymentUnit" placeholder="收款单位" />-->
        <a-select v-model:value="formItems.paymentUnit" :disabled="formItems.fangxiang=='2'" placeholder="收款单位" style="width: 20%">
          <a-select-option v-for="item in supList" :key="item.key" :value="item.key" >
            {{ item.label }}
          </a-select-option>
        </a-select>
...
<script setup="props, { content }" lang="ts">
  ...
  const [register, { closeModal }] = useModalInner((data) => {
    ...
    useRouteApi(cusFindAllByFlag,{schemaName: dynamicTenantId})('1').then(res=>{
      cusList.value = res.items.map(item=>{
        const item1:any = {}
        item1.key=item.uniqueCode
    ...
    useRouteApi(supFindAllByFlag,{schemaName: dynamicTenantId})('1').then(res=>{
      supList.value = res.items.map(item=>{
        const item1:any = {}
        item1.key=item.uniqueCode
    ...
    useCompanyOperateStoreWidthOut().getAccountList.forEach(item => {
      if (item.uniqueCode==data.defaultAdName && item.accName!='' && item.accName!=null){
