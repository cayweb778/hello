...
export function generateA4L(doc, printModel, etcArr, maxRowLen) {
  ...
  doc.autoTable({
    head: [['记账凭证', '', '', ''],
      ['', '', '', ''],
      ['核算单位：' + etc.unit, '', '第  ' + etc.number + '  号 -  1/1', ''],
      ['摘要', '科目', '借方', '贷方']],
    body: printList,
    // startY: 60,
    styles: theGlobalStyle,
...
function getTwoStyleParameter(code: number) {
  ...
  switch (code) {
    case 6:
      model = {bodySize: 9, headSize: 10, etcSize: 10, headRowHeight: 16, bodyRowHeight: 22,colOneLength: 28,colTwoLength: 44}
      break;
    case 7:
      model = {bodySize: 9, headSize: 10, etcSize: 10, headRowHeight: 16, bodyRowHeight: 17,colOneLength: 28,colTwoLength: 44}
      break;
    case 8:
      model = {bodySize: 8, headSize: 10, etcSize: 10, headRowHeight: 16, bodyRowHeight: 14,colOneLength: 32,colTwoLength: 48}
      break;
    default:
      model = {bodySize: 10, headSize: 12, etcSize: 10, headRowHeight: 16, bodyRowHeight: 25,colOneLength: 26,colTwoLength: 38}
  }
  return model
}
...
function getFPStyleParameter(code: number,type: number) {
  ...
  switch (code) {
    case 6:
      model = {bodySize: 10, headSize: 12, etcSize: 10, headRowHeight: 16, bodyRowHeight: 26,colOneLength: type==4?28:22,colTwoLength: type==4?46:38}
      break;
    case 7:
      model = {bodySize: 10, headSize: 12, etcSize: 10, headRowHeight: 16, bodyRowHeight: 22,colOneLength: 28,colTwoLength: 46}
      break;
    case 8:
      model = {bodySize: 10, headSize: 12, etcSize: 10, headRowHeight: 16, bodyRowHeight: 18,colOneLength: 28,colTwoLength: 46}
      break;
    default:
      model = {bodySize: 10, headSize: 12, etcSize: 10, headRowHeight: 16, bodyRowHeight: 30,colOneLength: 28,colTwoLength: 46}
  }
  return model
}
...
function A4TwoConvert(items, isWb,varLimitModel,total) {
  ...
  items.forEach(it => {
    ...
    item1[0] = it.cdigest == null ? '' :it.cdigest  // !isWb?it.cdigest:`${it.cdigest}【${it.foreignCurrency || ''}/${it.ljSlMd || it.ljSlMc}】`
...
function A4FPConvert(items, isWb,varLimitModel,code,total) {
  ...
  items.forEach(it => {
    ...
    if (code == 5 || code == 6){
      item1[2] = ''//code == 5?`${it.mdF+'/' || ''}${it.ljWbMd=='0'?it.ljWbMc:it.ljWbMd}`:`${it.ndS=='0'?it.ncS:it.ndS}/${it.cunitPrice}`
  ...
  do {
    ...
    if (elem.done) {//不存在
      ...
      for (let i = 0; i < tnum; i++) {
        ...
        o.list = A4TwoConvert(todys, false,varLimitModel,sum)
        pageModel.push(o)
    ...
    if (elem.done) {//不存在
      ...
      for (let i = 0; i < tnum; i++) {
        ...
        if (todys.length < rNumber) { // 不足时 补充空白
          let cnum = rNumber - todys.length
          for (let j = 0; j < cnum; j++) {
            todys.push({mdF:'',ljMd: '0',ljMc: '0',ndS: '0',ncS: '0',md: '0.00', mc: '0.00'})
          }
        }
