  ...
  <div class="app-container">
    ...
    <PageWrapper dense content-full-height fixed-height content-class="flex">
      ...
      >
        ...
        <template #cashProject="{record,index }">
          <a-select
            show-search
            v-model:value="record.cashProject"
            placeholder=""
            style="width: 100%"
          >
            <template #suffixIcon><search-outlined /></template>
            <a-select-option  key="0" value="" label="01-收到的销售收入">
              01-收到的销售收入
            </a-select-option>
          </a-select>
        </template>
...
import {
  ...
  SettingFilled,
  SyncOutlined,
  FilterFilled,CheckOutlined,PrinterOutlined,UsbOutlined,EditOutlined,SearchOutlined
...
import { useAccvoucherStore } from '/@/store/modules/accvoucher-flow'
...
const pageParameter = reactive({
  ...
  filterMark: 'bcash',   // bcash , bbank ,equivalence
  condition: {},
  searchConditon: {
    requirement: 'inoId',
    value: '',
...
const lanMuData = {'accId':'','menuName':'凭证列表','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
