    ...
    <div class="app-container-top">
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 40%;text-align: center;margin-top: 20px;">
          <b class="noneSpan" style="font-size: 26px;">现金银行余额表</b>
          <div style="font-size: 14px;text-align: center;margin-top: 20px;">
          <span style="color: black;font-size: 14px;">
            期间：<span style="color: black;font-weight: bold">{{
              time.strDate
            }} - {{ time.endDate }}</span>
          </span>
          </div>
      ...
      <div>
        <div style="margin-top: -30px;margin-left: 90px;">
          ...
          <div style="display: inline-block;float: left;margin-left: 5px;font-weight: bold;color: #666666;">
            ...
            &emsp;&emsp;显示累计发生：<span style="color: black;font-weight: bold"><a-checkbox
...
async function findAll() {
  ...
  let num1 = tableDataAll.value.length%1000
  if (num1<50) {
...
onMounted(async () => {
  val.openOne = 1
  val.showRulesSize = pageParameter.showRulesSize
  openQueryPageM(true, {
    data: val
  })
  loadMark.value = true
...
function printData(){
  ...
  useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
    ...
    doc.autoTable({
      head: [['现金银行余额表','', '', '',  '', '', '', '','',''],
        ['核算单位：'+pageParameter.companyName, '', '', '截止期间:'+time.strDate+' - '+time.endDate, '', '', '', '币种：人民币','',''],
        ['科目编码', '科目名称', '期初余额', '', '本期发生', '', '累计发生', '', '期末余额', ''],
        ['', '', '本币借方', '本币贷方', '本币借方', '本币贷方', '本币借方', '本币贷方', '本币借方', '本币贷方']
      ],
      body: printList,
      // startY: 60,
      styles: tableStyle(),
...
const excelData = () => {
  ...
  arrData.forEach(item=>{
    let item1 = {}
    item1[0] = item.ccode == null ? '' : item.ccode
    item1[1] = item.ccodeName == null ? '' : item.ccodeName
    item1[2] = item.qcMd == null ? '' : item.qcMd
    item1[3] = item.qcMc == null ? '' : item.qcMc
    item1[4] = item.pzMd == null ? '' : item.pzMd
    item1[5] = item.pzMc == null ? '' : item.pzMc
    item1[6] = item.ljMd == null ? '' : item.ljMd
    item1[7] = item.ljMc == null ? '' : item.ljMc
    item1[8] = item.qmMd == null ? '' : item.qmMd
    item1[9] = item.qmMc == null ? '' : item.qmMc
    list.push(item1)
  })
  const sheet:any = [
    {
      ...
      merges: ['A2:C2', 'D2:F2', 'H2:J2','A3:A4','B3:B4','C3:D3','E3:F3','G3:H3','I3:J3'],
      sheetName: '现金银行余额表',
      cellStyle: [
        {
          cell: 'A1',
      ...
      colWidth: [10, 30, 12, 12, 12, 12, 12, 12, 12, 12]
    }
  ]
  // 处理数据前
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '现金银行余额表_'+pageParameter.companyName )
}
...
async function resetDynamicColumnData() {
  // 先从数据查询是否已经设置
  lanMuData.accId = defaultAdName.value
