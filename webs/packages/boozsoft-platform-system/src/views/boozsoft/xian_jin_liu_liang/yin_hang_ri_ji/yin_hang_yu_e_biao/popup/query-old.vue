...
<!--              <li
              ...
              ><span>样&emsp;&emsp;式：</span>
                <a-select default-value="1" style="width: 150px" @change="handleChange">
...
async function timechange() {
  ...
  if (riqi.value.length > 0) {
    ...
    kmList.value = await useRouteApi(findAllXjOrLlList, {schemaName: databaseTrue})({
      strDate: start,
      endDate: 'test',
    ...
    kmList.value = kmList.value.filter(item => {
      ...
      // return item.bend=='1' && item.bbank == '1'
      return item.bend == '1'
    })
  }
}
...
async function handleClose() {
  if (null != formItems.value.openOne && formItems.value.openOne == 1) {
    await closeCurrent();
    await router.push('/zhongZhang/home/welcome');
  }
}
...
//   }
// }
async function newfindAllCode(start) {
  // 会计科目
  let list = await useRouteApi(findAllXjOrLlList, {schemaName: databaseTrue})({
    strDate: start,
    endDate: 'test',
    accId: database.value,
    userId: userStore.getUserInfo.id
...
async function AllCondition() {
  ...
  //级次
  const num = await findMaxJc(database.value, year.value);
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'银行余额表')
