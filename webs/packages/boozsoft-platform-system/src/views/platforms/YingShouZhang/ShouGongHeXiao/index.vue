...
<!--    <div class="app-container">
      ...
      >
        ...
        <template #isum="{ record }">{{toThousandFilter(sub(record.isum,record.hxIsum))}}</template>
    ...
    <div class="app-container" style="height: 325px;padding-top: 10px;margin-top: 5px;">
      <span style="font-size: 18px;padding-left: 10px;color: #0096c7;font-weight: bold;">收款单</span>
...
import {
  findQuxiaohexiaoList, findWhxskd, saveSaleousing, saveStockXyCsource, saveWriteOff, saveYsyf,
...
async function reloadTable(){
  ...
  if (arSourceFlag.value!='1') {
    arBeginBalanceList.value = qcList.filter(item => item.busStyle == 'YSD')
  } else {
    arBeginBalanceList.value = qcList.filter(item => item.arStyle == 'YSD')
  }
  if (arBeginBalanceList.value.length>0) {
    ...
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收款期初余额',method:'编辑',caozuoModule:'ar'})
    } else {
      // 任务不是当前操作员的
      if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
        ...
        await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收款期初余额',method:'编辑',caozuoModule:'ar'})
      }
    }
  }
  const skdList = arBeginBalanceList.value.filter(item=>item.busStyle=='SKD')
  const ysdList = arBeginBalanceList.value.filter(item=>item.busStyle=='YSD')
  ...
  for (let i = 0; i < ysyfList.value.length; i++) {
    ...
    if(taskData.length==0){
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:pageParameter.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:danju,method:'审核',recordNum:item.ccode,caozuoModule:'ar'})
    } else {
      // 任务不是当前操作员的
      if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
        await balanceTaskDel()
