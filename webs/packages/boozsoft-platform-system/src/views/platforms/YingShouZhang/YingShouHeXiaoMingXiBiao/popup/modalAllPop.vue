...
import {
  ...
  company_findByAllCode,
  company_findByAllCodeTree,
  company_findByAllCodeTree2, company_findByYearsAllCode,
...
const [register, { closeModal }] = useModalInner( async (data) => {
  ...
  pageParameter.iyearList=data.iyearList
  findByAccStyleAll()
  reload()
});
...
async function findByAccStyleAll() {
  ...
  let map={ style:styleList.value.join(','),iyearList:pageParameter.iyearList }
  fetch(map)
}

async function fetch(map) {
  const deptTree = await useRouteApi(company_findByAllCodeTree2,{schemaName: database})(map)
