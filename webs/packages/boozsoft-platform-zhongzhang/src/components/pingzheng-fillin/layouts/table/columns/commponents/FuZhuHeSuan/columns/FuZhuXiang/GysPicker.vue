<template>
  <div ref="thisSelectRef" class="thisSelectRef" style="height: 40px">
    <ItemListPicker
      ...
      @change="selectBlur"
    />
    <GysSelect @throwData="saveSelectDept" @register="registerSelectDeptPage" />
...
import GysSelect from '/@/views/boozsoft/system/supplier/popup/modalPop.vue';
...
const [registerSelectDeptPage, { openModal: openMoalPopPage }] = useModal();

function  saveSelectDept(e) {
  showText.value=e.supName
  selectBlur(e.supName);
}
const datasourcePicker=inject('datasourcePicker')
function clickRecord() {
  openMoalPopPage(true, {
    database: datasourcePicker.value.params.accountMode,
