<template>
  <div ref="thisSelectRef" class="thisSelectRef" style="height: 40px">
    ...
    />
    <!--    <SelectRecord />-->
    <SelectRecord @save="saveSelectDept" @register="registerSelectDeptPage" />
...
import {ref, computed, watch, onMounted, inject} from 'vue';
  ...
  import SelectRecord from '/@/views/boozsoft/system/defined-record/popup/select-record.vue';
  import { ProfileOutlined } from '@ant-design/icons-vue';
  import { useModal } from '../../../../../../../../Modal';
  import ItemListPicker from './ItemListPicker.vue';
  import { findByCfield } from '../../../../../../../../../api/record/system/fuzhu-hesuan';
  const emit = defineEmits(['change', 'focus', 'left', 'right']);
  const props = defineProps(['list', 'modelValue', 'columns', 'item']);
...
function saveSelectDept(e) {
    selectBlur(e.recordName);
    showText.value = e.recordName;
  }
  async function clickRecord() {
    const result = await findByCfield({ cfield: 'cdfine' + props.item.key.split('beiyong')[1] });
    openSelectDeptPage(true, {
      dynamicTenantId: datasourcePicker.value.params.accountMode,
      definedCode: result.definedCode,
    });
  }
