             ...
             <a-col :span="8">
               <a-list-item style="width: 95%">
                 <a-card
                   ...
                   style="width: 100%"
                 >
                   <div class="abc" style="width: 100%;float: right;">
                     <div style="width: 88px;height: 88px;display: inline-block">

                       <Popover placement="right">
                         <template #content>
                           <p class="p_specifics" @click="openEditPageByAddFun(item, recordData)">
                             <FormOutlined/>
                             修改
                           </p>
                           <p class="p_specifics" @click="openEditPageByEditFun(item, recordData)">
                             <DeleteOutlined/>
                             删除
                           </p>
                         </template>
...
async function cardReload() {
  if (!activeKey.value) cardList.value = await findAllApi()
}
...
import {findAllApi} from "/@/api/boozsoft/group/UsedForeignCurrency";
