  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        ...
        <span style="line-height: 25px;font-size: 16px;">部门支出经济分类</span>
      </div>
    </template>
    <div
...
import {getDeptList, GetDeptTreeByFlag, GetZfTreeByFlag,getMaxCode} from '/@/api/record/group/dept-class'
...
async function changeFl() {
  ...
  if(formItems.value.parentId){
    ...
    //请求改编码下最大code
    const res = await getMaxCode(formItems.value.parentId)
    if(res != '0'){
