<template>
  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;">
          <FormOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;资产类别
        </span>
      </div>
    ...
    <div
      ...
      style="height: 100%"
    >
      <div class="open-content-up" style="text-align: center;margin-top: 45px;">
        ...
        <a-input v-model:value="formItems.ecName" placeholder="" :disabled="isEdit" class="abc" style="width: 55%;" />
        ...
        <label>资产编码：</label>
        <a-input style="width: 23%;"  v-model:value="formItems.ecCode" :disabled="isEdit" placeholder="编码规范：1-2-2-2-2"/>
        <span class="red_span">*</span>
        <br><br>

        <label>使用年限：</label>
        <a-input style="width: 25%;"  v-model:value="formItems.serviceLife"  placeholder=""/>

        <label>净残值率：</label>
        <a-input style="width: 25%;"  v-model:value="formItems.netSalvage"   placeholder=""/>
        <br><br>
    ...
    <template #footer>
      ...
      <a-button @click="handleOk()" :disabled="saveClick" type="primary">修改</a-button>
    </template>
  </BasicModal>
</template>
...
import {
  ...
  getFaProperty, getFaZhejiuMethod,
   getSysUnitOfMeaType
} from '/@/api/record/group/fa-asset-type'
...
const [register, { closeModal }] = useModalInner((data) => {
  ...
  formItems.value = {
    ...
    serviceLife: data.data.serviceLife,
    netSalvage: data.data.netSalvage,
    assetAttId: data.data.assetAttId,
    depMethodId: data.data.depMethodId,
    unitId: data.data.unitId,
    isAccrual: data.data.isAccrual,
    inputTax: data.data.inputTax,
...
async function handleOk() {
  ...
  isChanged = !(formItems.value.uniqueCode == changeBeforeModel.uniqueCode
    ...
    && formItems.value.isAccrual == changeBeforeModel.isAccrual
    && formItems.value.serviceLife == changeBeforeModel.serviceLife
    && formItems.value.netSalvage == changeBeforeModel.netSalvage
    && formItems.value.assetAttId == changeBeforeModel.assetAttId
    && formItems.value.depMethodId == changeBeforeModel.depMethodId
    && formItems.value.inputTax == changeBeforeModel.inputTax
    && formItems.value.cardStyle == changeBeforeModel.cardStyle)
  if(isChanged){
    if(formItems.value.parentId){
