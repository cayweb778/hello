...
<!--        <img :src="getThisIndexImg()" class="container-head-img" draggable="false" >-->
        <div class="container-head-title" style="margin-left: 20px;">
          <b class="noneSpan"><UnorderedListOutlined style="font-size: 26px;" />&nbsp;&nbsp;集团-项目样式</b>
        </div>
      <div
        class="ant-btn-group"
        data-v-a1ccd506=""
...
import { cateFindAll,cateFindStateFlag,editFlag,deleteCate, saveCate } from '/@/api/group_project_category/project_category'
import {
  Select as ASelect,
  Input as AInput,
  List as AList,
...
import {getProjectList} from "/@/api/record/system/group-project";
...
const del = async(data:any) => {
  ...
  // alert('删除成功！')
  await reload()*/
  const itemList: any = await findAllProjectItemList()
  ...
  const res: any = await getProjectList({projectCateCode: data.projectCateCode})
...
async function delList() {
  if (checkRow.value.length > 0) {
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      const res: any = await getProjectList({projectCateCode: item.projectCateCode})
      if (res.items.length > 0) {
        createWarningModal({
          iconType: 'warning',
          title: '删除',
...
async function reloadCurrentPage() {
  const res: any = await cateFindAll()
