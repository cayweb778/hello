...
import {
  ...
  findProCateByName,
  saveCate
} from '/@/api/group_project_category/project_category'
...
async function maxCode(){
  const buCate = await findBukongProjectCateCode()
  if (buCate!=null && buCate.projectCateCode!=null && buCate.projectCateCode!=''){
    ...
    const cate = await findMaxProjectCateCode()
...
async function repeatCateName() {
  if (itemConfig.value.base.projectCateName != null && itemConfig.value.base.projectCateName != '') {
    const res: any = await findProCateByName(itemConfig.value.base.projectCateName)
    if (res.length != 0) {
      createErrorModal({
        iconType: 'warning',
        title: '提示',
