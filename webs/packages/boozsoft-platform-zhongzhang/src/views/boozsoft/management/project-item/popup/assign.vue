    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;集团授权分配 － 项目大类
        </span>
        </div>
      </template>
          ...
          <div style="background:#0096c7;padding:10px;border-radius: 2px;margin-bottom:10px;display: flex;justify-content : space-between;">
            ...
            <div class="a1">
              <a-select v-model:value="formItems.selectType" style="width: 120px;font-size: 12px;" class="special_select">
                <a-select-option style="font-size: 12px;" value="itemCode">大类编码</a-select-option>
                <a-select-option style="font-size: 12px;" value="itemName">大类名称</a-select-option>
              </a-select>
              <a-input-search
                placeholder=""
...
import {getProjectItemList} from "/@/api/record/system/group-project-item";
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await getProjectItemList()
  projectItemList.value = res
  await reloadProjectItem()
...
const projectItemAccountList:any = ref([])
const projectItemList:any = ref([])
async function reloadProjectItem(){
  checkRow.value = []
  checkRow1.value = []
  state.selectedRowKeys = []
  state1.selectedRowKeys = []
  const projectItemAccount = await findByOriginId(originId.value)
  projectItemAccountList.value = projectItemAccount.items
  // const res:any = await projectItemFindAll()
  // projectItemList.value = res.items
  if(activeKey.value==1){
    tableDataAll.value = projectItemAccountList.value
  } else {
    tableDataAll.value = projectItemList.value.filter(item=>{
      return projectItemAccount.items.map(aa=>aa.uniqueCode).indexOf(item.itemCode) ==-1
    })
  }
...
async function onSearch(){
  tableData.value = tableDataAll.value.filter(item => {
    if (formItems.value.selectType == 'itemCode' && formItems.value.selectValue != '') {
      return item.itemCode.indexOf(formItems.value.selectValue) != -1
    }
    if (formItems.value.selectType == 'itemName' && formItems.value.selectValue != '') {
