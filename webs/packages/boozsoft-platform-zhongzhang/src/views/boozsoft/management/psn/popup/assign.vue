    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;集团授权分配 － 人员信息
        </span>
        </div>
      </template>
...
import {findByOriginIdAndUniqueCode,save as savePsnTypeAccount} from "/@/api/record/system/group-psn-type-account";
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await psnFindByFlag({})
  psnList.value = res
  await reloadPsn()
...
async function reloadPsn(){
  ...
  loadMark.value = true
  const psnAccount = await findByOriginId(originId.value)
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await save(item1)
      const res = await findByOriginIdAndUniqueCode(originId.value,item.uniquePsnType)
      if (res.length==0){
        const item2 = {
          id: null,
