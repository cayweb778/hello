...
async function checkExcel(){
  ...
  if (list.value.length>0) {
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      //判断是否为自动编码
      if (isAutoCode.value=='1' && excelValue.value == '1') {
        item.psnCode = ccode.value.substring(0, qianzhui.value) + '' +
...
async function getMaxCode(){
  ...
  if(temp.prefixOneIs=='true'){
    ...
    if (temp.prefixOne=='99'){
      //手动录入
      str = str + pad(temp.psnTypeCode,temp.prefixOneCustomize)
    }
    //分隔符
    if (temp.delimiter=='2'){
      str = str + '.'
  ...
  if(temp.prefixTwoIs=='true'){
    ...
    if (temp.prefixTwo=='99'){
      //手动录入
      str = str + pad(temp.psnTypeCode,temp.prefixTwoCustomize)
    }
    //分隔符
    if (temp.delimiter=='2'){
      str = str + '.'
  ...
  if(temp.prefixThreeIs=='true'){
    ...
    if (temp.prefixThree=='99'){
      //手动录入
      str = str + pad(temp.psnTypeCode,temp.prefixThreeCustomize)
    }
    //分隔符
    if (temp.delimiter=='2'){
      str = str + '.'
