<template>
  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;">
          ...
          <FormOutlined v-if="isEdit" style="font-size: 34px;font-weight: bold"/> &nbsp;&nbsp;发票识别参数
        </span>
      </div>
    ...
    >
      <div class="open-content-up" style="text-align: center;margin-top: 18%;">

        <label style="font-size: 18px;margin-left: 0;">secretId：</label>
        <a-input v-model:value="formItems.secretId" placeholder="secretId" class="abc" style="width: 55%;" />
        <span class="red_span">*</span>
        <br/><br/><br/><br/>

        <label>secretKey:</label>
        <a-input v-model:value="formItems.secretKey"  placeholder="secretKey" />
...
import {findBySettModes,findByBsName} from "/@/api/record/system/group-budget-source";
...
async function handleOk(fls) {
  isChanged = !(formItems.value.secretId == changeBeforeModel.bsCode
    && formItems.value.secretId == changeBeforeModel.secretId)
  if (formItems.value.secretId==undefined || formItems.value.secretId=='') {
    // alert('请输入预算来源编码！')
    createErrorModal({
      iconType: 'error',
      title: '提示',
  ...
  if (formItems.value.secretKey==undefined || formItems.value.secretKey=='') {
    // alert('请输入预算来源名称！')
    createErrorModal({
      iconType: 'error',
      title: '提示',
      content: '请输入secretKey！'
    })
    return false
  }
