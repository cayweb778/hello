...
import {findByUniqueCode, getOrganizeList} from "/@/api/record/group/im-organize";
...
import {
  findAllByCusAssignLeftJoinGroupCusDataAndTenantId,
  findByOrgAssigAccount,
  groupCusAssignDelById,
...
import {
  assignOrgSave,
  findByOrgAssigAccount as findByOrgCusClassAssigAccount, findByOrgAssigTenantId
  ...
  if(a==0){
    getSelectRows().forEach(async (a)=>{
      ...
      let map={
        ...
        optRange:'0',
        optAction:'分配',
        optContent:'操作内容【取消分配】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
      }
      await saveLog(map)
      /************** 记录操作日志 END ****************/
    })
...
async function orgCusPermission() {
  // 集团档案权限分配表-集团客户
  let findByPermission=await findByUserIdAndTenantId(useUserStoreWidthOut().getUserInfo.id,accId.value,'customer')
...
async function handleOk() {
  ...
  getSelectRows().forEach(async (a)=>{
    ...
    let map3={
      ...
      optRange:'0',
      optAction:'分配',
      optContent:'操作内容【组织客户分类分配】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类编码【'+a.cusClass+'】,客户名称【'+a.cusCclassName+'】',
    }
    await saveLog(map3)
    /************** 记录操作日志 END ****************/
  })
  ...
  if(accCusClassAssignArr.length>0){
    accCusClassAssignArr.forEach(async (a)=>{
      ...
      let map3={
        ...
        optRange:'0',
        optAction:'分配',
        optContent:'操作内容【组织客户信息分配给账套】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
      }
      await saveLog(map3)
      /************** 记录操作日志 END ****************/
    })
