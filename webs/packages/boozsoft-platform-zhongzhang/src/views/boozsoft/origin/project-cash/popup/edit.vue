  ...
  >
    ...
    >
      <div class="open-content-up" style="text-align: center;margin-top: 5%;">
        ...
        <div style="text-align: left;padding-left:  8%;">
          <label>编码：</label>
          <a-input v-model:value="itemConfig.base.projectCode" placeholder="现金流量项目编码"   style="width: 30%;" />

          <label style="padding-left: 20px;">类别：</label>
          <a-select v-model:value="itemConfig.base.projectType" placeholder="所属类别" style="width: 30%;" >
            <a-select-option v-for="item in typeList" :key="item.projectTypeName" :value="item.projectType">
...
import {
  getTypeList
} from "/@/api/record/system/origin-project-cash";
import projectCashItemConfigModelHelper from "./helper/projectCashItemConfigModelHelper"
...
import {findCodeByIyear, findCodeByProjectCode, findCodeByYearAndBend} from "/@/api/record/system/origin-project-cash";
...
async function reloadCode(data) {
  kemuList.value = await findCodeByYearAndBend({
    originId: data,
    year:year.value
  })
}
...
const [register, { closeModal }] = useModalInner(async (data) => {
  ...
  if (data.data.projectCode != null && data.data.projectCode != '') {
    await findCodeByProjectCode({
      projectCode:itemConfig.value.base.projectCode,
      accStandard: data.accStandard,
      originId: data.originId,
      iyear:year.value
    }).then(res => {
...
async function handleOk() {
  itemConfig.value.base.table.forEach((row: any) => {
    ...
    row.originId = originId.value
    row.accStandard = accStandard.value
  })
  itemConfig.value.base.iyear =year.value
  itemConfig.value.base.originId =originId.value
  itemConfig.value.base.accStandard =accStandard.value
