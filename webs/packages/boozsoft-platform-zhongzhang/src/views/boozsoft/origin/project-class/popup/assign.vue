<template>
  ...
  >
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权分配 － 项目分类
        </span>
      </div>
    </template>
    ...
    <SelectItem
      @save="saveSelectItemData"
      @register="registerSelectItemPage"
    />
...
import {findClassByBend} from "/@/api/record/system/origin-project-class";
...
const projectClassCtl:any = ref("0")
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  if (accountList.value.length>0){
    ...
    projectClassCtl.value = accountList.value[0].projectClassCtl
  ...
  const res:any = await findClassByBend(data.originId)
...
async function changeOrigin() {
  for (const item of accountList.value) {
    if (item.accId == accId.value){
      projectClassCtl.value = item.projectClassCtl
...
const projectClassList:any = ref([])
async function reloadProjectClass(){
  checkRow.value = []
  checkRow1.value = []
  state.selectedRowKeys = []
  state1.selectedRowKeys = []
  const projectClassAccount = await findByTenantId(tenantId.value)
  projectClassAccountList.value = projectClassAccount.items
  if(activeKey.value==1){
    tableDataAll.value = projectClassAccountList.value
  } else {
    tableDataAll.value = projectClassList.value.filter(item=>{
      return projectClassAccount.items.map(aa=>aa.uniqueCode).indexOf(item.uniqueCode) ==-1
    })
  }
...
const [registerSelectItemPage, {openModal: openSelectItemPage}] = useModal()
async function openSelectItem() {
  openSelectItemPage(true, {
    originId: originId.value
  })
}

async function saveSelectItemData(data) {
  let dateTime = new_Date()
  for (let i = 0; i < checkRow.value.length; i++) {
    const item = checkRow.value[i]
    const item1 = {
