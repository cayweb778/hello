...
<!--        <img :src="getThisIndexImg()" class="container-head-img" draggable="false" >-->
      <div class="container-head-title" style="margin-left: 20px;">
        <b class="noneSpan"><UnorderedListOutlined style="font-size: 26px;" />&nbsp;&nbsp;组织-项目信息</b>
      </div>
...
<!--        <button
      ...
      <div style="margin-top: 10px;">
        <div style="display: inline-block;float: left;font-size: 16px;margin-left: 10px">
          <span>所属组织：</span>
          <a-select v-model:value="originId" @change="changeOrigin()" style="width: 250px;">
        ...
        <div style="float: left;font-size: 16px;margin-left: 10px;">
          <span>项目样式：</span>
          <a-select v-model:value="cateCode" @change="useProjectStoreWidthOut().commitNum" placeholder="请选择项目大类" style="width: 250px;">
            <a-select-option v-for="cate in cateList" :key="cate.projectCateCode" :value="cate.projectCateCode">
              ({{ cate.projectCateCode }}){{ cate.projectCateName }}
            </a-select-option>
          </a-select>
...
import {getPsnList, psnFindAll} from "/@/api/record/system/origin-psn";
import {projectClassFindAll} from "/@/api/record/system/origin-project-class";
...
async function reloadPsn() {
  const res:any = await getPsnList({originId:originId.value})
  psnList.value = res
...
async function reloadCurrentPage() {
  loadMark.value = true
  const res:any = await findProjectByCateCode(cateCode.value,originId.value)
...
async function exportExcel() {
  loadMark.value = true
  await reloadCurrentPage()
  const columnList = await getFromEdit(cateCode.value,originId.value)
...
const lanMuData = {'accId':'','menuName':'项目信息','type':'组织',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
