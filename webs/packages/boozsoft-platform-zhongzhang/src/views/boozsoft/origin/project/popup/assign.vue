    ...
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权分配 － 项目信息
        </span>
      </div>
    </template>
...
import {findProClassByCode} from "/@/api/record/system/origin-project-class";
import {
...
async function reloadProject(){
  ...
  const projectAccount = await findByTenantId(tenantId.value)
...
async function assignData() {
  ...
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await save(item1)
      const projectClass = await findProClassByCode(item.projectClassCode,originId.value)
      if (projectClass.length>0){
        const res = await findByTenantIdAndUniqueCode(tenantId.value,projectClass[0].uniqueCode)
        if (res.length == 0){
          const item2 = {
            id: null,
