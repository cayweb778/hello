    ...
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权引入 － 项目信息
        </span>
      </div>
    </template>
...
import {saveProject} from "/@/api/record/system/origin-project";
import {findByUniqueCode} from "/@/api/record/system/group-project";
import {findById, findProClassByCode} from "/@/api/record/system/group_project_class";
import {
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      let item1 = await findByUniqueCode(item.uniqueCode)
      item1.id = null
      item1.originId = originId.value
      await saveProject(item1);
      item.flag = '1'
