  ...
  >
    <template #title>
      <div style="display: flex;height:30px;margin-top: 10px;" class="vben-basic-title">
        <span style="font-size: 24px;line-height:30px;">
          <ShareAltOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;组织授权分配 － 人员信息
        </span>
      </div>
    </template>
    <div class="nc-open-content">
      <div class="open-content-up" style="margin-top: 10px;">
        ...
        <div style="height: 400px;padding:0;display: flex;justify-content : space-between;overflow:auto;">
          <a-tabs size="small" tab-position="left" v-model:activeKey="activeKey" @change="changeOrigin">
            <a-tab-pane key="1" tab="已分配">
              <div style="margin-top: -10px;overflow:auto;max-height: 400px;margin-left: -15px;">
                <BasicTable
...
import {psnFindByFlag} from "/@/api/record/system/origin-psn";
import {
  deleteById,
...
const [register, {closeModal,setModalProps}] = useModalInner(async(data) => {
  ...
  const res:any = await psnFindByFlag({originId:originId.value})
  psnList.value = res
  await reloadPsn()
...
async function reloadPsn(){
  ...
  loadMark.value = true
  const psnAccount = await findByTenantId(tenantId.value)
  psnAccountList.value = psnAccount.items
  // const res:any = await psnTypeFindAll()
  // psnTypeList.value = res.items
  if(activeKey.value==1){
    tableDataAll.value = psnAccountList.value
  } else {
    tableDataAll.value = psnList.value.filter(item=>{
      if (psnAccountList.value.length > 0) {
        return psnAccount.items.map(aa => aa.uniqueCode).indexOf(item.uniqueCode) == -1
      }
      return item
    })
  }
...
async function assignData() {
  if (checkRow.value.length > 0) {
    ...
    for (let i = 0; i < checkRow.value.length; i++) {
      ...
      await save(item1)
      const res = await findByTenantIdAndUniqueCode(originId.value,item.uniquePsnType)
      if (res.length==0){
        const item2 = {
          id: null,
          uniqueCode: item.uniquePsnType,
          ctype: '2',
          originId: originId.value,
          tenantId: tenantId.value,
          assignUser: useUserStoreWidthOut().getUserInfo.id,
        ...
        await savePsnTypeAccount(item2)
      }
    }
