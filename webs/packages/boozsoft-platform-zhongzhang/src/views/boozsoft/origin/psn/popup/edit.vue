    ...
    <div style="height: 100%" :class="isEdit?'nc-open-content':'nc-open-show-content'">
      <div class="open-content-up">
        ...
        <div>
          ...
          <span v-if="isLock">
            <a-input v-model:value="formItems.cellPhoneNum" :disabled="true" :placeholder="isEdit?'请输入手机号（不允许重复）':''"/>
            <span class="red_span"></span>
          </span>
          <span v-else>
            <a-input v-model:value="cellPhoneNum" :readonly="true" :disabled="true" :placeholder="isEdit?'请输入手机号（不允许重复）':''"/>
