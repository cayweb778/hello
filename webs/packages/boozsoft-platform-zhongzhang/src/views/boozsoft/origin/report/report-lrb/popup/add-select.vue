...
const [register, { closeModal }] = useModalInner((data) => {
        ...
        initBasisTabAccoutData().then(list => {
          ...
          levelList.forEach(item => {
            if (res.uniqueAccStandard == item.id) {
              ...
              findByAccStandardAndReportName(item.uniqueAccStandard, 'lrb').then(template => {
  ...
  formItems.value.templateName = '利润表'
...
async function changTitleName() {
  ...
  const res = await findByTitleName({reportName: 'lrb',titleName:formItems.value.titleName,originId:formItems.value.originId})
  if(res.length!=0){
    // alert('模板标识已存在，请重新输入！')
    createErrorModal({
      iconType: 'warning',
