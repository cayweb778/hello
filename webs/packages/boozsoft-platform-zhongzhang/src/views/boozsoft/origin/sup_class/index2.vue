...
<script setup lang="ts">
  ...
  import { GroupCustomerSaveApi } from '/@/api/record/system/supplier_class_group';
...
import {
  ...
  import { customerSaveApi } from "/@/api/record/system/supplier_class";
  import {useTabs} from "/@/hooks/web/useTabs";
  import {useMessage} from "/@/hooks/web/useMessage";
  import { BasicTree } from '/@/components/Tree';
  import { PageWrapper } from '/@/components/Page';
  import {
    deloriginSupplierClassById,
    findAllOrgCusClass,
    saveAll,
    treeCustomerClass
  } from "/@/api/org/sup_class/org_sup_class";
  ...
  const del = async () => {
    ...
    createConfirm({
      ...
      onOk: async() => {
        await deloriginSupplierClassById({orgUnique:orgUnique.value,list:JSON.stringify(getSelectRows())}).then(async (data)=>{
          if(data.length>0){
            createConfirm({
              iconType: 'warning',
              title: '警告',
            ...
            if(arr.length>0){
              for (let i = 0; i < arr.length; i++) {
                let log='操作内容【删除供应商分类】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类编码【'+getSelectRows().map(a=>a.cusClass).filter(a=>data.indexOf(a)==-1)[i]+'】,分类名称【'+getSelectRows().map(a=>a.cusCclassName).filter(a=>data.indexOf(a)==-1)[i]+'】'
                /************** 记录操作日志 ****************/
                let logmap={
            ...
            for (let i = 0; i < [...new Set(getSelectRows().map(a=>a.ccode))].length; i++) {
              let log='操作内容【删除供应商分类】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类编码【'+[...new Set(getSelectRows().map(a=>a.cusClass))][i]+'】,分类名称【'+[...new Set(getSelectRows().map(a=>a.cusCclassName))][i]+'】'
              /************** 记录操作日志 ****************/
              let logmap={
                loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
                userId: useUserStoreWidthOut().getUserInfo.username,
  ...
  async function bringGroupDataFunction(data) {
    ...
    for (let i = 0; i < data.map(a=>a.cusClass).length; i++) {
      let log='操作内容【组织引入供应商分类】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,分类编码【'+data.map(a=>a.cusClass)[i]+'】,分类名称【'+data.map(a=>a.cusCclassName)[i]+'】'
      /************** 记录操作日志 ****************/
      let logmap={
        loginTime:new Date( +new Date() + 8 * 3600 * 1000 ).toJSON().substr(0,19).replace("T"," "),
        userId: useUserStoreWidthOut().getUserInfo.username,
