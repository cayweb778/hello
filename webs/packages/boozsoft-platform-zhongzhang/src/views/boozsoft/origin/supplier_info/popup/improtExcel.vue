...
const handleDownByData = async (val: any) => {
  ...
  const columns2 = ['供应商编码','供应商全称','供应商简称','税号','分类名称','母公司','对应供应商','国家','区划(省-市-区)','详细地址','电话',
      ...
      if (info[0].code === '200' || info[0].error === 'error') {
        ...
        info[0].list.forEach(async (tx)=>{
          ...
          let map={
            ...
            optAction:'新增申请',
            optContent:'操作内容【导入供应商信息】,组织代码【'+orgInfo.orgCode+'】,组织名称【'+orgInfo.orgName+'】,供应商编码【'+tx.custCode+'】,供应商全称【'+tx.custName+'】,供应商简称【'+tx.custAbbname+'】',
          }
          await saveLog(map)
