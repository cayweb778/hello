              ...
              <div class="acco-sideline">
                ...
                <div class="acco-sideline-content" style="margin-left: 20px;">
                  ...
                  <Select v-model:value="formItems.stockValuationType" style="width: 15%;">
                    ...
                    <SelectOption value="相同批号">相同批号</SelectOption>
                    <SelectOption value="先进先出">先进先出</SelectOption>
                    <SelectOption value="手工计价">手工计价</SelectOption>
                    <template #suffixIcon>
                      <CaretDownOutlined style="color:#666666;"/>
                    </template>
                  </Select>
...
async function handleOk(flag) {
  ...
  if(hasBlank(trim(formItems.value.stockName))){
    ...
    if(!hasBlank(trim(formItems.value.stockGgxh))){
      ...
      let item = await useRouteApi(findByStockNameAndGgxh, {schemaName: database})(map)
  ...
  if(formItems.value.stockMeasurementType=='多计量'){
    ...
    formItems.value.stockUnitBarcode1=hasBlank(stockUnitBarcode1)?formItems.value.stockBarcode:trim(stockUnitBarcode1)
    formItems.value.stockUnitGgxh1=hasBlank(stockUnitBarcode1)?formItems.value.stockGgxh:trim(stockUnitGgxh1)

    formItems.value.stockUnitId2=stockUnitId2
    formItems.value.stockUnitName2=stockUnitName2
    formItems.value.stockUnitBarcode2=hasBlank(stockUnitBarcode2)?formItems.value.stockBarcode:trim(stockUnitBarcode2)
    formItems.value.stockUnitGgxh2=hasBlank(stockUnitBarcode2)?formItems.value.stockGgxh:trim(stockUnitGgxh2)
  }
  //价格表添加
