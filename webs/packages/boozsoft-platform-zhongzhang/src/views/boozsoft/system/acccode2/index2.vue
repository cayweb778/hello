  ...
  <div>
    ...
    <div class="app-container">
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 10px;">
          <b class="noneSpan" style="font-size: 36px;">
            会计科目
          </b>
          <p/>
          <a-select v-model:value="iyearselected" style="width: 120px;text-align: center;" @change="findAllData">
          ...
          <span style="float: right;">
            ...
            <span v-if="roweditflg">
              ...
              <span v-if="independent">
                <button type="button" class="ant-btn ant-btn-me" @click="copyCodeView"><span>复制</span></button>
              </span>
              <!-- 公司：引入 -->
              <span v-if="!independent">
      ...
      <div style="margin-top: -40px;margin-left: 80px;">
        ...
        <div style="display: inline-block;position:absolute;top:85px;left:110px;font-size: 14px;font-weight: bold;">
        ...
        <div style="float: right; margin-left: 10px;">
          ...
          <a-popover placement="bottom" v-model:visible="visible3">
            <template #content>
              <DynamicColumn :defaultData="initDynamics()['J']" :dynamicData="dynamicColumnData"
          ...
          <span v-if="roweditflg">
            ...
            <a-button title="回收站" v-if="roweditflg" @click="openCodeDel()">
        ...
        <div style="float: right; position: relative">
          <span style="float: left; position: relative">
            ...
            >
              <a-select-option v-for="item in searchConditonList" :value="item.value" >
                {{ item.title }}
              </a-select-option>
            </a-select>
            <a-input-search
    ...
    <div class="app-container" :style="{height: (windowHeight+82)+'px'}">
      ...
      <div class="app-container-bottom" style="width: calc(100% - 104px); float: right; margin-top: 10px;">
        <BasicTable
          ref="tableRef"
          :loading="loading"
        ...
        >
          <template #ccode="{ record }">
            <u :class="'tableUStyle'" @click="roweditflg?openRowEdit():openLock(record)">{{ record.ccode }}</u>
          </template>
          <template #ccodeName="{ record }">
            <u :class="'tableUStyle'" v-if="!hasBlank(record.ccodeName)" @click="roweditflg?openRowEdit():openLock(record)">{{
    ...
    <a-modal v-model:visible="contrastModel" title="选择对照年度">
      <template #footer>
        <a-button key="back" @click="contrastModel=false">取消</a-button>
        <a-button key="submit" type="primary" :loading="loadMark" @click="contrastModelOk">确定</a-button>
      </template>
      <br />
      <div style="margin-left: 30px">
        当前年度：
        <a-select v-model:value="thisiyear" style="width: 130px" class="special_select">
          <template v-for="item in iyearlist">
            <a-select-option :value="item.iyear">
              {{ item.iyear }}
            </a-select-option>
          </template>
        </a-select>
...
const windowHeight = (window.innerHeight - (340))
  ...
  const searchConditonList = ref([{title:'科目编码',value:'ccode'},{title:'科目名称',value:'ccodeName'},{title:'科目分类',value:'cclass'},
    {title:'方向',value:'bprogerty'},{title:'辅助项',value:'fuzhu'},{title:'数量单位',value:'menterage'},{title:'外币名称',value:'currencyType'}]);
...
const loadingRef = ref(false);
  ...
  async function findByTaskModuleAndUserName() {
    const task1 = await useRouteApi(findByFunctionModule, { schemaName: database })(iyearselected.value);
    let taskdata = task1.filter((t) => t.functionModule === '会计科目'&&t.caozuoUnique==useUserStoreWidthOut().getUserInfo.id);
  ...
  async function codeContrastView() {
    if(iyearlist.value.length==1){
      return createWarningModal({ content: '当前账套只有一个会计年度，没有其他年度可进行对照！' });
    }
  ...
  async function contrastModelOk() {
    ...
    thisyearCode.value = await useRouteApi(company_findAllByIyear, {
      schemaName: database,
    })({ iyear: thisiyear.value, databasenum: '' });

    upyearCode.value = await useRouteApi(company_findAllByIyear, {
      schemaName: database,
    })({ iyear: contrastiyear.value, databasenum: '' });
...
async function editflg(data) {
  clearSelectedRowKeys()
    ...
    if (data === '编辑') {
      ...
      if (taskdata.length > 0 && taskdata[0].caozuoUnique != null) {
        ...
        taskinfo.value = taskdata[0];
        // 判断是不是自己；动态获取
        if (taskdata[0].caozuoUnique !== String(useUserStoreWidthOut().getUserInfo.id)) {
          createConfirmPop(
            '提示：任务冲突！操作员【' +
              taskdata[0].username +
              '】正在进行当前单位的会计科目编辑操作，本功能属于独占任务，请等待其他操作员完成编辑任务，或手动清除该操作员的会计科目编辑任务！',
          );
          return false;
        }
      }
      ...
      if (taskQcData.length > 0 && taskQcData[0].caozuoUnique != null) {
        // 判断是不是自己；动态获取
        if (taskQcData[0].caozuoUnique !== String(useUserStoreWidthOut().getUserInfo.id)) {
          createConfirmPop(
            '提示：任务冲突！操作员【' +
              taskQcData[0].username +
  ...
  // 判断 凭证、期初、现金流量是否使用
  async function verifyData2(data) {
    // 判断 凭证、期初、现金流量是否使用
...
async function verifyData(data) {
  ...
  if (delFirstMsg.code === '200') {
    ...
    if (delFirstMsg.msg === 'accvoucher' || delFirstMsg.msg === 'qc' || delFirstMsg.msg=='accvoucherSetting') {
      return false;
    } else {
      return true;
    }
  ...
  const openAddPage = async (flag) => {
    ...
    if(checkBoxCcodeInfo[0]?.flag=='0'){
      return createConfirmPop('停用科目不能进行修改！')
    }
    if(!independent.value&&checkBoxCcodeInfo[0].lowerControl=='1'){
      return createConfirmPop('新增下级科目控制已开启,不能增加下级科目！')
    }
    if(flag=='1'&&getSelectRows().length==0){
      return createConfirmPop('请选择上级科目！')
    }
    openSavePage(true, {
      data: val,
      independent: val,
      ...
      superiorData:JSON.stringify(checkBoxCcodeInfo[0])
    });
  };
  ...
  const openRowEdit = async () => {
    ...
    if(getSelectRows()[0]?.flag=='0'){
      return createConfirmPop('停用科目不能进行修改！')
    }
    let verifyEditData= await verifyData2(getSelectRows()[0])
    if(verifyEditData.code=='200'){
      openEditPage(true, {
        data: val,
        independent: val,
        use:'0',
        ibudgetAccounting: ibudgetAccounting.value,
        superiorData:JSON.stringify(getSelectRows()[0])
      });
    }
    else{
      let msg=verifyEditData.msg == 'accvoucher'?'凭证':verifyEditData.msg=='qc'?'期初':verifyEditData.msg=='accvoucherSetting'?'常用凭证':''
      createConfirm({
         iconType: 'warning',
         title: '警告',
         content: '此科目已做' + msg + '，原数据将会覆盖。确定继续操作吗？',
         onOk: async () => {
           openEditPage(true, {
             data: val,
             independent: val,
  ...
  const delCode = async () => {
    ...
    // 常用凭证
    let accvoucherSetting = await useRouteApi(findAllApi, { schemaName: database.value })('');

    // 是否存在非末级科目
    if(getSelectRows().filter(a=>a.bend=='0').length>0){
      return createConfirmPop('只能删除末级科目,以下科目【'+getSelectRows().filter(a=>a.bend=='0').map(a=>a.ccode).join(',')+'】不是末级科目!')
    }
    ...
    let accSetting: any = getArrEqual(accvoucherSetting, codenum);
    if (pz.length > 0) {
      return createConfirmPop(setString(Array.from(new Set(pz)).toString(), 60) + '会计科目已经在凭证中使用，不能删除！')
    }
    if (qc.length > 0) {
      return createConfirmPop(setString(Array.from(new Set(qc)).toString(), 60) + '会计科目已经在期初余额中使用，不能删除！')
    }
    if (accvoucherSetting.length > 0) {
      return createConfirmPop(setString(Array.from(new Set(accSetting)).toString(), 60) + '会计科目已经在常用凭证中使用，不能删除！')
    }
    ...
    createConfirm({
      ...
      onOk: async () => {
        await useRouteApi(company_delCodekemu, { schemaName: database.value })({
          id: getSelectRows().map(a=>a.id).join(','),
          iyear: iyearselected.value,
          userName: useUserStoreWidthOut().getUserInfo.realName,
          independent: independent.value,
        })
          .then(async (temp)=>{
            ...
            for (let i = 0; i < temp.length; i++) {
              ...
              let map={
                ...
                billDate:iyearselected.value,
                optAction:'删除',
                optContent:'操作内容【删除会计科目】,账套代码【'+pageParameter.companyCode+'】,账套名称【'+pageParameter.companyName+'】,年度【'+iyearselected.value+'】,科目编码【'+temp[i]+'】',
              }
              await saveLog(map)
              /************** 记录操作日志 END ****************/
            }
    ...
    //                 billDate: iyearselected.value,
    //                 optAction:'删除',
    //                 optContent:'操作内容【删除会计科目】,账套代码【'+pageParameter.companyCode+'】,账套名称【'+pageParameter.companyName+'】,年度【'+iyearselected.value+'】,科目编码【'+arr[i]+'】',
    //               }
    //               await saveLog(map)
    //               /************** 记录操作日志 END ****************/
    //               console.log(map,'aaa')
