...
export const checkMeetSpecifyConditions = async (pageVariable, yearMonths,flag,thisDbName) => {
  ...
  if (closeMonth.length > 0) {
    createWarningModal({title: '开始记账前检测', content: '凭证' + closeMonth.toString() + '月份已经结账，不能进行记账操作！'})
    return false
  }
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    if (checkMenu.indexOf(item.functionModule) != -1 && item.state == '1') {
      ...
      if (item.functionModule == '月末结账' && item.time != '' && !compareTime(offsetToStr(item.time))
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.imonth + '月份' + item.functionModule + '处理!不能进行记账操作，请销后再试，或联系财务主管清理该月末结账任务后再进行相关操作!'
        break;
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.functionModule + '处理!不能继续进行取消记账操作，该操作不能多人同时并发进行，请销后再试，或联系财务主管清理该凭证记账任务后再进行操作!'
        break;
      } else if (!pageMark && item.functionModule == '凭证记账' && item.time != '' && compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.functionModule + '处理!不能继续进行凭证记账操作，该操作不能多人同时并发进行，请销后再试，或联系财务主管清理该审核凭证任务后再进行操作!'
        break;
      } else if (pageMark && item.functionModule == '审核凭证' && item.time != '' && compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
        ...
        msg = '提示：任务冲突！操作员【' +
          item.caozuoUnique +
          '】正在进行' + item.functionModule + '处理!不能继续进行取消记账操作，该操作不能多人同时并发进行，请销后再试，或联系财务主管清理该主管签字任务后再进行操作!'
        break;
      } else if (!pageMark && item.functionModule == '主管签字' && item.time != '' && compareTime(offsetToStr(item.time))) { // 超时
        msg = '提示：发现操作员【' +
          item.caozuoUnique +
