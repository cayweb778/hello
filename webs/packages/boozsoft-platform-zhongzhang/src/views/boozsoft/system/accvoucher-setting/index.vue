    ...
    <div class="app-container">
      <div class="app-container-head">
        ...
        <div class="container-head-title" style="padding-left: 35%;text-align: center;margin-top: 30px;">
          <b class="noneSpan" style="font-size: 36px;">常用凭证设置</b>
        </div>
    ...
    <div class="app-container">
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        <div class="w-1/4 xl:w-1/5"
             style="width: 250px;min-height: 250px;border: 1px #cccccc solid;margin-right: .2%;margin-bottom: 58px;">
        ...
        >
          ...
          <template #remarks="{ record }">
            <a class="tableUStyle" @click="openEdit(record,false)">{{ record.remarks }}</a>
          </template>
...
async function saveBatchData(data) {
  checkRow.value.forEach(item => {
    if (data.ctype != null && data.ctype != '') {
      item.ctype = data.ctype
    }
    if (data.cnum != null && data.cnum != '') {
      item.cnum = data.cnum
    }
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let i = 0; i < checkRow.value.length; i++) {
          ...
          await useRouteApi(deleteByParentId, {schemaName: dynamicTenantId})(item.id)
        }
        checkRow.value = []
        message.success('删除成功！')
        reloadTable()
...
const showPageNum:any = ref(false)
