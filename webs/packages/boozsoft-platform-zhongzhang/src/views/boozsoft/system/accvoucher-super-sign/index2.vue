    ...
    <div class="app-container-top">
      <div class="app-container-top-one">
        ...
        <div class="act-one-d-title">
          <span class="acto-d-title-span">主管签字</span>
        </div>
      ...
      <div style="position: relative;">
          ...
          >
            <template #ibook="{ record }">
              <a-tag  v-if="record.inoId!=null" :color="record.cdirector != null ? 'green' : 'red'">
                {{ record.cdirector != null ? '已签字' : '未签字' }}
              </a-tag>
            </template>
...
import Review from "/@/views/boozsoft/system/accvoucher-super-sign/popup/sign.vue";
import Query from "/@/views/boozsoft/system/accvoucher-super-sign/popup/query.vue";
...
const CrudApi = reactive({
  columns: JsonTool.parseProxy(accvoucherStore.getColumns(pageParameter.queryMark)).map(it=>{if (it.title=='记账状态'){it.title = '签字状态';} return it}).slice(0, 10)
})
...
import {checkMeetSpecifyConditions} from "/@/views/boozsoft/system/accvoucher-super-sign/bus";
...
function resetDynamicColumnData() {
  ...
  findDbLanMuList(lanMuData).then(res => {
    ...
    let statiList = JsonTool.parseProxy(initDynamics()['DATA' + pageParameter.queryMark]).filter(item => item.name != '记账人' && item.name != '出纳人' && item.name != '审核人')
      .map(it=>{if (it.name=='记账状态'){it.name = '签字状态';it.nameNew = '签字状态'} return it})
    if (dbList.length > 0) {
...
const reloadColumns = () => {
  ...
  let newA = (JSON.parse(JSON.stringify(accvoucherStore.getColumns(pageParameter.queryMark)))).filter(item => item.title != '记账人' && item.title != '出纳人' && item.title != '审核人')
