    ...
    <div class="app-container">
      <div class="app-container-top">
        <div style="width: 100%;height: 100px;">
          <div style="width: 40%;">
            <div>
              <AccountPicker theme="three" readonly="" style="margin-top: -5px;"/>
            </div>
          ...
          <div style="width: 60%;float: right;margin-top: -60px;">
      ...
      <div style="margin-top: -40px;" class="app-container-bottom">
          ...
          >
            ...
            <template #wbmoney="{ record }">
              <span v-if="record.md!=='0.00'&&record.csign!=null" style="float: right">
               {{moneyformat(record.nfratMd)}}
              </span>
              <span v-if="record.mc!=='0.00'&&record.csign!=null" style="float: right">
           ...
           {{numformat(record.ncS)}}
            ...
            <template #mdmoney="{ record }">
              <span v-if="record.csign!=null" style="float: right">
            ...
            <template #summary>
              <TableSummary fixed>
                <TableSummaryRow style="background-color: #cccccc;">
                  ...
                  <TableSummaryCell class="nc-summary" style="border-right: none;background-color: #cccccc;" v-for="cell in getCurrSummary()" :index="cell.ind" :align="cell.align"><span class="a-table-font-arial">{{null == summaryTotals[cell.dataIndex]?'':summaryTotals[cell.dataIndex]}}</span></TableSummaryCell>
                </TableSummaryRow>
              </TableSummary>
            </template>
          </BasicTable>
        <div class="pagination-text" :style="{top: (windowHeight+70)+'px'}" v-show="showPaginationText">
          {{`共 ${showPageNumber}条记录&emsp;每页 200 条`}}
        </div>
      </div>
    </div>
    <a-drawer
...
const showPageNumber=ref(0)
  ...
  if (res != null && res.items.length > 0){
    ...
    showPageNumber.value=getDataSource().length
    setTimeout(()=>{
      for (let i = getDataSource().length; i < (50); i++) {
        getDataSource().push({serialNumber:getDataSource()+i,csign:null})
      }
      calculateTotal()
...
const getCurrSummary  = () => {
  return (getColumns().filter(it=>it.title != '序号').map((it,ind)=>{it['ind']=ind+2;return it;}))
}
const summaryTotals = ref({})
const calculateTotal = () => {
  let list = getDataSource().filter(a=>a.csign!=null)
  if (list.length == 0){
    summaryTotals.value = {}
    return false;
  }
