  ...
  >
    <template #title>
      <div style="display: flex;" class="vben-basic-title">
        <span v-if="openType=='yue'" style="line-height: 25px;font-size: 16px;">发生及余额表查询</span>
      </div>
    </template>
            ...
            <a-tab-pane key="0" tab="集团模式">
              <ul>
                <li>
                  ...
                  <a-select v-model:value="naturalYear" style="width: 15%" @change="customizeChange">
                    <a-select-option :key="item" :value="item"
                                     v-for="item in orgDateList">
                      {{ item }}
                    </a-select-option>
                    <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template>
                  </a-select>
<!--                  <a-date-picker style="width: 114px" placeholder="" v-model:value="naturalYear" :disabled-date="disabledDate" picker="year" @change="customizeChange" @openChange="onOpenChange"/>-->
...
async function handleOk() {
  ...
  if (activeKey.value !== '0') {
    ...
    if (strDate.value.split('-')[0] !== naturalYear.value) {
      return createConfirmPop('日期年度与组织期间不一致!');
    }
  ...
  if (strDate.value.split('-')[0] !== endDate.value.split('-')[0]) {
    return createConfirmPop('日期不允许跨年!');
  }
...
async function dynamicAdReload(data) {
  ...
  await findDataBase(data.accId,+data.year).then(async (item)=>{
    // 进页面执行
    getThisAdInfoData({ accId: item.accountId }).then(async (res) => {
      ...
      .then((temp)=>{
        ...
        endDate.value=temp[0].iyear+'-'+month
        databaseyear.value=temp[0].iyear
      })
    });
