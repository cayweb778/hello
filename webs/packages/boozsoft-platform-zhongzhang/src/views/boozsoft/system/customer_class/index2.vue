        ...
        <div class="ant-btn-group" style="float: right;">
          <span v-if="independent=='1'">
            ...
            <button type="button" class="ant-btn ant-btn-me"
                    @click="openEdit()"><span>修改</span></button>
          ...
          <span v-else>
            <button type="button" class="ant-btn ant-btn-me" @click="del()"><span>删除</span></button>
            <button type="button" class="ant-btn ant-btn-me"
                    @click="bringGroupDataBtn()"><span>引入</span></button>
          </span>
          <button type="button" class="ant-btn ant-btn-me"
...
<!--        </div>-->
        ...
        <div style="float: right; position: relative">
          <a-input-search placeholder="" style="width: 200px; border-radius: 4px"
                          v-model:value="pageParameter.searchValue" @search="pageReload"/>
        </div>
      </div>
    ...
    <div class="app-container">
      <PageWrapper dense content-full-height fixed-height content-class="flex">
        <div class="bg-white m-4 mr-0 overflow-hidden"
          ...
          <div
            style="width: 100%; height: 32px;background-color: #d9d9d9;padding-top:5px;font-weight: bold;text-align: center;">
            客户分类
          </div>
          <BasicTree
            defaultExpandAll
        ...
        <div style="width: calc(100% - 250px); float: right;margin-left: 5px;">
          <BasicTable ref="tableRef" @fetch-success="fetchSuccess" @register="registerTable" :scroll="{y: windowHeight }"
                      :class="'a-table-font-size-12'" :bordered="true" v-if="tableshow"
                      @row-click="rowClick">
          ...
          <div class="pagination-text" :style="{top: (windowHeight+45)+'px',left:'900px'}" v-show="showPaginationText">
...
import {editFlagByCtypeAndTenantId} from "/@/api/record/system/customer_class_group";
import {findByStockAccountAccId} from "/@/api/record/system/stock-account";
import {editUniqueCustclass} from "/@/api/record/costomer_data/customer";
...
async function bringOrgDataFunction(data) {
  ...
  await useRouteApi(customerSaveCusClass, {schemaName: database})(data).then(async () => {
    ...
    for (let i = 0; i < data.length; i++) {
      let log = '操作内容【账套引入客户分类】,账套代码【' + accountInfo.value.coCode + '】,账套名称【' + accountInfo.value.companyName + '】,分类编码【' + data[i].cusClass + '】,分类名称【' + data[i].cusCclassName + '】'
      /************** 记录操作日志 ****************/
      let logmap = {
    ...
    onOk: async () => {
      await useRouteApi(delCustomerClass, {schemaName: database})(getSelectRows()).then(async (data) => {
        ...
        getSelectRows().forEach(async (a) => {
          let log = '操作内容【账套删除客户分类】,账套代码【' + accountInfo.value.coCode + '】,账套名称【' + accountInfo.value.companyName + '】,分类编码【' + a.cusClass + '】,分类名称【' + a.cusCclassName + '】'
          /************** 记录操作日志 ****************/
          let logmap = {
            loginTime: new Date(+new Date() + 8 * 3600 * 1000).toJSON().substr(0, 19).replace("T", " "),
            userId: useUserStoreWidthOut().getUserInfo.username,
...
async function saveData(data: any) {
  ...
  await useRouteApi(customerSaveApi, {schemaName: database})(data)
    .then(async (a) => {
      await useRouteApi(editUniqueCustclass, {schemaName: database})({
        uniqueCustclass: a.uniqueCustclass,
        oldUniqueCustclass: a.parentId
      })
    })
...
const dynamicAdReload = async (obj) => {
  ...
  let datainfo: any = await findByAccId(obj.accId);
