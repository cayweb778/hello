...
import {
  editFlagByCtypeAndOrgUnique, editFlagByCtypeAndTenantId,
  findAllCustomerClass
} from "/@/api/record/system/customer_class_group";
import {
  findByBringGroupClassAccount,
  findByBringOrgTenantId
...
const [register, {closeModal}] = useModalInner(async (data) => {
  pageParameter.tenantId=data.tenantId
  orgClassDataList.value=data.list
  clearSelectedRowKeys()
  reload()
});
async function handleOk() {
  if(getSelectRows().length==0){
    return createWarningModal({content:'请选择分类!!'})
  }

  // 过滤出上级，判断上级是否已引入，防止数据重复
  let parentId=[...new Set(getSelectRows().filter(a=>a.parentId!=='0').map(a=>a.parentId))]
  for (let i = 0; i < parentId.length; i++) {
    let temp=orgClassDataList.value.filter(f=>f.uniqueCustclass==parentId[i])
    if(temp.length==0){
      // 从集团表中查找上级信息
      let map={
        searchValue: '',
      ...
      let groupData=await findAllCustomerClass(map)
      let temp1=groupData.items.filter(g=>g.uniqueCustclass==parentId[i])
      getSelectRows().push(temp1[0])
    }
  }
  ...
  let map={
    flag:'1',
    ctype:'2',
    tenantId:pageParameter.tenantId,
    list:uniques,
  }
  // 批量修改集团引入状态
