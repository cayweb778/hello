...
import Edit from '/@/views/boozsoft/system/customer_info/popup/apply-edit.vue';
import Add from '/@/views/boozsoft/system/customer_info/popup/apply-save.vue';
import ImprotExcel from '/@/views/boozsoft/system/customer_info/popup/apply-improtExcel.vue';
import {
  GroupCustomerAddApi,
  findGroupCusByOrg,
...
import {findAllByCusBendEq1, findAllCustomerClass} from "/@/api/record/system/customer_class";
  ...
  if(orgSaveFlag=='0'){
    ...
    await GroupCustomerAddApi(data).then(async (a)=>{
      ...
      let map={
        ...
        optAction:'新增申请',
        optContent:'操作内容【新增客户信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
      }
      await saveLog(map)
      /************** 记录操作日志 End ****************/
    })
    ...
    await groupCusAssignOrgSave([data]).then(async (a)=>{
      ...
      let map2={
        ...
        uniqueCode:orgUnique.value,
        optAction:'分配',
        optContent:'操作内容【分配客户信息】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
      }
      await saveLog(map2)
      /************** 记录操作日志 End ****************/
    })
