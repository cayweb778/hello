...
import {
  editFlagByCtypeAndOrgUnique, editFlagByCtypeAndOrgUniqueAndTenantId,
  findAllByOrgUniqueAndCtypeAndFlag, findAllByOrgUniqueAndTenantIdAndCtypeAndFlag
...
const [registerTable, {reload, getSelectRows, getDataSource, setTableData,clearSelectedRowKeys}] = useTable({
  api: findAllByOrgUniqueAndTenantIdAndCtypeAndFlag,
  columns: columns,
...
async function handleOk() {
  if(getSelectRows().length>0){
    ...
    // 批量修改集团引入状态
    await editFlagByCtypeAndOrgUniqueAndTenantId(map)
