<template>
  ...
  >
    ...
    <SupModalPop @throwData="throwModalData" @register="registerSupModalPopPage" />
  </BasicModal>
</template>
...
import {findAllSupplierByFlag} from '/@/api/record/system/supplier_group';
...
import SupModalPop from '/@/views/boozsoft/system/supplier_group/popup/modalPop.vue';
/******************* 弹框加载中 **************************/
...
const loadingRef = ref(false);
  ...
  const [register, { closeModal }] = useModalInner(async (data) => {
    ...
    // 全部供应商信息
    findAllSupplierByFlag('1').then((res) => {
 ...
 async function getCodingRuleNum(){
  ...
  async function handleOk(closeflag) {
    ...
    if (formItems.value.custAbbname === '') {
      createWarningModal({ content: '请填写客户简称！' });
      return;
    ...
    if(openType.value=='add'){
      optAction='新增'
      optContent='【新增客户信息】,客户编码【'+formItems.value.custCode+'】,客户全称【'+formItems.value.custName+'】,客户简称【'+formItems.value.custAbbname+'】'
    }else{
      optAction='修改'
      optContent='操作内容【修改客户信息】,客户编码【'+formItems.value.custCode+'】,客户全称【'+formItems.value.custName+'】,客户简称【'+formItems.value.custAbbname+'】'
    }
    let map={
