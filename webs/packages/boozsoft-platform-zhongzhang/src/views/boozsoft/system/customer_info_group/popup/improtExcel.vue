...
import {sysimportCusInfoGroup} from '/@/api/record/system/customer_group';
...
const handleDownByData = async (val: any) => {
  ...
  const columns2 = ['客户编码','客户全称','客户简称','税号','分类名称','母公司','对应供应商','国家','省','市','区','详细地址','电话',
    ...
    .then(async (info: any) => {
      ...
      if (info[0].code === '200' || info[0].error === 'error') {
        ...
        if(info[0].list.length>0){
          excelResultInfo.successInfo.forEach(async (a)=>{
            /************** 记录操作日志 ****************/
            let map={
              ...
              optRange:'2',
              optAction:'导入',
              optContent:'操作内容【导入客户信息】,客户编码【'+a.custCode+'】,客户全称【'+a.custName+'】,客户简称【'+a.custAbbname+'】',
            }
            await saveLog(map)
            /************** 记录操作日志 END ****************/
          })
