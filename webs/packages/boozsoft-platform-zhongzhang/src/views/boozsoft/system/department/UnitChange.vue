  ...
  <div>
    <div style="display: inline-block;float: left;margin-left: 1%;font-size: 14px;">
      ...
      <a-input
        ...
        style="width: 70px;border-radius: 4px;text-align: center"
      >
        <template #suffix> <SearchOutlined @click="openSelection"/>{{}}</template>
      </a-input>
      &emsp;&emsp;{{pageParameter.ifUnit?'单位':'公司'}}简称：<span style="color: black;font-weight: bold" class="clickSpan">{{pageParameter.companyName}}</span>
    </div>
  </div>
</template>
...
let codeChange =  computed(()=>props.modelValue.companyCode)
const modify = (code)=>{
...
watch(totalChange, async (a)=>{
    if (a <= 0){
      ...
      let flushCode = tempValue.value || defalutCompanyCodeValue
      let defaultObj = authorizedList.value.filter(item=>item.coCode == flushCode)[0]
      pageParameter.companyCode = defaultObj.coCode
      pageParameter.companyName = defaultObj.accNameCn
    }else {
      // 切换成之后未取消焦点 导致返回一次切换失败
    }
  }
)
watch(codeChange, async (a)=>{
  if (defalutCompanyCodeValue != props.modelValue.companyCode){
    pageParameter.companyCode = props.modelValue.companyCode
    pageParameter.companyName = props.modelValue.companyName
  }
  }
