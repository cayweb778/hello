...
import {excelPsn as excelSysPsn} from "/@/api/record/system/psn";
...
async function checkExcel(){
  ...
  if (list.value.length>0) {
    ...
    for (let i = 0; i < list.value.length; i++) {
      ...
      //判断是否为自动编码
      if (isAutoCode.value=='1' && excelValue.value == '1') {
        if (item.psnCode == null || item.psnCode == '') {
...
async function handleOk() {
  ...
  if (msg=='') {
    ...
    if (isAuto.value=='0'){
      ...
      //账套表插入数据
      await useRouteApi(excelSysPsn,{schemaName:tenantId})(list.value)
    }
...
async function getMaxCode(){
  // 读取编码规则-人员
  let str = ""
  let temp= await getCodingRule('1-1')
  if(temp.id==null){
    return false
  }
