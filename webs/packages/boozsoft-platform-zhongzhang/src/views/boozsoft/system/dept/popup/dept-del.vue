    ...
    >
      <template #title>
        ...
        <span style="font-size: 24px;line-height:30px;margin-top: 10px;margin-left: 10px;">
          <DeleteOutlined style="font-size: 28px;font-weight: bold"/>&nbsp;&nbsp;部门回收站
        </span>
        </div>
      </template>
      <div class="nc-open-content" style="margin:0;padding:0;">
        <div class="open-content-up" style="margin:0;padding:0;">
          <div style="background:#0096c7;padding:10px;border-radius: 2px;margin-bottom:10px;margin-top: 10px;display: flex;justify-content : space-between;">
            <div class="a1">
...
async function reloadDept(){
  const res:any = await useRouteApi(findDeptDelList,{schemaName: dynamicTenantId})({})
  psnList.value = res
    ...
    createConfirm({
      ...
      onOk: async () => {
        for (let i = 0; i < checkRow.value.length; i++) {
          ...
          item.delName = null
          item.delDate = null
          await useRouteApi(saveDept, {schemaName: dynamicTenantId})(item)
