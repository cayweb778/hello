    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">资产组</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.bsCode = item['资产组编码']
      item1.bsName = item['资产组名称']
      item1.flag = item['状态']
      //匹配对应参数
      item1.isEnd = '1'
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      if (deptCode==null || deptCode==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行资产组编码为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      //判断导入名称是否为空
      const deptName = item1.bsCode
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行资产组名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
        ...
        if (i!=j){
          if (item1.bsCode==item2.bsCode){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行资产组编码信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
          if (item1.bsName==item2.bsName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行资产组名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function exportExcel() {
  const arrHeader = ['资产组编码','资产组名称'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '资产组导入模板.xlsx',
  });
}
const handleClose = () => {
  closeModal()
