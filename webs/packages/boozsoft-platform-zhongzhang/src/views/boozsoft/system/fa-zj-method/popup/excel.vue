    ...
    <div class="import-centent-div" >
      <div class="import-info-div">
        <div class="import-div-top">
          ...
          <div>
            <span>导入内容：</span><span style="font-weight: bold;font-size: 20px;">折旧方法</span><br/><br/>
            <span>模板样式：</span>
            <Select
...
import {getDeptList} from '/@/api/record/system/fa-zj'
const {
  createErrorModal
} = useMessage()
...
function loadDataSuccess(excelDataList) {
  ...
  if (items.length>0) {
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const item1: any = {}
      item1.zjName = item['折旧方法']
      item1.lformula = item['月折旧率公式']
      item1.eformula = item['月折旧额公式']
      item1.zjType = item['类型']
      item1.flag = item['状态']
      //匹配对应参数
      item1.bend = '1'
      //上级名称
      const pname = item1.zjName
      if (pname!=null || pname!=''){
        const d = deptList.value.filter(v=> v.zjName === pname)
        if(d.length > 0){
          createErrorModal({
            iconType: 'warning',
            title: '提示',
            content: '第'+(i+1)+'行折旧方法不能重复,不能进行信息导入'
          })
        }
      }
    ...
    for (let i=0; i<list.value.length; i++){
      ...
      //判断导入名称是否为空
      const deptName = item1.zjName
      if (deptName==null || deptName==''){
        createErrorModal({
          iconType: 'warning',
          title: '提示',
          content: '第'+(i+1)+'行折旧方法名称为空,不能进行信息导入'
        })
        list.value = []
        return false
      }
      for (let j=0; j<list.value.length; j++) {
        ...
        if (i!=j){
          ...
          if (item1.zjName==item2.zjName){
            createErrorModal({
              iconType: 'warning',
              title: '提示',
              content: '第'+i+'行折旧方法名称信息与第'+j+'行的信息重复，请修改后重新导入！'
            })
            list.value = []
            return false
          }
...
function exportExcel() {
  const arrHeader = ['折旧方法','月折旧率公式','月折旧额公式'];
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '折旧方法导入模板.xlsx',
  });
}
const handleClose = () => {
  closeModal()
