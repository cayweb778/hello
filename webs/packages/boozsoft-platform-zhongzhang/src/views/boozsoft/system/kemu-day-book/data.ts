...
} from "/@/api/ri_ji_zhang/ri_ji_zhang";
...
export async function initPageBasicData(condition, dynamicTenantId) {
  ...
  const res = await useRouteApi(findAccountByBank, {schemaName: dynamicTenantId})(condition.variable)
...
async function getTheLatestOpeningBalance(condition, dynamicTenantId) {
  ...
  let iyperiod
  if (condition.variable.periodStart != null || condition.variable.periodStart != null) {
    iyperiod = condition.variable.periodStart.split('-')[0]
  } else {
    iyperiod = condition.variable.dateStart.split('-')[0]
  }
  let list = await useRouteApi(findAccountByBankAndQc, {schemaName: dynamicTenantId})({
    iyperiod: iyperiod + '00',
    ccode: condition.variable.accountNum
  })
  let startYearMonth = condition.variable.periodStart
  if (startYearMonth.substring(5, 7) == thisYearOneQcMonth) {
    return list
  } else {
    // 获取中间的期间
    let startOld = condition.variable.periodStart
    let endOld = condition.variable.periodEnd
    ...
    data.variable.periodStart = year + thisYearOneQcMonth
    data.variable.periodEnd = year + supplementMonth
    const res = await useRouteApi(findAccountByBank, {schemaName: dynamicTenantId})(data)
    condition.variable.periodStart = startOld
    condition.variable.periodEnd = endOld
