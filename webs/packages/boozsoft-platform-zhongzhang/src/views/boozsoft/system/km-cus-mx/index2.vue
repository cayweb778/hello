      ...
      <div class="app-container-top">
        <div style="width: 100%;margin-top: 5px;height: 100px;">
          <div style="width: 40%;padding-top:10px;">
            <div>
              <AccountPicker theme="one" readonly="" @reloadTable="dynamicAdReload" style="margin-top: 5px;margin-left: 5px;"/>
            </div>
          ...
          <div style="width: 60%;float: right;margin-top: -54px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目客户明细账</span>
            ...
            <span style="display: block;color: black;font-size: 14px;margin-left: 10px;margin-top: 10px;">期间：{{ time.strDate }} - {{ time.endDate }}
            <span style="float: right;margin-top: 5px;">
              <div>
                ...
                <a-popover placement="bottom">
                  <template #content>
                    <span class="group-btn-span-special2" @click="onChangeSwitch('J')" :style="pageParameter.queryMark=='J'?{backgroundColor: '#0096c7',color: 'white'}:''"><SortDescendingOutlined/>&nbsp;金额式&emsp;&ensp;<CheckOutlined v-if="pageParameter.queryMark=='J'"/></span><br/>
                    <span class="group-btn-span-special2" @click="onChangeSwitch('SJ')" :style="pageParameter.queryMark=='SJ'?{backgroundColor: '#0096c7',color: 'white'}:''"><SortAscendingOutlined/>&nbsp;数量金额式<CheckOutlined v-if="pageParameter.queryMark=='SJ'"/></span><br/>
                    <span class="group-btn-span-special2" @click="onChangeSwitch('WJ')" :style="pageParameter.queryMark=='WJ'?{backgroundColor: '#0096c7',color: 'white'}:''"><SortDescendingOutlined/>&nbsp;外币金额式<CheckOutlined v-if="pageParameter.queryMark=='WJ'"/></span><br/>
                  </template>
                  <a-button>
            ...
            <template #summary>
              <TableSummary fixed>
                ...
                <TableSummaryRow style="background-color: #cccccc;" v-if="pageParameter.queryMark=='SJ' || pageParameter.queryMark=='WJ'">
                  <TableSummaryCell class="nc-summary" :index="0" :align="'left'" :colSpan="7" style="border-right: none;">合计</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='SJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.ndS}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='WJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.nfrat_md}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.md}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='SJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.ncS}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='WJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.nfrat_mc}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.mc}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;"/>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='SJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue_num}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" v-if="pageParameter.queryMark=='WJ'" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue_nfrat}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue}}</TableSummaryCell>
                </TableSummaryRow>

                <TableSummaryRow style="background-color: #cccccc;" v-if="pageParameter.queryMark=='SWJ'">
                  <TableSummaryCell class="nc-summary" :index="0" :align="'left'" :colSpan="8" style="border-right: none;">合计</TableSummaryCell>
                  ...
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.mc}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;"/>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue_num}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue_nfrat}}</TableSummaryCell>
                  <TableSummaryCell class="nc-summary" :index="0" :align="'right'" style="border-right: none;">{{summaryTotals.yue}}</TableSummaryCell>
                </TableSummaryRow>
              </TableSummary>
            </template>
          </BasicTable>
        <div class="pagination-text" :style="{top: (pageParameter.queryMark=='J'?windowHeight+60:windowHeight+90)+'px'}" v-show="showPageNumber">
          {{`共 ${pageNumber}条记录&emsp;每页 1000 条`}}
        </div>
      </div>
    </div>
    <Query @save="loadPage" queryType="科目客户明细表" @register="registerQueryPage" />
...
import { findAll,exportAll, exportAll2 } from '/@/api/record/km-cus-mx/data';
import Query from "/@/views/boozsoft/system/km-cus-yue/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-cus-mx/popup/print.vue";
//////////////////////////////// END /////////////////////////////////////////
...
import {kmCusMxGlStore} from "/@/api/record/km-cus-mx/cusKmMxColumn";
...
const lanMuData = {'accId':'','menuName':'科目客户明细账','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
function thisPageImport() {
  ...
  let ccodeName=kmList.value.filter(v=>v.ccode===pageParameter.km)[0].ccodeName
  const title = ['科目：'+pageParameter.km+'-'+ccodeName+'              期间：'+ time.strDate +' - '+ time.endDate +'         单位：元']
  ...
  data2.forEach(v=>{
    v.cunitPrice=v.cunitPrice==null|| v.cunitPrice==0?'':v.cunitPrice
    ...
    v.ljmc=v.ljmc==null|| v.ljmc==0?'':moneyformat(v.ljmc)
    v.ljmd=v.ljmd==null|| v.ljmd==0?'':moneyformat(v.ljmd)
    v.mc=v.mc==null || v.mc==0?'':moneyformat(v.mc)
    v.md=v.md==null|| v.md==0?'':moneyformat(v.md)
    v.mdF=v.mdF==null|| v.mdF==0?'':v.mdF
    v.ncS=v.ncS==null|| v.ncS==0?'':v.ncS
    v.ndS=v.ndS==null|| v.ndS==0?'':v.ndS
    v.nfrat_md=v.nfrat_md==null|| v.nfrat_md==0?'':v.nfrat_md
    v.nfrat_mc=v.nfrat_mc==null|| v.nfrat_mc==0?'':v.nfrat_mc
    v.unitMeasurement=v.unitMeasurement==null?'':v.unitMeasurement
    v.yue=v.yue==0?'':moneyformat(v.yue)
    v.yue_num=v.yue_num==0?'':v.yue_num
    v.yue_nfrat=v.yue_nfrat==0?'':v.yue_nfrat
  })
  //样式靠右列
  let rightrow:any=[]
  //样式靠左列
  ...
  exportExcel(sheet, 'xlsx',accNameAll.value+'-科目客户明细账-'+time.strDate+'-'+time.endDate)
...
async function companyExportView() {
  ...
  pageParameter.ccodeList=kmList.value.map(v=>v.ccode).join(',')
...
async function onlySheetExport() {
  ...
  let temp1:any=[{title: '科目编码',key:'temp1',dataIndex:'temp1'},{title: '科目名称',key:'temp2',dataIndex:'temp1'}]
  ...
  if(pageParameter.queryMark === 'J'){
    ...
    rightrow=['I','J','K','L','N','O']
  }else{
    leftrow=['A','B','C','D','E','F','G']
    rightrow=['J','K','L','M','N','O','Q','R','S']
  }
  pageParameter.ccodeList=kmList.value.map(v=>v.ccode).join(',')
  ...
  .then(list=>{
    ...
    exportExcel2(sheet, 'xlsx',accNameAll.value+'-科目客户明细账-'+time.strDate+'-'+time.endDate)
    compState.loading = false;
  })
}
...
const loadPrint = async (obj) => {
  ...
  if(pageParameter.queryMark==='J'){
    ...
    data.forEach((item,index) => {
      ...
      item1[2]=item.ccode==null?'':item.ccode
      item1[3]=item.ccodeName==null?'':setString(item.ccodeName,37)
      item1[4]=item.cdigest==null?'':item.cdigest
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '科目客户明细账', '', '', '', '', '', ''],
          ['会计科目：' + pageParameter.km+'-'+ccodeName,'', '期间:' + time.strDate+' - '+time.endDate,'', '', '',printBz,'','单位：元'],
          ['期间', '凭证号','客户编码','客户名称', '摘要', '借方', '贷方', '方向', '余额']],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
        margin: {
  ...
  if(pageParameter.queryMark==='SJ'){
    ...
    data.forEach((item,index) => {
      ...
      item1[0]=item.dbillDate==null?'':item.dbillDate
      item1[1]=item.inoId==null?'':item.inoId
      item1[2]=item.ccode==null?'':setString(item.ccode,10)
      item1[3]=item.ccodeName==null?'':setString(item.ccodeName,30)
      item1[4]=item.cdigest==null?'':setString(item.cdigest,15)
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '科目客户明细账', '', '', '', '', '', '', '', ''],
          ['会计科目：' + pageParameter.km+'-'+ccodeName,'', '','期间:'+ time.strDate+' - '+time.endDate, '', '', '', '', printBz,'','','单位：元'],
          ['期间', '凭证号','客户编码','客户名称', '摘要','单位', '借方', '', '贷方', '','余额',''],
  ...
  if(pageParameter.queryMark==='WJ'){
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '科目客户明细账', '', '', '', '', '', '', ''],
          ['会计科目：' + pageParameter.km+'-'+ccodeName+currencyType,'', '','期间:'+ time.strDate+' - '+time.endDate, '', '', '', printBz,'','','单位：元'],
          ['期间', '凭证号','客户编码','客户名称', '摘要', '借方', '', '贷方', '','余额',''],
