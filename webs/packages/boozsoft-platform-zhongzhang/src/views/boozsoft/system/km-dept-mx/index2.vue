<template>
  <div>
      ...
      <div class="app-container-top">
        <div style="width: 100%;margin-top: 5px;height: 100px;">
          ...
          <div style="width: 60%;float: right;margin-top: -40px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目部门明细账</span>
    ...
    <Query @save="loadPage" queryType="科目部门明细表" @register="registerQueryPage" />
...
import { findAll,exportAll, exportAll2 } from '/@/api/record/km-dept-mx/data';
import Query from "/@/views/boozsoft/system/dept-km-mx/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-dept-mx/popup/print.vue";
//////////////////////////////// END /////////////////////////////////////////
...
import {kmDeptMxGlStore} from "/@/api/record/km-dept-mx/deptKmMxColumn";
...
const lanMuData = {'accId':'','menuName':'科目部门明细账','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
onMounted( async ()=>{
  if (JSON.stringify(routemsg.value).length === 2) {
    ...
    deptList.value = await useRouteApi(findDeptAll,{schemaName:dynamicTenantId.value})('');
...
function thisPageImport() {
  ...
  exportExcel(
    sheet,
    'xlsx',
    accNameAll.value + '-科目部门明细账-' + time.strDate + '-' + time.endDate,
  );
...
async function companyExportView() {
  ...
  await useRouteApi(exportAll, { schemaName: dynamicTenantId })(pageParameter).then((list) => {
    for (var key in list) {
...
async function onlySheetExport() {
  ...
  let temp1: any = [
    { title: '科目编码', key: 'temp1', dataIndex: 'temp1' },
    { title: '科目名称', key: 'temp2', dataIndex: 'temp1' },
  ];
  ...
  pageParameter.ccodeList = kmList.value.map((v) => v.ccode).join(',');
  await useRouteApi(exportAll2, { schemaName: dynamicTenantId })(pageParameter).then((list) => {
    const data = list;
    data.forEach((v) => {
      v.ccode = v.ccode == null ? '' : v.ccode;
      v.ccodeName = v.ccodeName == null ? '' : v.ccodeName;
...
const loadPrint = async (obj) => {
  ...
  if (pageParameter.queryMark === 'J') {
    ...
    data.forEach((item, index) => {
      ...
      item1[3] = item.ccodeName == null ? '' : item.ccodeName
      item1[4] = item.cdigest == null ? '' : setString(item.cdigest, 37);
      ...
      if (item.ccode != '合计') {
        ...
        if (index > 0 && index % 27 == 0) {
          ...
          mc = 0;
          yue = 0;
          if (index > 0 && index % 27 == 0 && index < data.length - 1) {
            let item2 = {};
            item2[0] = '';
            item2[1] = '承前页';
            item2[2] = '';
    ...
    useNewPrint({ data: ['l', 'px', 'a4', true] }, (doc) => {
      ...
      doc.autoTable({
        head: [
          ['', '', '', '', '科目部门明细账', '', '', '', ''],
          [
            '会计科目：' + pageParameter.km + '-' + ccodeName,
            '',
            '',
            '',
            '期间:' + time.strDate + ' - ' + time.endDate,
            '',
            printBz,
            '',
            '单位：元',
          ],
          ['期间', '凭证号', '部门编码', '部门名称', '摘要', '借方', '贷方', '方向', '余额'],
        ],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
        ...
        addPageContent: (data) => {
          ...
          doc.autoTableText('核算单位：' + accNameAll.value, tabMarginLeft, data.cursor.y + 3, 0);
  ...
  if (pageParameter.queryMark === 'SJ') {
    ...
    data.forEach((item, index) => {
      ...
      item1[1] = item.inoId == null ? '' : item.inoId;
      item1[2] = item.ccode == null ? '' : item.ccode;
      item1[3] = item.ccodeName == null ? '' : setString(item.ccodeName, 20);
      item1[4] = item.cdigest == null ? '' : setString(item.cdigest, 30);
  ...
  if (pageParameter.queryMark === 'WJ') {
    ...
    data.forEach((item, index) => {
      ...
      item1[3] = item.ccodeName == null ? '' : setString(item.ccodeName, 15);
      item1[4] = item.cdigest == null ? '' : setString(item.cdigest, 28);
    ...
    // 查询科目外币名称
    let code = await useRouteApi(company_findByCcode, { schemaName: dynamicTenantId })({
      ccode: pageParameter.km,
    });
    let currencyType = '';
    ...
    useNewPrint({ data: ['l', 'px', 'a4', true] }, (doc) => {
      ...
      doc.autoTable({
        head: [
          ['', '', '', '', '科目部门明细账', '', '', '', '', '', ''],
          [
            '会计科目：' + pageParameter.km + '-' + ccodeName + currencyType,
            '',
            '',
            '',
            '期间:' + time.strDate + ' - ' + time.endDate,
        ...
        didParseCell(data) {
          ...
          if (data.section == 'head' && data.row.index == 1) {
            ...
            if (data.column.index == 0) {
              ...
              data.cell.styles.cellPadding = { left: 18 };
              data.cell.colSpan = 4;
              data.cell.styles.halign = 'center';
  ...
  if (pageParameter.queryMark === 'SWJ') {
    message.warn('暂不支持 数量外币式 打印!');
    compState.loading = false;
    return false;
  }
};
