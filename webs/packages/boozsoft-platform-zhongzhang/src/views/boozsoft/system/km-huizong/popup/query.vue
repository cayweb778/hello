<template>
  ...
  >
    ...
    <div class="nc-open-content" style="height: 100%" >
      ...
      <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;科目汇总表</span>
...
async function dynamicAdReload(data) {
  ...
  pageParameter.companyCode=data.coCode
  pageParameter.companyName=data.companyName
  pageParameter.accId=data.accId
  pageParameter.database=data.accountMode
  // 获取账套信息
...
async function handleOk1() {
  ...
  let map={
    ...
    optAction:'查询',
    optContent:'操作内容【科目汇总查询】,账套代码【'+pageParameter.companyCode+'】,账套名称【'+pageParameter.companyName+'】,查询条件【查询日期：'+strDate.value+','+endDate.value+'】,' +
      '科目编码【'+minKm.value+','+maxKm.value+'】,科目类型【'+styleValue.value+'】,凭证类型【'+pzType.value+'】,级次【'+minJc.value+','+maxJc.value+'】,包含未记账【'+ishaveRjz.value+'】'
  }
  await saveLog(map)
  /************** 记录操作日志 ****************/
