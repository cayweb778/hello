<template>
 <div>
     ...
     <div class="app-container-top">
       <div style="width: 100%;margin-top: 5px;height: 100px;">
         ...
         <div style="width: 60%;float: right;margin-top: -40px;">
           <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目供应商明细账</span>
   ...
   <Query @save="loadPage" queryType="科目供应商明细表" @register="registerQueryPage" />
...
import { findAll,exportAll,exportAll2 } from '/@/api/record/km-sup-mx/data';
import Query from "/@/views/boozsoft/system/km-sup-yue/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-sup-mx/popup/print.vue";
import { kmSupMxGlStore } from "/@/api/record/km-sup-mx/supKmMxColumn";
//////////////////////////////// END /////////////////////////////////////////
...
import { findSupAll} from "/@/api/record/generalLedger/data";
...
const lanMuData = {'accId':'','menuName':'科目供应商明细账','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
onMounted( async ()=>{
  if (JSON.stringify(routemsg.value).length === 2) {
    ...
    // 供应商
    deptList.value = await useRouteApi(findSupAll,{schemaName: routemsg.value.dynamicTenantId})('');
    // 用于显示
...
function thisPageImport() {
  ...
  exportExcel(sheet, 'xlsx',accNameAll.value+'-科目供应商明细账-'+time.strDate+'-'+time.endDate)
...
async function onlySheetExport() {
  ...
  await useRouteApi(exportAll2,{schemaName:dynamicTenantId})(pageParameter)
    .then(list=>{
      ...
      exportExcel2(sheet, 'xlsx',accNameAll.value+'-科目供应商明细账-'+time.strDate+'-'+time.endDate)
      compState.loading = false;
    })
}
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '科目供应商明细账', '', '', '', '', ''],
          ...
          ['期间', '凭证号','供应商编码','供应商名称', '摘要', '借方', '贷方', '方向', '余额']],
        body: printList,
        // startY: 60,
        styles: tableStyle(),
        margin: {
  ...
  if(pageParameter.queryMark==='SJ'){
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '', '科目供应商明细账', '', '', '', '', '', '', ''],
          ['会计科目：' + pageParameter.km+'-'+ccodeName,'', '','','期间:'+ time.strDate+' - '+time.endDate, '', '', '', printBz,'','','单位：元'],
          ['期间', '凭证号','供应商编码','供应商名称', '摘要','单位', '借方', '', '贷方', '','余额',''],
    ...
    useNewPrint({data: ['l', 'px', 'a4', true]}, (doc) => {
      ...
      doc.autoTable({
        head: [['', '', '', '', '科目供应商明细账', '', '', '', '', '', ''],
          ...
          ['期间', '凭证号','供应商编码','供应商名称', '摘要', '借方', '', '贷方', '','余额',''],
