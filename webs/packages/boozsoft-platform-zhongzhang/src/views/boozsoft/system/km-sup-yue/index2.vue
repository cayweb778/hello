<template>
  <div>
      ...
      <div class="app-container-top">
        <div style="width: 100%;margin-top: 5px;height: 100px;">
          ...
          <div style="width: 60%;float: right;margin-top: -40px;">
            <span style="font-size: 24px;color: #0096c7;font-weight: bold;">科目供应商余额表</span>
    ...
    <div class="app-container" style="margin-top: -40px;">
      ...
      <Query @save="loadPage" queryType="科目供应商余额表" @register="registerQueryPage" />
...
import { findAll ,exportAll,exportAll2} from '/@/api/record/km-sup-yue/data';
import Query from "/@/views/boozsoft/system/km-sup-yue/popup/query.vue";
import Print from "/@/views/boozsoft/system/km-sup-yue/popup/print.vue";
import { kmSupYueGlStore } from "/@/api/record/km-sup-yue/kmSupYueColumn";
//////////////////////////////// END /////////////////////////////////////////
...
const lanMuData = {'accId':'','menuName':'科目供应商余额表','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
// 表格参数
...
async function goRouter(uniqueCode) {
  ...
  router.push({
    path: '/zhongZhang/account-book/abf-supplier-contacts/abfsc-kemu-mxable',
    query: pageParameter,
  });
}
onMounted( async ()=>{
...
function thisPageImport(){
  ...
  exportExcel(sheet, 'xlsx',accNameAll.value+'-科目供应商余额表-'+time.strDate+'-'+time.endDate)
...
async function onlySheetExport() {
  ...
  .then(list=>{
    ...
    exportExcel2(sheet, 'xlsx',accNameAll.value+'-科目供应商余额表-'+time.strDate+'-'+time.endDate)
    compState.loading = false;
  })
}
