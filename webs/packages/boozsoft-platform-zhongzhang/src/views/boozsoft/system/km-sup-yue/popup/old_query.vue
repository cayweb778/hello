...
async function handleOk() {
  if(kmList.value.length===0){
    message.error('请先设置供应商辅助科目!');
    return;
  }
  if(deptList.value.length===0){
    message.error('请先设置供应商信息!');
    return;
  }
...
async function newfindAllCode(s,start) {
  ...
  return list.filter(ck=>ck.bsup==='1' && ck.bend==='1');
}
...
async function AllCondition() {
  ...
  // 供应商
  deptList.value = await useRouteApi(findSupAll,{schemaName:database.value})('');
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'科目供应商余额表')
