  ...
  <div class="app-container">
    <div class="app-container-head">
      <div class="container-head-title" style="float: none;text-align: center;display: revert;">
        <b class="noneSpan">{{ pageParameter.projectName }}-项目明细总账</b>
      </div>
      ...
      <div style="margin-top: .5%;">
        <div style="display: inline-block;float: left">
          <span style="font-size: 16px;color: #aaa;font-weight: bold">部门：</span>
          <a-select
            show-search
            placeholder="部门选择"
        ...
        <div style="display: inline-block;float: left;margin-top: .8%;font-size: 16px;">
          <!--&emsp;&emsp;项目大类：<span style="font-size: 16px;font-weight: bold">{{ pageParameter.projectName }}</span>-->
          &emsp;&emsp;币种：<span style="font-size: 16px;font-weight: bold">{{ bzName }} </span>
          &emsp;&emsp;样式：<span style="color: black;font-weight: bold">{{ styleName }}</span>
          &emsp;&emsp;&emsp;期间：<span style="color: black;font-weight: bold">{{ pageParameter.strDate }}~{{ pageParameter.endDate }}</span>
        </div>
...
<script setup lang="ts">
  ...
  import { useProjectGlStore } from '/@/api/record/generalLedger/projectGeneralLedger'

  import {onMounted, reactive, ref} from "vue";
  import moment from "moment";
  import {useCompanyOperateStoreWidthOut} from "/@/store/modules/operate-company";
  import { initDynamics,assemblyDynamicColumn} from "./data";
  import Query from "/@/views/boozsoft/system/projcet-generalledger/popup/query.vue";
  ...
  import {findAllProject} from "/@/api/record/generalLedger/data";
  ...
  // 全局常量
  const glStore = useProjectGlStore()
  ...
  const lanMuData = {'accId':'','menuName':'项目总账','type':'',objects: '',username:useUserStoreWidthOut().getUserInfo.username}
  // 表格参数
  ...
  const CrudApi = {
    list: useRouteApi(findAllProject,{schemaName:databasesName.value}),
  ...
  const loadPage = (data)=>{
    ...
    //筛选科目 如果有部门核算 则显示所有  没有只显示全部
    let flg = false
    kmList.value.forEach(v=>{
      if(v.bdept === '1'){
        flg = true
        return
      }
