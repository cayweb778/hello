...
import {
  ...
  findBzAll,
  findCodeKmByPeriod, findProject, findProjectCategory, findProjectClass,
...
async function handleOk() {
  ...
  formItems.value.minDept = minDept.value===undefined || minDept.value===''?deptList.value[0].uniqueCode:minDept.value;
  formItems.value.maxDept = maxDept.value===undefined|| maxDept.value===''?deptList.value[0].uniqueCode:maxDept.value;
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'项目科目明细账')
