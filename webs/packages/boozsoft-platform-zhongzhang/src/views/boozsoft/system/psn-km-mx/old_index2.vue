...
import Query from "/@/views/boozsoft/system/psn-km-mx/popup/query.vue";
...
onMounted( async ()=>{
  if (JSON.stringify(routemsg.value).length === 2) {
    ...
    pageParameter.minPsn=routemsg.value.minDept
...
const loadPage = async (data)=>{
  ...
  pageParameter.minPsn = data.minPsn
  pageParameter.maxPsn = data.maxPsn
...
function thisPageImport() {
  ...
  let ccodeName=deptList.value.filter(v=>v.uniqueCode===pageParameter.minPsn)[0].label
