        ...
        <a-tabs type="card" tabPosition="left" style="height: 400px" v-model:activeKey="currentTabsKey" @change="tabChange">
          <a-tab-pane key="" tab="标准模式">
            <ul>
              ...
              <li>
                <span>科目类型：</span>
                <a-select style="width: 150px" v-model:value="styleValue" @change="styleChange">
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;
                <span>凭证类别：</span>
                <a-select mode="tags" v-model:value="pzType" style="width: 155px;" :class="pzType=='未发现凭证类型授权'?'selectColorRed':'selectColorBlack'">
              ...
              <li>
                ...
                <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template></a-select>
                <span>字号：</span>
                <a-select style="width: 180px" v-model:value="fontSize">
...
async function handleOk() {
  ...
  formItems.value.minPsn = minDept.value===undefined?deptList.value[0].uniqueCode:minDept.value;
  formItems.value.maxPsn = maxDept.value===undefined?deptList.value[0].uniqueCode:maxDept.value;
  ...
  const queryPlanEntity={
    ...
    queryConditions:{
      ...
      minDept:formItems.value.minPsn,
      maxDept:formItems.value.maxPsn,
      bz:bz.value,
...
async function newfindAllCode(s,start) {
  ...
  return list.filter(ck=>ck.bperson==='1' && ck.bend==='1');
}
...
async function getQueryPlan() {
  let queryPlanIngo= await findByQueryPlan(database.value,'个人科目明细账')
