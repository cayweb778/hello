...
<!--        <div style="font-size: 16px">
    ...
    <div class="app-container" style="padding:10px;height:690px;overflow: auto !important;">
      ...
      <div
        style="margin-left: 10%;margin-right: 10%;display: flex;width: 80%;justify-content : space-between;">
      ...
      <div style="margin-left: 10%;margin-right: 10%;display: flex;width: 80%;">
        <table
          ...
          <tr class="odd" v-for="row in itemConfig.table">
            ...
            <td style="width: 200px;text-align: right;">{{ toThousandFilter(row.bennianLeijiMoney) }}</td>
          </tr>
          </tbody>
        </table>
      </div>
...
async function saveData(data) {
  ...
  if (itemConfig.value.templateId != '' && itemConfig.value.templateId != null) {
    ...
    const benyueLeijiList = await useRouteApi(findByAccvoucherOrderAndTemplateId, {schemaName: dynamicTenantId})({
      templateId: data.templateId,
      iyperiod1: data.iyperiod1,
      iyperiod2: data.iyperiod2,
      jizhang: data.jizhang
    })
    const bennianLeijiList = await useRouteApi(findByAccvoucherOrderAndTemplateId, {schemaName: dynamicTenantId})({
      templateId: data.templateId,
...
async function printDefaultPDF() {
  ...
  if (itemConfig.value.dataType == '1') {
    pdfName = '利润表_' + itemConfig.value.iyear + '年' + itemConfig.value.iperiod + '月_'+pageParameter.companyName+'.pdf'
  } else if (itemConfig.value.dataType == '2') {
    pdfName = '利润表_' + itemConfig.value.iyear + '年第' + itemConfig.value.jidu + '季度_'+pageParameter.companyName+'.pdf'
  } else {
    pdfName = '利润表_' + itemConfig.value.iyear + '年_'+pageParameter.companyName+'.pdf'
  }
...
function exportExcel () {
  ...
  // 类型默认为xlsx
  if (itemConfig.value.dataType == '1') {
    writeExcel(wb, bookType, '利润表_' + itemConfig.value.iyear + '年' + itemConfig.value.iperiod + '月_'+pageParameter.companyName)
  } else if (itemConfig.value.dataType == '2') {
    writeExcel(wb, bookType, '利润表_' + itemConfig.value.iyear + '年第' + itemConfig.value.jidu + '季度_'+pageParameter.companyName)
  } else {
    writeExcel(wb, bookType, '利润表_' + itemConfig.value.iyear + '年_'+pageParameter.companyName)
  }
}
/************************导出excel结束**************************************/
