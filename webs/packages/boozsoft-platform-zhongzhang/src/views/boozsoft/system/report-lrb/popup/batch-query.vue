    ...
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;批量生成利润表</span>
        </div>
        <div class="nc-open-content">
          <div class="open-content-up">
            ...
            <div class="border-div">
              <span>业务范围</span>
              <AccountPicker theme="three" @reloadTable="dynamicAdReload" readonly class="account" style="text-align: center;"/>
            </div>
            <div class="border-div">
              <span>报表模板</span>
              <label>选择模板：</label>
              <a-select v-model:value="selectModel" style="width: 350px;text-align: center;font-weight: bold;">
...
async function batchSave() {
  if (radiovalue.value == '1') {
    ...
    for (const item of monthList) {
      ...
      template.iyperiod1 = formItems.value.year + template.iperiod
      template.iyperiod2 = formItems.value.year + template.iperiod
...
async function saveOne(timeDate) {
  ...
  if (itemConfig.value.templateId != '' && itemConfig.value.templateId != null) {
    ...
    const benyueLeijiList = await useRouteApi(findByAccvoucherOrderAndTemplateId, {schemaName: dynamicTenantId})({
      templateId: template.templateId,
      iyperiod1: template.iyperiod1,
      iyperiod2: template.iyperiod1,
      jizhang: template.jizhang
    })
    const bennianLeijiList = await useRouteApi(findByAccvoucherOrderAndTemplateId, {schemaName: dynamicTenantId})({
      templateId: template.templateId,
      iyperiod1: template.iyear + '00',
      iyperiod2: template.iyperiod2,
      jizhang: template.jizhang
    })
