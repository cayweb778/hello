...
<!--        <div class="container-head-title">
        ...
        <div class="container-head-title" style="padding-left: 40%;text-align: center;margin-top: 30px;">
          <b class="noneSpan" style="font-size: 36px;">系统模板</b>
        </div>
...
import { findTemplateBySysFlag, findByReportNameAndFlag, deleteTemplate, saveTemplate, editFlag } from '/@/api/record/system/report-template'

import { BasicTable, useTable } from '/@/components/Table'
import Edit from './popup/edit.vue'
import ZcfzbEdit from './popup/zcfzb-edit.vue'
...
const {register:registerZcfzbEdit,open :openZcfzbEditPage2}=openZcfzbEditPage()
const dynamicTenantId = ref(getCurrentAccountName(true))
const defaultAdName = ref(useCompanyOperateStoreWidthOut().getSchemaName)
const CrudApi = {
  list: useRouteApi(findTemplateBySysFlag, {schemaName: dynamicTenantId}),
  columns: [
    {
      title: 'ID',
      dataIndex: 'id',
...
const addModelPage = async (data) => {
  /*loadMark.value = true
  await useRouteApi(saveTemplate, {schemaName: dynamicTenantId})(data)
...
const del = async(data:any) => {
  await useRouteApi(deleteTemplate,{schemaName: dynamicTenantId})(data)
  // alert('删除成功！')
  createSuccessModal({
    iconType: 'success',
    title: '删除',
...
async function delList() {
  if (checkRow.value.length>0) {
    ...
    if (num==0) {
      for (let i = 0; i < checkRow.value.length; i++) {
        const item = checkRow.value[i]
        await useRouteApi(deleteTemplate,{schemaName: dynamicTenantId})(item)
      }
      createSuccessModal({
        iconType: 'success',
        title: '删除',
...
async function reloadCurrentPage() {
  const res = await useRouteApi(findTemplateBySysFlag,{schemaName: dynamicTenantId})({sysFlag:'1'})
