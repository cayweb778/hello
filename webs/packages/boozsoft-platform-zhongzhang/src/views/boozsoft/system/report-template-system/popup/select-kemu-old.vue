    ...
    <template #title>
      <div style="display: flex;margin-top: 10px;margin-left: 10px;" class="vben-basic-title">
        <span style="line-height:40px;font-size: 28px;" >
          <FormOutlined style="font-size: 34px;font-weight: bold"/>&nbsp;&nbsp;报表栏目公式设置
        </span>
      </div>
    </template>
    <div class="nc-open-content" style="min-height: 375px;margin-top: 20px;">
      <div class="open-content-up">
        <a-radio-group v-model:value="formItems.base.formulaMethod" @change="checkMethod()" style="width:415px;">
          <a-radio value="2" style="width:130px;">
            <span style="color: #000000;font-weight: bold;font-size: 13px;">按科目计算</span>
          </a-radio>
          <a-radio value="1" style="width:130px;margin-left: -10px;">
            <span style="color: #000000;font-weight: bold;font-size: 13px;">按级次计算</span>
          </a-radio>
          <a-radio value="" style="width:110px;">
        ...
        <a-select v-model:value="formItems.base.valueMethod" :disabled="!showKemu" style="width: 200px;">
          ...
          <a-select-option v-if="reportName!='lrb'" value="1">余额</a-select-option>
          <a-select-option v-if="reportName!='lrb'" value="2">借方余额</a-select-option>
          <a-select-option v-if="reportName!='lrb'" value="3">贷方余额</a-select-option>
          <a-select-option v-if="reportName!='zcfzb'" value="4">借方发生</a-select-option>
          <a-select-option v-if="reportName!='zcfzb'" value="5">贷方发生</a-select-option>
        </a-select>
        <br/>

        <label style="width: 130px;">栏目显示名称：</label>
        <a-input v-model:value="formItems.base.columnShowName" placeholder="栏目名称" style="width: 30%"/>
        <label style="width: 100px;">显示行次：</label>
        ...
        <label style="width: 100px;">显示级次：</label>
        <a-select v-model:value="formItems.base.jici" style="width: 80px;">
          <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template>
          <a-select-option value="">请选择</a-select-option>
...
<!--            <th style="width: 100px;">操作</th>-->
          ...
          <tr class="odd" v-for="(row,rowIndex) in formItems.base.formulaTable">
            <td style="width: 80px;border-left: none;">
              <a-checkbox :value="rowIndex" style="width: auto;text-align: left;"/>
            </td>
...
<!--            <th style="width: 100px;;">操作</th>-->
          ...
          <tr class="odd" v-for="(row,rowIndex) in formItems.base.formulaTable">
            <td style="width: 80px;border-left: none;text-align: left;">
              <a-checkbox :value="rowIndex" style="width: auto;text-align: left;"/>
            </td>
            <td style="text-align: left;border-left: none;">
              <a-select v-model:value="row.ccode" @change="chengeKemu(row);handleChange"
                        placeholder="会计科目" show-search :filter-option="false" @search="handleSearch"
                        style="width: 100%;" class="none-border">
                <template #suffixIcon><CaretDownOutlined style="color:#666666;" /></template>
...
import {Select as ASelect,Input as AInput,InputNumber as AInputNumber,Radio as ARadio,Checkbox as ACheckbox} from "ant-design-vue"
import {useRouteApi} from "/@/utils/boozsoft/datasource/datasourceUtil";
import {useCompanyOperateStoreWidthOut} from "/@/store/modules/operate-company";
import {useMessage} from "/@/hooks/web/useMessage";
import {company_findAllCodeOrderByAsc} from "/@/api/codekemu/codekemu";
...
const [register, { closeModal }] = useModalInner( (data) => {
  ...
  //根据会计准则获取科目信息
  useRouteApi(company_findAllCodeOrderByAsc, {schemaName: dynamicTenantId})({})
    .then((res:any)=> {
      if (reportName.value == 'lrb') {
        kemuList.value = res.filter(item => item.cclass == '损益').map(item => {
          item.key = item.ccode + '%'
          return item
        })
      } else {
...
async function handleOk() {
  ...
  formItems.value.base.formulaTable = formItems.value.base.formulaTable.filter(item =>{
    return item.ccode!=null && item.ccode!=''
  })
...
function checkMethod(){
  if (formItems.value.base.formulaMethod=='1'){
    ...
    if (reportName.value!='lrb') {
      ...
      formItems.value.base.valueMethod = '4'
    }
    showHangci.value = false
    showKemu.value = true
  } else {
...
.none-border .ant-select-selector {
    border: none !important;
  }
