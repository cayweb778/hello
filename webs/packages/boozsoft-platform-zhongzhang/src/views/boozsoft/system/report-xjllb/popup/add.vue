...
<!--        <img :src="getThisIndexImg()" class="container-head-img" draggable="false">-->
        ...
        <div class="container-head-title" style="padding-left: 40%;text-align: center;margin-top: 30px;">
          <b class="noneSpan" style="font-size: 32px;">现金流量表</b>
        </div>
...
<!--        <div>{{ itemConfig.menu2 }}</div>-->
      ...
      <div style="margin-left: 10%;margin-right: 10%;display: flex;width: 80%;">
        <table
          ...
          <tbody>
          <tr class="odd" v-for="row in itemConfig.table">
      ...
      <div
        style="margin-left: 10%;margin-right: 10%;display: flex;width: 80%;display: flex;justify-content : space-between">
...
async function saveData(data) {
  ...
  if (itemConfig.value.templateId != '' && itemConfig.value.templateId != null) {
    ...
    const benyueLeijiList = await useRouteApi(findByCashOrderAndTemplateId,{schemaName: dynamicTenantId})({templateId:data.templateId,iyear:data.iyear,iperiod1:data.iperiod1,iperiod2:data.iperiod2})
    const bennianLeijiList = await useRouteApi(findByCashOrderAndTemplateId,{schemaName: dynamicTenantId})({templateId:data.templateId,iyear:data.iyear,iperiod1:'01',iperiod2:data.iperiod2})
    columnList.forEach(column=>{
      ...
      benyueLeijiList.forEach(item=>{
        if (column.id==item.columnId){
          ...
          if (column.formulaMethod=='1'){//行次计算
            if (item.fuhao=='+') {
              benyue = add(benyue,columnList.filter(item1=>{return item.ccode==item1.hangci}).map(item1=>item1.benyueMoney))
            } else {
              benyue = sub(benyue,columnList.filter(item1=>{return item.ccode==item1.hangci}).map(item1=>item1.benyueMoney))
            }
          }
        }
      })
      ...
      bennianLeijiList.forEach(item=>{
        if (column.id==item.columnId){
          ...
          if (column.formulaMethod=='1'){//行次计算
            if (item.fuhao=='+') {
              bennianLeiji = add(bennianLeiji,columnList.filter(item1=>{return item.ccode==item1.hangci}).map(item1=>item1.bennianLeijiMoney))
            } else {
              bennianLeiji = sub(bennianLeiji,columnList.filter(item1=>{return item.ccode==item1.hangci}).map(item1=>item1.bennianLeijiMoney))
            }
          }
        }
      })
    ...
    table.value = itemConfig.value.table.map((item, index) => {
      const item1 = {}
      if (item.jici=='2'){
      ...
      item1[1] = item.hangci
      item1[2] = toThousandFilter(item.benyueMoney)
      item1[3] = toThousandFilter(item.bennianLeijiMoney)
      return item1
    })
  }
}
...
// onMounted(async () => {
//   const res = await findByReportNameAndFlag("xjllb")
...
async function printDefaultPDF() {
  ...
  if (itemConfig.value.dataType == '1') {
    pdfName = '现金流量表_' + itemConfig.value.iyear + '年' + itemConfig.value.iperiod + '月_'+pageParameter.companyName+'.pdf'
  } else if (itemConfig.value.dataType == '2') {
    pdfName = '现金流量表_' + itemConfig.value.iyear + '年第' + itemConfig.value.jidu + '季度_'+pageParameter.companyName+'.pdf'
  } else {
    pdfName = '现金流量表_' + itemConfig.value.iyear + '年_'+pageParameter.companyName+'.pdf'
  }
...
function exportExcel () {
  ...
  // 类型默认为xlsx
  if (itemConfig.value.dataType == '1') {
    writeExcel(wb, bookType, '现金流量表_' + itemConfig.value.iyear + '年' + itemConfig.value.iperiod + '月_'+pageParameter.companyName)
  } else if (itemConfig.value.dataType == '2') {
    writeExcel(wb, bookType, '现金流量表_' + itemConfig.value.iyear + '年第' + itemConfig.value.jidu + '季度_'+pageParameter.companyName)
  } else {
    writeExcel(wb, bookType, '现金流量表_' + itemConfig.value.iyear + '年_'+pageParameter.companyName)
  }
}
/************************导出excel结束**************************************/
