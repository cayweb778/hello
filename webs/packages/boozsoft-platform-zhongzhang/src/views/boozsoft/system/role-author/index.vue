    ...
    <div class="app-container">
      ...
      <div class="app-container-neck">
        ...
        <div style="display: inline-block;float: left;margin-left: 2%;">
          <span>角色：</span>
          <a-select style="width: 120px;" v-model:value="saveDataModel.roleUniqueCode"
                    @change="operChange">
...
import LeftTree from '/@/views/boozsoft/system/operation-author/popup/LeftTree.vue';
...
import {
  ...
  getPlatformColumn,
  savePlatformColumn,
  getPlatformAndMenu, savePlatformAndMenu
} from '/@/api/caozuoyuan/caozuoyuan';
import {useTabs} from "/@/hooks/web/useTabs";
import router from "/@/router";
import {usePlatformsStore} from "/@/store/modules/platforms";
import {assemblyDynamicColumn, initDynamics} from "/@/views/boozsoft/system/operation-author/data";
...
const dynamicAdReload = async (e) => {
  ...
  if (operList.value.length == 0) {
    operList.value = ((await findAll()))
    if (operList.value.length > 0 && hasBlank(saveDataModel.roleUniqueCode)) {
      saveDataModel.roleUniqueCode = operList.value[1].id
...
const operChange = async (v) => {
  if (!hasBlank(saveDataModel.ztUniqueCode) && !hasBlank(saveDataModel.ztYear) && !hasBlank(saveDataModel.roleUniqueCode)) {
    loadMark.value = true
    let res = await findUserAuthor2({
      userId: saveDataModel.roleUniqueCode,
...
async function saveData() {
  ...
  if (hasBlank(saveDataModel.ztUniqueCode)) {
    ...
    showOk.value = true
  } else if (hasBlank(saveDataModel.roleUniqueCode)) {
    createWarningModal({content: '请选择角色！'})
    showOk.value = true
  } else {
    ...
    let res = await saveAuthor2(saveDataModel)
    if (res != null) {
      message.success('保存成功！')
      // operChange(saveDataModel.roleUniqueCode)
...
async function writeLog(action, a, content) {
  /************** 记录操作日志 ****************/
  if (null == content){
    content = `操作内容【${action}角色权限】,` + '账套代码【' + a.ztUniqueCode + '】,年度【' + a.ztYear + '】,角色【' + transformText('roleUniqueCode', a.roleUniqueCode) + '】'
    if (a.supervisor == '1') {
      content += '被授予的账套主管权限！'
    } else {
...
function transformText(key, value) {
  ...
  switch (key) {
    case "roleUniqueCode":
      ...
      text = arr.length == 0 ? value : arr[0].roleName
      break;
  }
  return text
}
function copyCallback(u1,u2) {
  writeLog("复制",{},`操作内容【复制角色权限】将角色【${transformText('roleUniqueCode', u1)}】的权限复制给角色【${transformText('roleUniqueCode', u2)}】！`)
}
