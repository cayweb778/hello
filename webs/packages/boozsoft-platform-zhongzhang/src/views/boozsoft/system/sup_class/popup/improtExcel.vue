...
import {importCusClass} from "/@/api/record/system/supplier_class";
...
const handleOk = async () => {
  ...
  await useRouteApi(importCusClass,{schemaName: database})(formData)
    .then(async (info:any)=>{
      if(info[0].code==='200' || info[0].error==='error'){
        ...
        for (let i = 0; i < info[0].list.length; i++) {
          // 埋点-操作日志
          let log='操作内容【导入供应商分类】,账套代码【'+accountInfo.value.coCode+'】,账套名称【'+accountInfo.value.companyName+'】,分类编码【'+info[0].list[i].cusClass+'】,分类编码【'+info[0].list[i].cusCclassName+'】'
          /************** 记录操作日志 ****************/
          let logmap={
