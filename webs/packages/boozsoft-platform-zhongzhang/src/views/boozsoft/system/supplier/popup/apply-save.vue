...
import {customerSaveCusClass, findAllByCusBendEq1,getCusClassAuthorData} from "/@/api/record/system/supplier_class";
...
const ASelectOption = ASelect.Option;
  ...
  async function findByClass() {
    // 是否设置权限控制
    let temp=await getCusTypeSeeSwitch({accId:tenantId.value,dataBaseName:"supplier_class"})
    if(temp!==''){
      let cusClassAuthorData=await getCusClassAuthorData({tableName:'supplier_class',userId:useUserStoreWidthOut().getUserInfo.id,accId:tenantId.value})
