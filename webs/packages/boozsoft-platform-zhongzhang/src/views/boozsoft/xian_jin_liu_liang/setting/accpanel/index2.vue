...
<!--      <BasicTable @register="registerTable" :row-selection="{ type: 'checkbox' }" :scroll="{ y: windowHeight }" :class="'a-table-font-size-12'">-->
...
import { findAllByflag ,savePanel,delPanel,countByFlag,findAllByGroupSysUserFlag } from '/@/api/record/accpanel/data';
...
const pageParameter=reactive({
  panelFlag:'1',
  iyear: useCompanyOperateStoreWidthOut().getLoginDate.split('-')[0],
...
const findAll = async () => {
  let noPanelName1=['自定义1','自定义2','自定义3','自定义4','自定义5']
  ...
  if(accpanel.length==0){
    noPanelName1.forEach(async (v,i)=>{
      let data={
        ...
        panelName:v,
        panelFlag:'1',
        panelIyear:database.value.split('-')[2],
        panelOrder: i+1,
        panelType:'cn'
      }
  ...
  setTimeout(async ()=>{
    accPanelList.value=await useRouteApi(findAllByflag,{schemaName: database})(pageParameter)
  },200)
}
async function dynamicAdReload(data) {
  await findDataBase(data.accId,data.year).then(async (item)=>{
    ...
    pageParameter.iyear=item.accountYear
    findAll()
  })
}
// 类型下拉触发事件
