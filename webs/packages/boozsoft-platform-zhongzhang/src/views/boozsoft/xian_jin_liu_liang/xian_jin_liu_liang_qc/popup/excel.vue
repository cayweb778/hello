...
function checkExcel(){
  ...
  for (let i=0; i<list.value.length; i++){
    ...
    //判断导入名称是否为空
    const projectName = item.beiyong1
    if (projectName==null || projectName==''){
      msg="第"+(i+1)+'行项目金额为空,不能进行信息导入!'
      return false
    }
    //编码不存在 不能导入
    console.log(projectList.value.find(v=> v.projectCode === projectCode))
    if(!projectList.value.find(v=> v.projectCode === projectCode)){
      msg = '项目编码重复,不能进行信息导入!'
      return false
    }
  }
...
function exportExcel() {
  const arrHeader = ['项目编码','金额'];
  console.log(arrHeader)
  // 保证data顺序与header一致
  aoaToSheetXlsx({
    data: [],
    header: arrHeader,
    filename: '现金流量期初模板.xlsx',
  });
}
