...
import {findByKemuAndDate} from "/@/api/wei_da_zhang/wei_da_zhang";
...
function printData(){
  ...
  arrData.forEach(item=>{
    ...
    item1[0] = item.statementDate
    item1[1] = item.settModes
    item1[2] = item.piaohao
    item1[3] = setString(item.duifangUnit,27)
    item1[4] = setString(item.remarks,27)
    item1[5] = item.jie==0?'':toThousandFilter(item.jie)
    item1[6] = item.dai==0?'':toThousandFilter(item.dai)
...
const excelData = () => {
  ...
  sheet.forEach((item, index) => {
    ...
    if (foot || foot === '') {
      foot && data.push(foot);
      const str = ('A' + data.length) + (':G' + data.length)
      merges.push(str)
    }
    ...
    (function () {
      Object.keys(dataInfo).forEach(i => {
        if (i == '!ref' || i == '!merges' || i == '!cols') {
          ...
          } else if (i.substring(0, 1) == 'B' ||i.substring(0, 1) == 'C' || i.substring(0, 1) == 'D' || i.substring(0, 1) == 'E') {
            dataInfo[i.toString()].s = {
              border: border === {} ? border : border || globalStyle.border,
              font: font || globalStyle.font,
              fill: fill || globalStyle.fill,
          ...
          } else if (i.substring(0, 1) == 'F' || i.substring(0, 1) == 'G') {
            dataInfo[i.toString()].s = {
              border: border === {} ? border : border || globalStyle.border,
              font: font || globalStyle.font,
              fill: fill || globalStyle.fill,
  ...
  const bookType = 'xlsx'
  // 类型默认为xlsx
  writeExcel(wb, bookType, '银行对账-银行未达账项_'+pageParameter.companyName)
}
