<template>
  <div class="nc-hr">
    ...
    <div class="nc-hr-middle">
      ...
      <div>
          <span>操作日志</span><br/>
          <span>区间：{{null == queryParameter.dates?'':queryParameter.dates.join(' - ')}}</span>
      </div>
      <div>
        <div style="text-align: right;">
          <Button class="ant-btn-me" @click="openFilterPage">过滤</Button>
          <Button class="ant-btn-me" style="border-left: none;" @click="exportOrPrint('1')">导出</Button>
          <Button class="ant-btn-me" style="border-left: none;" @click="exportOrPrint('0')">打印</Button>
          <Button class="ant-btn-me" style="border-left: none;" @click="outToNcPage">退出</Button>
        </div>
        ...
        <div style="margin-top: 10px">
          ...
          <InputSearch placeholder="" v-model:value="searchCondition.value" @search="onSearch"  style="width: 200px; border-radius: 4px;border-color: #c9c9c9;margin-right: 1em;margin-top: 4px;"/>
          <Button @click="tableReload">
...
<!--          <Button>
    ...
    <div class="nc-hrm-body">
      ...
      <div style="display: inline-block;height: 100%; float: left;position: relative;" :style="showTree?{width: 'calc(100% - 202px)',marginLeft: '5px'}:{width: '99.8%'}">
        <BasicTable
        :loading="loadMark"
        ref="tableRef"
        ...
        size="small"
      >
          <template #optAction="{ record }"><Tag color="green" v-if="record.optAction!='booz'">{{ record.optAction }}</Tag></template>
      </BasicTable>
        <div class="pagination-text" :style="{left: (totalColumnWidth-280)+'px'}" v-show="showPaginationText">
          {{`共 ${paginationNumber} 条记录 每页 200 条`}}
        </div>
      </div>
    </div>
    <LogFilter @query="queryFilter" @register="registerFilterPage"/>
  </div>
...
import {Input, Select, Button, Table,Radio,Avatar, Steps, Rate,Checkbox,Popconfirm,Popover, message,Tag} from "ant-design-vue";
...
const dynamicAdReload = async (obj) => {
  ...
  queryParameter.key = obj.accId
  let userStore = useUserStoreWidthOut()
  queryParameter.account = userStore.getUserInfo.username
  const serverDate = useCompanyOperateStoreWidthOut().getCurrServeDate.replaceAll(/-/g,'.')
  let startDate = (DateTool().offsetToStr(new Date(serverDate + ' 00:00:00'),(1000*60*60*24*7*-1)))?.substring(0,10).replaceAll(/-/g,'.')
  queryParameter.dates = [startDate,serverDate]
  resetDynamicColumnData()
}
// 这是示例组件
const [registerTable, { reload,getDataSource,setTableData,setColumns,getColumns,getPaginationRef: testgetPaginationRef }] = useTable({
  api: CrudApi.list,
...
const fetchSuccess = (data) => {
  if(data.items.length<50){
    for (let i =  data.items.length; i < 50; i++) {
      data.items.push({optAction: 'booz'})
    }
  }
...
const treeFilter = async (e) => {
  queryParameter.dates = e.date
  await tableReload()
}
const tableReload = async () => {
  await reload()
  setTimeout(()=>{
    paginationNumber.value = testgetPaginationRef().total
  },1000)
}
...
const reloadColumns = () => {
  ...
  newA = newA.map(it=>{if (it.title == '操作模块')it.format =(t)=>moduleForMat(t);return it})
  setColumns(newA)
  initTableWidth(newA)
  tableReload()
...
function resetDynamicColumnData() {
  // 先从数据查询是否已经设置
  lanMuData.accId = dynamicTenant.value?.accId
...
import LogFilter from "/@/views/dashboard/analysis/components/new/etc-components/LogFilter.vue";
import {useCompanyOperateStoreWidthOut} from "/@/store/modules/operate-company";
const [registerFilterPage, {openModal: openFilter}] = useModal()
const openFilterPage = () => {
  openFilter(true,dynamicTenant.value)
}

const qijianText = ref('')
const queryFilter = (e) => {
  if (!hasBlank(e.variable.periodStart)){
    qijianText.value = formatText(e.variable.periodStart)+ ' - '+formatText(e.variable.periodEnd)
    queryParameter.dates = [e.variable.periodStart+'-01',e.variable.periodEnd+'-31']
  }else{
    qijianText.value = e.variable.dateStart.replaceAll(/-/g,'.')+ ' - '+e.variable.dateEnd.replaceAll(/-/g,'.')
    queryParameter.dates = [e.variable.dateStart,e.variable.dateEnd]
  }
  tableReload()
}
