      ...
      <div class="acb-head">
        ...
        <div class="acbgead-two" :style="status == 3?{ pointerEvents: 'none'}:{}">
          ...
          <Select v-model:value="formItems.busStyle" @change="cvencodeChange()" style="width: 15%;">
            ...
            <SelectOption value="FKD">付款单</SelectOption>
            <SelectOption value="YFD">应付单</SelectOption>
          </Select>
          <label>供应商：</label>
...
<!--          <label>折扣金额：</label>
      ...
      <div v-show="formItems.busStyle=='FKD'" class="acb-centent">
        <div>
...
<!--            <template #footer>
      ...
      <div v-show="formItems.busStyle=='YFD'" class="acb-centent">
        <div>
          <span style="font-size: 18px;font-weight: bold;color: #0096c7;">蓝字</span>
          <div style="float: right;">
            <Button class="ant-btn-sm actod-btn" @click="tableAdd">增行</Button>
...
import {
  findArApYsyfByCcode,findWhxskd,
  saveYsyf,
...
async function cvencodeChange(){
  ...
  if (formItems.value.cvencode!=''&&formItems.value.cvencode!=null){
    if(formItems.value.busStyle=='FKD') {
      ...
      for (let i = 0; i < skdList.value.length; i++) {
        ...
        if(taskData.length==0){
          await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:danju,method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
        } else {
          // 任务不是当前操作员的
          if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
            ...
            await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:dynamicYear.value,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:danju,method:'审核',recordNum:item.ccode,caozuoModule:'ap'})
          }
        }
      }
...
async function contentSwitch(action) {
  loadMark.value = true
  const res = await useRouteApi(findArApHzhcdList,{schemaName: dynamicTenantId})({billStyle:'FKD',iyear:dynamicYear.value})
      ...
      createConfirm({
        ...
        onOk: async () => {
          ...
          if (formItems.value.busStyle=='FKD') {
            ...
            for (let j = 0; j < list.length; j++) {
              let item1 = list[j]
              const saleousing = await useRouteApi(findWarehousingByCcode,{schemaName: dynamicTenantId})({ccode:item1.sourcecode,billStyle:item1.sourceType})
...
async function saveData() {
  ...
  if (formItems.value.busStyle=='FKD'){
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      item.hxMoney = item.hxIsum
      if(!hasBlank(item.hxMoney) && item.hxMoney!=0) {
        const saleousing = await useRouteApi(findWarehousingByCcode, {schemaName: dynamicTenantId})({ccode:item.sourcecode,billStyle:item.sourceType})
