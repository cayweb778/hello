  ...
  >
    <template #title>
      <div style="height: 30px;width: 100%;background-color: #5f375c;color: white;line-height: 30px;">
        ...
        <span style="font-size: 14px">应收款管理</span>
      </div>
    </template>
    <div style="display: inline-flex;justify-content: space-between;width: 100%;">
      <div style="width: calc(100% - 150px);height: 100%;">
        <div style="text-align: center;padding: 10px 0 5px;">
          ...
          <span style="line-height:30px;font-size: 24px;color: #0096c7;font-weight: bold;">&ensp;收款明细账</span>
        </div>
...
<!--                <a-range-picker
              ...
              <!--              <div>
                              ...
                              <label>客户分类：</label>
                              <a-select
                                v-model:value="formItems.sup"
                                show-search
                                style="width: 240px;"
                              >
                                <a-select-option
                                  v-for="item in custClassList"
                              ...
                              &nbsp;
                              <a style="font-weight: bold;font-size: 18px;"><LinkOutlined @click="openHeadSelectContent('cvencode')" /></a>
                            </div>-->
              <!--              <div>
                              <span class="red_span"></span>
                              <label>客户：</label>
                              <a-select
                                v-model:value="formItems.customerList"
                                show-search
                                style="width: 240px;"
                              >
...
function changeDdate1(){
  if (!hasBlank(formItems.value.ddate1) && !hasBlank(formItems.value.ddate2)){
    if (formItems.value.ddate1.split('-')[0]!=formItems.value.ddate2.split('-')[0]){
      formItems.value.ddate2 = ''
    }
  }
}
...
function changeDdate2(){
  if (!hasBlank(formItems.value.ddate1) && !hasBlank(formItems.value.ddate2)){
    if (formItems.value.ddate1.split('-')[0]!=formItems.value.ddate2.split('-')[0]){
      formItems.value.ddate1 = ''
    }
  }
}
...
async function handleOk() {
  ...
  // console.log(formItems.value.riqi[0].format('YYYY-MM-DD'))
  // console.log(formItems.value.riqi[1].format('YYYY-MM-DD'))
  formItems.value.year = formItems.value.ddate1.split('-')[0]
