...
import {GetCustomerClassTree} from "/@/api/record/system/customer_class";
...
async function handleOk() {
  ...
  //是否存在锁定任务（应收款期初余额、收款单、退款单、应收单、销货单）
  let qichu= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:formItems.value.year,name:'应收款期初余额',method:'编辑'})
  if(qichu.length>0){
    return createWarningModal({ content: qichu[0]?.username+'正在编辑应收款期初余额,不能同时进行操作！' });
  }
  let shoukuan= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:formItems.value.year,name:'收款单',method:'修改,删除,审核,弃审'})
  if(shoukuan.length>0){
    return createWarningModal({ content: shoukuan[0]?.username+'正在'+shoukuan[0]?.method+'收款单,不能同时进行操作！' });
  }
  ...
  let yingshou= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:formItems.value.year,name:'应收单',method:'修改,删除,审核,弃审'})
  if(yingshou.length>0){
    return createWarningModal({ content: yingshou[0]?.username+'正在'+yingshou[0]?.method+'应收单,不能同时进行操作！' });
  }
  let xiaohuodan= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:formItems.value.year,name:'销货单',method:'修改,删除,审核,弃审'})
  if(xiaohuodan.length>0) {
    return createWarningModal({content: xiaohuodan[0]?.username + '正在' + xiaohuodan[0]?.method + '销货单,不能同时进行操作！'});
  }
  let taskData= await useRouteApi(getByStockBalanceTask, { schemaName: dynamicTenantId })({iyear:formItems.value.year,name:'应收自动核销',method:'核销'})
  if(taskData.length==0){
    await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({iyear:formItems.value.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收自动核销',method:'核销',caozuoModule:'ar'})
  } else {
    // 任务不是当前操作员的
    if(String(taskData[0]?.caozuoUnique)!==String(useUserStoreWidthOut().getUserInfo.id)){
      return createWarningModal({ content: taskData[0]?.username+'正在应收自动核销,不能同时进行操作！' });
    }else{
      await useRouteApi(saveTaskApi, { schemaName: dynamicTenantId })({id:taskData[0]?.id,iyear:formItems.value.year,caozuoUnique:useUserStoreWidthOut().getUserInfo.id,functionModule:'应收自动核销',method:'核销',caozuoModule:'ar'})
    }
  }
