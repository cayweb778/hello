<template>
  <div>
    ...
    >
      ...
      </FormItem><Input size="large" v-model:value="formData.account"
...
export default defineComponent({
  ...
  setup() {
    ...
    async function handleLogin() {
      ...
      let result = (await loginxxxx({username: data.account, password: data.password})).data.result
      console.log(result)
      ...
      async function abcd(result){
        const userinfo=result.content
        // mock auto mock token
        if (userinfo.token == 'mock token') {
          userinfo.token = guid()
        }
      ...
      if (result.code == '30001') {
        createConfirm({
          title: '登录提示',
          content: '当前账户已经登录，是否重新登录' || t('sys.api.networkExceptionMsg'),
          getContainer: () => document.body.querySelector(`.${prefixCls}`) || document.body,
          async onOk(){
            result = await loginxxxx({username: data.account, password: data.password,force:"true"})
            abcd(result)
          }
        });
        return
